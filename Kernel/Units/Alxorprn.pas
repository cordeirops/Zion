...
Uses
    SysUtils, Classes, Forms, Dialogs, Windows, IBQuery, IBDataBase, DB, Buttons, AlxMessage,
    messages, Printers, ULancEnt, UDMEstoque, UMenu, Alxor32, UDMMacs, Graphics, UDMSaida,
    UDMCaixa, UPedVenda, abarra, ComObj, Excelxp, Winspool, URelEstoque, UnitDeclaracoes,
    RpBase, RpSystem, RpDefine, RpRave;

    //FUNÇÕES DA ECF SCHALTER 
    // Vende Item
    Function ecfVenda_Item(codigo: string; descricao: string; qtdeItemInt: string; qtdeItemFrac: string; valor: string; taxa: integer): integer;stdcall; external 'dll32phi.dll';
    // Imprimi cabeçalho
    ...
    function ecfCancItemDef(item: string; descricao: string): integer;stdcall; external 'dll32phi.dll';
    ...
    function ecfInicCupomNFiscal(byTipo: integer): integer;stdcall; external 'dll32phi.dll';
    // serve para realizar o pagamento assim como para cancelar o último pagamento realizado
    function ecfPagamento (tipo: integer; pos: string; valor: string; linhaMens: integer): integer;stdcall; external 'dll32phi.dll';
    // serve para realizar um pagamento à vista assim como para cancelar o último pagamento à vista realizado
    function ecfPagCancPag (tipo: integer; descr: string; valor: string; mens: integer; linhaMens: integer): integer;stdcall; external 'dll32phi.dll';
    // serve para realizar um pagamento à prazo assim como para cancelar o último pagamento à prazo realizado.
    function ecfPrazoCancPrazo (tipo: integer; descr: string; valor: string; mens: integer; linhaMens: integer): integer;stdcall; external 'dll32phi.dll';
    ...
    function ecfCancAcresDescSubTotal (acres: integer; tipo: integer; descr: string; valor: string): integer;stdcall; external 'dll32phi.dll';
    ...
    function ecfFimTrans(operador: string): integer;stdcall; external 'dll32phi.dll';
    // Leitura X
    ...
    function ecfPayPatterns (szPosTab: string; szTitulo: string): integer;stdcall; external 'dll32phi.dll';
    // FUNÇÃO DE VENDA
    function ecfVendaItem3d(codigo: string; descricao: string; qtde: string; valor: string; taxa: integer; unidade: string; digito: string): integer;stdcall; external 'dll32phi.dll';
    //Este comando fornece o status de uma determinada forma de pagamento.
    function ecfStatusPayTypes (posicao: integer): PChar; stdcall; external 'dll32phi.dll';
    // Cancela ultimo Item
    function ecfCancItem(descricao: string): integer;stdcall; external 'dll32phi.dll';
    //Abre gaveta
    ...
    function ecfStatusCupom (flag_geral : integer): PChar; stdcall; external 'dll32phi.dll';
    // faz desconto ou cancelamento de desconto
    function ecfDescItem (tipo: integer; descr: string; valor: string): integer;stdcall; external 'dll32phi.dll';
   ...
   //VARIAVEIS UTILIZADA EM IMPRESSÃO DE ETIQEUTEAS DE PRODUTO TIPO PERSONALIZADAS
	  XALTPAG, XLARGPAG, XQTDETQPAG, XLINHAINI, XCOLUNAINI, XQTDCOLPAG, XLARGCODBARRA, XCOMPCODBARRA, XINTERVLIN, XINTERVCOL, XTAMFONTE, XTAMDESC: Integer;
     XTXTPERSONALIZADO: String;
   ////////
   //VARIAVEIS UTILIZADAS EM IMPRESSAO DE NF
     XNUMLIN, XImpDup, XQTDPAG, XQTDPAGIMP, XQTDITEMIMPPAG, XQTDITEM, XQTDDESPIMPPAG, XQTDDESP: Integer;
	  XTipoNF: String;
   ...
   //ACUMULA ERROS EM UM TEXT FILE
     ...
     XRetorno: integer; // eh o retorno que a função Codigo_Retorno traz para a variavel para futura verificação
     ...
     XERRO_FUNCAO : BOOLEAN; // SERVE PARA COMPARAR O RESULTADO DAS FUNÇÕES, SE VERDADEIRA OU FALSA
     ...
     XTOTFORNEC: INTEGER; // armazena a qtde de fornecedores que a empresa ainda possui pendencias
     XNumNotaFiscal: String; //Armazena temporariamente o numero da nota fiscal para impressão
     ...
     xCodPlnCtaVista, xCodPlnCtaPrazo: Integer;//variaveis utilizadas para marcar o codigo das contas de compra a prazo e compra a vista
	{************************
...
//   Function LexVendItem:Boolean;
   ...
   Function NFFaturaOS(Cod_Ordem: Integer; XSQLTabela :String; XCONTROLEREL: Integer): Boolean;
...
//   Procedure Analisa_iRetorno;
   // FUNCTION VENDE ITEM
   Function ecfVendeItem(Codigo: string; Descricao: string; Qtde: String; ValorUnit: string; IndiceAliquota: String; Unidade: String; Digito: string; Desconto: String): integer;
   ...
   function SubstituiCaracter(Valor: String; XCaracApag: String; XCaracInsert: String): String;
   ...
   Function ImpReciboFatura(Cliente:String; Endereco:String; Valor:String; ValorPorExtenso:String; Historico:String; CodPessoa:String; Parcela:String) : Boolean;
   //Função para gerar comprovante do faturamente de varias faturas (FaturaContas)
   Function ImpReciboFaturaContas(COD_GERADOR:Integer;TIPOGERADOR:String;TIPOOP:String) : Boolean;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           StrPCopy(Executa,#27+'.10'+'01'+ConcatZero(SubstituiCaracter(XVLRDESC, ',',''),12)+'{DINHEIRO                                                                          }');
           Resposta := EnviaComando(Executa);

           // função para localizar a palavra ERRO. Se encontrar exibe a msg p o usuario
           If localiza_palavra(Resposta, 'ERRO') = true
           Then Begin
...
begin
   ...
   THEN BEGIN
       ...
       if XNum = '1'
       THEN BEGIN
           Mensagem('N O T I F I C A Ç Ã O', 'Papel proximo do fim!', '', 1, 1, False, 'C');
       END;
       XNum := copy(aux, 3, 5);
       if (XNum = '113') or (XNum = '108')
       THEN BEGIN
           If Mensagem(' A T E N Ç Ã O ', 'ERRO: 113' + #13 + #13 + 'DESCRIÇÃO: Fechamento do dia não realizado.' + #13 + #13 + 'Deseja realizar a operação?' , '', 2, 3, False, 'E') = 3
           Then Begin
               Exit;
           End
           Else Begin
...
begin
   ...
   Else Begin
       ...
       If (MODELO_IMPRESSORA = 'BEMATECH')
       THEN BEGIN
           XRetorno := Bematech_FI_VendeItem('1234567890123','TESTE DE VENDA', '01','I','1',2,'2,00','$','0,00');
       END;
       IF (MODELO_IMPRESSORA = 'ELGIN')
       THEN BEGIN
           // FUNÇÃO VENDE ITEM
           XRetorno := ecfVendaItem3d('20', 'PRODUTO TESTE', '001,000', '200', 1, 'UN', '3');

           if XRetorno <> 0
           Then Begin
               if Codigo_Retorno(XRetorno) = false
               then
               ecfCancVenda('0005');
               Exit;
           End;
       ...
       IF (MODELO_IMPRESSORA = 'SWEDA')
       THEN BEGIN
           StrPCopy(Executa,#27+'.0112345678912340001000000000500000000000500TESTE DE VENDA ORION    T02}');
           Resposta := EnviaComando(Executa);

           // função para localizar a palavra ENCERRADA. Se encontrar exibe a msg p o usuario
           If localiza_palavra(Resposta,'ENCERRADA') = true
           Then Begin
               If Mensagem('   S W E D A   ', Resposta + #13 + #13 + 'Deseja cancelar cupom?' , '', 2, 3, False, 'E') = 3
               Then Begin
                   Exit;
               End
               Else Begin
               ...
               End;
           End;
           If (localiza_palavra(Resposta,'ENCERRAR O DIA') = true)
           Then Begin
               If Mensagem('   S W E D A   ', 'DESCRIÇÃO: Fechamento do dia não efetuado.' + #13 + #13 + 'Deseja realizar a operação?' , '', 2, 3, False, 'E') = 3
               Then Begin
                   Exit;
               End
               Else Begin
               ...
               End;
           End;
           If localiza_palavra(Resposta,'.-P002') = TRUE
           THEN BEGIN
               Mensagem('   S W E D A   ', 'É necessario abrir porta da impressora.', '', 1, 1, False, 'A');
           END;
       END;
   End;
end;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           // função para localizar a palavra ERRO. Se encontrar exibe a msg p o usuario
           If localiza_palavra('ERRO',Resposta) = true
           Then Begin
               Mensagem('   S W E D A   ', Resposta, '', 1, 1, False, 'E');
           End;
           If (localiza_palavra(Resposta,'ENCERRAR O DIA') = true)
           Then Begin
               Mensagem('   S W E D A   ', 'Efetue o encerramento do dia com a Redução Z.', '', 1, 1, False, 'A');
           End;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           Then Begin
               // DESCONTO NO ITEM
               StrPCopy(Executa,#27+'.03'+'Desconto  '+ ConcatZero(SubstituiCaracter(XVLRDESC, ',',''), 12)+'S}');
               Resposta := EnviaComando(Executa);
...
begin
   ...
   If DMCAIXA.TCaixa.FieldByName('COD_CAIXA').AsInteger <> 0
   Then Begin
       MODELO_IMPRESSORA := DMCAIXA.TCaixa.FieldByName('MODELO_IMPRESSORA').AsString;
...
begin
   ...
   // eh feito
   if (Codigo > 00) and (Codigo < 157) and (Codigo <> 81) and (Codigo <> 108) and (Codigo <> 113) and (Codigo <> 68)
   THEN BEGIN
       Mensagem(' A T E N Ç Ã O ', 'ERRO: ' + IntToStr(Codigo) + #13 + #13 +'DESCRIÇÃO: ' + descricao, '', 1, 1, False, 'E');
       Result := false;
   END;
   IF (Codigo = 81)
   THEN BEGIN
   ...
   IF(Codigo = 108) or (Codigo = 113)
   THEN BEGIN
       If Mensagem('   E L G I N   ', 'ERRO: ' + IntToStr(Codigo) + #13 + 'DESCRIÇÃO: ' + descricao + #13 + #13 + 'Deseja realizar a Redução Z ?' , '', 2, 3, False, 'E') = 3
       Then Begin
           result := false;
           Exit;
       End
...
begin
   ...
   THEN BEGIN
       ...
       Mensagem('N O T I F I C A Ç Ã O', 'O número do Cupom atual é: ' + XNumeroCupom, '', 1, 1, False, 'C');
   END;
   IF (MODELO_IMPRESSORA = 'ELGIN')
   THEN BEGIN
       ...
       Mensagem('N O T I F I C A Ç Ã O', 'Número do Cupom: ' + XValor , '', 1, 1, False, 'C');
   END;
end;
...
begin
   ...
   Result := IntToStr(Trunc(StrToFloat(DMESTOQUE.TSlave.FieldByname('QTD').AsString)));
end;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           if XNum = '0'
           THEN BEGIN
               Mensagem('   E L G I N   ', 'Papel Sufuciente na impressora!', '', 1, 1, False, 'C');
           END
           Else Begin
       ...
       THEN BEGIN
           ...
           If Procura_Caracter(Resposta,'-') = '-'
           Then Begin
               Mensagem('   S W E D A   ', 'Substitua o papel.', '', 1, 1, False, 'A');
           End
           Else Begin
               Mensagem('   S W E D A   ', 'Papel Sufuciente na impressora!', '', 1, 1, False, 'C');
           End;
       END;
   End;
end;
...
begin
   ...
   THEN BEGIN
   	If DMCAIXA.TCaixa.FieldByName('TIPOCUPOM').AsString='ECF' Then
       	Mensagem(' A T E N Ç Ã O', 'SUAS CONFIGURAÇÕES SÃO PARA IMPRIMIR ECF, PORÉM NÃO HÁ IMPRESSORA DEFINIDA PARA O USUÁRIO ATUAL.', '', 1, 1, False, 'E');
       Result := false;
   END
   ELSE
       result := true;
...
//   XString := FloatToStr(Frac(StrToFloat(IntToStr(DMESTOQUE.TSlave.FieldByName('QTD').AsInteger))));
   XString := FloatToStr(Frac(StrToFloat(DMESTOQUE.TSlave.FieldByName('QTD').AsString)));
...
begin
  ...
  THEN BEGIN
       ...
       Then Begin
           ...
           XVALOR_PAGO := FormatFloat('0.00', DMESTOQUE.Alx.FieldByName('TOTAL').AsCurrency);
       End
       Else Begin
           for I := 1 to 14 do
           Begin
           ...
           Except
               XValor_Pago_Formatado:=FormatFloat('0.00',DMESTOQUE.Alx.FieldByName('TOTAL').AsCurrency);
           End;
       End;
       //Final do descobrir total do pedido

       // Verifica se é Acréscimo ou Desconto: 'A' para Acréscimo - 'D' para Desconto
       If DMESTOQUE.Alx.FieldByName('DESCONTO').AsCurrency  > 0
       Then Begin
           ...
           XDescontoReal:=FormatFloat('0.00', DMESTOQUE.Alx.FieldByName('TOTAL').AsCurrency);
           XDescontoReal:=FormatFloat('0.00', ( StrToFloat(XValor_Pago_Formatado)-DMESTOQUE.Alx.FieldByName('TOTAL').AsCurrency) );
           XVlrReal:=  FormatFloat('0.00', (StrToFloat(XValor_Pago_Formatado)-StrToFloat(XDescontoReal)));
			XRetorno := Bematech_FI_FechaCupom('Dinheiro', 'D', '$', XDescontoReal, XVlrReal, XMENS);
       End
  		Else Begin
			XRetorno := Bematech_FI_FechaCupomResumido(pchar('Dinheiro'),pchar (XMENS));
       End;
       Result := true;
   END;
   IF (MODELO_IMPRESSORA = 'ELGIN')
   THEN BEGIN
       ...
       if StrToInt(XValor_Fechamento) > 0
       Then Begin
           // a variavel XCod recebe o retorno da funçã e verifica o caodigo na função Codigo_Retorno
           XRetorno := ecfCancAcresDescSubTotal(0, 0, 'Desconto: ', XValor_Fechamento);
           if XRetorno <> 0
           then begin
       ...
       Then Begin
          ...
          XRetorno := ecfCancAcresDescSubTotal(1, 0, 'Acrescimo: ', SubstituiCaracter(XValor_Fechamento, '-', ''));
           if XRetorno <> 0
           then begin
               If Codigo_Retorno(XRetorno) = false then
               ecfCancVenda('0005');
   ...
   Then Begin
       ...
       Then Begin
           // DESCONTO NO ITEM
           StrPCopy(Executa,#27+'.03'+'          '+ ConcatZero(SubstituiCaracter(XVLRDESC, ',',''), 12)+'S}');
           Resposta := EnviaComando(Executa);
       End;
       If(XVLRDESC < '0')
       Then Begin
           ...
           XVLRDESC := SubstituiCaracter(XVLRDESC,'-','');
           // efetua o acrescimo no valor
           StrPCopy(Executa,#27+'.1151'+ ConcatZero(SubstituiCaracter(XVLRDESC, ',',''), 15) +'S}');
           Resposta := EnviaComando(Executa);
           Result := true;
       End;
   End;
...
begin
   ...
   THEN BEGIN
       ...
       Then Begin
           ...
           XValor_Pago_Formatado:=FloatToStr(StrToFloat(XValor_Pago_Formatado)-XTOTDESC);
       End;
       // FUNÇÃO DA DLL PARA EFETUAR PAGAMENTO
       If DMESTOQUE.Alx.FieldByName('PAGFISC').AsString<>'' Then
       	XRetorno := Bematech_FI_EfetuaFormaPagamento(DMESTOQUE.Alx.FieldByName('PAGFISC').AsString, XValor_Pago_Formatado)
       Else
       	XRetorno := Bematech_FI_EfetuaFormaPagamento('DINHEIRO', XValor_Pago_Formatado);
       If XRetorno<>1
       Then Begin
           Result := False;
   ...
   THEN BEGIN
      ...
      XRetorno := ecfPagamento(0, 'Dinheiro', SubstituiCaracter(FormatFloat('0.00',XVALOR-XTOTDESC),',',''), 2);

       IF XRetorno <> 0
       THEN BEGIN
           Codigo_Retorno(XRetorno);
           ecfCancVenda('0005');
           Mensagem('N O T I F I C A Ç Ã O', 'Cupom fiscal cancelado.', '', 1, 1, False, 'C');
           result := false;
       END
       Else Begin
       	Result:=True;
...
begin
   ...
   THEN BEGIN
       ...
       XFracao := frac(StrToFloat(Qtde)); // seleciona soh a parte fracionaria do numero
       If XFracao = 000 // ou seja, se for = 000 pode passar o XTipoQtde como inteiro
       Then Begin
       ...
       XRetorno := Bematech_FI_VendeItem(Codigo,copy(Descricao, 0, 28), IndiceAliquota, XTipoQtde,Qtde,2,ValorUnit,XTipoDesc,Desconto);
       If XRetorno <> 1 then
          Result := 0
       Else
          Result := 1;
   ...
   THEN BEGIN
       ...
       If (Unidade = '') or (Length(Unidade) < 2)
       Then Begin
           Unidade := 'UN';
       End;
       //Formata Valores
       ...
       XPrecoElgin:=PChar(SubstituiCaracter(FormatFloat('0.00', StrToFloat(ValorUnit)), ',', '' ));
       XDigitoElgin:=PChar(Digito);
       XUnidElgin:=PChar(Unidade);
       // FUNÇÃO DE VENDA PARA ELGIN
       XRetorno := ecfVendaItem3d(XCodigoElgin, XDescricaoElgin, XQuantElgin, XPrecoElgin, StrToInt(Trim(IndiceAliquota)), XUnidElgin, XDigitoElgin);
       if XRetorno <> 0
       then begin
   ...
   THEN BEGIN
       ...
       If (IndiceAliquota <> 'II') OR (IndiceAliquota <> 'FF') OR (IndiceAliquota <> 'NN')
       THEN BEGIN
       ...
       StrPCopy(Executa,#27+'.01'+ConcatEspaco(Codigo, 13)+ ConcatZero(SubstituiCaracter(FormatFloat('0.000', StrToFloat(Qtde)), ',', ''), 7) + ConcatZero(SubstituiCaracter(FormatFloat('0.00', StrToFloat(ValorUnit)), ',', ''), 9)+ ConcatZero(IntToStr(XQtde * XValor),12) + ConcatEspacoDir(Descricao, 24) + IndiceAliquota + '}');
       Resposta := EnviaComando(Executa);
   END;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           if XRetorno <> 0
           then begin
           	XRetorno := ecfCancVenda('0'); // parametro tem que ser registro do operador, número do terminal... coloquei esse ai
               If XRetorno<>0 Then
               	Codigo_Retorno(XRetorno);
           end;
       END;
       ...
       THEN BEGIN
           ...
           Then Begin
               ...
               Else Begin
                   ...
                   End
                   Else Begin
                       Mensagem('   S W E D A   ', 'Redução Z realizada com sucesso', '', 1, 1, False, 'C');
                   End;
               End;
           End;
       END;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           FiltraTabela(DMPESSOA.TCliente,'CLIENTE','COD_CLIENTE',DMESTOQUE.Alx.FieldByName('COD_CLIENTE').AsString,'');
           ...
           If (ValidCPF(DMPESSOA.TPessoa.FieldByName('CPFCNPJ').AsString)=True) Or (ValidaCGC(DMPESSOA.TPessoa.FieldByName('CPFCNPJ').AsString)=True) Then
               XRetorno:= Bematech_FI_AbreCupom(DMPESSOA.TPessoa.FieldByName('CPFCNPJ').AsString)
           Else
               XRetorno:= Bematech_FI_AbreCupom('');
           If XRetorno <> 1
           Then Begin
              Result := false;
              Mensagem('   B E M A T E C H   ', 'Erro ao abrir cupom.', '', 1, 1, False, 'E');
              Exit;
           End
           Else Begin
              Result := true;
       ...
       THEN BEGIN
           ...
           Then Begin
               ecfCancVenda('0005');
               Mensagem('   E L G I N   ', 'Para não haver duplicidade de Nº Fiscal, o cabeçalho já impresso foi cancelado.', '', 1, 1, False, 'A');
               Result := false;
               Exit;
           End;
...
{           // Função utilizada sempre apos a impressao do cabecalho
       ...
       THEN BEGIN
           ...
           If localiza_palavra(Resposta,'OPERACAO NAO ENCERRADA') = true
           Then Begin
               If Mensagem('   S W E D A   ', 'OPERAÇÃO NÃO ENCERRADA' + #13 + #13 + 'Deseja cancelar cupom?' , '', 2, 3, False, 'E') = 3
               Then Begin
                   Exit;       
               End
               Else Begin
                   ...
                   Then Begin
                       ...
                       If Mensagem('   S W E D A   ', 'É necessário executar a Redução Z.' + #13 + #13 + 'Deseja continuar?' , '', 2, 3, False, 'E') = 3
                       Then Begin
                           Exit;
                       End
                       Else Begin
                           ...
                           If Procura_Caracter(Resposta,'+') = '-'
                           Then Begin
                               Mensagem('   S W E D A   ', 'Redução Z efetuada com sucesso.', '', 1, 1, False, 'C');
                           End
                           Else Begin
                   ...
                   Else Begin
                       ...
                       Mensagem('   S W E D A   ', Resposta + #13 + #13 + 'Não foi possível abrir cupom.', '', 1, 1, False, 'E');
                   End;
               End;
           End;
           ...
           If (localiza_palavra(Resposta,'ENCERRAR O DIA') = true)
           Then Begin
               If Mensagem('   S W E D A   ', 'DESCRIÇÃO: Fechamento do dia não efetuado.' + #13 + #13 + 'Deseja realizar efetuar a Redução Z?' , '', 2, 3, False, 'E') = 3
               Then Begin
               ...
               Else Begin
                   ...
                   Else Begin
                       ...
                       If localiza_palavra(Resposta,'ERRO') = TRUE
                       Then Begin
                           Mensagem('   S W E D A   ', 'Não foi possivel abrir cupom.'+ #13 + #13 + Resposta, '', 1, 1, False, 'E');
                           Exit;
                       End;
                   End;
               End;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           END
           ELSE BEGIN
               Mensagem('   E L G I N   ', 'Leitura X realizada com sucesso!', '', 1, 1, False, 'C');
           END;
       END;
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
           ...
           Then Begin
              Result := false;
              Mensagem('   BEMATECH   ', 'Erro ao finalizar cupom.', '', 1, 1, False, 'A');
              Exit;
           End
           Else Begin
...
Function FechaPorta: Boolean;
begin
   if(MODELO_IMPRESSORA = 'BEMATECH') or (MODELO_IMPRESSORA = 'SWEDA')
   THEN BEGIN
...
begin
   ...
   Else Begin
       ...
       THEN BEGIN
          ...
          If (PORTA_IMPRESSORA = 'COM1')
          then Begin
              XERRO_FUNCAO := AbrePorta(1,5);
              If (XERRO_FUNCAO = false) then
                Mensagem('   S W E D A   ', 'Porta COM1 não aberta.', '', 1, 1, False, 'E');
          END;
          If (PORTA_IMPRESSORA = 'COM2')
          then Begin
              XERRO_FUNCAO := AbrePorta(2,5);
              If (XERRO_FUNCAO = false) then
                Mensagem('   S W E D A   ', 'Porta COM2 não aberta.', '', 1, 1, False, 'E');
          END;
          If (PORTA_IMPRESSORA = 'COM3')
          then Begin
              XERRO_FUNCAO := AbrePorta(3,5);
              If (XERRO_FUNCAO = false) then
                Mensagem('   S W E D A   ', 'Porta COM3 não aberta.', '', 1, 1, False, 'E');
          End;
          If (PORTA_IMPRESSORA = 'COM4')
          then Begin
              XERRO_FUNCAO := AbrePorta(4,5);

              If (XERRO_FUNCAO = false) then
                Mensagem('   S W E D A   ', 'Porta COM4 não aberta.', '', 1, 1, False, 'E');
          End;
       End;
   End;
end;
...
begin
   ...
   Else Begin
       ...
       If XRetorno = 1 then
           Mensagem('   B E M A T E C H   ', 'Redução Z realizada com sucesso!', '', 1, 1, False, 'C')
       Else
           Mensagem('   B E M A T E C H   ', 'Ocorreu erro ao realizar Redução Z!', '', 1, 1, False, 'A');
       END;
       ...
       THEN BEGIN
           ...
           END
           ELSE BEGIN
               Mensagem('   E L G I N   ', 'Redução Z realizada com sucesso!', '', 1, 1, False, 'C');
           END;
       END;
...
Begin
   ...
   Try
       Printer.BeginDoc;
           ...
           //Imprime Cabeçalho
           Printer.Canvas.TextOut(Coluna, Linha, DMMACS.TLoja.FieldByName('TITULOPRIM').AsString);
           Linha:=Linha+XINTERVLIN;
           //Printer.Canvas.Font.Size:=8;
           Printer.Canvas.TextOut(COLUNA, Linha, DMMACS.TLoja.FieldByName('TITULOSEC').AsString);
           ...
           //////////////////
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Cliente: '+DMESTOQUE.Alx.FieldByName('NOMECLI').AsString);
           Coluna:=Coluna+(Tamanho*70);
           Printer.Canvas.TextOut(COLUNA, Linha, 'CPF: '+DMESTOQUE.Alx.FieldByName('CPFCNPJ').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Nº Ped.: '+DMESTOQUE.Alx.FieldByName('NUMPED').AsString);
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(XCOLUNAINI, Linha, 'Endereço: '+DMESTOQUE.Alx.FieldByName('ENDERECO').AsString);
           Coluna:=Coluna+(Tamanho*70);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Cidade: '+DMESTOQUE.Alx.FieldByName('NOME').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           Printer.Canvas.TextOut(COLUNA, Linha, 'DT Pedido: '+DMESTOQUE.Alx.FieldByName('DTPEDVEN').AsString);
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Endereço de Entrega: '+DMESTOQUE.Alx.FieldByName('endentraga').AsString);
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Vendedor: '+DMEstoque.Alx1.FieldByName('nome').AsString);
...
//               Linha:=Linha+XINTERVLIN;
           ...
           Begin
               Coluna:=XCOLUNAINI;
               Linha:=Linha+XINTERVLIN;
               Printer.Canvas.TextOut(COLUNA, Linha, DMESTOQUE.TSlave.FieldByName('PRIMCOD').AsString);
               Coluna:=(XCOLUNAINI+(Tamanho*26))-(length(DMESTOQUE.TSlave.FieldByName('QTDEPROD').AsString)*Tamanho) ;
               Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('QTDEPROD').AsString);
               Coluna:=XCOLUNAINI+(Tamanho*28);
               Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('SIGLA_UNID').AsString);
               Coluna:=XCOLUNAINI+(Tamanho*34);
               Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('DESCRICAO').AsString);
               Coluna:=XCOLUNAINI+(Tamanho*74);
               Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('MARCA').AsString);
               Coluna:=(XCOLUNAINI+(Tamanho*108))-(length(FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('VALUNIT').AsCurrency))*Tamanho) ;
               Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('VALUNIT').AsCurrency));
               Coluna:=(XCOLUNAINI+(Tamanho*118))-(length(FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('DESCPRO').AsCurrency))*Tamanho) ;
               Printer.Canvas.TextOut(COLUNA, Linha,  FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('DESCPRO').AsCurrency));
               Coluna:=(XCOLUNAINI+(Tamanho*130))-(length(FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('VALORTOTAL').AsCurrency))*Tamanho) ;
               Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('VALORTOTAL').AsCurrency));
               if fmenu.XPedFatura = ''
               then begin
                    XTotProdNF:=XTotProdNF+DMESTOQUE.TSlave.FieldByName('VALORTOTAL').AsCurrency;
               end
               else begin
           ...
           Linha:=Linha+XINTERVLIN;
           IncLine:=Linha;
           Printer.Canvas.TextOut(XCOLUNAINI, Linha, DMESTOQUE.ALX.FieldByName('Descricao').AsString);
           Coluna:=(XCOLUNAINI+(Tamanho*131))-(length('Produtos: '+FormatFloat('0.00', XTotProdNF))*Tamanho) ;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Produtos: '+ FormatFloat('0.00', XTotProdNF));
           Linha:=Linha+XINTERVLIN;
           Coluna:=(XCOLUNAINI+(Tamanho*131))-(length('Desc. (%): '+FormatFloat('0.00', XTotProdNF-DMESTOQUE.Alx.FieldByName('Valor').AsCurrency))*Tamanho) ;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Desc. (%): '+FormatFloat('0.00', XTotProdNF-DMESTOQUE.Alx.FieldByName('Valor').AsCurrency));
           Linha:=Linha+XINTERVLIN;
           Coluna:=(XCOLUNAINI+(Tamanho*129))-(length('TOTAL: '+FormatFloat('0.00', DMESTOQUE.Alx.FieldByName('Valor').AsCurrency))*Tamanho) ;
           Printer.Canvas.TextOut(COLUNA, Linha, 'TOTAL: '+ FormatFloat('0.00', DMESTOQUE.Alx.FieldByName('Valor').AsCurrency));
           //Imprime Parcelas
           ...
           Then Begin
               ...
               Begin
                   Linha:=Linha+XINTERVLIN;
                   Printer.Canvas.TextOut(COLUNA, Linha, DMMACS.TALX.FieldByName('dtvenc').AsString+'  /  '+DMMACS.TALX.FieldByName('cobranca').AsString+'  -  R$ '+FormatFloat('0.00', DMMACS.TALX.FieldByName('valor').AsCurrency));
                   DMMACS.TALX.Next;
               End;
           End;
           //Imprime Rodape
...
IncLine: Integer; //incremento de linha
XBarra: String;//Separadores de paragrafo
XLinAuxImp01, XLinAuxImp02: String;//Linhas auxiliares para imprimir campos memo
Begin
   //RECEBE VALORES CONFIGURADOS EM BANCO
   Try
       SetarImpressora; //força valores de impresao
   ...
   Try
       Printer.BeginDoc;
           ...
           ///////////////////
           //1º linha
           Printer.Canvas.TextOut(Coluna, Linha, DMMACS.TLoja.FieldByName('DESCRICAO').AsString);
           ...
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(Coluna, Linha, 'CNPJ: '+DMMACS.TEmpresa.FieldByName('CNPJ').AsString);
           Coluna:=(Coluna+(Tamanho*89))-(length(DMServ.Tord.FieldByName('NUMERO').AsString)*Tamanho) ;
           Printer.Canvas.TextOut(Coluna, Linha, DMServ.Tord.FieldByName('NUMERO').AsString);
           Coluna:=XCOLUNAINI+(Tamanho*122);
           Printer.Canvas.TextOut(Coluna, Linha, DateToStr(Date()));
           //3º linha
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(Coluna, Linha, 'Endereço: '+DMMACS.TEmpresa.FieldByName('ENDERECO').AsString);
           Coluna:=(Coluna+(Tamanho*80));
           ...
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(Coluna, Linha, 'Bairro: '+DMMACS.TEmpresa.FieldByName('BAIRRO').AsString);
           Coluna:=(Coluna+(Tamanho*88))-(length(DMServ.Tord.FieldByName('LOCALAT').AsString)*Tamanho);
           Printer.Canvas.TextOut(Coluna, Linha, DMServ.TOrd.FieldByName('LOCALAT').AsString);
           //5º LINHA
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(Coluna, Linha, 'Fone: '+DMMACS.TEmpresa.FieldByName('FONE').AsString);
           Coluna:=(XCOLUNAINI+(Tamanho*129))-(length(DMServ.Tord.FieldByName('STATUS').AsString)*Tamanho);
           Printer.Canvas.TextOut(Coluna, Linha, DMServ.TOrd.FieldByName('STATUS').AsString);
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           For I:=0 to XCOMPCODBARRA Do
               XBarra:=XBarra+'= ';
           ...
           /////////////////
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Cliente: '+DMPESSOA.TPessoa.FieldByName('NOME').AsString);
           Coluna:=Coluna+(Tamanho*70);
           Printer.Canvas.TextOut(COLUNA, Linha, 'CPF: '+DMPESSOA.TPessoa.FieldByName('CPFCNPJ').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           Printer.Canvas.TextOut(COLUNA, Linha, 'FONE: '+DMPESSOA.TPessoa.FieldByName('TELREL').AsString);
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(XCOLUNAINI, Linha, 'Endereço: '+DMPESSOA.TPessoa.FieldByName('ENDERECO').AsString);
           Coluna:=Coluna+(Tamanho*70);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Cidade: '+DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           Printer.Canvas.TextOut(COLUNA, Linha, 'CEP: '+DMPESSOA.TPessoa.FieldByName('CEP').AsString);
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Endereço de Entrega: '+DMPESSOA.TCliente.FieldByName('ENDENTRAGA').AsString);
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(COLUNA, Linha, XBarra);
           Linha:=Linha+XINTERVLIN;
           ...
           //1º Linha
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Equipamento: '+DMPESSOA.TEquip.FieldByName('DESCRICAO').AsString);
           //2º Linha
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(XCOLUNAINI, Linha, 'Marca: '+DMPESSOA.TEquip.FieldByName('MARCA').AsString);
           Coluna:=Coluna+(Tamanho*50);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Modelo: '+DMPESSOA.TEquip.FieldByName('MODELO').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Motor: '+DMPESSOA.TEquip.FieldByName('MOTOR').AsString);
           //2º Linha
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(XCOLUNAINI, Linha, 'Km/Hs: '+DMPESSOA.TEquip.FieldByName('KMATUAL').AsString);
           Coluna:=Coluna+(Tamanho*50);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Ano Fabr.: '+DMPESSOA.TEquip.FieldByName('ANO').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Ano Motor: '+DMPESSOA.TEquip.FieldByName('ANOMOTOR').AsString);
           //3º Linha
           Linha:=Linha+XINTERVLIN;
           Coluna:=XCOLUNAINI;
           Printer.Canvas.TextOut(XCOLUNAINI, Linha, 'Placa: '+DMPESSOA.TEquip.FieldByName('PLACA').AsString);
           Coluna:=Coluna+(Tamanho*50);
           Printer.Canvas.TextOut(COLUNA, Linha, 'Chassi: '+DMPESSOA.TEquip.FieldByName('ANO').AsString);
           Coluna:=COLUNA+(Tamanho*35);
           ...
           Then Begin
               ...
               Begin
                   Coluna:=XCOLUNAINI;
                   Printer.Canvas.TextOut(COLUNA, Linha, DMESTOQUE.TSlave.FieldByName('PRIMCOD').AsString);
                   Coluna:=(XCOLUNAINI+(Tamanho*26))-(length(DMESTOQUE.TSlave.FieldByName('QTD').AsString)*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('QTD').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*28);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('UNIDADE').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*34);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('DESCRICAO').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*74);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlave.FieldByName('MARCA').AsString);
                   Coluna:=(XCOLUNAINI+(Tamanho*108))-(length(FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('VLRUNIT').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('VLRUNIT').AsCurrency));
                   Coluna:=(XCOLUNAINI+(Tamanho*118))-(length(FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('DESCONTO').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha,  FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('DESCONTO').AsCurrency));
                   Coluna:=(XCOLUNAINI+(Tamanho*130))-(length(FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('TOTAL').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TSlave.FieldByName('TOTAL').AsCurrency));
                   XTotProdNF:=XTotProdNF+DMESTOQUE.TSlave.FieldByName('TOTAL').AsCurrency;
                   DMESTOQUE.TSlave.Next;
               End;
           End;
           //////////////////
           ...
           Then Begin
               ...
               Begin
                   Coluna:=XCOLUNAINI;
                   Printer.Canvas.TextOut(COLUNA, Linha, DMESTOQUE.TSlaveServ.FieldByName('cod_itservord').AsString);
                   Coluna:=(XCOLUNAINI+(Tamanho*26))-(length(DMESTOQUE.TSlaveServ.FieldByName('QTD').AsString)*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlaveServ.FieldByName('QTD').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*28);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlaveServ.FieldByName('UNIDADE').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*34);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TSlaveServ.FieldByName('DESCRICAO').AsString);
                   Coluna:=(XCOLUNAINI+(Tamanho*108))-(length(FormatFloat('0.00', DMESTOQUE.TSlaveServ.FieldByName('VLRUNIT').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TSlaveServ.FieldByName('VLRUNIT').AsCurrency));
                   Coluna:=(XCOLUNAINI+(Tamanho*118))-(length(FormatFloat('0.00', DMESTOQUE.TSlaveServ.FieldByName('DESCONTO').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha,  FormatFloat('0.00', DMESTOQUE.TSlaveServ.FieldByName('DESCONTO').AsCurrency));
                   Coluna:=(XCOLUNAINI+(Tamanho*130))-(length(FormatFloat('0.00', DMESTOQUE.TSlaveServ.FieldByName('TOTAL').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TSlaveServ.FieldByName('TOTAL').AsCurrency));
                   XTotProdNF:=XTotProdNF+DMESTOQUE.TSlaveServ.FieldByName('TOTAL').AsCurrency;
           ...
           Then Begin
               ...
               Begin
                   Coluna:=XCOLUNAINI;
                   Printer.Canvas.TextOut(COLUNA, Linha, DMESTOQUE.TDesp.FieldByName('COD_DESPADIC').AsString);
                   Coluna:=(XCOLUNAINI+(Tamanho*26))-(length(DMESTOQUE.TDesp.FieldByName('QTD').AsString)*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TDesp.FieldByName('QTD').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*28);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TDesp.FieldByName('UNIDADE').AsString);
                   Coluna:=XCOLUNAINI+(Tamanho*34);
                   Printer.Canvas.TextOut(COLUNA, Linha,  DMESTOQUE.TDesp.FieldByName('DESPESA').AsString);
                   Coluna:=(XCOLUNAINI+(Tamanho*108))-(length(FormatFloat('0.00', DMESTOQUE.TDesp.FieldByName('VLRFINAL').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TDesp.FieldByName('VLRFINAL').AsCurrency));
                   Coluna:=(XCOLUNAINI+(Tamanho*118))-(length(FormatFloat('0.00', DMESTOQUE.TDesp.FieldByName('DESCONTO').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha,  FormatFloat('0.00', DMESTOQUE.TDesp.FieldByName('DESCONTO').AsCurrency));
                   Coluna:=(XCOLUNAINI+(Tamanho*130))-(length(FormatFloat('0.00', DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency))*Tamanho) ;
                   Printer.Canvas.TextOut(COLUNA, Linha, FormatFloat('0.00', DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency));
                   XTotProdNF:=XTotProdNF+DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency;
           ...
           Printer.Canvas.TextOut(COLUNA, Linha, XBarra);
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Responsável: '+DMPESSOA.VWFuncionario.FieldByName('COD_INTERNO').AsString+' - '+DMPESSOA.VWFuncionario.FieldByName('NOME').AsString);
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Total de Produtos: '+FormatFloat('00.00', DMServ.TOrd.FieldByName('TOTPROD').AsCurrency));
           If Tipo<>'ABERTURA'
           Then Begin
               //fechamento
               Printer.Canvas.TextOut(COLUNA+(Tamanho*60), Linha, 'Entrada    Data: '+DMServ.TOrd.FieldByName('DTABERT').AsString);
               Printer.Canvas.TextOut(COLUNA+(Tamanho*95), Linha, 'Entrega    Data: '+DMServ.TOrd.FieldByName('DTENTREGA').AsString);
           End
           Else Begin
               //abertura
               Printer.Canvas.TextOut(COLUNA+(Tamanho*60), Linha, 'Previsão de    Data: '+DMServ.TOrd.FieldByName('DTPREV').AsString);
               Printer.Canvas.TextOut(COLUNA+(Tamanho*95), Linha, 'Entrada        Data: '+DMServ.TOrd.FieldByName('DTABERT').AsString);
           End;
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Total de Serviços: '+FormatFloat('00.00', DMServ.TOrd.FieldByName('TOTSERV').AsCurrency));
           Coluna:=XCOLUNAINI;
           If Tipo<>'ABERTURA'
           Then Begin
               //Fechamento
               Printer.Canvas.TextOut(COLUNA+(Tamanho*69), Linha, 'Hora: '+DMServ.TOrd.FieldByName('HRABERT').AsString);
               Printer.Canvas.TextOut(COLUNA+(Tamanho*104), Linha, 'Hora: '+DMServ.TOrd.FieldByName('HRENTREGA').AsString)
           End
           Else Begin
               //abertura
               Printer.Canvas.TextOut(COLUNA+(Tamanho*62), Linha, 'Entrega      Hora: '+DMServ.TOrd.FieldByName('HRPREV').AsString);
               Printer.Canvas.TextOut(COLUNA+(Tamanho*106), Linha, 'Hora: '+DMServ.TOrd.FieldByName('HRABERT').AsString);
           End;
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'Descontos: '+FormatFloat('00.00', DMServ.TOrd.FieldByName('DESCONTO').AsCurrency));
           Linha:=Linha+XINTERVLIN;
           Printer.Canvas.TextOut(COLUNA, Linha, 'TOTAL: '+FormatFloat('00.00', DMServ.TOrd.FieldByName('TOTORD').AsCurrency));
           Linha:=Linha+XINTERVLIN;
           Linha:=Linha+XINTERVLIN;
           If Tipo='ABERTURA'
           Then Begin
               ...
               Begin
                   ...
                   LinImp:=LinImp+80;
                   Coluna:=XQTDETQPAG-Round((Length(Trim(XLinAuxImp02))*Tamanho)/2);
                   Printer.Canvas.TextOut(COLUNA, Linha, XLinAuxImp02);
                   Linha:=Linha+XINTERVLIN;
               End;
           End
           ...
           Printer.Canvas.TextOut(COLUNA, Linha, '____________________________________________');
...
Begin
   ...
   Try
           ...
           Then Begin
               ...
               ImpCampo('Natureza', DMESTOQUE.TCFOP.FieldByName('DESCRICAO').AsString, 'S');
...
// 				 ImpCampo('IE Subs.', DMESTOQUE.TCFOP.FieldByName('DESCRICAO').AsString, 'S');
               ...
               then begin
                    ImpCampo('Nome', DMPESSOA.TCliente.FieldByName('COD_INTERNO').AsString+'-'+ DMPESSOA.TPessoaJ.FieldByName('RAZAO_SOCIAL').AsString, 'S');
               end
               else begin
               ...
               if FMenu.XPedFatura = ''
               then begin
                    ImpCampo('Data Emissao', DMSAIDA.TFiscPV.FieldByName('DTEMISSAO').AsString, 'S');
               end
               else begin
                    ImpCampo('Data Emissao', DateToStr(Date()), 'S');
               end;

               ImpCampo('Endereco', DMPESSOA.TPessoa.FieldByName('ENDERECO').AsString, 'S');
               ...
               if FMenu.XPedFatura = ''
               then begin
                    ImpCampo('Data Saida', DMSAIDA.TFiscPV.FieldByName('DTENTSAID').AsString, 'S');
               end
               else begin
               ...
               If Not DMGEOGRAFIA.TCidade.IsEmpty Then
                   ImpCampo('Municipio', DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString, 'S');
               ImpCampo('Fone', DMPESSOA.TPessoa.FieldByName('TELREL').AsString, 'S');
               If Not DMGEOGRAFIA.TEstado.IsEmpty Then
                   ImpCampo('UF', DMGEOGRAFIA.TEstado.FieldByName('UF_ESTADO').AsString, 'S');
               ...
               if FMenu.XPedFatura = ''
               then begin
                    ImpCampo('Hora Saida', DMSAIDA.TFiscPV.FieldByName('HORASAID').AsString, 'S');
               end
               else begin
               ...
               If (DMESTOQUE.TCFOP.FieldByName('DEVOLUCAO').AsString<>'1') OR (DMESTOQUE.TCFOP.FieldByName('DEVOLUCAO').AsString= NULL)
               Then Begin
                   ...
                   Then Begin
                       ...
                       Begin
                           ImpCampo('Fat. Vencimento 0'+IntToStr(I), DMCONTA.WParR.FieldByName('DTVENC').AsString, 'S');
                           ImpCampo('Fat. Valor 0'+IntToStr(I), DMCONTA.WParR.FieldByName('VALOR').AsString, 'M');
                           DMCONTA.WParR.Next;
                           I:=I+1;
                       End;
                       ...
                       While not DMCONTA.WParR.Eof Do
                       Begin
                           ImpCampo('Venc. Duplicata', DMCONTA.WParR.FieldByName('DTVENC').AsString, 'S');
                           ImpCampo('Valor Duplicata', DMCONTA.WParR.FieldByName('VALOR').AsString, 'M');
                           ...
                           DMCONTA.WParR.Next;
                           XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINRECIBO').AsInteger;
                       End;
                   End
                   Else Begin
                       ImpCampo('Fat. Vencimento 01', FormatDateTime('dd/mm/yyyy', Date())+ ' - À VISTA', 'S');
                       ImpCampo('Fat. Valor 01', DMSAIDA.TFiscPV.FieldByName('VLRTOTDOCNF').AsString, 'M');
                   End;
               End;
           End;
           If XTipoNF='OS'
...
///					ImpCampo('IE Subs.', DMESTOQUE.TCFOP.FieldByName('DESCRICAO').AsString, 'S');
               ...
               ImpCampo('CPF/CNPJ', DMPESSOA.TPessoa.FieldByName('CPFCNPJ').AsString, 'S');
               ImpCampo('Data Emissao', DMServ.TFiscOrd.FieldByName('DTEMISSAO').AsString, 'S');
               ImpCampo('Endereco', DMPESSOA.TPessoa.FieldByName('ENDERECO').AsString, 'S');
               ImpCampo('Bairro', DMPESSOA.TPessoa.FieldByName('BAIRRO').AsString, 'S');
               ImpCampo('CEP', DMPESSOA.TPessoa.FieldByName('CEP').AsString, 'S');
               ImpCampo('Data Saida', DMServ.TFiscOrd.FieldByName('DTENTSAID').AsString, 'S');
               If Not DMGEOGRAFIA.TCidade.IsEmpty Then
                   ImpCampo('Municipio', DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString, 'S');
               ImpCampo('Fone', DMPESSOA.TPessoa.FieldByName('TELREL').AsString, 'S');
               If Not DMGEOGRAFIA.TEstado.IsEmpty Then
               ...
               If Not DMPESSOA.TPessoaJ.IsEmpty Then
                   ImpCampo('IE', DMPESSOA.TPessoaJ.FieldByName('INSC_EST').AsString, 'S');
               ImpCampo('Hora Saida', DMServ.TFiscOrd.FieldByName('HORASAID').AsString, 'S');
               //IMPRIME DUPLICATAS EM FATURAMENTOS
               I:=1;
               ...
               DMCONTA.TParcCR.ParamByName('COD').AsString:=DMServ.TFiscOrd.FieldByName('cod_ordem').AsString;
               DMCONTA.TParcCR.Open;

                   DMCONTA.TParcCR.First;
                   While not DMCONTA.TParcCR.Eof Do
                   Begin
                       ImpCampo('Fat. Vencimento 0'+IntToStr(I), DMCONTA.TParcCR.FieldByName('DTVENC').AsString, 'S');
                       ImpCampo('Fat. Valor 0'+IntToStr(I), DMCONTA.TParcCR.FieldByName('VALOR').AsString, 'M');
                   ...
                   While not DMCONTA.TParcCR.Eof Do
                   Begin
                       ImpCampo('Venc. Duplicata', DMCONTA.TParcCR.FieldByName('DTVENC').AsString, 'S');
                       ImpCampo('Valor Duplicata', DMCONTA.TParcCR.FieldByName('VALOR').AsString, 'M');
...
///					ImpCampo('IE Subs.', DMESTOQUE.TCFOP.FieldByName('DESCRICAO').AsString, 'S');
               ...
               ImpCampo('CPF/CNPJ', DMPESSOA.TPessoa.FieldByName('CPFCNPJ').AsString, 'S');
               ImpCampo('Data Emissao', DMENTRADA.TFiscPC.FieldByName('DTEMISSAO').AsString, 'S');
               ImpCampo('Endereco', DMPESSOA.TPessoa.FieldByName('ENDERECO').AsString, 'S');
               ImpCampo('Bairro', DMPESSOA.TPessoa.FieldByName('BAIRRO').AsString, 'S');
               ImpCampo('CEP', DMPESSOA.TPessoa.FieldByName('CEP').AsString, 'S');
               ImpCampo('Data Saida', DMENTRADA.TFiscPC.FieldByName('dtentsaid').AsString, 'S');
               If Not DMGEOGRAFIA.TCidade.IsEmpty Then
                   ImpCampo('Municipio', DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString, 'S');
               ImpCampo('Fone', DMPESSOA.TPessoa.FieldByName('TELREL').AsString, 'S');
               If Not DMGEOGRAFIA.TEstado.IsEmpty Then
               ...
               If Not DMPESSOA.TPessoaJ.IsEmpty Then
                   ImpCampo('IE', DMPESSOA.TPessoaJ.FieldByName('INSC_EST').AsString, 'S');
               ImpCampo('Hora Saida', DMENTRADA.TFiscPC.FieldByName('HORASAID').AsString, 'S');
           End;
   Except
       Result:=False;
   End;
...
{         ImpCampo('Fat. Vencimento 0'+IntToStr(I), DMMACS.TMP.FieldByName('DESC1').AsString, 'S');
   ...
   Try
       ...
       if (XSQLTabela = 'ORDEM')Then Begin
       ...
       If (XTIPOPAG<>'Multiplo')
       Then Begin
           //Tenta encontras a Forma de pagamento
           If (XFORMAPAG='À VISTA')
           Then Begin
               DMMACS.TMP.Insert;
           ...
           Else Begin
           	...
           	DMCONTA.TCtaR.SQL.Add('select * from '+XTABELA+' where ('+XTABELA+'.cod_gerador=:CodGerador) and ('+XTABELA+'.tipogerador='+#39+XTIPOGER+#39+')');
               ...
               Then Begin
                   ...
                   DMCONTA.TParcCR.Close;
                   DMCONTA.TParcCR.SQL.Clear;
                   DMCONTA.TParcCR.SQL.Add('select * from '+XPARCELA+' where '+XPARCELA+'.'+XCODIGO+'=:ctareceber');
       ...
       Else Begin
           ...
           DMCAIXA.TAlx.Close;
           DMCAIXA.TAlx.SQL.Clear;
           DMCAIXA.TAlx.SQL.Add(' SELECT '+XPARCELA+'.valor, '+XPARCELA+'.dtvenc FROM '+XPARCELA);
           DMCAIXA.TAlx.SQL.Add(' LEFT JOIN '+XTABELA+' ON '+XPARCELA+'.'+XCODIGO+' = '+XTABELA+'.'+XCODIGO);
           DMCAIXA.TAlx.SQL.Add(' WHERE ('+XTABELA+'.cod_gerador=:Codigo) and (c'+XTABELA+'.tipogerador='+#39+XTIPOGER+#39+')');
           DMCAIXA.TAlx.ParamByName('codigo').AsInteger:=Cod_Ordem;
           DMCAIXA.TAlx.Open;
           If not DMCAIXA.TAlx.IsEmpty
           Then Begin
           ...
           DMCAIXA.TAlx.SQL.Add(' where (movbanco.tipogerador='+#39+XTIPOGER+#39+') and (movbanco.cod_gerador=cod_gerador) ');
           DMCAIXA.TAlx.ParamByName('codigo').AsInteger:=Cod_Ordem;
           DMCAIXA.TAlx.Open;
           If not DMCAIXA.TAlx.IsEmpty
           Then Begin
...
Begin
   ...
   Try
      ...
      DMMACS.TALX.SQL.Add(' Where ((cst.substituicao='+#39+'0'+#39+') or (cst.substituicao is null)) and (itprodord.cod_ordem=:codigo) ');
      DMMACS.TALX.ParamByName('CODIGO').AsInteger:=DMServ.TOrd.FieldByName('COD_ORDEM').AsInteger;
      DMMACS.TALX.Open;
      ...
      DMMACS.TALX.SQL.Add(' Select cst.substituicao from itprodord ');
      DMMACS.TALX.SQL.Add(' Left Join cst on itprodord.cod_cst = cst.cod_cst ');
      DMMACS.TALX.SQL.Add(' Where (cst.substituicao='+#39+'1'+#39+') and (itprodord.cod_ordem=:codigo) ');
      DMMACS.TALX.ParamByName('CODIGO').AsInteger:=DMServ.TOrd.FieldByName('COD_ORDEM').AsInteger;
      DMMACS.TALX.Open;
      ...
      If (XCSTNormal=true) and (XCstSubst=True)
      Then Begin
          ...
          If Fmenu.XEstadoOrigem<>Fmenu.XEstadoDestino Then
          		DMMACS.TALX.ParamByName('codigo').AsInteger:=DMMACS.TLoja.FieldByName('CFOPEXPERTOCST').AsInteger//para cfop fora do estado
          Else
          		DMMACS.TALX.ParamByName('codigo').AsInteger:=DMMACS.TLoja.FieldByName('CFOPINTERNOCST').AsInteger;//para cfop fora do estado
          DMMACS.TALX.Open;
          ...
          Else Begin
              ...
              ImpCampo('Natureza', DMESTOQUE.TCFOP.FieldByName('DESCRICAO').AsString+'/'+DMMACS.TALX.FieldByName('DESCRICAO').AsString, 'S');
              ImpCampo('CFOP', DMESTOQUE.TCFOP.FieldByName('CFOP').AsString+'/'+DMMACS.TALX.FieldByName('CFOP').AsString, 'S');
          End;
      End;

      //APENAS CFOP DE SUBSTITUIÇÃO
      If (XCSTNormal=False) and (XCstSubst=True)
      Then Begin
          ...
          Else Begin
          		//SE ENCONTROU O CFOP DE SUBSITUIÇÃO IMPRIME OS DOIS
              ImpCampo('Natureza', DMMACS.TALX.FieldByName('DESCRICAO').AsString, 'S');
...
Begin
   ...
   Try
       ...
       Begin
           ...
           FiltraTabela(DMESTOQUE.TEstoqueLote, 'ESTOQUELOTE', 'COD_ESTOQUE', DMServ.TPOrd.FieldByName('COD_ESTOQUE').AsString, '');
           ...
           ImpCampo('Cód. Produto', DMESTOQUE.TSubProd.FieldByName(DMMACS.TLoja.FieldByName('CODPRODREL1').AsString).AsString, 'S');
           ImpCampo('Produto', DMESTOQUE.WSimilar.FieldByName('DESCRICAO').AsString, 'S');
           ImpCampo('Desc. Prod.', DMServ.TPOrd.FieldByName('DESCONTO').AsString, 'M');
           ...
           End
           Else Begin
                ImpCampo('S.T. Prod.',  DMESTOQUE.TCST.FieldByName('COD_SIT_TRIB').AsString, 'S');
           End;
           ImpCampo('Unid. Prod.',  DMESTOQUE.TUnidade.FieldByName('SIGLA_UNID').AsString, 'S');
           ImpCampo('Quant. Prod.', DMServ.TPOrd.FieldByName('QTD').AsString, 'M');
           ImpCampo('Vlr. Unit. Prod.',  DMServ.TPOrd.FieldByName('vlrunit').AsString, 'M');
           ImpCampo('Vlt. Tot. Prod.',  DMServ.TPOrd.FieldByName('total').AsString, 'M');
           ImpCampo('Aliq. ICMS Prod.',  DMServ.TPOrd.FieldByName('aliqicms').AsString, 'M');
   ...
   Try
           ...
           Then Begin
               ...
               Begin
                   ...
                   //PARA CADA ITEM DE VENDA SELECIONA O LOTE REFERENTE
                   FiltraTabela(DMESTOQUE.TEstoqueLote, 'ESTOQUELOTE', 'COD_ESTOQUE', DMSAIDA.TItemPV.FieldByName('COD_ESTOQUE').AsString, '');
                   //PARA CADA ITEM DE VENDA SELECIONA O PRODUTO REFERENTE
                   FiltraTabela(DMESTOQUE.TProduto, 'PRODUTO', 'COD_PRODUTO', DMESTOQUE.WSimilar.FieldByName('COD_PRODUTO').AsString, '');
                   //PARA CADA ITEM DE VENDA SELECIONA O SUBPRODUTO REFERENTE
                   FiltraTabela(DMESTOQUE.TSubProd, 'SUBPRODUTO', 'COD_SUBPRODUTO', DMESTOQUE.WSimilar.FieldByName('COD_SUBPRODUTO').AsString, '');
                   ...
                   DMMACS.TLote.ParamByName('CODIGO').AsString:=DMSAIDA.TItemPV.FieldByName('COD_LOTE').AsString;
                   DMMACS.TLote.Open;
                   If Not DMMACS.TLote.IsEmpty Then
                       ImpCampo('Lote Prod.',IntToStr(DMMACS.TLote.FieldByName('NUMERO').AsInteger), 'S');
                   ImpCampo('Desc. Prod.', DMSAIDA.TItemPV.FieldByName('DESCPRO').AsString, 'M');
                   ImpCampo('CFOP Prod.',  DMESTOQUE.TCFOP.FieldByName('CFOP').AsString, 'S');
                   //Tratamento para cst
                   If DMESTOQUE.TCST.FieldByName('substituicao').AsString='1'
                   Then Begin
                   ...
                   End;
                   ImpCampo('Unid. Prod.',  DMESTOQUE.TUnidade.FieldByName('SIGLA_UNID').AsString, 'S');
                   ImpCampo('Quant. Prod.', DMSAIDA.TItemPV.FieldByName('QTDEPROD').AsString, 'M');
                   ImpCampo('Vlr. Unit. Prod.',  DMSAIDA.TItemPV.FieldByName('VALUNIT').AsString, 'M');
                   ImpCampo('Vlt. Tot. Prod.',  DMSAIDA.TItemPV.FieldByName('VALORTOTAL').AsString, 'M');
                   ImpCampo('Aliq. ICMS Prod.',  DMSAIDA.TItemPV.FieldByName('ALIQICMS').AsString, 'M');
                   XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
                   XQTDITEMIMPPAG:=XQTDITEMIMPPAG+1;
           ...
           Then Begin
               ...
               Begin
                   ...
                   //PARA CADA ITEM DE VENDA SELECIONA O LOTE REFERENTE
                   FiltraTabela(DMESTOQUE.TEstoqueLote, 'ESTOQUELOTE','COD_ESTOQUE',DMENTRADA.TItemPC.FieldByName('COD_ESTOQUE').AsString, '');
                   //PARA CADA ITEM DE VENDA SELECIONA O PRODUTO REFERENTE
                   FiltraTabela(DMESTOQUE.TProduto, 'PRODUTO', 'COD_PRODUTO', DMESTOQUE.WSimilar.FieldByName('COD_PRODUTO').AsString, '');
                   //PARA CADA ITEM DE VENDA SELECIONA O SUBPRODUTO REFERENTE
                   FiltraTabela(DMESTOQUE.TSubProd, 'SUBPRODUTO', 'COD_SUBPRODUTO', DMESTOQUE.WSimilar.FieldByName('COD_SUBPRODUTO').AsString, '');
                   //PARA CADA ITEM DE VENDA SELECIONA O CST REFERENTE
                   FiltraTabela(DMESTOQUE.TCST, 'CST', 'COD_CST', DMESTOQUE.TProduto.FieldByName('COD_CST').AsString, '');
                   //PARA CADA ITEM DE VENDA SELECIONA A UNIDADE REFERENTE
                   FiltraTabela(DMESTOQUE.TUnidade, 'UNIDADE', 'COD_UNIDADE', DMESTOQUE.TSubProd.FieldByName('COD_UNIDVENDA').AsString, '');
                   //Paulo 26/11/2010: Seleciona o numero do lote
                   FiltraTabela(DMMACS.TLote,'LOTE','COD_LOTE',IntToStr(DMESTOQUE.TEstoqueLote.FieldByName('COD_LOTE').AsInteger),''); 

                   ImpCampo('Cód. Produto', DMESTOQUE.TSubProd.FieldByName(DMMACS.TLoja.FieldByName('CODPRODREL1').AsString).AsString, 'S');
                   ImpCampo('Produto', DMESTOQUE.WSimilar.FieldByName('DESCRICAO').AsString, 'S');
                   ImpCampo('Lote Prod.',IntToStr(DMMACS.TLote.FieldByName('NUMERO').AsInteger), 'S');
                   ImpCampo('Desc. Prod.', DMENTRADA.TItemPC.FieldByName('DESCPRO').AsString, 'M');
                   ImpCampo('CFOP Prod.',  DMESTOQUE.TCFOP.FieldByName('CFOP').AsString, 'S');
                   ImpCampo('S.T. Prod.',  DMESTOQUE.TCST.FieldByName('COD_SIT_TRIB').AsString, 'S');
                   ImpCampo('Unid. Prod.',  DMESTOQUE.TUnidade.FieldByName('SIGLA_UNID').AsString, 'S');
                   ImpCampo('Quant. Prod.', DMENTRADA.TItemPC.FieldByName('QTDEPROD').AsString, 'M');
                   ImpCampo('Vlr. Unit. Prod.',  DMENTRADA.TItemPC.FieldByName('VALUNIT').AsString, 'M');
                   ImpCampo('Vlt. Tot. Prod.',  DMENTRADA.TItemPC.FieldByName('VALORTOTAL').AsString, 'M');
                   ImpCampo('Aliq. ICMS Prod.',  DMENTRADA.TItemPC.FieldByName('ALIQICMS').AsString, 'M');
                   XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
                   XQTDITEMIMPPAG:=XQTDITEMIMPPAG+1;
...
Result:=True;
   Try
           ...
           Then Begin
               ...
               While not DMESTOQUE.TDesp.Eof Do
               Begin
                   ImpCampo('Desc. Desp.', DMESTOQUE.TDesp.FieldByName('DESPESA').AsString, 'S');
                   ImpCampo('Vlr. Desc. Desp.', DMESTOQUE.TDesp.FieldByName('DESCONTO').AsString, 'M');

                   ImpCampo('Qnt. Desp.', DMESTOQUE.TDesp.FieldByName('QTD').AsString, 'M');
                   ImpCampo('Vlr. Unit. Desp.',  DMESTOQUE.TDesp.FieldByName('vlrfinal').AsString, 'M');
                   ImpCampo('Vlt. Tot. Desp.',  DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsString, 'M');
...
Begin
   ...
   Result:=True;
   Try
       ImpCampo('Base ICMS', DMServ.TFiscOrd.FieldByName('baseicms').AsString, 'M');
       ImpCampo('Vlr. ICMS', DMServ.TFiscOrd.FieldByName('vlricms').AsString, 'M');

        If FMenu.XEstadoOrigem<>FMenu.XEstadoDestino
        Then Begin
             ImpCampo('Base ICMS Subs.', DMServ.TFiscOrd.FieldByName('BASCALSUBS').AsString, 'M');
             ImpCampo('Vlr. ICMS Subs.', DMServ.TFiscOrd.FieldByName('VLRICMSUBS').AsString, 'M');
        End;
       ImpCampo('Vlr. Frete', DMServ.TFiscOrd.FieldByName('VLRFRETE').AsString, 'M');
       ImpCampo('Vlr. Seguro', DMServ.TFiscOrd.FieldByName('VLRSEG').AsString, 'M');
       ImpCampo('Outras Desp.', DMServ.TFiscOrd.FieldByName('VLROUTRASDESP').AsString, 'M');
       ImpCampo('Vlr Tot. IPI', DMServ.TFiscOrd.FieldByName('VLRTOTIPI').AsString, 'M');
       ImpCampo('Vlt Tot. NF', DMServ.TFiscOrd.FieldByName('vlrtotdocnf').AsString, 'M');

       //verifica e imprime decontos para produtos
       If XTotProdNF<>DMServ.tOrd.FieldByName('totprod').AsCurrency
       Then Begin
           ImpCampo('Linha Desc. Produto', 'Desconto em produtos R$ '+FormatFloat('0.00', (XTotProdNF-DMServ.tOrd.FieldByName('totprod').AsCurrency)), 'S');
           ImpCampo('Vlr. Tot. Prod.', FloatToStr((DMServ.TOrd.FieldByName('totprod').AsCurrency)), 'M');
       End
       Else Begin
       ...
       If XTotServNF<>DMServ.tOrd.FieldByName('totserv').AsCurrency
       Then Begin
           ImpCampo('Linha Desc. Serviço', 'Desconto em serviço R$ '+FormatFloat('0.00', (XTotServNF-DMServ.tOrd.FieldByName('totserv').AsCurrency)), 'S');
           ImpCampo('Vlr. Tot. Serv.', FloatToStr(DMServ.TOrd.FieldByName('totserv').AsCurrency), 'M');
       End
       Else Begin
       ...
       Then Begin
           ...
           ImpCampo('Vlr. ISS', DMServ.TFiscOrd.FieldByName('VLRTOTISS').AsString, 'M');
       End;
   Except
       Result:=False;
   End;
...
Begin
   ...
   Try
           If XTipoNF='SAIDA'
           Then Begin
               ImpCampo('Base ICMS', DMSAIDA.TFiscPV.FieldByName('baseicms').AsString, 'M');
               ImpCampo('Vlr. ICMS', DMSAIDA.TFiscPV.FieldByName('vlricms').AsString, 'M');

                If FMenu.XEstadoOrigem<>FMenu.XEstadoDestino
                Then Begin
                     ImpCampo('Base ICMS Subs.', DMSAIDA.TFiscPV.FieldByName('BASCALSUBS').AsString, 'M');
                     ImpCampo('Vlr. ICMS Subs.', DMSAIDA.TFiscPV.FieldByName('VLRICMSUBS').AsString, 'M');
                End
                Else Begin
					If XCalcImpostos=0
                   then Begin
                   ...
                   End;
                End;
                ImpCampo('Vlr. Frete', DMSAIDA.TFiscPV.FieldByName('VLRFRETE').AsString, 'M');
                ImpCampo('Vlr. Seguro', DMSAIDA.TFiscPV.FieldByName('VLRSEG').AsString, 'M');
                ImpCampo('Outras Desp.', DMSAIDA.TFiscPV.FieldByName('VLROUTRASDESP').AsString, 'M');
                ImpCampo('Vlr Tot. IPI', DMSAIDA.TFiscPV.FieldByName('VLRTOTIPI').AsString, 'M');
                ImpCampo('Vlt Tot. NF', DMSAIDA.TFiscPV.FieldByName('vlrtotdocnf').AsString, 'M');

               if FMenu.XPedFatura = '' //caso seja direto do pedido de venda
               then begin
                   DMESTOQUE.TCFOP.Close;
                   DMESTOQUE.TCFOP.SQL.Clear;
                   DMESTOQUE.TCFOP.SQL.Add('SELECT * FROM CFOP WHERE CFOP.COD_CFOP=:CODIGO');
                   DMESTOQUE.TCFOP.ParamByName('CODIGO').AsInteger:=DMSAIDA.TFiscPV.FieldByName('COD_CFOP').AsInteger;
                   ...
                   //verifica e imprime decontos para produtos
                   If XTotProdNF<>DMSAIDA.WPedV.FieldByName('VALOR').AsCurrency
                   Then Begin
                       If XCalcImpostos<>0
                       Then Begin
                           ImpCampo('Linha Desc. Produto', 'Desconto em produtos R$ '+FormatFloat('0.00', XTotProdNF-DMSAIDA.WPedV.FieldByName('VALOR').AsCurrency), 'S');
                           ImpCampo('Vlr. Tot. Prod.', FloatToStr(XTotProdNF-(XTotProdNF-DMSAIDA.WPedV.FieldByName('VALOR').AsCurrency)), 'M');
                       End
                       Else Begin
                           ImpCampo('Vlr. Tot. Prod.', FloatToStr(DMSAIDA.TFiscPV.FieldByName('VLRTOTPROD').AsCurrency), 'M');
                       End;
                   End
                   Else Begin
                       ImpCampo('Vlr. Tot. Prod.', FloatToStr(XTotProdNF), 'M');
               ...
               else begin //caso seja apartir do faturar
                   //verifica e imprime decontos para produtos
                   If XTotProdNF<>FMenu.XValorFatura
                   Then Begin
                       ImpCampo('Linha Desc. Produto', 'Desconto em produtos R$ '+FormatFloat('0.00', XTotProdNF-DMSAIDA.WPedV.FieldByName('VALOR').AsCurrency), 'S');
                       ImpCampo('Vlr. Tot. Prod.', FloatToStr(XTotProdNF-(XTotProdNF-DMSAIDA.WPedV.FieldByName('VALOR').AsCurrency)), 'M');
                   End
           ...
           Then Begin
               ...
               Then Begin
                   ImpCampo('Linha Desc. Produto', 'Desconto em produtos R$ '+FormatFloat('0.00', (XTotProdNF-DMServ.tOrd.FieldByName('totprod').AsCurrency)), 'S');
                   ImpCampo('Vlr. Tot. Prod.', FloatToStr(XTotProdNF-(DMServ.TOrd.FieldByName('totprod').AsCurrency)), 'M');
               End
               Else Begin
                   ImpCampo('Vlr. Tot. Prod.', FloatToStr(XTotProdNF), 'M');
               End;
           ...
           If XTipoNF='ENTRADA'
           Then Begin
               ImpCampo('Base ICMS', DMENTRADA.TFiscPC.FieldByName('BASEICMS').AsString, 'M');
               ImpCampo('Vlr. ICMS', DMENTRADA.TFiscPC.FieldByName('VLRICMS').AsString, 'M');
               ImpCampo('Base ICMS Subs.', DMENTRADA.TFiscPC.FieldByName('BASCALSUBS').AsString, 'M');
               ImpCampo('Vlr. ICMS Subs.', DMENTRADA.TFiscPC.FieldByName('VLRICMSUBS').AsString, 'M');
               ImpCampo('Vlr. Tot. Prod.', FloatToStr(XTotProdNF), 'M');
               ImpCampo('Vlr. Frete', DMENTRADA.TFiscPC.FieldByName('VLRFRETE').AsString, 'M');
               ImpCampo('Vlr. Seguro', DMENTRADA.TFiscPC.FieldByName('VLRSEG').AsString, 'M');
               ImpCampo('Outras Desp.', DMENTRADA.TFiscPC.FieldByName('VLROUTRASDESP').AsString, 'M');
               ImpCampo('Vlr Tot. IPI', DMENTRADA.TFiscPC.FieldByName('VLRTOTIPI').AsString, 'M');
               ImpCampo('Vlt Tot. NF', DMENTRADA.WPedc.FieldByName('VALOR').AsString, 'M');
               //verifica e imprime decontos para produtos
               If XTotProdNF<>DMENTRADA.WPedc.FieldByName('VALOR').AsCurrency
               Then Begin
                   ImpCampo('Linha Desc. Produto', 'Desconto em produtos R$ '+FormatFloat('0.00', XTotProdNF-DMENTRADA.WPedc.FieldByName('VALOR').AsCurrency), 'S');
               End;
           End;
   Except
       Result:=False;
...
Begin
   ...
   Then Begin
       ...
       ImpCampo('Frete por Conta', DMSAIDA.TFiscPV.FieldByName('FRETECONTA').AsString, 'S');
       ImpCampo('Placa Veiculo', DMSAIDA.TFiscPV.FieldByName('PLACA').AsString, 'S');
       ImpCampo('UF Veiculo', DMSAIDA.TFiscPV.FieldByName('UFPLACA').AsString, 'S');
       ImpCampo('CPF/CNPJ Transp.', DMPESSOA.TALX.FieldByName('CPFCNPJ').AsString, 'S');
       ImpCampo('Endereco Transp.', DMPESSOA.TALX.FieldByName('ENDERECO').AsString, 'S');
       ImpCampo('Municipio Transp.', DMPESSOA.TALX.FieldByName('CIDADE').AsString, 'S');
       ImpCampo('UF Transp.', DMPESSOA.TALX.FieldByName('UF').AsString, 'S');
       ImpCampo('IE Transp.', DMPESSOA.TALX1.FieldByName('insc_est').AsString, 'S');
       ImpCampo('Quant. Transp.', DMSAIDA.TFiscPV.FieldByName('QUANTFRETE').AsString, 'S');
       ImpCampo('Especie Transp.', DMSAIDA.TFiscPV.FieldByName('ESPECIEFRETE').AsString, 'S');
       ImpCampo('Marca Transp.', DMSAIDA.TFiscPV.FieldByName('MARCAFRETE').AsString, 'S');
       ImpCampo('Numero Transp.', DMSAIDA.TFiscPV.FieldByName('NUMCONHEC').AsString, 'S');
       ImpCampo('Peso Bruto Transp.', DMSAIDA.TFiscPV.FieldByName('PESOBRUTO').AsString, 'S');
       ImpCampo('Peso Liquido Transp.', DMSAIDA.TFiscPV.FieldByName('PESOLIQ').AsString, 'S');
   End;
   If XTipoNF='OS'
   Then Begin
       ImpCampo('Nome Transp.', DMPESSOA.TALX.FieldByName('NOME').AsString, 'S');
       ImpCampo('Frete por Conta', DMServ.TFiscOrd.FieldByName('FRETECONTA').AsString, 'S');
       ImpCampo('Placa Veiculo', DMServ.TFiscOrd.FieldByName('PLACA').AsString, 'S');
       ImpCampo('UF Veiculo', DMServ.TFiscOrd.FieldByName('UFPLACA').AsString, 'S');
       ImpCampo('CPF/CNPJ Transp.', DMPESSOA.TALX.FieldByName('CPFCNPJ').AsString, 'S');
       ImpCampo('Endereco Transp.', DMPESSOA.TALX.FieldByName('ENDERECO').AsString, 'S');
       ImpCampo('Municipio Transp.', DMPESSOA.TALX.FieldByName('CIDADE').AsString, 'S');
       ImpCampo('UF Transp.', DMPESSOA.TALX.FieldByName('UF').AsString, 'S');
       ImpCampo('IE Transp.', DMPESSOA.TALX1.FieldByName('insc_est').AsString, 'S');
       ImpCampo('Quant. Transp.', DMServ.TFiscOrd.FieldByName('QUANTFRETE').AsString, 'S');
       ImpCampo('Especie Transp.', DMServ.TFiscOrd.FieldByName('ESPECIEFRETE').AsString, 'S');
       ImpCampo('Marca Transp.', DMServ.TFiscOrd.FieldByName('MARCAFRETE').AsString, 'S');
       ImpCampo('Numero Transp.', DMServ.TFiscOrd.FieldByName('NUMCONHEC').AsString, 'S');
       ImpCampo('Peso Bruto Transp.', DMServ.TFiscOrd.FieldByName('PESOBRUTO').AsString, 'S');
       ImpCampo('Peso Liquido Transp.', DMServ.TFiscOrd.FieldByName('PESOLIQ').AsString, 'S');
   End;
   If XTipoNF='ENTRADA'
   Then Begin
       ImpCampo('Nome Transp.', DMPESSOA.TALX.FieldByName('NOME').AsString, 'S');
       ImpCampo('Frete por Conta', DMENTRADA.TFiscPC.FieldByName('FRETECONTA').AsString, 'S');
       ImpCampo('Placa Veiculo', DMENTRADA.TFiscPC.FieldByName('PLACA').AsString, 'S');
       ImpCampo('UF Veiculo', DMENTRADA.TFiscPC.FieldByName('UFPLACA').AsString, 'S');
       ImpCampo('CPF/CNPJ Transp.', DMPESSOA.TALX.FieldByName('CPFCNPJ').AsString, 'S');
       ImpCampo('Endereco Transp.', DMPESSOA.TALX.FieldByName('ENDERECO').AsString, 'S');
       ImpCampo('Municipio Transp.', DMPESSOA.TALX.FieldByName('CIDADE').AsString, 'S');
       ImpCampo('UF Transp.', DMPESSOA.TALX.FieldByName('UF').AsString, 'S');
       ImpCampo('IE Transp.', DMPESSOA.TALX1.FieldByName('insc_est').AsString, 'S');
       ImpCampo('Quant. Transp.', DMENTRADA.TFiscPC.FieldByName('QUANTFRETE').AsString, 'S');
       ImpCampo('Especie Transp.', DMENTRADA.TFiscPC.FieldByName('ESPECIEFRETE').AsString, 'S');
       ImpCampo('Marca Transp.', DMENTRADA.TFiscPC.FieldByName('MARCAFRETE').AsString, 'S');
       ImpCampo('Numero Transp.', DMENTRADA.TFiscPC.FieldByName('NUMCONHEC').AsString, 'S');
       ImpCampo('Peso Bruto Transp.', DMENTRADA.TFiscPC.FieldByName('PESOBRUTO').AsString, 'S');
       ImpCampo('Peso Liquido Transp.', DMENTRADA.TFiscPC.FieldByName('PESOLIQ').AsString, 'S');
   End;
End;
...
Begin
   ...
   End;
   If (XAux<>1) Then
       ImpCampo('Dados Adicionais', 'FOLHA: ' + IntToStr(XQTDPAGIMP)+'/'+IntToStr(XAux), 'S');
   ...
   If XTipoNF='SAIDA' Then
       FiltraTabela(DMSAIDA.TFiscPV, 'DOCFISSAIDA', 'COD_PEDIDO', IntToStr(XCodigoPedido), '');
       if FMenu.XPedFatura = ''
       then begin
     ...
     //       FMenu.MAux.Lines.Text:='Ped:'+FMenu.XPedFatura;
      ...
      Then Begin
          ...
          THEN BEGIN
            ...
            if FMenu.XEstadoOrigem = FMenu.XEstadoDestino
            then begin
                 DMESTOQUE.Alx.ParamByName('CODIGO').AsInteger:=DMMACS.TLoja.FieldByName('CFOPINTERNOCST').AsInteger;
            end
            else begin
                 DMESTOQUE.Alx.ParamByName('CODIGO').AsInteger:=DMMACS.TLoja.FieldByName('CFOPEXPERTOCST').AsInteger;
            end;
         END
         ELSE BEGIN
            DMESTOQUE.Alx.Close;
...
//             ImpCampo('Dados Adicionais', 'CFOP Subtituição Tributária: '+ DMESTOQUE.Alx.FieldByName('cfop').AsString+ ' - '+DMESTOQUE.Alx.FieldByName('DESCRICAO').AsString, 'S');
   ...
   If DMServ.TOrd.FieldByName('numfiscalaux').AsString<>''
   Then Begin
       ImpCampo('Dados Adicionais', 'Nota Fiscal Ref.: ' + DMServ.TOrd.FieldByName('numfiscalaux').AsString, 'S');
       XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
   End;
   ...
   If XTipoNF='SAIDA' Then
       ImpCampo('Nº NF Rodapé', DMSAIDA.TFiscPV.FieldByName('NUMDOCFIS').AsString, 'S');

   If (FMenu.XEncontrouST=True) and (DMESTOQUE.TCFOP.FieldByName('DEVOLUCAO').AsString<>'1')
   Then Begin
   ...
   If XTipoNF='OS'
   Then Begin
       ImpCampo('Nº NF Rodapé', DMServ.TFiscOrd.FieldByName('NUMDOCFIS').AsString, 'S');
   End;
   If XTipoNF='ENTRADA' Then
       ImpCampo('Nº NF Rodapé', DMENTRADA.TFiscPC.FieldByName('NUMDOCFIS').AsString, 'S');
...
//IMPRIME DADOS ADICIONAIS 01
   ...
   If DMMACS.TLoja.FieldByName('NFIMPOBS').AsString='1'
   Then Begin
   ...
   Then Begin
       ...
       Then Begin
			ImpCampo('Dados Adicionais', 'Placa: '+DMPESSOA.TEquip.FieldByName('PLACA').AsString+' - Frota: '+DMPESSOA.TEquip.FieldByName('NUM_FROTA').AsString+' - Km Atual:'+DMServ.TOrd.FieldByName('TOTKM').AsString, 'S');
           XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;           
       End;
   End;
   FMenu.MAux.Lines.Text:=DMServ.TFiscOrd.FieldByName('DADOSAD04').AsString;
       ...
       Then Begin
         ...
         then begin
          ...
          Then Begin
              ...
              DMESTOQUE.Alx.SQL.Clear;
              DMESTOQUE.Alx.SQL.Add(' SELECT * FROM CFOP WHERE CFOP.COD_CFOP=:CODIGO');
              DMESTOQUE.Alx.ParamByName('CODIGO').AsInteger:=DMMACS.TLoja.FieldByName('cod_cfopauto').AsInteger;
              DMESTOQUE.Alx.Open;
             { If Not DMESTOQUE.Alx.IsEmpty
              Then Begin
                   XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
              ...
              End; }
              XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
              ImpCampo('Reservado ao Fisco', 'Base de Cálculo ICMS S.T. R$ '+ DMServ.TFiscOrd.FieldByName('bascalsubs').AsString, 'S');
              XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
              XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;              
              ImpCampo('Reservado ao Fisco', 'Valor de ICMS S.T. R$ '+ DMServ.TFiscOrd.FieldByName('vlricmsubs').AsString, 'S');
          End;
       End;
       Result:=True;
     Except
         ...
         then begin
            ...
            Then Begin
               ...
               { If Not DMESTOQUE.Alx.IsEmpty
                ...
                End; }
                XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
                ImpCampo('Reservado ao Fisco', 'Base de Cálculo ICMS S.T. : R$ '+ DMSAIDA.TFiscPV.FieldByName('baseicms').AsString, 'S');
                XNUMLIN:=XNUMLIN+DMMACS.TLoja.FieldByName('NFENTLINPROD').AsInteger;
                ImpCampo('Reservado ao Fisco', 'Valor de ICMS S.T. : R$ '+ DMSAIDA.TFiscPV.FieldByName('vlricms').AsString, 'S');
            End;
...
Begin
   ...
   Then Begin
           Result:=True;
           If FiltraTabela(DMServ.TOrd, 'ordem', 'cod_ordem', IntToStr(COD_PEDIDO), '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O Pedido de Venda relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           //DOCUMENTO FISCAL DEVENDAS
           If FiltraTabela(DMServ.TFiscOrd, 'docfisord', 'cod_ordem', IntToStr(COD_PEDIDO), '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O Documento Fiscal de Venda relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
...
//           	If FiltraTabela(DMSAIDA.TItemPV, 'ITENSPEDVEN', 'COD_PEDVEN', IntToStr(COD_PEDIDO), ' Order by ')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'Os Itens de Venda relacionados não foram encontrados!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           ...
           //ClIENTE DE VENDA
           If FiltraTabela(DMPESSOA.TCliente, 'CLIENTE', 'COD_CLIENTE', DMServ.TOrd.FieldByName('COD_CLIENTE').AsString , '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O Cliente relacionado com o documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           ...
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'A Pessoa relacionado ao cliente do documento fiscal não foi encontrada!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           ...
           DMPESSOA.TALX.ParamByName('COD_FORNEC').AsInteger:=DMServ.TFiscOrd.FieldByName('COD_TRANSP').AsInteger;
           DMPESSOA.TALX.Open;
           ...
           DMPESSOA.TALX1.ParamByName('COD_PESSOA').AsInteger:=DMPESSOA.TALX.FieldByName('COD_PESSOA').AsInteger;
           DMPESSOA.TALX1.Open;

           //CFOP DE VENDA
           If FiltraTabela(DMESTOQUE.TCFOP, 'CFOP', 'COD_CFOP', DMServ.TFiscOrd.FieldByName('COD_CFOP').AsString , '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O C.F.O.P. relacionado ao documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           ...
           //Ctas a Receber
           If FiltraTabela(DMCONTA.TCtaR, 'CTARECEBER', 'COD_GERADOR', DMServ.TFiscOrd.FieldByName('cod_ordem').AsString, ' AND (CTARECEBER.TIPOGERADOR='+#39+'ORDEM'+#39+')')=True
           Then Begin
               //DUPLICATAS DE VENDA
   ...
   Then Begin
           ...
           if FMenu.XPedFatura = ''
           then begin
               If FiltraTabela(DMSAIDA.TPedV, 'PEDVENDA', 'COD_PEDVENDA', IntToStr(COD_PEDIDO), '')=False
               Then Begin
                   Mensagem('A T E N Ç Ã O!', 'O Pedido de Venda relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
                   Printer.Abort;
                   Result:=False;
           ...
           If FMenu.XPedFatura = ''
           then begin
               If FiltraTabela(DMSAIDA.TFiscPV, 'DOCFISSAIDA', 'COD_PEDIDO', IntToStr(COD_PEDIDO), '')=False
               Then Begin
                   Mensagem('A T E N Ç Ã O!', 'O Documento Fiscal de Venda relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
                   Printer.Abort;
                   Result:=False;
               ...
               End;
                ...
                DMMACS.TDocFiscFatura.SQL.Add('where docfiscfatura.cod_pedido in ('+FMenu.XPedFatura+')');
                DMMACS.TDocFiscFatura.Open;

                DMSAIDA.TFiscPV.Close;
                DMSAIDA.TFiscPV.SQL.Clear;
                DMSAIDA.TFiscPV.SQL.Add('select * from docfissaida');
                DMSAIDA.TFiscPV.SQL.Add('where docfissaida.cod_docfisc = '+DMMACS.TDocFiscFatura.fieldByName('cod_docfiscal').AsString);
                DMSAIDA.TFiscPV.Open;

               If DMSAIDA.TFiscPV.IsEmpty
               Then Begin
                   Mensagem('A T E N Ç Ã O!', 'O Documento Fiscal de Venda relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
                   Printer.Abort;
                   Result:=False;
           ...
           then begin
                ...
                DMSAIDA.TItemPV.SQL.Add('where itenspedven.cod_pedven in ('+FMenu.XPedFatura+')');
           end;
...
//           	If FiltraTabela(DMSAIDA.TItemPV, 'ITENSPEDVEN', 'COD_PEDVEN', IntToStr(COD_PEDIDO), ' Order by ')=False
           ...
           if FMenu.XPedFatura = ''
           then begin
               If FiltraTabela(DMPESSOA.TCliente, 'CLIENTE', 'COD_CLIENTE', DMSAIDA.TPedV.FieldByName('COD_CLIENTE').AsString , '')=False
               Then Begin
                   Mensagem('A T E N Ç Ã O!', 'O Cliente relacionado com o documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
                   Printer.Abort;
                   Result:=False;
               ...
               If FiltraTabela(DMPESSOA.TCliente, 'CLIENTE', 'COD_CLIENTE', FMenu.XCodCLiFatura , '')=False
               Then Begin
                   Mensagem('A T E N Ç Ã O!', 'O Cliente relacionado com o documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
                   Printer.Abort;
                   Result:=False;
           ...
           DMPESSOA.TALX.SQL.Add('         left join estado on cidade.cod_estado = estado.cod_estado');
           DMPESSOA.TALX.SQL.Add('			where vwfornec.cod_fornec=:COD_FORNEC ');
           DMPESSOA.TALX.ParamByName('COD_FORNEC').AsInteger:=DMSAIDA.TFiscPV.FieldByName('COD_TRANSP').AsInteger;
           DMPESSOA.TALX.Open;
           ...
           IF FMenu.XPedFatura = ''
           Then Begin
               If FiltraTabela(DMESTOQUE.TCFOP, 'CFOP', 'COD_CFOP', DMSAIDA.TFiscPV.FieldByName('COD_CFOP').AsString , '')=False
               Then Begin
                   Mensagem('A T E N Ç Ã O!', 'O C.F.O.P. relacionado ao documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
                   Printer.Abort;
                   Result:=False;
           ...
           then begin
                ...
                If FiltraTabela(DMCONTA.TCtaR, 'CTARECEBER', 'NUMDOC', IntToStr(DMMACS.TLoja.FieldByName('PROXDOCCTAREC').AsInteger-1), ' AND (CTARECEBER.TIPOGERADOR='+#39+'PEDVENDA'+#39+')')=True
                Then Begin
                     //DUPLICATAS DE VENDA
                     If FiltraTabela(DMCONTA.WParR, 'vwparcr', 'COD_CTARECEBER', DMCONTA.TCtaR.FieldByName('COD_CTARECEBER').AsString, '')=True
                     Then Begin
   ...
   Then Begin
           Result:=True;
           If FiltraTabela(DMENTRADA.TPedC, 'PEDCOMPRA', 'cod_pedcomp', IntToStr(COD_PEDIDO), '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O Pedido de Compra relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           //DOCUMENTO FISCAL DE COMPRAS
           If FiltraTabela(DMENTRADA.TFiscPC, 'DOCFIS', 'COD_PEDIDO', IntToStr(COD_PEDIDO), '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O Documento Fiscal de Compra relacionado não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
...
//           	If FiltraTabela(DMSAIDA.TItemPV, 'ITENSPEDVEN', 'COD_PEDVEN', IntToStr(COD_PEDIDO), ' Order by ')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'Os Itens de Compra relacionados não foram encontrados!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           ...
           XQTDITEM:=DMESTOQUE.Alx.FieldByName('QTDITEM').AsInteger;
           //ClIENTE DE VENDA
           If FiltraTabela(DMPESSOA.TFornecedor, 'FORNECEDOR', 'COD_FORNEC', DMENTRADA.TPedC.FieldByName('COD_FORNEC').AsString , '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O Fornecedor relacionado com o documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
               Exit;
           End;
           ...
           DMPESSOA.TALX.SQL.Add('         left join estado on cidade.cod_estado = estado.cod_estado');
           DMPESSOA.TALX.SQL.Add('			where vwfornec.cod_fornec=:COD_FORNEC ');
           DMPESSOA.TALX.ParamByName('COD_FORNEC').AsInteger:=DMENTRADA.TFiscPC.FieldByName('COD_TRANSP').AsInteger;
           DMPESSOA.TALX.Open;
           ...
           //CFOP DE VENDA
           If FiltraTabela(DMESTOQUE.TCFOP, 'CFOP', 'COD_CFOP', DMENTRADA.TFiscPC.FieldByName('COD_CFOP').AsString , '')=False
           Then Begin
               Mensagem('A T E N Ç Ã O!', 'O C.F.O.P. relacionado ao documento fiscal não foi encontrado!'+#13+'A impressao será cancelada!', '' , 1, 1, false, 'i');
               Printer.Abort;
               Result:=False;
...
Begin
    ...
    If RecalcFiscalOs('OS',COD_PEDIDO)=false
    Then begin
    ...
    Else Begin
       ...
       Then Begin
           Try
               ...
               End;
           Except
               MessageDlg('Não foi possível imprimir a Nota Fiscal do Pedido.', mtWarning, [mbOK], 0);
               Result := false;
    ...
    If PadraoImp(DMMACS.TLoja.FieldByName('NFIMP').AsString)=False
    Then Begin
    ...
    Try
       ...
       Then Begin
           ...
           If Mensagem('ATENÇÃO', 'Deseja cancelar a impressão corrente?', '', 1, 1, False, 'c')=2 Then
               Printer.Abort;
       End;
    Except
       Mensagem('ATENÇÃO', 'Ocorreu algum problema com o mecanismo ou instalação da impressora!', '', 1, 1, False, 'a');
       Exit;
    End;
    Try
    ...
    Try
       ...
       Then Begin
           ...
           Then Begin
               ...
               DMESTOQUE.Alx.ParamByName('NUMERO').AsString:=DMMACS.TEmpresa.FieldByName('PROX_NF').AsString;
               DMESTOQUE.Alx.ParamByName('codigo').AsString:=DMServ.TFiscOrd.FieldByName('cod_docfiscord').AsString;
               DMESTOQUE.Alx.Open;
               If not DMESTOQUE.Alx.IsEmpty
               Then Begin
                   Mensagem('ATENÇÃO', 'O processo de impressão da nota fiscal foi cancelado!'+#13+'A Numeração indicada para impressão ('+DMMACS.TEmpresa.FieldByName('PROX_NF').AsString+') já foi utilizada. Altere a numeração nas configurações do sistema', '', 1, 1, false, 'a');
                   Printer.Abort;
                   Exit;
               End;
               DMServ.TFiscOrd.Edit;
               DMServ.TFiscOrd.FieldByName('NUMDOCFIS').AsString:=DMMACS.TEmpresa.FieldByName('PROX_NF').AsString;
           ...
           Else Begin
               ...
               If FiltraTabela(DMServ.TFiscOrd, 'docfisord', 'cod_ordem', IntToStr(COD_PEDIDO), '')=True Then
       ...
       End
       Else Begin
           Mensagem('ATENÇÃO', 'As informações de configuração da empresa não foram encontrados.', '', 1, 1, false, 'a');
           Printer.Abort;
           Exit;
       End;
       ...
       Then Begin
           ...
           If Mensagem('CONFIRMAÇÃO', 'Deseja atribuir uma nova numeração para a nota fiscal e continuar a impressão?'+#13+'Nova numeração: '+DMMACS.TEmpresa.FieldByName('PROX_NF').AsString, '', 1, 1, false, 'a')=2
           Then Begin
               DMServ.TFiscOrd.Edit;
               DMServ.TFiscOrd.FieldByName('NUMDOCFIS').AsString:=DMMACS.TEmpresa.FieldByName('PROX_NF').AsString;
               DMServ.TFiscOrd.Post;
       ...
       Try
         ...
         Then Begin
           If (XQTDPAG<(XQTDDESP/DMMACS.TLoja.FieldByName('NFITMAXDESP').AsInteger))
           Then Begin
				XQTDPAG:=(Round(XQTDDESP/DMMACS.TLoja.FieldByName('NFITMAXDESP').AsInteger));
               If XQTDPAG<(XQTDDESP/DMMACS.TLoja.FieldByName('NFITMAXDESP').AsInteger)
               Then Begin
                   XQTDPAG:=XQTDPAG+1;
               End; 
       ...
       Then Begin
          ...
          then begin
              Result:=True;
              If FiltraTabela(DMServ.TOrd, 'ORDEM', 'COD_ORDEM', IntToStr(COD_PEDIDO), '') = True
              Then Begin
                 ...
                 End;
                 DMServ.TOrd.FieldByName('NUMFISCAL').AsString:=DMServ.TFiscOrd.FieldByName('NUMDOCFIS').AsString;
                 DMServ.TOrd.Post;
                 DMServ.IBT.CommitRetaining;
              End
              Else Begin
       ...
       Else Begin
           Mensagem('A T E N Ç Ã O', 'Verifique se a Nota Fiscal foi devidamente impresso. Este pedido não foi marcado como impresso, entre em contato com a equipe de suporte!'+#13+'Verifique o arquivo C:\erronf.txt', '', 1, 1, False, 'a');
       End;
    Except
       Mensagem('ATENÇÃO', 'Por favor, verifique a impressora!', '', 1, 1, False, 'a');
       Printer.EndDoc;
       Exit;
    End;
End;
...
Begin
   Try
       ...
       Then Begin
           // função para recalcular nota fiscal
           If RecalcFiscal('PV',COD_PEDIDO)=false
           Then begin
               Mensagem('   A T E N Ç Ã O   ','Erro de cálculo fiscal. Tente novamente, mas se o problema persistir, entre em contato com suporte.','',1,1,false,'I');
               Result := false;
               Exit;
           ...
           Try
               ...
               Then Begin
                   ...
                   Then Begin
                       ...
                       End
                       Else Begin
                           XNumNotaFiscal:=DMSAIDA.TFiscPV.FieldByName('NUMDOCFIS').AsString;
                       End;
                   End;
                   If XTipoNF='OS'
                   Then Begin
                   ...
                   Then Begin
                       ...
                       DMENTRADA.TFiscPC.FieldByName('NUMDOCFIS').AsString:=DMMACS.TEmpresa.FieldByName('PROX_NF').AsString;
               ...
               then begin
                   If XTipoNF='SAIDA' Then
                       FiltraTabela(DMSAIDA.TFiscPV, 'DOCFISSAIDA', 'COD_PEDIDO', IntToStr(COD_PEDIDO), '');
                   If XTipoNF='OS' Then
                       FiltraTabela(DMServ.TFiscOrd, 'docfisord', 'cod_ordem', IntToStr(COD_PEDIDO), '');
                   If XTipoNF='ENTRADA' Then
                       FiltraTabela(DMENTRADA.TFiscPC, 'DOCFIS', 'COD_PEDIDO', IntToStr(COD_PEDIDO), '');
               end
               else begin
                   If XTipoNF='SAIDA' Then
                       DMSAIDA.TAlx.Close;
                       ...
                       FiltraTabela(DMSAIDA.TFiscPV, 'DOCFISSAIDA', 'COD_DOCFISC', DMSAIDA.TAlx.fieldByName('cod_docfiscal').AsString, '');
               end;
               //ALEX: 09/03/2009 - VERIFICA SE O NUMERO DA NOTA FISCAL NÃO ESTA VAZIA
               If XNumNotaFiscal=''
               Then Begin
               ...
               Else Begin
                   XQTDPAG := (Round(XQTDITEM/DMMACS.TLoja.FieldByName('NFITMAXPROD').AsInteger));
                   If XQTDPAG<(XQTDITEM/DMMACS.TLoja.FieldByName('NFITMAXPROD').AsInteger) Then
                       XQTDPAG:=XQTDPAG+1;
               End;
               FMenu.XIMPFISC:=True;
               ...
               Then Begin
                  ...
                  then begin
                      ...
                      Then Begin
                       Result:=True;
                       If FiltraTabela(DMSAIDA.TPedV, 'PEDVENDA', 'COD_PEDVENDA', IntToStr(COD_PEDIDO), '') = True
                       Then Begin
                           //se filtra o pedido impresso mara-lo
                           DMSAIDA.TPedV.Edit;
                      ...
                      tHEN Begin
                       Result:=True;
                       If FiltraTabela(DMENTRADA.TPedC, 'PEDCOMPRA', 'COD_PEDCOMP', IntToStr(COD_PEDIDO), '') = True
                       Then Begin
                           //se filtra o pedido impresso mara-lo
                           DMENTRADA.TPedC.Edit;
                           DMENTRADA.TPedC.FieldByName('FISCO').AsString:='NF';
                           DMENTRADA.TPedC.FieldByName('NUMFISCAL').AsString:=DMENTRADA.TFiscPC.FieldByName('NUMDOCFIS').AsString;
...
Begin
    ...
    Then Begin
       ...
       XINFPROCON:= DMMACS.TLoja.fieldbyname('www_procon').AsString + ' ' +DMMACS.TLoja.fieldbyname('fone_procon').AsString + ' ' + DMMACS.TLoja.fieldbyname('end_procon').AsString;
       If XINFPROCON <> '  '
       Then Begin
           If Length(XINFPROCON)>47
           Then Begin
               If Length(XINFPROCON)>94
               Then Begin
       ...
       If length(DMESTOQUE.Alx.FieldByName('NOMECLI').AsString) >= 37
       Then Begin
           XNOMECLI := 'Cliente: ' + copy(DMESTOQUE.Alx.FieldByName('NOMECLI').AsString,1,37) + '.';
       End
       Else Begin
           XNOMECLI := 'Cliente: ' + ConcatEspacoDir(DMESTOQUE.Alx.FieldByName('NOMECLI').AsString,39);
       End;
       XMENS:=XMENS+XNOMECLI+XINFPROCON;

       // FUNÇÃO PARA FECHAR CUPOM
       if ecfFecha_Cupom(XMENS) = false
       then begin
           Exit;
       end;
    End
    Else Begin
       ...
       If length(DMESTOQUE.Alx.FieldByName('NOMECLI').AsString) >= 38
       Then Begin
           XNOMECLI := 'Cliente: ' + copy(DMESTOQUE.Alx.FieldByName('NOMECLI').AsString,1,38) + '.';
       End
       Else Begin
           XNOMECLI := 'Cliente: ' + ConcatEspacoDir(DMESTOQUE.Alx.FieldByName('NOMECLI').AsString,39);
       End;
...
VAR
   ...
   I: Integer;//Contador
   XVLRTOTCUPOM, XVlrProdCupom: Real; //armazena o total do cupom em arredondamento
   XVTTRUNCCUPOM, XVTTRUNCPROD, XQTDTRUNC, XVLRUNITTRUNC: Extended; ////armazena o total do cupom em truncamento
Begin
   FMenu.XIMPFISC:=True;
   ...
   Then Begin
       ...
       If ((TipoEmissor = 'ORDEM') And (Not DMESTOQUE.TDesp.IsEmpty))
       Then Begin
            XENCONTRAITEM := True;
       End;
   End;
   ...
   If XENCONTRAITEM=False
   Then Begin
       Mensagem('A T E N Ç Ã O', 'Não foram encontrados itens a serem impressos!', '', 1, 1, False, 'I');
       ECF:=False;
       Exit;
   End;
   ...
   Then Begin
       Mensagem('A T E N Ç Ã O', ' Os dados de cabeçalho não foram encontrados!', '', 1, 1, False, 'I');
       ECF:=False;
       Exit;
   End;
   ...
   If (MODELO_IMPRESSORA = '')
   THEN BEGIN
       Mensagem(' A T E N Ç Ã O ', 'Não há nenhuma impressora selecionada.', '', 1, 1, False, 'E');
       Exit;
   END;
   ...
   // função abre porta
   if ecfAbrePorta=false
   Then Begin
       If (MODELO_IMPRESSORA <> 'SWEDA')
       THEN BEGIN
           Mensagem('   A T E N Ç Ã O   ', 'Erro ao abrir porta.', '', 1, 1, False, 'E');
           Exit;
       END;
       EXIT;
   End
   Else Begin
       ...
       Else Begin
             ...
             Begin
               ...
               Then Begin
                   ...
                   XVLRUNITTRUNC:=DMESTOQUE.TSlave.FieldByName('TOTAL').AsCurrency/DMESTOQUE.TSlave.FieldByName('QTD').AsCurrency;
               End
               Else Begin
               ...
               if ecfVendeItem(DMESTOQUE.TSlave.FieldByName('CODPROD').AsString, DMESTOQUE.TSlave.FieldByName('DESCRICAO').AsString, DMESTOQUE.TSlave.FieldByname('QTD').AsString, FormatFloat('0.00', XVLRUNIT), DMESTOQUE.TSlave.FieldByName('indiceecf').AsString, DMESTOQUE.TSlave.FieldByName('UNIDADE').AsString, '3', FormatFloat('0.00', XDESCPROD)) = 0 Then
                   Exit;

               If DMMACS.TLoja.FieldByName('ECFTRUNC').AsInteger=1
               Then Begin
                   XVlrProdCupom:=(DMESTOQUE.TSlave.FieldByName('QTD').AsCurrency*XVLRUNIT)-XDESCPROD;
                   XVLRTOTCUPOM:=XVLRTOTCUPOM+XVlrProdCupom;
               End
               Else Begin
                   XVTTRUNCPROD:= Trunc(((DMESTOQUE.TSlave.FieldByName('QTD').AsCurrency*XVLRUNITTRUNC)-XDESCPROD) * 100) / 100;
             ...
             Then Begin
                   ...
                   Begin
                       ...
                       XDESCPROD:=(DMESTOQUE.TDesp.FieldByName('vlrfinal').AsCurrency*DMESTOQUE.TDesp.FieldByName('QTD').AsCurrency)-DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency;
                       If XDESCPROD<0
                       Then Begin
                           XDESCPROD:=0;
                           //Caso o desconto seja negativo significa que foi cobrado um vlr superior ao do banco de dados, então o vlr unitário deve ser corrigido
                           XVLRUNIT:=DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency/DMESTOQUE.TDesp.FieldByName('QTD').AsCurrency;
                           XVLRUNITTRUNC:=DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency/DMESTOQUE.TDesp.FieldByName('QTD').AsCurrency;
                       End
                       Else Begin
                       ...
                       // Função Vende Item - A função ecfAliquota informa o indice aliquota conforme o produto - as funções Numero Inteiro e Casas Decimais retornam a quant. com 7 numeros -
                      if ecfVendeItem(DMESTOQUE.TDesp.FieldByName('COD_DESPADIC').AsString, DMESTOQUE.TDesp.FieldByName('DESPESA').AsString, DMESTOQUE.TDesp.FieldByName('QTD').AsString, FormatFloat('0.00', XVLRUNIT), DMMACS.TLoja.FieldByName('ECFINDICEALIQSERV').AsString, DMESTOQUE.TDesp.FieldByName('UNIDADE').AsString, '0', FormatFloat('0.00', XDESCPROD))= 0 Then
                           Exit;

                       If DMMACS.TLoja.FieldByName('ECFTRUNC').AsInteger=1
                       Then Begin
                           XVlrProdCupom:=(DMESTOQUE.TDesp.FieldByName('QTD').AsCurrency*XVLRUNIT)-XDESCPROD;
                           XVLRTOTCUPOM:=XVLRTOTCUPOM+XVlrProdCupom;
                       End
                       Else Begin
                           XVTTRUNCPROD:= Trunc(((DMESTOQUE.TDesp.FieldByName('QTD').AsCurrency*XVLRUNITTRUNC)-XDESCPROD) * 100) / 100;
                           XVTTRUNCCUPOM:=XVTTRUNCCUPOM+XVTTRUNCPROD;
                       End;
                       DMESTOQUE.TDesp.Next;
                   End;
       ...
       Then Begin
           ...
           If (FiltraTabela(DMSAIDA.TPedV, 'PEDVENDA', 'COD_PEDVENDA', IntToStr(COD_EMISSOR), '') = True ) Or (FiltraTabela(DMServ.TOrd,'ORDEM','COD_ORDEM', IntToStr(COD_EMISSOR),'') = True)
           Then Begin
           ...
           End
           Else Begin
               Mensagem('A T E N Ç Ã O', 'O pedido não foi marcado como impresso.', '', 1, 1, False, 'a');
           End;
       End
       Else Begin
           Mensagem('A T E N Ç Ã O', 'Verifique se o Cupom foi devidamente impresso. Este pedido não foi marcado como impresso, entre em contato com a equipe de suporte!', '', 1, 1, False, 'a');
       End;
	End;
End;
...
Begin
   ...
   End
   Else Begin
       XTOTDESC:=StrToFloat(formatFloat('0.00', XTOTDESC))-StrToFloat(formatFloat('0.00',DMESTOQUE.Alx.FieldByName('TOTAL').AsCurrency));
   End;
...
Begin
   DMMACS.TLoja.Edit;
   DMMACS.TLoja.FieldByName('TMP1').AsString:='Consultado:'+#13+'Lançamento de Contas a Receber/Pagar'+#13+'Lançamentos em Banco e Cheques';
   DMMACS.TLoja.Post;
   DMMACS.Transaction.CommitRetaining;
   ...
   DMCONTA.TAlx.Close;
   DMCONTA.TAlx.SQL.Clear;
   DMCONTA.TAlx.SQL.Add('SELECT Distinct extract(month from VWPARCR.dtvenc) AS MES, extract(year from VWPARCR.dtvenc) AS ANO from VWPARCR WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL))  and (VWPARCR.dtvenc Is not Null)');
   DMCONTA.TAlx.Open;
   ...
   Try
       DMCONTA.TAlx.Close;
       DMCONTA.TAlx.SQL.Clear;
       DMCONTA.TAlx.SQL.Add('SELECT Distinct extract(month from VWPARCP.dtvenc) AS MES, extract(year from VWPARCP.dtvenc) AS ANO from VWPARCP WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL))  and (VWPARCP.dtvenc Is not Null)');
       DMCONTA.TAlx.Open;
       DMCONTA.TAlx.RecordCount;
       DMCONTA.TAlx.First;
   Except
   ...
   DMCONTA.TAlx.Close;
   DMCONTA.TAlx.SQL.Clear;
   DMCONTA.TAlx.SQL.Add('SELECT Distinct extract(month from vwchequerec.dtvenc) AS MES, extract(year from vwchequerec.dtvenc) AS ANO from vwchequerec  WHERE (FECH<>'+#39+'S'+#39+') or (FECH is null)  and (vwchequerec.dtvenc Is not Null) ');
   DMCONTA.TAlx.Open;
   DMCONTA.TAlx.First;
   ...
   DMCONTA.TAlx.Close;
   DMCONTA.TAlx.SQL.Clear;
   DMCONTA.TAlx.SQL.Add('SELECT Distinct extract(month from vwmovbanco.dtvenc) AS MES, extract(year from vwmovbanco.dtvenc) AS ANO from vwmovbanco  WHERE (FECH<>'+#39+'S'+#39+') or (FECH is null)  and (vwmovbanco.dtvenc Is not Null)');
   DMCONTA.TAlx.Open;
   DMCONTA.TAlx.First;
   ...
   Begin
       ...
       DMMACS.TALX.Close;
       DMMACS.TALX.SQL.Clear;
       DMMACS.TALX.SQL.Add('SELECT SUM(VWPARCP.VALOR) AS TOTAL FROM VWPARCP WHERE (COD_LOJA = :CODLOJA) AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) AND (extract(month FROM vwparcp.dtvenc)=:MES) AND (extract(YEAR FROM vwparcp.dtvenc)=:ANO) AND ( vwparcp.dtvenc Is not Null) ');
       DMMACS.TALX.ParamByName('CODLOJA').AsString:=FMenu.LCODLOJA.Caption;
       DMMACS.TALX.ParamByName('MES').AsString:=DMMACS.TMP.FieldByName('INT1').AsString;
       DMMACS.TALX.ParamByName('ANO').AsString:=DMMACS.TMP.FieldByName('DESC2').AsString;
       DMMACS.TALX.Open;
       DMMACS.TMP.Edit;
       DMMACS.TMP.FieldByName('VLR1').AsCurrency:=DMMACS.TALX.FieldByName('TOTAL').AsCurrency;
       DMMACS.TMP.Post;
       ///////////////////////
       ...
       DMMACS.TALX.Close;
       DMMACS.TALX.SQL.Clear;
       DMMACS.TALX.SQL.Add('SELECT SUM(VWPARCR.VALOR) AS TOTAL FROM VWPARCR WHERE (COD_LOJA = :CODLOJA) AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) AND (extract(month FROM vwparcr.dtvenc)=:MES) AND (extract(YEAR FROM vwparcr.dtvenc)=:ANO) AND (vwparcr.dtvenc Is not Null) ');
       DMMACS.TALX.ParamByName('CODLOJA').AsString:=FMenu.LCODLOJA.Caption;
       DMMACS.TALX.ParamByName('MES').AsInteger:=DMMACS.TMP.FieldByName('INT1').AsInteger;
       DMMACS.TALX.ParamByName('ANO').AsString:=DMMACS.TMP.FieldByName('DESC2').AsString;
       DMMACS.TALX.Open;
       DMMACS.TMP.Edit;
       DMMACS.TMP.FieldByName('VLR2').AsCurrency:=DMMACS.TALX.FieldByName('TOTAL').AsCurrency;
       DMMACS.TMP.Post;
       /////////////////////////
...
//           DMMACS.TALX.SQL.Add('SELECT SUM(VWPARCR.VALOR)     AS TOTAL FROM VWPARCR     WHERE ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) AND (extract(month FROM vwparcr.dtvenc    )=:MES) AND (extract(YEAR FROM vwparcr.dtvenc    )=:ANO) AND (vwparcr.dtvenc     Is not Null) ');
       ...
       DMMACS.TALX.SQL.Add(' WHERE ((vwchequerec.FECH<>'+#39+'S'+#39+')  OR (vwchequerec.FECH IS NULL)) AND (extract(month FROM vwchequerec.dtvenc)=:MES) AND (extract(YEAR FROM vwchequerec.dtvenc)=:ANO) AND (vwchequerec.dtvenc IS NOT NULL) and ((movbanco.cod_gerador<0) or (movbanco.cod_gerador is null))');
       DMMACS.TALX.ParamByName('MES').AsInteger:=DMMACS.TMP.FieldByName('INT1').AsInteger;
       DMMACS.TALX.ParamByName('ANO').AsString:=DMMACS.TMP.FieldByName('DESC2').AsString;
       DMMACS.TALX.Open;
       DMMACS.TMP.Edit;
       DMMACS.TMP.FieldByName('VLR2').AsCurrency:=DMMACS.TMP.FieldByName('VLR2').AsCurrency+DMMACS.TALX.FieldByName('TOTAL').AsCurrency;
       DMMACS.TMP.Post;
       /////////////////////////////
       ...
       DMMACS.TALX.SQL.Add(' WHERE ((movbanco.FECH<>'+#39+'S'+#39+')  OR (movbanco.FECH IS NULL)) AND (extract(month FROM vwmovbanco.dtvenc)=:MES) AND (extract(YEAR FROM vwmovbanco.dtvenc)=:ANO) AND (vwmovbanco.tipoop='+#39+'ENTRADA'+#39+') and ((movbanco.cod_gerador<0) or (movbanco.cod_gerador is null))');
       DMMACS.TALX.ParamByName('MES').AsInteger:=DMMACS.TMP.FieldByName('INT1').AsInteger;
       DMMACS.TALX.ParamByName('ANO').AsString:=DMMACS.TMP.FieldByName('DESC2').AsString;
       DMMACS.TALX.Open;
       DMMACS.TMP.Edit;
       ...
       DMMACS.TALX.Close;
       DMMACS.TALX.SQL.Clear;
       DMMACS.TALX.SQL.Add('SELECT SUM(vwmovbanco.valor) AS TOTAL FROM vwmovbanco WHERE ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) AND (extract(month FROM vwmovbanco.dtvenc)=:MES) AND (extract(YEAR FROM vwmovbanco.dtvenc)=:ANO) AND (vwmovbanco.tipoop='+#39+'SAIDA'+#39+')');
       DMMACS.TALX.ParamByName('MES').AsInteger:=DMMACS.TMP.FieldByName('INT1').AsInteger;
       DMMACS.TALX.ParamByName('ANO').AsString:=DMMACS.TMP.FieldByName('DESC2').AsString;
       DMMACS.TALX.Open;
       DMMACS.TMP.Edit;
       DMMACS.TMP.FieldByName('VLR1').AsCurrency:=DMMACS.TMP.FieldByName('VLR1').AsCurrency+DMMACS.TALX.FieldByName('TOTAL').AsCurrency;
       DMMACS.TMP.Post;
       DMMACS.TMP.Next;
   End;
   DMMACS.Transaction.CommitRetaining;
   ...
   Else
       Mensagem('NOTIFICAÇÃO AO USUÁRIO', 'Relatório retonou em branco!', '', 1, 1, False, 'a');                  
End;
...
begin
   ...
   BEGIN
       ...
       If XLOOP=1 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[+ DE 120 DIAS] - Vcidas. a + de 120 dias [<='+DateToStr(DT1)+']';
       If XLOOP=2 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[120 DIAS] - Vcidas. a 120 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=3 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[90 DIAS] - Vcidas. a 90 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=4 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[60 DIAS] - Vcidas. a 60 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=5 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[30 DIAS] - Vcidas. a 30 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=6 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[30 DIAS] - Vmento. em 30 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=7 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[60 DIAS] - Vmento. em 60 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=8 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[90 DIAS] - Vmento. em 90 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=9 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[120 DIAS] - Vmento em 120 dias [>='+DateToStr(DT1)+']/[<='+DateToStr(DT2)+']';
       If XLOOP=10 Then
           DMMACS.TMP.FieldByName('DESC1').AsString:='[+ de 120 DIAS] - Vmento em + de 120 dias [>='+DateToStr(DT1)+']';
       ...
       Begin
           ...
           DMMACS.TMP.FieldByName('COD_TMP').AsInteger:=I;
           I:=I+1;
           DMMACS.TMP.FieldByName('DESC2').AsString:=DMMACS.TALX.FieldByName('NUMPARC').AsString;
           DMMACS.TMP.FieldByName('DESC1').AsString:=DMMACS.TALX.FieldByName('NOME').AsString;
           DMMACS.TMP.FieldByName('DATA1').AsString:=DMMACS.TALX.FieldByName('DTVENC').AsString;
           DMMACS.TMP.FieldByName('DATA2').AsString:=DMMACS.TALX.FieldByName('DTLANC').AsString;
       ...
       Then Begin
           ...
           DMMACS.TALX.ParamByName('DT2').AsDate:=DT2;
       End;
       DMMACS.TALX.SQL.Add(' and (vwparcp.cod_loja=:CODLOJA)  AND (vwparcp.fech<>'+#39+'S'+#39+')');
...
begin
   ...
   BEGIN
       ...
       Then Begin
           ...
           // retorna o numero de clientes que estão com conta atrasada em um determinado periodo
           DMMACS.TMP.FieldByName('INT1').AsInteger := RetornaNumCliente(DateToStr(DT1),'');
           // retorna o numero de fornecedores que estão no ctas a pagar
           DMMACS.TMP.FieldByName('INT2').AsInteger := RetornaNumFornecedores(DateToStr(DT1),'');
           DMMACS.TMP.Post;
       End;
       If XLOOP=2
       Then Begin
           ...
           DMMACS.TMP.FieldByName('DESC1').AsString:='[>='+DateToStr(DT1)+' E <='+DateToStr(DT2)+']';
           // retorna o numero de clientes que estão com conta atrasada em um determinado periodo
           DMMACS.TMP.FieldByName('INT1').AsInteger := RetornaNumCliente(DateToStr(DT1),DateToStr(DT2));
           // retorna o numero de fornecedores que estão no ctas a pagar
           DMMACS.TMP.FieldByName('INT2').AsInteger := RetornaNumFornecedores(DateToStr(DT1),DateToStr(DT2));
           DMMACS.TMP.Post;
       End;
       If XLOOP=3
       Then Begin
       ...
       Then Begin
           DMMACS.TMP.FieldByName('COD_TMP').AsInteger:=InserReg(DMMACS.TMP, 'TMP', 'COD_TMP');
           DMMACS.TMP.FieldByName('DESC2').AsString:='60 DIAS ';
           DMMACS.TMP.FieldByName('DESC1').AsString:='[>='+DateToStr(DT1)+'/<='+DateToStr(DT2)+']';
           // retorna o numero de clientes que estão com conta atrasada em um determinado periodo
           DMMACS.TMP.FieldByName('INT1').AsInteger := RetornaNumCliente(DateToStr(DT1),DateToStr(DT2));
           // retorna o numero de fornecedores que estão no ctas a pagar
           DMMACS.TMP.FieldByName('INT2').AsInteger := RetornaNumFornecedores(DateToStr(DT1),DateToStr(DT2));
       ...
       Then Begin
           ...
           DMMACS.TMP.FieldByName('DESC1').AsString:='[>='+DateToStr(DT1)+']';
           // retorna o numero de clientes que estão com conta atrasada em um determinado periodo
           DMMACS.TMP.FieldByName('INT1').AsInteger := RetornaNumCliente('',DateToStr(DT2));
           // retorna o numero de fornecedores que estão no ctas a pagar
           DMMACS.TMP.FieldByName('INT2').AsInteger := RetornaNumFornecedores('',DateToStr(DT2));
           DMMACS.TMP.Post;
       End;
       //FILTRA CONTAS A RECEBER
       DMMACS.TALX.Close;
       ...
       DMMACS.TMP.Edit;
       DMMACS.TMP.FieldByName('VLR1').AsCurrency:=DMMacs.TALX.FieldByName('TOTAL').AsCurrency;
       DMMACS.TMP.FieldByName('VLR3').AsCurrency:=DMMACS.TALX.FieldByName('TOTAL').AsCurrency;
       DMMACS.TMP.Post;
...
Begin
   FiltraTabela(DMMACS.TLoja, 'LOJA', 'COD_LOJA', FMenu.LCODLOJA.Caption, '');
   If FiltraTabela(DMMACS.TEtqProd, 'ETQPROD', 'COD_ETQPROD', DMMACS.TLoja.FieldByName('COD_ETQPROD').AsString, '')=True
   Then Begin
       ...
       XTXTPERSONALIZADO:=DMMACS.TEtqProd.FieldByName('TEXTOPERSONALIZADO').AsString;
   End
   Else Begin
       Mensagem('A T E N Ç Ã O', 'Não existe configuração para impressão da etiqueta. Verifique em configuração de loja!', '', 1, 1, true, 'i');
       Exit;
   End;
End;
...
Begin
   FiltraTabela(DMMACS.TLoja, 'LOJA', 'COD_LOJA', FMenu.LCODLOJA.Caption, '');
   If FiltraTabela(DMMACS.TConfigDup, 'CONFIGDUP', 'COD_CONFIGDUP', DMMACS.TLoja.FieldByName('COD_DUPPERSONAL').AsString, '')=True
   Then Begin
   ...
   End
   Else Begin
       Mensagem('A T E N Ç Ã O', 'Não existe configuração para impressão da duplicata. Verifique em configuração de loja!', '', 1, 1, true, 'i');
       Exit;
   End;
End;
...
Begin
   FiltraTabela(DMMACS.TLoja, 'LOJA', 'COD_LOJA', FMenu.LCODLOJA.Caption, '');
   If FiltraTabela(DMMACS.TEtqProd, 'ETQPROD', 'COD_ETQPROD', DMMACS.TLoja.FieldByName('COD_IMPMAT').AsString, '')=True
   Then Begin
       XINTERVCOL:=DMMACS.TEtqProd.FieldByName('INTERVCOL').AsInteger;
       XINTERVLIN:=DMMACS.TEtqProd.FieldByName('INTERVLINHA').AsInteger;
       XCOMPCODBARRA:=DMMACS.TEtqProd.FieldByName('COMPCODBARRA').AsInteger;
   ...
   End
   Else Begin
       Mensagem('A T E N Ç Ã O', 'Não existe configuração para impressão do comprovante. Verifique em configuração de loja!', '', 1, 1, true, 'i');
       Exit;
   End;
End;
...
BEGIN
   ...
   NomeImp:='';
   Try
       If IMPRESSAO='ETIQPROD'
       Then Begin //SETAR IMPRESSORA PARA IMPRESSÃO DE ETIQUETAS DE PRODUTO\
       ...
       Begin
           If (Copy(FMenu.ListBox.Items[I], 1, Tam) = NomeImp)
           Then Begin
       ...
       If Flag=1
       Then Begin
           Mensagem('A T E N Ç Ã O', 'Falha ao setar impressora padrão para esta operação!', 'O sistema macs permite configurar um tipo de impressora para cada tipo de operações de impressão. Verifique, se o nome da impressora cadastrada na loja para esta operação, corresponde ao nome de alguma impressora instalada no sistema operacional' , 1, 1, true, 'i');
           SetImp:=False;
       End
       Else Begin
           SetImp:=True;
...
Begin
   ...
   Try
       Printer.BeginDoc;
           ...
           Begin
               ...
               If QTD>=0 Then
                   QtdEt:=QTD
               Else
                   QtdEt:=DMESTOQUE.TRel.FieldByName('QTD').AsInteger+DMESTOQUE.TRel.FieldByName('QTDEEXP').AsInteger;
               ...
               Begin
                   ...
                   If DMMACS.TLoja.FieldByName('IEPRECO').AsString='1'
                   Then Begin
                       If DMESTOQUE.TRel.FieldByName('PRECO').AsString<>'' Then
                           Printer.Canvas.TextOut(Coluna+5, 10, 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').Value))
                       Else
                           Printer.Canvas.TextOut(Coluna+5, 10, 'R$ 0,00');
                   End;
                   Printer.Canvas.Font.Size:=6;
                   //IMPRIME CTRL INTERNO
                   Printer.Canvas.TextOut(Coluna+5, 120, DMESTOQUE.TRel.FieldByName('CONTRINT').AsString);
                   //Verifica se deve Imprimir descricao
                   If DMMACS.TLoja.FieldByName('IEDESC').AsString='1' Then
                       Printer.Canvas.TextOut(Coluna+5, 140, DMESTOQUE.TRel.FieldByName('DESCR').AsString);
                   ...
                   Then Begin
                       ...
                       Then Begin//imprimir codigo de barra
                           CodBarraImp:=DMESTOQUE.TRel.FieldByName('CODBARRA').AsString;
                       End
                       Else Begin//codigo do fab
                           CodBarraImp:=DMESTOQUE.TRel.FieldByName('CODBARRA').AsString;
                       End
                   End
                   Else Begin
                   ...
                   If Length(TRIM(CodBarraImp))=13
                   Then Begin//IMPRIME EM FORMATO EAN 13
                   ...
                   End;
                   If Length(TRIM(CodBarraImp))=8
                   Then Begin//IMPRIME EM FORMATO EAN 8
       ...
       Printer.enddoc;
   Except
       Mensagem('A T E N Ç Ã O!', 'Falha ao imprimir!'+#13+'Verifique se a impressora esta ligada e devidamente configurada.', '' , 1, 1, false, 'i');
   End;
End;
               ...
               Begin
                   ...
                   Then Begin
                       ...
                       Else
                           Printer.Canvas.TextOut(Coluna+5, 10, 'R$ 0,00');
                   End;
                   If DMMacs.Tloja.FieldByName('IEPRECOPRAZO').AsInteger=1
                   Then Begin
                       If DMESTOQUE.TRel.FieldByName('PRECOPRAZO').AsString<>'' Then
                           Printer.Canvas.TextOut(Coluna+200, 10, 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECOPRAZO').Value))
                       Else
                           Printer.Canvas.TextOut(Coluna+200, 10, 'R$ 0,00');
                   End;
                   Printer.Canvas.Font.Size:=6;
                   //IMPRIME CTRL INTERNO
                   Printer.Canvas.TextOut(Coluna, 120, DMESTOQUE.TRel.FieldByName('CONTRINT').AsString);
                   //Verifica se deve Imprimir descricao
                   If DMMACS.TLoja.FieldByName('IEDESC').AsString='1' Then
                       Printer.Canvas.TextOut(Coluna+5, 140, Copy(DMESTOQUE.TRel.FieldByName('DESCR').AsString, 1, 35));
   ...
   Try
       ...
       begin
           ...
           begin
               ...
               Printer.Canvas.Font.Size:=8;
               //melhor coluna é 10
               Printer.Canvas.TextOut(1, 2, Copy(DMESTOQUE.TRel.FieldByName('DESCR').AsString, 0, 16));//Descrição do item
               if Trim(DMESTOQUE.TRel.FieldByName('COR').AsString)='' Then
                   Printer.Canvas.TextOut(1, 52, 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECOPRAZO').AsCurrency)) //Valor a Prazo
               else
                   Printer.Canvas.TextOut(1, 52, 'RX '+ Copy(DMESTOQUE.TRel.FieldByName('COR').AsString, 0, 16));//rx
               Printer.Canvas.TextOut(1, 135, Copy(DMESTOQUE.TRel.FieldByName('ESPECIFICACAO').AsString, 0, 16));//Especificação
               Printer.Canvas.TextOut(1, 180, 'REF.:'+Copy(DMESTOQUE.TRel.FieldByName('contrint').AsString, 0, 12));//controle interno
...
begin
   ...
   If (InttoStr(XTAMFONTE) = '') or (InttoStr(XINTERVLIN) = '') or (InttoStr(XLINHAINI) = '') or (InttoStr(XLARGPAG) = '') or (InttoStr(XALTPAG) = '')
   Then Begin
       Mensagem('   A T E N Ç Ã O   ', 'As configurações de páginas não estão atualizadas. Verifique:'+#13+'* Tamanho da fonte;'+#13+'* Entre linhas;'+#13+'* Linha inicial'+#13+'* Largura ou altura da página;', '' , 1, 1, false, 'E');
       Result := false;
       Exit;
   End
   Else
...
begin
   ...
   DMMACS.TConfigCheque.ParamByName('codigo').AsString := DMMACS.TLoja.FieldByName('cod_configcheque').AsString;
...
begin
   ...
   DMBANCO.TAlx.ParamByName('codigo').AsString := DMMACS.TConfigCheque.FieldByName('cod_configcheque').AsString;
   DMBANCO.TAlx.Open;
end;
...
Var
   ...
   X : string; // variavel usada para preencher o restante da linha logo apos o valor por extenso
   num_cheque: integer;
   XLinha : integer;
begin
//   Try
       ...
       If (GetWindowsVersion='Windows XP') Or  (GetWindowsVersion='Windows 2000') Or  (GetWindowsVersion='Windows NT')
       Then Begin
           SetPrinterPage_Xp(DMPAPER_USER, XLARGPAG, XALTPAG)
       End
       Else Begin
...
{   Except
   ...
   Try}
       ...
       Begin
           ...
           Then Begin
               ...
               Begin
                   ...
                   Printer.Canvas.Font.Name:= DMMACS.TItensCheque.FieldByName('NOME_FONTE').AsString;
                   Printer.Canvas.Font.Size:=DMMACS.TItensCheque.FieldByName('TAMANHO_FONTE').AsInteger;

                   //BUSCA VALOR A SER IMPRESSO
                   If DMBANCO.TAlx.FieldByName('CAMPO').AsString = 'Valor extenso'
                   Then Begin
                       // Formata valor extenso
                       FMenu.Extenso.Numero:=FormatFloat('0.00', StrToFloat(DMESTOQUE.TRel.FieldByName('VALOR').AsString));
                   ...
                   If DMBANCO.TAlx.FieldByName('CAMPO').AsString='Valor' Then
                       XVlrCampo:= 'R$ '+FormatFloat('0.00', StrToFloat(DMESTOQUE.TRel.FieldByName('VALOR').AsString));
                   ...
                   If DMBANCO.TAlx.FieldByName('CAMPO').AsString='Dia'
                   Then Begin
                       If Length(IntToStr(Day)) = 1
                       Then Begin
                       ...
                       End;
                   End;
                   If DMBANCO.TAlx.FieldByName('CAMPO').AsString='Mês'
                   Then Begin
                       If Length(IntToStr(Month)) = 1
                       Then Begin
                   ...
                   Printer.Canvas.TextOut(COLIMP+DMBANCO.TAlx.FieldByName('COLUNA').AsInteger, Linha+DMBANCO.TAlx.FieldByName('LINHA').AsInteger, COPY(XVlrCampo, 1, DMBANCO.TAlx.FieldByName('TAMANHO_CAMPO').AsInteger));
               DMBANCO.TAlx.Next;
               End;
           End
           Else Begin
               Mensagem('   A T E N Ç Ã O   ', 'Os itens não estão configurados para impressão de cheque.', '' , 1, 1, false, 'E');
               Exit;
           End;
       ...
       End;
       Printer.enddoc;
      	Mensagem('   A T E N Ç Ã O   ', 'cheque(s) enviados para impressora.', '' , 1, 1, false, 'a');
{   Except
       Mensagem('A T E N Ç Ã O!', 'Falha ao imprimir!'+#13+'Verifique se a impressora esta ligada e devidamente configurada.'+#13+'Se o problema continuar reinicie o computador e a impressora.', '' , 1, 1, false, 'i');
   End;}
end;
...
Procedure PrnEtiqPerson02(EAN13: Boolean; CODBAR: INTEGER; XQTD: Integer);
Var
   xNomeImpressoraPadrao, xNomeImpressoraEtiqueta: String;//recebe nome da impressora padrão
   QtdEt, NumCop, ContEt: Integer;
   Linha, Tamanho, LimImp: Integer;
   Coluna: Integer;
   CodBarraImp: String; // O código a ser impresso em formato barra (Cód. barra ou Fabricante)
   COLIMP: Integer;
   XVlrCampo: String;// Armazena temporariamente o valor do campo a ser impresso
Begin
   //RECEBE VALORES CONFIGURADOS EM BANCO
   Try
       ...
       xNomeImpressoraPadrao := GetDefaultPrinterName;//capturamos o nome da impressora padro atual para retornar ap/s operaço
       ...
       DMMACS.TALX.ParamByName('codigo').AsString:=DMMACS.TLoja.FieldByName('COD_ETQPROD').AsString;
       DMMACS.TALX.Open;
       if not DMMACS.TALX.IsEmpty
       Then Begin
           if DMMACS.TALX.FieldByName('NOMEIMPRESSORA').AsString <> ''
           Then Begin
               xNomeImpressoraEtiqueta := DMMACS.TALX.FieldByName('NOMEIMPRESSORA').AsString;
               mudaimpressoraPadrao(xNomeImpressoraEtiqueta);   //se temos uma impressora configurada para etiquetas, setamos ela como padro
           End
           Else Begin
               FMenu.PrintDialog1.Execute;
       ...
       End
       Else Begin
           MessageDlg('Nenhuma configuração de etiqueta foi encontrada', mtWarning, [mbOK], 0);
       End;
       ...
       ContEt:=0;
   Except
       Mensagem('A T E N Ç Ã O!', 'Falha ao coletar dados da impressora!'+#13+'Verifique se a impressora esta ligada e devidamente configurada.'+#13+'Se o problema continuar reinicie o computador e a impressora.', '' , 1, 1, false, 'i');
   End;
   Try 
       Printer.BeginDoc;
           Printer.canvas.pen.width:=5;
           ...
           Begin
               ...
               Begin
                   ...
                   If ContEt>=XQTDETQPAG
                   Then Begin
                       ContEt:=0;
                       Coluna:=1;
                       Linha:=XLINHAINI;
                       MessageDlg('Nova Pagina:'+#13+#10+''+#13+#10+'Linha Código: ' + IntToStr(Linha)+#13+'Linha config: '+DMMACS.TALX.FieldByName('LINHA').AsString, mtWarning, [mbOK], 0);
                       Printer.NewPage;
                   End;
                   If Coluna>XQTDCOLPAG
                   Then Begin
                       Linha:=Linha+XINTERVLIN;
                       LimImp:=LimImp+1;
                       Coluna:=1;
                   ...
                   DMMACS.TALX.ParamByName('CODETQ').AsString:=DMMACS.TLoja.FieldByName('COD_ETQPROD').AsString;
                   DMMACS.TALX.Open;
                   If Not DMMACS.TALX.IsEmpty
                   Then Begin
                       DMMACS.TALX.First;
                       While not DMMACS.TALX.Eof do
                       Begin
                           Printer.Canvas.Font.Name:=DMMACS.TALX.FieldByName('NOMEFONTE').AsString;
                           Printer.Canvas.Font.Size:=DMMACS.TALX.FieldByName('FONTE').AsInteger;
                           //BUSCA VALOR A SER IMPRESSO
                           If DMMACS.TALX.FieldByName('CAMPO').AsString='Cód. Fabricante' Then
                               XVlrCampo:=DMESTOQUE.TRel.FieldByName('CODFAB').AsString;
                           If DMMACS.TALX.FieldByName('CAMPO').AsString = 'Data'
                           Then Begin
                               // subbstitui o caracter '/' (barra) por espaço e copia somente o mes e ano
                               XVlrCampo := Copy(SubstituiCaracter(DMESTOQUE.TRel.FieldByName('DTCAD').AsString,'/',''),3,Length(DMESTOQUE.TRel.FieldByName('DTCAD').AsString));
                           End;
                           ...
                           If DMMACS.TALX.FieldByName('CAMPO').AsString='Preço Vista'
                           Then begin
                               if DMMACS.TLoja.FieldByName('DESCETIQUETA').AsInteger = 1 then
                                   XVlrCampo:= 'À VISTA: R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').AsCurrency)
                               else
                                   XVlrCampo:= 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').AsCurrency);
                           end;
                           If DMMACS.TALX.FieldByName('CAMPO').AsString='Preço Prazo'
                           Then begin
                               if DMMACS.TLoja.FieldByName('DESCETIQUETA').AsInteger = 1 then
                                   XVlrCampo:= 'À PRAZO: R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECOPRAZO').AsCurrency)
                               else
                                   XVlrCampo:= 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECOPRAZO').AsCurrency);
                           end;
                           ...
                           Then Begin
                               XVlrCampo:=DMESTOQUE.TRel.FieldByName('LARGURA').AsString+' x '+DMESTOQUE.TRel.FieldByName('ALTURA').AsString;
                           End;
                           ...
                           If DMMACS.TALX.FieldByName('CAMPO').AsString = 'Texto personalizado' Then
                               XVlrCampo := XTXTPERSONALIZADO;
                               
                           //Verifica c campo comum ou tipo barra
                           If DMMACS.TALX.FieldByName('CAMPO').AsString<>'Barras'
                           Then Begin
                               Printer.Canvas.TextOut(COLIMP+DMMACS.TALX.FieldByName('COLUNA').AsInteger, Linha+DMMACS.TALX.FieldByName('LINHA').AsInteger, COPY(XVlrCampo, 1, DMMACS.TALX.FieldByName('TAMANHO').AsInteger));
                           End
                           Else Begin
                               CodBarraImp:=DMESTOQUE.TRel.FieldByName('CODBARRA').AsString;
                               If (Length(TRIM(CodBarraImp))=13)
                               Then Begin//IMPRIME EM FORMATO EAN 13
                                   ...
                                   FMenu.PrnEan13.LinhaPrinter	 := Linha+DMMACS.TALX.FieldByName('LINHA').AsInteger;
                                   FMenu.PrnEan13.ColunaPrinter := COLIMP+DMMACS.TALX.FieldByName('COLUNA').AsInteger;
                                   FMenu.PrnEan13.Execute;
                               End;
                               If Length(TRIM(CodBarraImp))=8
                               Then Begin//IMPRIME EM FORMATO EAN 8
                                   ...
                                   FMenu.PrnEan8.LinhaPrinter	:= Linha+DMMACS.TALX.FieldByName('LINHA').AsInteger;
                                   FMenu.PrnEan8.ColunaPrinter := COLIMP+DMMACS.TALX.FieldByName('COLUNA').AsInteger;
                                   FMenu.PrnEan8.Execute;
                               End;
                               If (Length(TRIM(CodBarraImp))<>8) AND (Length(TRIM(CodBarraImp))<>12)
                               Then Begin
                                   ...
                                   FMenu.PrnCodBarra.LinhaPrinter	:= Linha+DMMACS.TALX.FieldByName('LINHA').AsInteger;
                                   FMenu.PrnCodBarra.ColunaPrinter := COLIMP+DMMACS.TALX.FieldByName('COLUNA').AsInteger;
                   ...
                   End
                   Else Begin
                       Mensagem('A T E N Ç Ã O!', 'Verifique as configurações de etiqueta!', '' , 1, 1, false, 'i');
                       Exit;
                   End;
                   //atualiza o nº de cópias impressas
                   NumCop:=NumCop+1;
   ...
   Try
       Printer.BeginDoc;
           ...
           Then Begin//neste caso deve ser impresso as parcelas ateh a dt selencionaa
               ...
               DMESTOQUE.TRel.SQL.Add('And (parcelacr.DTVENC<=:DTVENC)  AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) ');
               DMESTOQUE.TRel.ParamByName('CODCTARECEBER').AsInteger:=XCODCTA;
               DMESTOQUE.TRel.ParamByName('DTVENC').AsDateTime:=DMCONTA.WParR.FieldByName('DTVENC').AsDateTime;
           End;
           If XTIPO=3
           Then Begin//neste caso deve ser impresso as parcelas ateh a parcela selecionada
               DMESTOQUE.TRel.SQL.Add('Where (PARCELACR.MARK='+#39+'I'+#39+') ');
           End;
           DMESTOQUE.TRel.SQL.Add(' AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) ');
           DMESTOQUE.TRel.Open;
           ...
           Begin
               ...
               Begin
                   ...
                   DMMACS.TALX.ParamByName('CODETQ').AsString:=DMMACS.TLoja.FieldByName('cod_duppersonal').AsString;
                   DMMACS.TALX.Open;
                   If Not DMMACS.TALX.IsEmpty
                   Then Begin
                       DMMACS.TALX.First;
                       ...
                       Begin
                           ...
                           If DMMACS.TALX.FieldByName('CAMPO').AsString='Juro' Then
                               XVlrCampo:=FormatFloat('0.00', DMMACS.TLoja.FieldByName('JUROS').AsCurrency);
                           ...
                           If DMMACS.TALX.FieldByName('CAMPO').AsString='Valor' Then
                               XVlrCampo:= 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('valor').AsCurrency);
                           If DMMACS.TALX.FieldByName('CAMPO').AsString='Textos'
                           Then Begin
                           ...
                           If DMMACS.TALX.FieldByName('TAMANHO').AsInteger<>0 Then
                               Printer.Canvas.TextOut(COLIMP+DMMACS.TALX.FieldByName('COLUNA').AsInteger, Linha+DMMACS.TALX.FieldByName('LINHA').AsInteger, COPY(XVlrCampo, 1, DMMACS.TALX.FieldByName('TAMANHO').AsInteger))
                           Else
                               Printer.Canvas.TextOut(COLIMP+DMMACS.TALX.FieldByName('COLUNA').AsInteger, Linha+DMMACS.TALX.FieldByName('LINHA').AsInteger, XVlrCampo);
                           DMMACS.TALX.next;
                       End;
                   End
                   Else Begin
               ...
               Begin
                   ...
                   Printer.Canvas.Font.Size:=8+XTAMFONTE;
                     ...
                     Then Begin//IMPRIME EM FORMATO EAN 13
                       ...
                       if DMESTOQUE.TRel.FieldByName('COR').AsString <> '' THEN begin
                           Printer.Canvas.TextOut(COLIMP+600, (Linha*1)+10, 'RX-'+DMESTOQUE.TRel.FieldByName('COR').AsString);
                           Printer.Canvas.Font.Size:=6+XTAMFONTE;
                           Printer.Canvas.TextOut(COLIMP+600, (Linha*1)+80, Copy(DMESTOQUE.TRel.FieldByName('GRADE').AsString, 1, 13));
                           Printer.Canvas.TextOut(COLIMP, IncLine+Linha, DMESTOQUE.TRel.FieldByName('CONTRINT').AsString); //Paulo 07/01/2011: Esta imprimindo o contrint, caso quiser código de barra é só mudar para a valiavel CodBarraImp
                       end
                       else begin
                          Printer.Canvas.TextOut(COLIMP+600,(Linha*1)+10, 'R$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECOPRAZO').Value));
                          Printer.Canvas.Font.Size:=6+XTAMFONTE;
                           Printer.Canvas.TextOut(COLIMP+600, (Linha*1)+80, Copy(DMESTOQUE.TRel.FieldByName('GRADE').AsString, 1, 13));
                          Printer.Canvas.TextOut(COLIMP, IncLine+Linha, DMESTOQUE.TRel.FieldByName('CONTRINT').AsString);
                       end;
           ...
           Begin
               ...
               Begin
                   ...
                   If DMMACS.TLoja.FieldByName('IECODFAB').AsString='1' Then
                       Printer.Canvas.TextOut(COLIMP, (Linha*1), DMESTOQUE.TRel.FieldByName('CODFAB').AsString);
                   Printer.canvas.font.style:=[fsbold];
                   ...
                   if DMMACS.TLoja.FieldByName('IECLASS2').AsString='1' THEN
                       Printer.Canvas.TextOut(COLIMP, (Linha*1), DMESTOQUE.TRel.FieldByName('COR').AsString);
                   Printer.canvas.font.style:=[fsbold];
                   ...
                   Then Begin
                       If DMESTOQUE.TRel.FieldByName('PRECO').AsString<>'' Then
                           Printer.Canvas.TextOut(COLIMP+(Tamanho*11),(Linha*1), '$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').Value))
                       Else
                           Printer.Canvas.TextOut(COLIMP+(Tamanho*11), (Linha*1), '$ 0,00');
                   End;
                   Printer.canvas.font.style:=[];
                   //Verifica se deve Imprimir Cód Composto
                   If DMMACS.TLoja.FieldByName('IECODCOMPOSTO').AsString='1'  Then
                      	Printer.Canvas.TextOut(COLIMP, (Linha+(Tamanho*2)), DMESTOQUE.TRel.FieldByName('CODCOMPOSTO').AsString);
                   if DMMACS.TLoja.FieldByName('IECONTRINT').AsString='1'  Then
                      	Printer.Canvas.TextOut(COLIMP, (Linha+(Tamanho*2)), DMESTOQUE.TRel.FieldByName('CONTRINT').AsString);
                   Printer.Canvas.Font.Size:=7+XTAMFONTE;

                   //Verifica se deve Imprimir descricao
                   If DMMACS.TLoja.FieldByName('IEDESC').AsString='1' Then
                       Printer.Canvas.TextOut(COLIMP, (Linha+(Tamanho*4)), Copy(DMESTOQUE.TRel.FieldByName('DESCR').AsString, 0, XTAMDESC));

                   //Verifica se deve Imprimir a marca
                   If DMMACS.TLoja.FieldByName('IEMARCA').AsString='1' Then
                       Printer.Canvas.TextOut(COLIMP, (Linha+(Tamanho*6)), DMESTOQUE.TRel.FieldByName('MARCA').AsString);

                   Printer.Canvas.Font.Size:=6+XTAMFONTE;
                   //Verifica se deve Imprimir a classificacao
                   If DMMACS.TLoja.FieldByName('IECLASS').AsString='1'
                   Then Begin
                       Printer.Canvas.TextOut(COLIMP, Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('CLASSIFICACAO').AsString);
                       IncLine:=2;
                   End;
                   //Verifica se deve Imprimir a Composição
                   If DMMACS.TLoja.FieldByName('IECOMPOSICAO').AsString='1'
                   Then Begin
                       Printer.Canvas.TextOut(COLIMP, IncLine+Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('COMPOSICAO').AsString);
                   End;
                   //Verifica se deve Imprimir a altura e largura
                   If DMMACS.TLoja.FieldByName('IEALTLARG').AsString='1'
                   Then Begin
                       Printer.Canvas.TextOut(COLIMP+200, IncLine+Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('LARGURA').AsString+'x'+DMESTOQUE.TRel.FieldByName('ALTURA').AsString);
                   End;

                   If (DMMACS.TLoja.FieldByName('IECOMPOSICAO').AsString='1') or (DMMACS.TLoja.FieldByName('IEALTLARG').AsString='1') Then
                       IncLine:=IncLine+2;
                   ...
                   End;
                   Printer.Canvas.Font.Size:=8+XTAMFONTE;
                   If (DMMACS.TLoja.FieldByName('IEBARRA').AsString='1')
                   Then Begin
                     If (Length(TRIM(CodBarraImp))=13)
                     Then Begin//IMPRIME EM FORMATO EAN 13
                       CodBarraImp:=Copy(CodBarraImp, 0, 12);
                     ...
                     begin
                         Printer.Canvas.Font.Size:=6+XTAMFONTE;
                         Printer.Canvas.TextOut(COLIMP, IncLine+Linha+(Tamanho*7), CodBarraImp);
                     end
                     else begin
                   ...
                   If DMMACS.TLoja.FieldByName('IEFABR').AsString='1'
                   Then Begin
                       IncLine:=IncLine+2;
                       Printer.Canvas.TextOut(COLIMP, IncLine+Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('FABRICANTE').AsString);
                   End;
                   Printer.Canvas.Font.Size:=8+XTAMFONTE;
                   //atualiza o nº de cópias impressas
                   NumCop:=NumCop+1;
           ...
           Begin
               ...
               Begin
                   ...
                   Then Begin
                       ...
                       Then Begin
                           //usara campo código de barras
                           If Length(DMESTOQUE.TRel.FieldByName('CODBARRA').AsString)=13
                           Then Begin//imprimir codigo de barra
                               CodBarraImp:=DMESTOQUE.TRel.FieldByName('CODBARRA').AsString;
                               EAN13:=True;
                           End
                       ...
                       //Verifica se deve Imprimir Cód Fabricante
                       If DMMACS.TLoja.FieldByName('IECODFAB').AsString='1' Then
                           Printer.Canvas.TextOut(20, Linha*1, DMESTOQUE.TRel.FieldByName('CODFAB').AsString);

                       Printer.Canvas.Font.Size:=8;
                       //Verifica se deve Imprimir descricao
                       If DMMACS.TLoja.FieldByName('IEDESC').AsString='1' Then
                           Printer.Canvas.TextOut(10, Linha+(Tamanho*6), DMESTOQUE.TRel.FieldByName('DESCR').AsString);

                       //Verifica se deve Imprimir a marca
                       If DMMACS.TLoja.FieldByName('IEMARCA').AsString='1' Then
                           Printer.Canvas.TextOut(10, Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('MARCA').AsString);
                       Printer.Canvas.Font.Size:=6;
                       //Verifica se deve Imprimir a classificacao
                       If DMMACS.TLoja.FieldByName('IECLASS').AsString='1' Then
                           Printer.Canvas.TextOut(10, Linha+(Tamanho*10), DMESTOQUE.TRel.FieldByName('CLASSIFICACAO').AsString);
                       //Verifica se deve Imprimir o fabricante
                       ...
                       If DMMACS.TLoja.FieldByName('IECOMPOSICAO').AsString='1' Then
                           Printer.Canvas.TextOut(10, Linha+(Tamanho*12), DMESTOQUE.TRel.FieldByName('COMPOSICAO').AsString);

                       //Verifica se deve Imprimir a altura e largura
                       If DMMACS.TLoja.FieldByName('IEALTLARG').AsString='1' Then
                           Printer.Canvas.TextOut(200, Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('ALTURA').AsString+'x'+DMESTOQUE.TRel.FieldByName('LARGURA').AsString);

                       Printer.Canvas.Font.Size:=8;
                       //Verifica se deve Imprimir Cód Composto
                       If DMMACS.TLoja.FieldByName('IECODCOMPOSTO').AsString='1' Then
                           Printer.Canvas.TextOut(200, Linha*1, DMESTOQUE.TRel.FieldByName('CODCOMPOSTO').AsString)
                       Else
                           Printer.Canvas.TextOut(230, Linha*1, DMESTOQUE.TRel.FieldByName('CONTRINT').AsString);
                       //Verifica se deve Imprimir preço Vista
                       If DMMACS.TLoja.FieldByName('IEPRECO').AsString='1'
                       Then Begin
                           If DMESTOQUE.TRel.FieldByName('PRECO').AsString<>'' Then
                               Printer.Canvas.TextOut(230, Linha+(Tamanho*3), 'V $ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').Value))
                           Else
                               Printer.Canvas.TextOut(230, Linha+(Tamanho*3), 'V $ 0,00');
                       End;
                       //Verifica se deve Imprimir preço Prazo
                       If DMMACS.TLoja.FieldByName('IEPRECOPRAZO').AsString='1'
                       Then Begin
                           If DMESTOQUE.TRel.FieldByName('PRECOPRAZO').AsString<>'' Then
                               Printer.Canvas.TextOut(150, Linha+(Tamanho*3), 'P $ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').Value))
                           Else
                   ...
                   Then Begin
                       ...
                       //Verifica se deve Imprimir Cód Fabricante
                       If DMMACS.TLoja.FieldByName('IECODFAB').AsString='1' Then
                           Printer.Canvas.TextOut(370, Linha*1, DMESTOQUE.TRel.FieldByName('CODFAB').AsString);

                       Printer.Canvas.Font.Size:=8;
                       //Verifica se deve Imprimir descricao
                       If DMMACS.TLoja.FieldByName('IEDESC').AsString='1' Then
                           Printer.Canvas.TextOut(370, Linha+(Tamanho*6), DMESTOQUE.TRel.FieldByName('DESCR').AsString);
                       //Verifica se deve Imprimir a marca
                       If DMMACS.TLoja.FieldByName('IEMARCA').AsString='1' Then
                           Printer.Canvas.TextOut(370, Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('MARCA').AsString);
                       Printer.Canvas.Font.Size:=6;
                       //Verifica se deve Imprimir a classificacao
                       If DMMACS.TLoja.FieldByName('IECLASS').AsString='1' Then
                           Printer.Canvas.TextOut(370, Linha+(Tamanho*10), DMESTOQUE.TRel.FieldByName('CLASSIFICACAO').AsString);
                       //Verifica se deve Imprimir o fabricante
                       If DMMACS.TLoja.FieldByName('IEFABR').AsString='1' Then
                           Printer.Canvas.TextOut(370, Linha+(Tamanho*10), DMESTOQUE.TRel.FieldByName('FABRICANTE').AsString);
                       //Verifica se deve Imprimir a Composição
                       If DMMACS.TLoja.FieldByName('IECOMPOSICAO').AsString='1' Then
                           Printer.Canvas.TextOut(370, Linha+(Tamanho*12), DMESTOQUE.TRel.FieldByName('COMPOSICAO').AsString);
                       //Verifica se deve Imprimir a altura e largura
                       If DMMACS.TLoja.FieldByName('IEALTLARG').AsString='1' Then
                           Printer.Canvas.TextOut(550, Linha+(Tamanho*8), DMESTOQUE.TRel.FieldByName('ALTURA').AsString+'x'+DMESTOQUE.TRel.FieldByName('LARGURA').AsString);

                       Printer.Canvas.Font.Size:=8;
                       //Verifica se deve Imprimir Cód Composto
                       If DMMACS.TLoja.FieldByName('IECODCOMPOSTO').AsString='1' Then
                           Printer.Canvas.TextOut(550, Linha*1, DMESTOQUE.TRel.FieldByName('CODCOMPOSTO').AsString)
                       Else
                           Printer.Canvas.TextOut(580, Linha*1, DMESTOQUE.TRel.FieldByName('CONTRINT').AsString);

                       //Verifica se deve Imprimir preço
                       If DMMACS.TLoja.FieldByName('IEPRECO').AsString='1'
                       Then Begin
                           If DMESTOQUE.TRel.FieldByName('PRECO').AsString<>'' Then
                               Printer.Canvas.TextOut(580, Linha+(Tamanho*3), '$ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').Value))
                           Else
                       ...
                       Then Begin
                           If DMESTOQUE.TRel.FieldByName('PRECOPRAZO').AsString<>'' Then
                               Printer.Canvas.TextOut(500, Linha+(Tamanho*3), 'P $ '+FormatFloat('0.00', DMESTOQUE.TRel.FieldByName('PRECO').Value))
                           Else
...
Begin
   ...
   DMESTOQUE.Alx1.SQL.Add('select '+XTABELA2+'.'+XCAMPO+',cfop.cfop,cfop.descricao from '+XTABELA2+' left join cfop ');
   if(XTABELAAUX = 'ORDEM')Then
	   DMESTOQUE.Alx1.SQL.Add('on '+XTABELA2+'.'+XCAMPO+' = cfop.cod_cfop where '+XTABELA2+'.cod_ordem = :numdocfis')
   Else
      DMESTOQUE.Alx1.SQL.Add('on '+XTABELA2+'.'+XCAMPO+' = cfop.cod_cfop where '+XTABELA2+'.cod_pedido = :numdocfis');
...
Begin
	if(XTABELA3 = 'PEDCOMPRA')Then Begin
      	XCONTROLE := 0;
       XPARAM2 := 'DOCFIS';
   ...
   if(XTABELA3 = 'PEDVENDA')Then Begin
       XPARAM2 := 'DOCFISSAIDA';
       XTABELAAUX := 'PEDVENDA';
   End;
   if(XTABELA3 = 'ORDEM')Then Begin
       XPARAM2 := 'DOCFISORD';
       XCONTROLE := 2;
       XTABELAAUX := 'ORDEM';
   End;
   ...
   DMESTOQUE.Alx1.ParamByName('cod_empresa').AsInteger := DMMACS.TEmpresa.FieldByName('cod_empresa').AsInteger;
   DMESTOQUE.Alx1.Open;
   ...
   if (XTABELAAUX = 'ORDEM')Then Begin
   ...
   End
   Else Begin
   	If (XTABELAAUX = 'PEDCOMPRA')Then Begin
   ...
   End;
   if(XTABELAAUX = 'ORDEM')Then
   	DMESTOQUE.Alx4.ParamByName('numdocfis').AsInteger := DMESTOQUE.Alx2.ParamByName('cod_ordem').AsInteger
   Else
   	DMESTOQUE.Alx4.ParamByName('numdocfis').AsInteger := DMESTOQUE.Alx2.ParamByName('cod_pedido').AsInteger;
   DMESTOQUE.Alx4.Open;
   ...
   Else Begin
       ...
       DMMACS.TMP.FieldByName('COD_TMP').AsInteger:=XCod_tmp;
       DMMACS.TMP.FieldByName('DESC1').AsString:='À VISTA';
       DMMACS.TMP.FieldByName('VLR1').AsCurrency:= DMESTOQUE.Alx2.FieldByName('VLRTOTDOCNF').AsInteger;
       DMMACS.TMP.FieldByName('INT2').AsInteger:=1;
       DMMACS.TMP.Post;
   End;
   ...
   DMMACS.TALX.Close;
   DMMACS.TALX.SQL.Clear;
   DMMACS.TALX.SQL.Add('insert into tmp(int1,int2,desc3,desc1) values ('+#39+IntToStr(XCONTROLE)+#39+',2,'+#39+XTEXTCFOP+#39+','+#39+XTEXTCODCFOP+#39+')');
   DMMACS.TALX.ExecSQL;
   DMMACS.Transaction.CommitRetaining;
   DMMACS.TALX.Close;
   ...
   if(XTABELA = 'PEDCOMPRA')Then Begin
   ...
   DMSAIDA.TAlx.SQL.Clear;
   if(XTABELA3 = 'PEDCOMPRA')Then Begin
   	DMSAIDA.TAlx.SQL.Add('select subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS BARRAS, cst.cod_sit_trib cod_cst,itenspedc.cfop,itenspedc.qtdeprod,itenspedc.valunit,itenspedc.valortotal,itenspedc.baseicms,itenspedc.vlricms,');
   ...
   Else begin
   	if(XTABELA3 = 'PEDVENDA')Then Begin
		   	DMSAIDA.TAlx.SQL.Add('select subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS BARRAS, cst.cod_sit_trib cod_cst,itenspedven.cfop,itenspedven.qtdeprod,itenspedven.valunit,itenspedven.valortotal,itenspedven.baseicms,itenspedven.vlricms,');
       ...
       End
       Else begin
       	DMSAIDA.TAlx.SQL.Add('select subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS BARRAS, cst.cod_sit_trib cod_cst,itprodord.cfop,itprodord.qtd AS QTDEPROD,itprodord.vlrunit AS valunit,itprodord.total AS valortotal,itprodord.baseicms,itprodord.vlricms,');
   ...
   if (XTABELA3 = 'ORDEM')Then
   	DMBANCO.TAlx.ParamByName('numdocfis').AsInteger := DMESTOQUE.Alx2.FieldByName('COD_ORDEM').AsInteger
   Else
   	DMBANCO.TAlx.ParamByName('numdocfis').AsInteger := DMESTOQUE.Alx2.FieldByName('COD_PEDIDO').AsInteger;
...
Begin
   ...
   Then Begin
       ...
       begin
           DMCONTA.TAlx.ParamByName('param').AsInteger := COD_GERADOR;//DMMACS.TALX.FieldByName('cod_faturaconta').AsInteger;
           DMCONTA.TAlx.Open;
           DMCONTA.TAlx.First;
       end;
   End;
	...
	Try
       ...
       Then Begin
           ...
           If DMCONTA.TAlx.FieldByName('COD_USUARIO').AsString<>'111522' Then
		   		FiltraTabela(DMMACS.TUsuario, 'USUARIO', 'CODUSUARIO', DMCONTA.TAlx.FieldByName('COD_USUARIO').AsString, '');
   		FiltraTabela(DMFINANC.TFormPag, 'FORMPAG', 'COD_FORMPAG', IntToStr(DMCONTA.TAlx.FieldByName('cod_formpag').AsInteger), '');
   		xTotal := DMCONTA.TAlx.FieldByName('vlrtotfatura').AsFloat;
   		Total := FormatFloat('0.00', xTotal);
   		//CABECALHO GERAL
   		XLinImp:=XLinImp+5;
   		Imprime(FMenu.RvFaturas, xLinImp,5, 'Data da fatura: '+ DateToStr(DMCONTA.TAlx.FieldByName('dtfatura').AsDateTime) , 'Verdana',8,clBlack, false);
           If DMCONTA.TAlx.FieldByName('COD_USUARIO').AsString<>'111522' Then
   			Imprime(FMenu.RvFaturas, XLinImp,50, 'Usuário: '+ DMMACS.TUsuario.FieldByName('LOGIN').AsString, 'Verdana',8,clBlack, false)
           Else
   			Imprime(FMenu.RvFaturas, XLinImp,50, 'Usuário: Mestre', 'Verdana',8,clBlack, false);

   		Imprime(FMenu.RvFaturas, XLinImp,78, 'Forma Pag.: '+ Copy(DMFINANC.TFormPag.FieldByName('DESCRICAO').AsString, 0, 15) , 'Verdana',8,clBlack, false);
   		Imprime(FMenu.RvFaturas, XLinImp,127, 'Cobrança: '+ DMCONTA.TAlx.FieldByName('COBRANCA').AsString, 'Verdana',8,clBlack, false);
   		Imprime(FMenu.RvFaturas, XLinImp,160, 'Total Fatura: '+ Total, 'Verdana',9,clBlack, TRUE);
   		XLinImp:=XLinImp+1;
   		...
   		XLinImp:=XLinImp+4;
   		Imprime(FMenu.RvFaturas, XLinImp,5, 'Dívida Renegociada', 'Verdana',9,clBlack, TRUE);
     		XLinImp:=XLinImp+1;
   		...
   		Begin
   			...
   			XLinImp:=XLinImp+5;
   			Imprime(FMenu.RvFaturas, xLinImp,5, DMCONTA.TAlx.FieldByName('NUMPARC').AsString, 'Verdana',8,clBlack, false);
   			Imprime(FMenu.RvFaturas, xLinImp,25, DateToStr(DMCONTA.TAlx.FieldByName('DTREF').AsDateTime), 'Verdana',8,clBlack, false);
   			Imprime(FMenu.RvFaturas, xLinImp,50, DateToStr(DMCONTA.TAlx.FieldByName('DTVENC').AsDateTime), 'Verdana',8,clBlack, false);
   			Imprime(FMenu.RvFaturas, xLinImp,84, Desconto, 'Verdana',8,clBlack, false);
   			Imprime(FMenu.RvFaturas, xLinImp,107, Multa, 'Verdana',8,clBlack, false);
       		Imprime(FMenu.RvFaturas, xLinImp,127, Valor, 'Verdana',8,clBlack, false);
   			DMCONTA.TAlx.Next;
   		End;
   		XLinImp:=XLinImp+2;
   		Imprime(FMenu.RvFaturas,XLinImp,5,'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ _','Verdana',10,clBlack, true);
       End;
   	//CABECALHO ITENS ORD
       if (DMCONTA.TAlx.FieldByName('TIPOGERA').AsString = 'ORD')
       Then Begin
           if XColImp = 0
           Then Begin
               Imprime(FMenu.RvFaturas,XLinImp,5,'____________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
               XLinImp:=XLinImp+4;
               Imprime(FMenu.RvFaturas, XLinImp,5, 'OS Prestados', 'Verdana',9,clBlack, TRUE);
               XLinImp:=XLinImp+1;
               Imprime(FMenu.RvFaturas,XLinImp,5,'____________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
       ...
       Imprime(FMenu.RvFaturas,XLinImp,5,'____________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
  		XLinImp:=XLinImp+4;
       Imprime(FMenu.RvFaturas, XLinImp,5, 'Resultado da Renegociação', 'Verdana',9,clBlack, TRUE);
       XLinImp:=XLinImp+1;
       Imprime(FMenu.RvFaturas,XLinImp,5,'____________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
   	...
   	begin 
           xTotalPago := xTotalPago + DMFINANC.TAlx.FieldByName('VLRPAGO').AsCurrency;
           ...
           Imprime(FMenu.RvFaturas, xLinImp,5, DMFINANC.TAlx.FieldByName('PARCELA').AsString, 'Verdana',8,clBlack, false);
           Imprime(FMenu.RvFaturas, xLinImp,30, FormatFloat('0.00', DMFINANC.TAlx.FieldByName('VLRTOTAL').AsCurrency), 'Verdana',8,clBlack, false);
           Imprime(FMenu.RvFaturas, xLinImp,55, DMFINANC.TAlx.FieldByName('DTVENCIMENTO').AsString, 'Verdana',8,clBlack, false);
           Imprime(FMenu.RvFaturas, xLinImp,90, FormatFloat('0.00', (xTotalConta - xTotalPago)), 'Verdana',8,clBlack, false);
           Imprime(FMenu.RvFaturas, xLinImp,115, DMFINANC.TAlx.FieldByName('DTPAGAMENTO').AsString, 'Verdana',8,clBlack, false);
           Imprime(FMenu.RvFaturas, xLinImp,150, FormatFloat('0.00', DMFINANC.TAlx.FieldByName('VLRPAGO').AsCurrency), 'Verdana',8,clBlack, false);
           Imprime(FMenu.RvFaturas, xLinImp,180, DMFINANC.TAlx.FieldByName('COBRANCA').AsString, 'Verdana',8,clBlack, false);

           DMFINANC.TAlx.Next;
       end;
       XLinImp:=XLinImp+3;       
   	Imprime(FMenu.RvFaturas,XLinImp,5,'_________________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);       
       XLinImp:=XLinImp+4;
       Imprime(FMenu.RvFaturas, xLinImp,5, 'Valor total devido pelo cliente: R$'+FormatFloat('0.00', (xTotalConta - xTotalPago)), 'Verdana',8,clBlack, true);
       //Imprimi rodapé
   	XLinImp:=XLinImp+2;
   	Imprime(FMenu.RvFaturas,XLinImp,5,'_________________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
   	XLinImp:=XLinImp+3;
   	Imprime(FMenu.RvFaturas, xLinImp,80, 'MZR Sistemas - Sistemas em gestão empresarial ', 'Verdana',7,clBlack, false);
   	XLinImp:=XLinImp+2;
   	Imprime(FMenu.RvFaturas, xLinImp,95, 'www.mzrsistemas.com ', 'Verdana',7,clBlack, false);
   Except
       Result := False;
   End;
End;
   ...
   try
       ...
       DMCONTA.TAlx.First;
        ...
        End;
   			...
   			Imprime(FMenu.RvFaturas, xLinImp,84, Desconto, 'Verdana',8,clBlack, false);
   			Imprime(FMenu.RvFaturas, xLinImp,107, Multa, 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,125, Valor, 'Verdana',8,clBlack, false);
   			DMCONTA.TAlx.Next;
   	 End;
   	 XLinImp:=XLinImp+2;
   	 Imprime(FMenu.RvFaturas,XLinImp,5,'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ _','Verdana',10,clBlack, true);
...
Begin
	Try
       ...
       If FRelGeralMovimentacoesFinanceiras.CBPedCompra.Checked=True
       Then Begin
...
Var
   ...
   XTotalCarteira, XTotalCCartao, XTotalDCartao, XTotalCheque, XTotalOutros: Real;
Begin
	Try
      ...
      DMServ.TAlx1.ParamByName('dt1').AsDateTime:=StrToDate(FRelGeralMovimentacoesFinanceiras.edData1.Text);
      DMServ.TAlx1.ParamByName('dt2').AsDateTime:=StrToDate(FRelGeralMovimentacoesFinanceiras.edData2.Text);
      DMServ.TAlx1.Open;
      ...
      XTotalOutros:=(XtotalPrevisto+XtotalRealizado)-XTotalCarteira-XTotalCheque-XTotalCCartao-XTotalDCartao;

      //Impressão do Resultado
      XLinImp:=XLinImp+4;
      Imprime(FMenu.RvFaturas, XLinImp,100, 'A Prazo: ','Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, 'A Vista: ','Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, 'Total: ','Courier New',11,clBlack, false);
      //Total de Ordens
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,005, '(+) Ordens de Serviço: ','Courier New',11,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XtotalPrevisto),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XtotalRealizado),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XtotalPrevisto+XtotalRealizado),'Courier New',10,clBlack, true);
      //Total de Produtos
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,15, 'Produtos:', 'Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotProdutosPrevisto),'Courier New',10,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotProdutosRealizado),'Courier New',10,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotProdutosPrevisto+XTotProdutosRealizado),'Courier New',10,clBlack, false);
      //Total de Serviços
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,15, 'Serviços:', 'Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotServicosPrevisto),'Courier New',10,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotServicosRealizado),'Courier New',10,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotServicosPrevisto+XTotServicosRealizado),'Courier New',10,clBlack, false);
      //Formas de pagamento
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Formas de Pagamento:', 'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Carteira:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalCarteira),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cheques:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalCheque),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cartão de Crédito:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalCCartao),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cartão de Débito:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalDCartao),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Outros Pagamentos:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalOutros),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas,XLinImp,5,'......................................................................................................................................................','Verdana',10,clBlack, true);
...
Begin
	Try
      ...
      XTotalOutros:=(XPVSinteticoPrevisto+XPVSinteticoRealizado)-XTotalCarteira-XTotalCheque-XTotalCCartao-XTotalDCartao;
      ...
      //Total de PV
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,005, '(+) Pedidos de Venda: ','Courier New',11,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XPVSinteticoPrevisto),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XPVSinteticoRealizado),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XPVSinteticoPrevisto+XPVSinteticoRealizado),'Courier New',10,clBlack, true);
...
Begin
	Try
      ...
      XTotalOutros:=(XPcSinteticoPrevisto+XPcSinteticoRealizado)-XTotalCarteira-XTotalCheque-XTotalCCartao-XTotalDCartao;
      ...
      //Total de PV
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,005, '(-) Pedidos de Compra: ','Courier New',11,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XPcSinteticoPrevisto),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XPcSinteticoRealizado),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XPcSinteticoPrevisto+XPCSinteticoRealizado),'Courier New',10,clBlack, true);
      ...
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalCCartao),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cartão de Débito:', 'Courier New',10,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalDCartao),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Outros Pagamentos:', 'Courier New',10,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalOutros),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas,XLinImp,5,'......................................................................................................................................................','Verdana',10,clBlack, true);
	Except
...
Begin
	Try
      ...
      Then Begin
          //Impressão do Resultado
          XLinImp:=XLinImp+4;
          Imprime(FMenu.RvFaturas, XLinImp,100, 'Qtd. Est.: ','Courier New',11,clBlack, false);
          Imprime(FMenu.RvFaturas, XLinImp,130, 'Vlr. Compra: ','Courier New',11,clBlack, false);
          Imprime(FMenu.RvFaturas, XLinImp,170, 'Vlr. Venda: ','Courier New',11,clBlack, false);
          //Total de PV
          XLinImp:=XLinImp+3;
          Imprime(FMenu.RvFaturas, XLinImp,005, '( ) Posição Geral de Estoque: ','Courier New',11,clBlack, true);
          Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', DMServ.TAlx1.FieldByName('Estoque').AsCurrency),'Courier New',10,clBlack, true);
          Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', DMServ.TAlx1.FieldByName('COMPRA').AsCurrency),'Courier New',10,clBlack, true);
          Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', DMServ.TAlx1.FieldByName('VENDA').AsCurrency),'Courier New',10,clBlack, true);
          XLinImp:=XLinImp+3;
          Imprime(FMenu.RvFaturas,XLinImp,5,'......................................................................................................................................................','Verdana',10,clBlack, true);
      End;
...
Begin
     Try
        ...
        //Mostra saldo inicial
	     If DMCAIXA.TAlx.FieldByName('FECHADO').AsString='S'
        Then Begin
	...
	Try
      ...
      Then Begin
           ...
           Begin
               ...
               DMServ.TAlx2.ParamByName('CodigoCaixa').AsInteger:=DMServ.TAlx1.FieldByName('cod_ctacor').AsInteger;
               DMServ.TAlx2.ParamByName('Periodo').AsDateTime:=StrToDate(FRelGeralMovimentacoesFinanceiras.edData2.Text);
               DMServ.TAlx2.Open;
               ...
               DMServ.TAlx3.ParamByName('CodigoCaixa').AsInteger:=DMServ.TAlx1.FieldByName('cod_ctacor').AsInteger;
               DMServ.TAlx3.ParamByName('Periodo').AsDateTime:=StrToDate(FRelGeralMovimentacoesFinanceiras.edData2.Text);
               DMServ.TAlx3.Open;
               //Determina qual o melhor abcaixa a pegar
               If (DMServ.TAlx2.FieldByName('dtfech').AsDateTime>DMServ.TAlx3.FieldByName('data_abert').AsDateTime) Then
                  XCodAbcaixa:=DMServ.TAlx2.FieldByName('cod_abbanco').AsInteger
               Else
                  XCodAbcaixa:=DMServ.TAlx3.FieldByName('cod_abbanco').AsInteger;

               If not DMServ.TAlx2.IsEmpty
               Then Begin
                    DMMACS.TMP.Insert;
                    DMMACS.TMP.FieldByName('COD_TMP').AsInteger:=XCodTmp;
                    DMMACS.TMP.FieldByName('DESC1').AsString:=DMServ.TAlx1.FieldByName('DESCBANCO').AsString;
           ...
           //Impressão do Resultado
           XLinImp:=XLinImp+4;
           Imprime(FMenu.RvFaturas, XLinImp,005, '(+) Posição Geral de Bancos: ','Courier New',11,clBlack, true);
           Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XVlrTotalCaixa),'Courier New',10,clBlack, true);
           //Formas de pagamento
           XLinImp:=XLinImp+3;
           Imprime(FMenu.RvFaturas, XLinImp,25, 'Saldo Individual por Ctas Corrente:', 'Courier New',09,clBlack, false);
           DMMACS.TMP.First;
           While not DMMACS.TMP.Eof do
           Begin
               XLinImp:=XLinImp+3;
               Imprime(FMenu.RvFaturas, XLinImp,25, DMMACS.TMP.FieldByName('DESC1').asstring, 'Courier New',09,clBlack, false);
               Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', DMMACS.TMP.FieldByName('VLR1').AsCurrency),'Courier New',09,clBlack, false);
               DMMACS.TMP.Next;
           End;   
           XLinImp:=XLinImp+3;
           Imprime(FMenu.RvFaturas,XLinImp,5,'......................................................................................................................................................','Verdana',10,clBlack, true);
	...
	Try
      ...
      Then Begin
           ...
           Begin
               ...
               DMServ.TAlx2.ParamByName('CodigoCaixa').AsInteger:=DMServ.TAlx1.FieldByName('cod_caixa').AsInteger;
               DMServ.TAlx2.ParamByName('Periodo').AsDateTime:=StrToDate(FRelGeralMovimentacoesFinanceiras.edData2.Text);
               DMServ.TAlx2.Open;
               //Filtro abcaixas aberto para tentarmos pegar a melhor data
               DMServ.TAlx3.Close;
               ...
               DMServ.TAlx3.ParamByName('CodigoCaixa').AsInteger:=DMServ.TAlx1.FieldByName('cod_caixa').AsInteger;
               DMServ.TAlx3.ParamByName('Periodo').AsDateTime:=StrToDate(FRelGeralMovimentacoesFinanceiras.edData2.Text);
               DMServ.TAlx3.Open;
               //Determina qual o melhor abcaixa a pegar
               If (DMServ.TAlx2.FieldByName('dtfecha').AsDateTime>DMServ.TAlx3.FieldByName('dtabertura').AsDateTime) Then
               ...
               Then Begin
                    DMMACS.TMP.Insert;
                    DMMACS.TMP.FieldByName('COD_TMP').AsInteger:=XCodTmp;
                    DMMACS.TMP.FieldByName('DESC1').AsString:=DMServ.TAlx1.FieldByName('DESCRICAO').AsString;
                    XResultadoCx:=ImpRelGMovFinanceiras_CaixaSintetico_Saldo(XCodAbcaixa);
                    DMMACS.TMP.FieldByName('VLR1').AsCurrency:= XResultadoCx;
                    DMMACS.TMP.Post;
                    XVlrTotalCaixa:=XVlrTotalCaixa+XResultadoCx;
               End;
           ...
           //Impressão do Resultado
           XLinImp:=XLinImp+4;
           Imprime(FMenu.RvFaturas, XLinImp,005, '(+) Posição Geral dos Caixas: ','Courier New',11,clBlack, true);
           Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XVlrTotalCaixa),'Courier New',10,clBlack, true);
           //Formas de pagamento
           XLinImp:=XLinImp+3;
           Imprime(FMenu.RvFaturas, XLinImp,25, 'Saldo Individual de Caixas:', 'Courier New',09,clBlack, false);
           DMMACS.TMP.First;
           While not DMMACS.TMP.Eof do
           Begin
               XLinImp:=XLinImp+3;
...
Var
   ...
   XTotalCarteiraReal, XTotalCCartaoReal, XTotalDCartaoReal, XTotalChequeReal, XTotalOutrosReal: Real;
   XTotalCarteiraPrev, XTotalCCartaoPrev, XTotalDCartaoPrev, XTotalChequePrev, XTotalOutrosPrev: Real;
Begin
	Try
      ...
      If not DMServ.TAlx1.IsEmpty
      Then Begin
           XRecebimentosSinteticoRealizado:=DMServ.TAlx1.FieldByNAme('TOTAL').AsCurrency;
      End;
      DMServ.TAlx1.Close;
      DMServ.TAlx1.SQL.Clear;
      DMServ.TAlx1.SQL.Add(' Select Sum(vwparcr.valorpg) as TOTAL from vwparcr ');
      ...
      End;
      XTotalOutrosReal:=XRecebimentosSinteticoRealizado- XTotalCarteiraReal - XTotalCCartaoReal - XTotalDCartaoReal - XTotalChequeReal;
      ...
      If not DMServ.TAlx1.IsEmpty
      Then Begin
           XRecebimentosSinteticoPrevisto:=DMServ.TAlx1.FieldByNAme('TOTAL').AsCurrency;
      End;
      DMServ.TAlx1.Close;
      DMServ.TAlx1.SQL.Clear;
      DMServ.TAlx1.SQL.Add(' Select Sum(vwparcr.valorpg) as TOTAL from vwparcr ');
      ...
      End;
      XTotalOutrosPrev:=XRecebimentosSinteticoPrevisto - XTotalCarteiraPrev - XTotalCCartaoPrev - XTotalDCartaoPrev - XTotalChequePrev;

      //Impressão do Resultado
      XLinImp:=XLinImp+4;
      Imprime(FMenu.RvFaturas, XLinImp,100, 'A Receber: ','Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, 'Recebido: ','Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, 'Total: ','Courier New',11,clBlack, false);
      //Total de PV
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,005, '(+) Recebimentos de Clientes: ','Courier New',11,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XRecebimentosSinteticoPrevisto),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XRecebimentosSinteticoRealizado),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XRecebimentosSinteticoRealizado+XRecebimentosSinteticoPrevisto),'Courier New',10,clBlack, true);
      ...
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Carteira:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotalCarteiraReal),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotalCarteiraPrev),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalCarteiraPrev+XTotalCarteiraReal),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cheques:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotalChequeReal),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotalChequePrev),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalChequePrev+XTotalChequeReal),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cartão de Crédito:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotalCCartaoReal),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotalCCartaoPrev),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalCCartaoPrev+XTotalCCartaoReal),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Cartão de Débito:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotalDCartaoReal),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotalDCartaoPrev),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalDCartaoPrev+XTotalDCartaoReal),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,25, 'Outros Pagamentos:', 'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XTotalOutrosReal),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XTotalOutrosPrev),'Courier New',09,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XTotalOutrosPrev+XTotalOutrosReal),'Courier New',09,clBlack, false);
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas,XLinImp,5,'......................................................................................................................................................','Verdana',10,clBlack, true);
	Except
...
Begin
	Try
      ...
      Then Begin
           ...
           If (xCodPlnCtaVista<=0) or (xCodPlnCtaPrazo<=0)
           Then Begin
      ...
      If FRelGeralMovimentacoesFinanceiras.CBSeparar.Checked=True
      Then Begin
      ...
      //Impressão do Resultado
      XLinImp:=XLinImp+4;
      Imprime(FMenu.RvFaturas, XLinImp,100, 'A Pagar: ','Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,130, 'Pago: ','Courier New',11,clBlack, false);
      Imprime(FMenu.RvFaturas, XLinImp,170, 'Total: ','Courier New',11,clBlack, false);
      //Total de PV
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,005, '(-) Pagamento de Outras Despesas: ','Courier New',11,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XRecebimentosSinteticoPrevisto),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XRecebimentosSinteticoRealizado),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XRecebimentosSinteticoRealizado+XRecebimentosSinteticoPrevisto),'Courier New',10,clBlack, true);
...
Begin
	Try
      ...
      //Total de PV
      XLinImp:=XLinImp+3;
      Imprime(FMenu.RvFaturas, XLinImp,005, '(-) Pagamento de Fornecedores: ','Courier New',11,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,100, FormatFloat('###,##0.00', XRecebimentosSinteticoPrevisto),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,130, FormatFloat('###,##0.00', XRecebimentosSinteticoRealizado),'Courier New',10,clBlack, true);
      Imprime(FMenu.RvFaturas, XLinImp,170, FormatFloat('###,##0.00', XRecebimentosSinteticoRealizado+XRecebimentosSinteticoPrevisto),'Courier New',10,clBlack, true);
...
Begin
	Try
       XLinImp:=XLinImp+7;
		Imprime(FMenu.RvFaturas, XLinImp,5, 'Filtros: '+FMenu.XDecreto,'Courier New',12,clBlack, false);
	    XLinImp:=XLinImp+5;
		Imprime(FMenu.RvFaturas, XLinImp,5, FMenu.XProtocolo,'Courier New',12,clBlack, false);
	    XLinImp:=XLinImp+5;
		Imprime(FMenu.RvFaturas, XLinImp,5, FMenu.XVersaoNovidades,'Courier New',12,clBlack, false);
	    XLinImp:=XLinImp+5;
       Imprime(FMenu.RvFaturas,XLinImp,5,'____________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
	Except
...
Begin
	Try
   	...
   	DMServ.Alx.ParamByName('param').AsInteger := DMServ.TAlx2.FieldByName('COD_ORDEM').AsInteger;
   	DMServ.Alx.Open;
   	DMServ.Alx.First;

   	FiltraTabela(DMPESSOA.TFuncionario, 'FUNCIONARIO', 'COD_FUNC', IntToStr(DMServ.Alx.FieldByName('cod_vendedor').AsInteger), '');
   	FiltraTabela(DMPESSOA.TPessoa, 'PESSOA', 'COD_PESSOA', IntToStr(DMPESSOA.TFuncionario.FieldByName('COD_PESSOA').AsInteger), '');
   	...
   	Then Begin
   		...
   		Begin
       		...
       		XLinImp:=XLinImp+4;
       		Imprime(FMenu.RvFaturas, xLinImp,5, FloatToStr(DMServ.Alx.FieldByName('NUMERO').AsFloat), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,17, DMServ.Alx.FieldByName('cod_interno').AsString, 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,30, DateToStr(DMServ.Alx.FieldByName('data').AsDateTime), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,50, Copy('S-'+DMServ.Alx.FieldByName('descricao').AsString, 0, 35), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,122, CurrToStr(DMServ.Alx.FieldByName('qtd').AsCurrency), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,140, VlrUnit, 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,158, VlrTotal, 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,180, DMPESSOA.TPessoa.FieldByName('NOME').AsString, 'Verdana',8,clBlack, false);
           	XColImp :=1;
       		DMServ.Alx.Next;
       	End;
   	End
   	Else Begin
       	...
       	Begin
       		...
       		XLinImp:=XLinImp+4;
           	Imprime(FMenu.RvFaturas, xLinImp,5, FloatToStr(DMServ.Alx.FieldByName('NUMERO').AsFloat), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,17, ' ', 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,30, DateToStr(DMServ.Alx.FieldByName('dtfech').AsDateTime), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,50,  Copy('S-'+DMServ.Alx.FieldByName('despesa').AsString, 0, 35), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,122, CurrToStr(DMServ.Alx.FieldByName('qtd').AsCurrency), 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,140, VlrUnit, 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,158, VlrTotal, 'Verdana',8,clBlack, false);
           	Imprime(FMenu.RvFaturas, xLinImp,180, DMPESSOA.TPessoa.FieldByName('NOME').AsString, 'Verdana',8,clBlack, false);
...
Begin
	Try
   	...
   	DMESTOQUE.Alx.ParamByName('param').AsInteger := DMServ.TAlx2.FieldByName('COD_ORDEM').AsInteger;
   	DMESTOQUE.Alx.Open;

   	FiltraTabela(DMPESSOA.TFuncionario, 'FUNCIONARIO', 'COD_FUNC', IntToStr(DMESTOQUE.Alx.FieldByName('cod_vendedor').AsInteger), '');
   	FiltraTabela(DMPESSOA.TPessoa, 'PESSOA', 'COD_PESSOA', IntToStr(DMPESSOA.TFuncionario.FieldByName('COD_PESSOA').AsInteger), '');
   	...
   	Begin
   		...
   		XLinImp:=XLinImp+4;
       	Imprime(FMenu.RvFaturas, xLinImp,5, FloatToStr(DMESTOQUE.Alx.FieldByName('NUMERO').AsFloat), 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,17, DMESTOQUE.Alx.FieldByName('contrint').AsString, 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,30, DateToStr(DMESTOQUE.Alx.FieldByName('data').AsDateTime), 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,50, Copy('P-'+DMESTOQUE.Alx.FieldByName('descricao').AsString, 0, 35), 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,122, CurrToStr(DMESTOQUE.Alx.FieldByName('qtd').AsCurrency), 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,140, VlrUnit, 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,158, VlrTotal, 'Verdana',8,clBlack, false);
       	Imprime(FMenu.RvFaturas, xLinImp,180, DMPESSOA.TPessoa.FieldByName('NOME').AsString, 'Verdana',8,clBlack, false);
       	XColImp :=1;
...
Begin
	try
       ...
       DMServ.TAlx2.ParamByName('param').AsInteger := DMServ.TAlx1.FieldByName('cod_faturaconta').AsInteger;
       DMServ.TAlx2.Open;
       DMServ.TAlx2.First;
   Except
   	Result := False;
...
Begin
   ...
   Imprime(FMenu.RvFaturas, xLinImp,15, 'Interno ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,35, 'Data ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,50, 'Descrição ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,120, 'Qtde ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,135, 'Vlr. Unit. ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,155, 'Vlr. Total ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,180, 'Vendedor ', 'Verdana',10,clBlack, true);
   XLinImp:=XLinImp+2;
   Imprime(FMenu.RvFaturas,XLinImp,5,'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ _','Verdana',10,clBlack, false);
   XLinImp:=XLinImp+2;
End;
...
Begin
	Imprime(FMenu.RvFaturas,XLinImp,5,'____________________________________________________________________________________________________________________________','Verdana',10,clBlack, true);
   XLinImp:=XLinImp+5;
   Imprime(FMenu.RvFaturas, xLinImp,5, 'Parcelas ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,25, 'Data Lanç. ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,50, 'Data Venc. ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,80, 'Desconto ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,105, 'Multa ', 'Verdana',10,clBlack, true);
   Imprime(FMenu.RvFaturas, xLinImp,125, 'Valor ', 'Verdana',10,clBlack, true);
   XLinImp:=XLinImp+2;
   Imprime(FMenu.RvFaturas,XLinImp,5,'_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ __ _ _ _ _ _ _ _','Verdana',10,clBlack, true);
End;
...
begin
   XLinImp:=XLinImp+5;
	...
	Imprime(FMenu.RvFaturas, xLinImp,55, 'Data Venc.', 'Verdana',10,clBlack, true);
	Imprime(FMenu.RvFaturas, xLinImp,85, 'Valor Devido', 'Verdana',10,clBlack, true);
	Imprime(FMenu.RvFaturas, xLinImp,115, 'Data Pag.', 'Verdana',10,clBlack, true);
	Imprime(FMenu.RvFaturas, xLinImp,145, 'Valor Pago', 'Verdana',10,clBlack, true);
	Imprime(FMenu.RvFaturas, xLinImp,170, 'Tipo Pagamento', 'Verdana',10,clBlack, true);
   XLinImp:=XLinImp+2;   
end;
...
Begin
   ...
   DMFINANC.TAlx.ParamByName('param').AsInteger := DMCONTA.TAlx.FieldByName('cod_faturaconta').AsInteger;
