...
Uses
    ...
    Printers, Registry, ShellAPI, URLMON;
...
// function VerifExitProd(Xtabela: String; XVALORPK:STRING; XCAMPO: STRING; XValor:String):Boolean;
   ...
   Function LancConta(Pedido:String; CODPEDIDO:INTEGER; CodPessoa: Integer; Cobranca: String; ValorProd: Real; ValorServ: Real; CodConta: Integer; CodContaServ: Integer; CodCota: Integer; Historico: String; CodFormPag: Integer; Documento: String; GERADOR: String; DATA: TDateTime; COMIS: Real; DATAREF:String;  NumFiscal: String): Boolean;
...
uses IdMessage, UPrecoServ, Math, UFechaConta, UDMExporta, UnitDeclaracoes,
           ...
           Then Begin //se foi encontrado alguma conta
               ...
               Begin
                   ...
                   Then Begin
                       ...
                       If (DMCAIXA.TLanCaixa.FieldByName('COD_ABCAIXA').AsString <> FMenu.LABCAIXA.Caption) And (FiltraTabela(DMBANCO.TMovBanco, 'MOVBANCO', 'COD_MOVBANCO', '', '(COD_GERADOR = ' + #39 + DMCONTA.TParcCR.FieldByname('COD_PARCELACR').AsString + #39 + ') AND (TIPOGERADOR = ' + #39 + 'PCHR' + #39 + ')') = false)
...
Begin
   Try
       ...
       Then Begin
           result:=true;
           If ((DMMACS.TUsuario.FieldByName('FILTRAPEDDATA01').AsString<>'') or (DMMACS.TUsuario.FieldByName('FILTRAPEDDATA01').AsString<>'  /  /   ')) and ((DMMACS.TUsuario.FieldByName('FILTRAPEDDATA02').AsString<>'') or (DMMACS.TUsuario.FieldByName('FILTRAPEDDATA02').AsString<>'  /  /   ')) Then
               Result := True
           Else
               Result :=False;
       End
...
Begin
	...
	Try
       ...
       Then Begin
           ...
           Registra('PEDIDO COMPRA', 'CANCELA NFe', DMENTRADA.TPedC.FieldByName('DTPEDCOMP').AsString, 'Nº '+DMENTRADA.TPedC.FieldByName('NUMPED').AsString+' - '+DMENTRADA.WPedc.FieldByName('NOME').AsString, 'Vlr:'+DMENTRADA.TPedC.FieldByName('VALOR').AsString+'-'+DMENTRADA.WPedc.FieldByName('FORMPAG').AsString);
...
Begin
	...
	Try
       ...
       Then Begin
       	//REGISTRA COMANDO DO USUARIO
           Registra('PEDIDO VENDA', 'CANCELA NFe', DMSAIDA.TPedV.FieldByName('DTPEDVEN').AsString, 'Nº '+DMSAIDA.TPedV.FieldByName('NUMPED').AsString+'-'+DMSAIDA.TPedV.FieldByName('NOMECLI').AsString, 'Vlr:'+DMSAIDA.TPedV.FieldByName('VALOR').AsString+'-'+DMSAIDA.WPedV.FieldByName('FORMPAG').AsString);
           //INSERE COMANDOS PARA FILTRAR
           DMESTOQUE.Alx.Close;
           DMESTOQUE.Alx.SQL.Clear;
...
Begin
	...
	Try
       ...
       Then Begin
           //REGISTRA COMANDO DO USUARIO
           Registra('ORDEM SERVICO', 'CANCELA NFe', DMServ.WOrdem.FieldByName('DTABERT').AsString, 'Nº '+DMServ.WOrdem.FieldByName('NUMERO').AsString+'-'+DMServ.WOrdem.FieldByName('CLIENTE').AsString, 'Vlr:'+DMServ.WOrdem.FieldByName('TOTAL').AsString+'-'+DMServ.WOrdem.FieldByName('FORMAPAG').AsString);
           //INSERE COMANDOS PARA FILTRAR
           DMESTOQUE.Alx.Close;
           DMESTOQUE.Alx.SQL.Clear;
               ...
               Then Begin
                       ...
                       Then Begin
                             ...
                             Then Begin
                                 ...
                                 //ICMS ST
                                  DMSAIDA.TItemPV.FieldByName('baseicmssubs').AsCurrency := (((DMSAIDA.TItemPV.FieldByName('VALORTOTAL').AsCurrency + DMSAIDA.TItemPV.FieldByName('VLRIPI').AsCurrency) * XMva)/100) + DMSAIDA.TItemPV.FieldByName('VALORTOTAL').AsCurrency + DMSAIDA.TItemPV.FieldByName('VLRIPI').AsCurrency;
           ...
           Then Begin
              ...
              // Else Begin
               ...
               filtraTabela(DMGEOGRAFIA.TEstado, 'ESTADO', 'UF_ESTADO', Fmenu.XEstadoDestino, '');
                                  ...
                                  Then Begin
                                       ...
                                       Then Begin
                                        // - 30/03/2009: calcula ICMS ST
                                         ...
                                         DMSAIDA.TFiscPV.FieldByName('bascalsubs').AsCurrency := DMESTOQUE.Alx3.fieldByName('TOTAL_BASEST').AsCurrency; // + ((( DMSAIDA.TFiscPV.FieldByName('vlroutrasdesp').AsCurrency + DMSAIDA.TFiscPV.FieldByName('vlrseg').AsCurrency + DMSAIDA.TFiscPV.FieldByName('VLRFRETE').AsCurrency + DMSAIDA.TFiscPV.FieldByName('VLRTOTIPI').AsCurrency) * XMva)/100) + DMESTOQUE.Alx3.fieldByName('TOTAL_BASEST').AsCurrency ;
                             ...
                             Then Begin
                                 if (DMSAIDA.WPedV.FieldByName('valor').AsCurrency > 0)
                                  Then Begin
                                    IF XDadosAdicionais2 <> '' Then
                                       XDadosAdicionais2 := XDadosAdicionais2 + #13;
                                    XDadosAdicionais2 := XDadosAdicionais2 + ' -Permite aproveit. crédito de ICMS valor de R$ ' + FormatFloat('#,##0.00', (DMSAIDA.WPedV.FieldByName('valor').AsCurrency * DMMACS.TEmpresa.FieldByName('ALIQICMS').AsCurrency)/100) + ' corresp. aliq. ' + DMMacs.TEmpresa.FieldByName('ALIQICMS').AsString + '% nos termos Art. 23LC123/2006.'
                                  End;
                             End
                             Else Begin
                                DMESTOQUE.Alx1.Close;
                               ...
                               Then Begin
                                    If ((XEmitIndustria) And (XDestIndustria))
                                    Then Begin   // - 30/03/2009:       industria p/ industria calcular sobre total dos produtos
                                      if (DMSAIDA.WPedV.FieldByName('valor').AsCurrency > 0)
                                      Then Begin
                                        IF XDadosAdicionais2 <> '' Then
                                       XDadosAdicionais2 := XDadosAdicionais2 + #13;
                                    ...
                                    Else Begin
                                       ...
                                       Then Begin
                                            IF XDadosAdicionais2 <> '' Then
                                               XDadosAdicionais2 := XDadosAdicionais2 + #13;
                                            XDadosAdicionais2 := XDadosAdicionais2 + '-Permite aproveit. crédito de ICMS valor de R$ ' + FormatFloat('#,##0.00', (DMESTOQUE.Alx1.FieldByName('TOTAL').AsCurrency * DMMACS.TEmpresa.FieldByName('ALIQICMS').AsCurrency)/100) + ' corresp. aliq. ' + DMMacs.TEmpresa.FieldByName('ALIQICMS').AsString + '% nos termos Art. 23LC123/2006.';
                                       End;
                                    End;
                                 End;
                              End;
                       ...
                       if FMenu.XPedFatura = ''
                       then begin  // se vem do pedido de venda
                            ...
                            // - 30/03/2009: industria para industrica
                              ...
                              Then Begin
                               ...
                               Then Begin
                                    DMSAIDA.TFiscPV.Edit;
                                    DMSAIDA.TFiscPV.FieldByName('vlrtotdocnf').AsCurrency:=DMSAIDA.TFiscPV.FieldByName('VLRTOTPROD').AsCurrency+DMSAIDA.TFiscPV.FieldByName('vlroutrasdesp').AsCurrency+DMSAIDA.TFiscPV.FieldByName('vlrseg').AsCurrency+DMSAIDA.TFiscPV.FieldByName('vlricmsubs').AsCurrency + DMSAIDA.TFiscPV.FieldByName('VLRTOTIPI').AsCurrency;
                                    If DMSAIDA.TFiscPV.FieldByName('FRETECOMP').AsString='1' Then
                                       DMSAIDA.TFiscPV.FieldByName('vlrtotdocnf').AsCurrency:=DMSAIDA.TFiscPV.FieldByName('vlrtotdocnf').AsCurrency+DMSAIDA.TFiscPV.FieldByName('vlrfrete').AsCurrency;
                       ...
                       else begin  // se vem do faturar calculo de soma de produtos diferenciado
                   ...
                   End
                   Else Begin  //começa recálculos para ordens de serviço
...
End;
   ...
   End;
	...
	begin
		...
		registro.RootKey:=HKEY_LOCAL_MACHINE; registro.openkey('System\CurrentControlSet\Services\VXD\VNETSUP',false); result:=registro.readstring('ComputerName');
	end;
           ...
           Then Begin
       		...
       		Begin
       			...
       			begin
                       ...
                       Then Begin//se o tag for diferente de 5 significa que a tabela deve ser ativada
                       	Try
   ...
   Begin
   	Try
       	...
       	Then BEGIN
               ...
               Then Begin//Se o usuario é administrado ele tem acesso a tudo
                   Result:=True;
               End
   ...
   Var
    ...
    Begin
	    ...
	    Then Begin//Verifica se o formulário já foi criado
    ...
    Begin
    	...
    	Try
         ...
         Else Begin
            ...
            If (XSQLAux<>'')
            Then Begin//passa sql auxiliar se o valor em branco vai passar apenas a sql auxiliar
            	If Xvalor='' Then
            ...
            If XSql<>''
            Then Begin  //se não foi passado nada para consulta prepara uma consuilta para retornar tabela vazia
            	Try
                   Xtabela.Prepare;
	         		Xtabela.Open;
               except
    ...
    Begin
        ...
        Begin
            If not (STR[I] IN ['0'..'9'])
            Then Begin//a letra em questão não é um numero
            	If FLAG=1
               Then Begin//se depois de ter encontrado uma strin numerica voltou a encontrar uma string literal, junta as duas como strin literal e zera a numérica
                   FLAG:=0;
...
{***    function VerifExitProd(Xtabela: String; XVALORPK:STRING; XCAMPO: STRING; XValor:String):Boolean;
   ...
   if nResto <> CharToInt(dig2[1]) then exit;
    ...
    begin
       ...
       if (length(ie) <> 14) and (length(ie) <> 9) then exit;
       if not IsNumero(ie) then exit;
       ...
       begin
           ...
           for i := 1 to 9 do nro9[i] := CharToInt(ie[i]);
   ...
   begin
       ...
       if Sigla = 'AC'  then Result := ValidaInscEst_AC(Insc_est) else
       if Sigla = 'AL'  then Result := ValidaInscEst_AL(Insc_est) else
       if Sigla = 'AP'  then Result := ValidaInscEst_AP(Insc_est) else
       if Sigla = 'AM'  then Result := ValidaInscEst_AM(Insc_est) else
       if Sigla = 'BA'  then Result := ValidaInscEst_BA(Insc_est) else
       if Sigla = 'CE'  then Result := ValidaInscEst_CE(Insc_est) else
       if Sigla = 'DF'  then Result := ValidaInscEst_DF(Insc_est) else
       if Sigla = 'ES'  then Result := ValidaInscEst_ES(Insc_est) else
       if Sigla = 'GO'  then Result := ValidaInscEst_GO(Insc_est) else
       if Sigla = 'MA'  then Result := ValidaInscEst_MA(Insc_est) else
       if Sigla = 'MG'  then Result := ValidaInscEst_MG(Insc_est) else
       if Sigla = 'MT'  then Result := ValidaInscEst_MT(Insc_est) else
       if Sigla = 'MS'  then Result := ValidaInscEst_MS(Insc_est) else
       if Sigla = 'PA'  then Result := ValidaInscEst_PA(Insc_est) else
       if Sigla = 'PB'  then Result := ValidaInscEst_PB(Insc_est) else
       if Sigla = 'PR'  then Result := ValidaInscEst_PR(Insc_est) else
       if Sigla = 'PE'  then Result := ValidaInscEst_PE(Insc_est) else
       if Sigla = 'PI'  then Result := ValidaInscEst_PI(Insc_est) else
       if Sigla = 'RJ'  then Result := ValidaInscEst_RJ(Insc_est) else
       if Sigla = 'RO'  then Result := ValidaInscEst_RO(Insc_est) else
       if Sigla = 'RN'  then Result := ValidaInscEst_RN(Insc_est) else
       if Sigla = 'RS'  then Result := ValidaInscEst_RS(Insc_est) else
       if Sigla = 'SC'  then Result := ValidaInscEst_SC(Insc_est) else
       if Sigla = 'SP'  then Result := ValidaInscEst_SP(Insc_est) else
       Result := false;
   end;
...
END;
    ...
    Begin
        If Calculo<>0
        Then Begin//não pode efetuar a função sem que tenha sido passado o calculo
            If Percento=0
            Then begin //calculo como retorno o valor de percentual
                AlxPercento:=(Valor*100)/Calculo;
                AlxValor:=Valor;
            End
            Else Begin // calcula como retorno para valor
...
//            Transaction.StartTransaction;
    ...
    {Procedure Utilizada p/ Abrir o help on line}
	 Procedure AbrirHelp(xHelp: xTypeManual);
       var xLink: array[0..220] of WideChar;
       xNomeArquivo: String;
	 Begin
...
//Paulo 11/05/10: Verifica se existe o txt no ftp.
Procedure VerificaArquivo;
var LineFtp, dirDowFTP, ArquivoFtp :String;
Begin
   LineFtp := '';
...
//Paulo 11/05/2010: Rele o txt em seguida adiciona novas atualizações
Procedure ReescreveArquivo;
var  XArquivoVelho,XArquivoNovo, XArquivo:TextFile;
...
//Paulo 16/05/2010:Substitui o arquivo atual no ftp por um txt em branco
Procedure ZeraArquivo;
var LineFtp2, XArq, XArqNovo, LineFtp, dirDowFTP, ArquivoFtp :String;
begin
...
Begin
	if(FileExists('C:\MZR\Arquivos\AtualizaBanco\sincro.txt'))Then Begin
   	...
   	if not directoryExists('C:\MZR\Arquivos\AtualizaBanco\Upload\') then
		...
		//(Download de arquivo)
		if(DMMACS.TLoja.FieldByName('RECEBEARQFTP').AsString = '1')Then Begin
   		//Caminho onde sera gravado o arquivo
   		LineFtp := 'C:\MZR\Arquivos\AtualizaBanco\Downloads\Atualiza.txt';
   		//Se o diretorio nao existir
   		if not(DirectoryExists(DirecDownFtp))Then Begin
   		...
   		if not (FileExists(lineFtp)) then Begin
      			with FMenu.IdFTP1 do try
      				//Conectando ao servidor
   				Username := DMMACS.TLoja.FieldByName('USERNAMEFTP').AsString;
   				Password := Fmenu.Cripto.CriptoHexToText(DMMACS.TLoja.FieldByName('PASSWORDFTP').AsString);
   	...
   	if(DMMACS.TLoja.FieldByName('CRIAARQFTP').AsString = '1')Then Begin
   ...
   Begin
       ...
       XValor1:=AnsiUpperCase(Copy(XLinha,125,3));
           ...
           //Se arquivo ja existe entao insere no banco senao update(atualiza)
			if((XTable = 'CST')and(DMESTOQUE.Alx1.IsEmpty))Then begin
       	...
       	End
       	Else Begin
       		if((XTable = 'CST')and(not(DMESTOQUE.Alx1.IsEmpty)))Then begin
   	...
   	if(XTable = 'UNIDADE')Then Begin
   		ComparaTables('UNIDADE','SIGLA_UNID',Trim(XValor));
   		if((XTable = 'UNIDADE')and(DMESTOQUE.Alx1.IsEmpty))Then begin
       	...
       	End
       	Else Begin
       		if((XTable = 'UNIDADE')and(not(DMESTOQUE.Alx1.IsEmpty)))Then begin
   	...
   	if(XTable = 'SUBGRUPOPROD')Then Begin
   		ComparaTables('SUBGRUPOPROD','DESCRICAO',Trim(XValor));
   		if((XTable = 'SUBGRUPOPROD')and(DMESTOQUE.Alx1.IsEmpty))Then begin
       	...
       	End
       	Else Begin
       		if((XTable = 'SUBGRUPOPROD')and(not(DMESTOQUE.Alx1.IsEmpty)))Then begin
       ...
       if(XTable = 'GRUPOPROD')Then Begin
       	ComparaTables('GRUPOPROD','DESCRICAO',Trim(XValor));
   		if((XTable = 'GRUPOPROD')and(DMESTOQUE.Alx1.IsEmpty))Then begin
       	...
       	End
       	Else Begin
       		if((XTable = 'GRUPOPROD')and(not(DMESTOQUE.Alx1.IsEmpty)))Then begin
       ...
       if(XTable = 'PRODUTO')Then Begin
           ...
           XSUBGRUPO := AnsiUpperCase(Copy(XLinha,437,100));
			...
			if((XTable = 'PRODUTO')and(DMESTOQUE.Alx1.IsEmpty))Then begin
       	...
       	End
       	Else Begin
       		if((XTable = 'PRODUTO')and(not(DMESTOQUE.Alx1.IsEmpty)))Then begin
       ...
       if(XTable = 'SUBPRODUTO')Then Begin
           ...
           XCodProd:= DMESTOQUE.Alx4.FieldByName('COD_PRODUTO').AsInteger;
			if((XTable = 'SUBPRODUTO')and(DMESTOQUE.Alx1.IsEmpty))Then begin
       ...
       if(XTable = 'ESTOQUE')Then Begin
           ...
           dmestoque.Alx1.RecordCount;
			if((XTable = 'ESTOQUE')and(DMESTOQUE.Alx2.IsEmpty))Then begin
           ...
           End
       	Else Begin
       		if((XTable = 'ESTOQUE')and(NOT(DMESTOQUE.Alx1.IsEmpty)))Then begin
   ...
   begin
       ...
       begin
           if (Pos('@', EMail)=1) or (Pos('@', EMail)= Length(EMail)) or (Pos('.', EMail)=1) or (Pos('.', EMail)= Length(EMail)) or (Pos(' ', EMail)<>0) then
               Result := False
           else begin // @ seguido de . e vice-versa
