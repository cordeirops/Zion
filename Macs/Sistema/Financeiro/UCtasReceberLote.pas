...
uses
  ...
  QRCtrls, DB{, MDOCustomDataSet, MDOQuery};
...
type
  ...
  public
    ...
    //Procedure utilizada para a//Testa Clientes sem data de vencimento informadas no cadasatro
    Function TestaDataVencimentoClientes: Boolean;
...
uses UDMFinanc, UDMPessoa, UDMConta, UDMMacs, UMenu , Alxor32, AlxMessage,
  UConsPlncta, UDmBanco, UCtaCor, UDMSaida, UPadrao, Alxorprn, UMDO;
...
{   //FILTRA LOJA PARA VERIFICAR O TIPO DE IMPRESSÃO
   ...
   Do Begin
   	...
   	DMFINANC.TRel.Next;
   End;}
   ShowMessage('Opção de Impressão de Boleto desativada.');
End;
...
//Testa Clientes sem data de vencimento informadas no cadasatro
Function TFCtasReceberLote.TestaDataVencimentoClientes: Boolean;
Var
 ClientesVencimento: String;
Begin
   Try
       Result:=True;
       Mdo.QAlx1.Close;
       MDO.QAlx1.SQL.Clear;
       MDO.QAlx1.SQL.Add(' select cliente.diavencimento, vwcliente.nome as CLIENTE  from vwcliente ');
       MDO.QAlx1.SQL.Add(' left join cliente on vwcliente.cod_cliente = cliente.cod_cliente ');
       MDO.QAlx1.SQL.Add(' where ((cliente.diavencimento<1) or (cliente.diavencimento>28) or (cliente.diavencimento is null)) and (vwcliente.export = ''X'') and ((vwcliente.ativo=''1'') or (vwcliente.ativo is null)) and ((vwcliente.separar<>''1'') or (vwcliente.separar is null)) ');
       MDO.QAlx1.Open;
       ...
       Then Begin
           ...
           Begin
               ClientesVencimento:=' '+MDO.QAlx1.FieldByName('CLIENTE').AsString+#13;
               MDO.QAlx1.Next;
           End;
           MessageDlg('O(s) Clientes(s):'+#13+#10+''+#13+  #10+'Não apresentam a data de vencimento em seu cadastro.', mtWarning, [mbOK], 0);
       End;
...
Begin
   ...
   DMFINANC.TAlx.SQL.Add('SELECT * FROM vwCLIENTE');
   DMFINANC.TAlx.SQL.Add('LEFT JOIN PESSOA ON vwCLIENTE.COD_PESSOA=PESSOA.COD_PESSOA');
   DMFINANC.TAlx.SQL.Add(' left join cliente on vwcliente.cod_cliente = cliente.cod_cliente ');
   DMFINANC.TAlx.SQL.Add('WHERE (vwCLIENTE.EXPORT='+#39+'X'+#39 + ') and ((vwcliente.ativo=''1'') or (vwcliente.ativo is null)) and ((vwcliente.separar<>''1'') or (vwcliente.separar is null)) order by vwcliente.Nome');
   DMFINANC.TAlx.Open;
...
End;
    ...
    Begin
        ...
        DMFINANC.TAlx.Close;
        DMFINANC.TAlx.SQL.Clear;
        DMFINANC.TAlx.SQL.Add(' select * from vwcliente  WHERE ((vwcliente.ativo<>0) or (vwcliente.ativo is null)) and ((vwcliente.separar<>1) or (vwcliente.separar  is null)) order by vwcliente.nome ');
        DMFINANC.TAlx.Open;
    End;
...
begin
  inherited;
	Caption:='Cadastro de Ctas em Receber em Lote';
   SelecionaCliente;
   BtnLancamento.Click();
   LimpaEdits;
...
begin
  inherited;
   ...
   DMPESSOA.TALX.SQL.Add('UPDATE CLIENTE SET CLIENTE.EXPORT='+#39+'X'+#39 +' where ((cliente.ativo<>0) or (cliente.ativo is null)) and ((cliente.separar<>1) or (cliente.separar  is null))');
   ...
   Then Begin
       ...
       Then Begin
           ...
           Then Begin
			...
			End;
       End;
       If (CBTipoValor.Text = 'DO CADASTRO') and (TestaDataVencimentoClientes=False) Then
           Exit;
...
begin
    ...
    DMPESSOA.TALX.Close;
    DMPESSOA.TALX.SQL.Clear;
    DMPESSOA.TALX.SQL.Add('select count(vwcliente.cod_cliente) AS NUMERO_CLIENTES from vwcliente where (vwcliente.export = ' + #39 + 'X' + #39+') and ((vwcliente.ativo=''1'') or (vwcliente.ativo is null)) and ((vwcliente.separar<>''1'') or (vwcliente.separar is null))');
    DMPESSOA.TALX.Open;
