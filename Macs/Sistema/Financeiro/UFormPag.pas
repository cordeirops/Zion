...
begin
    ...
    THEN Begin
        DBDESC.Text:= 'GARANTIA'; //forma de pagamento será a vista
        EDPRIMPARCELA.Value:=1;
        EDQUANTPARCELA.Value:=1;
        If FiltraTabela(DMFINANC.TAlx, 'FORMPAG', 'DESCRICAO', '', ' DESCRICAO = '+#39+'GARANTIA'+#39+' AND COD_LOJA='+FMenu.LCODLOJA.Caption)=TRUE
        Then Begin//não pode haver duas formas de pagamento=GARANTIAS
	         Mensagem('OPÇÃO BLOQUEADA', 'Esta forma de pagamento é descrita com pagto GARANTIA. Porém ja existe uma forma de pagamento deste tipo!', '', 1, 1, False, 'a');
       	 exit;
        End;
    End;
    If CBCortesia.Checked=True
    THEN Begin
        DBDESC.Text:= 'CORTESIA'; //forma de pagamento será a vista
        EDPRIMPARCELA.Value:=1;
        EDQUANTPARCELA.Value:=1;
        If FiltraTabela(DMFINANC.TAlx, 'FORMPAG', 'DESCRICAO', '', ' DESCRICAO = '+#39+'CORTESIA'+#39+' AND COD_LOJA='+FMenu.LCODLOJA.Caption)=TRUE
        Then Begin//não pode haver duas formas de pagamento=CORTESIA
	         Mensagem('OPÇÃO BLOQUEADA', 'Esta forma de pagamento é descrita com pagto CORTESIA. Porém ja existe uma forma de pagamento deste tipo!', '', 1, 1, False, 'a');
       	 exit;
        End;
    End;
...
begin
  inherited;
   ...
   Else
       CBFORMPAGVISTA.Checked:=False;
	If DMFINANC.TFormPag.FieldByName('DESCRICAO').AsString='GARANTIA' Then
       CBGarantia.Checked:=True
   Else
       CBGarantia.Checked:=False;
	If DMFINANC.TFormPag.FieldByName('DESCRICAO').AsString='CORTESIA' Then
