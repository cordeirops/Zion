...
type
  TFFaturaContas = class(TFPadrao)
    ...
    Label11: TLabel;
    EdTxCartao: TEdit;
    cbImpReciboModelo01: TCheckBox;
    EDDescPercento: TEdit;
    cbImpComprovante: TCheckBox;
    cbImpReciboModelo02: TCheckBox;
...
begin
  inherited;
   ...
   Then Begin
       ...
       Exit;
   End;
   If (cbPagamento.Text<>'Banco') And  (cbPagamento.Text<>'Carteira')  And (cbPagamento.Text<>'Pix') And (cbPagamento.Text<>'Cheque') And (cbPagamento.Text<>'Cartão') And (cbPagamento.Text<>'Boleto')  And (cbPagamento.Text<>'C - Cartão')  And (cbPagamento.Text<>'D - Cartão') And (cbPagamento.Text<>'Crédito')
   Then Begin
   ...
   Then Begin
       ...
       Then Begin
           ...
           then begin
               ...
               If cbImpReciboModelo01.Checked = True
               then Begin
               ...
               if cbImpReciboModelo02.Checked = true then
               begin
					FMenu.TIPOREL := 'RECIBOFATCONTAMODELO2';
                   FMenu.Extenso.Numero:=FormatFloat('0.00', StrToFloat(edTotal.Text));
                   ImpReciboFatura(edNome.Text, xEndereco, edTotal.Text,UpperCase(FMenu.Extenso.Extenso),XHist, edCodigo.Text, DMFINANC.TAlx.FieldByName('NUMPARC').AsString);
                   FMenu.RvFaturas.SystemPreview.FormState := wsMaximized;
                   FMenu.RvFaturas.DefaultDest := rdPreview;  //- 19/02/2009: visualizar o relatório direto
                   FMenu.RvFaturas.SystemPrinter.Units := unMM; //- 19/02/2009: trabalha nos milimetros da página
                   FMenu.RvFaturas.SystemPReview.RulerType := rtBothCm; //- 19/02/2009: mostra régua superior e lateral
       ...
       Else Begin
           ...
           If xTipo = 'CR'
           Then Begin
		        IF LancConta('PEDVENDA', xCodFaturaConta, xCodPessoa, cbPagamento.Text, StrToFloat(edTotal.Text), 0, xCodConta, 0,  -1, XHist, XCodFormPag, EdDoc.Text, 'R', StrToDateTime(EdDataRef.Text), 0, EdDataRef.Text, '')=True
               Then  //Se lançou
                   Flag:=1
               Else
                   Flag:=0;
           End
           Else Begin
			    IF LancConta('FATCP'{'PEDCOMPRA'}, xCodFaturaConta{StrToInt(EdDoc.Text)}, xCodPessoa, cbPagamento.Text, StrToFloat(edTotal.Text), 0, xCodConta , 0,  -1, XHist, xCodFormpag, EdDoc.Text, 'P', StrToDateTime(EdDataRef.Text), 0, EdDataRef.Text, '')=True
               Then
                   Flag:=1
               Else
                   Flag:=0;
       ...
       then Begin
           ...
           ImpReciboFaturaContas(xCodFaturaConta,'CONTAS'{'FATURA'}, 'CR');
           FMenu.RvFaturas.SystemPreview.FormState := wsMaximized;
           FMenu.RvFaturas.DefaultDest := rdPreview;  //- 19/02/2009: visualizar o relatório direto
           FMenu.RvFaturas.SystemPrinter.Units := unMM; //- 19/02/2009: trabalha nos milimetros da página
           FMenu.RvFaturas.SystemPReview.RulerType := rtBothCm; //- 19/02/2009: mostra régua superior e lateral
...
begin
  inherited;
       xCodCtaCor:=-1;
		if xTipo = 'CR'
       then begin
           Caption := 'Faturar Contas a Receber';
           ConsultaParcelasCR;
           preparaDados;
       ...
       if xTipo = 'CP'
       then begin
           Caption := 'Faturar Contas a Pagar';
           ConsultaParcelasCP;
           preparaDados;
       End;
end;
