...
type
  ...
  private
    ...
    //procedure utilizada para verificar a forma de lançamento referente a diferença de valores Maior
    procedure VerificaFormaLancamentoDiferencaMaior;
    //procedure utilizada para verificar a forma de lançamento referente a diferença de valores Menor
...
var
  ...
  xSucessoBaixa: Integer;//somente irá comitar em financ quando xSucessoBaixa igual a 1 que significa que todas as operações foram concluídas
  xNaoLancTroco: Integer;
  xGeraNovaParcela: String;
  XCOD_PARCELA_BAIXADA: Integer;
  //Edmar - 19/01/2015 - Variável que vai armazenar o código da nova parcela que foi criada para o cheque lançado
  //Caso o usuário escolha baixar apenas o valor pago de uma conta em cheque, deve-se criar uma nova conta
  //vinculando o cheque lançado a nova conta
  xCodNewParc: Integer;  
implementation
...
begin
   LVlrCred.Caption:='0' ;
  inherited;
  //Edmar - 15/04/2015 - Armazena o código da PARCELACR para usar na hora de baixar a conta
  //variavel necessária pois depois que criar a nova parcela, o sistema filtrava o WParR
...
begin
  inherited;
    ...
    //Edmar - 08/01/2015 - Foi trocado a condição pois ele testava o valor antes
    //de alguma alteração, caso fosse dado algum desconto na conta
    //[Solicitação 282]
  	 //If XVLRDEV>XVLRFINAL
 	 {Edmar - 12/06/2015 - Alterado toda a condição para buscar o valor do desconto
    e também validar se foi selecionado alguma opção
    If XVLRFINAL>=XVLRPG
    Then Begin
       ...
       Then Begin
           Mensagem('INFORMAÇÃO AO USUÁRIO', 'Por favor, informe o que deve ser feito com a diferença de valores. Escolha uma opção!.', '', 1, 1, False, 'i');
           Exit;
       End;
    End
    Else Begin
       ...
       //Edmar - 13/01/2015 - Aqui a condição descrita acima também foi trocada
    ...
    //Edmar - 12/06/2015 - Valida se foi seleciona alguma opção caso os valores estejam diferentes
    ...
    //para calcular o valor pago antes validar se foi selecionada alguma opção
    //[Solicitação 282]
	 //XVLRPG:=EDVALORPAGO.ValueNumeric-xvlrlanc;

    // - 19/01/2009: incluido multiplo
    If (DBCOB.Text<>'Crédito') and (DBCOB.Text<>'Chq. Terc.') and (DBCOB.Text<>'Pix') and (DBCOB.Text<>'Banco') And  (DBCOB.Text<>'Carteira') And (DBCOB.Text<>'Cheque') And (DBCOB.Text<>'Cartão') And (DBCOB.Text<>'Boleto')  And (DBCOB.Text<>'C - Cartão')  And (DBCOB.Text<>'D - Cartão')
    ...
    if ((DBCOB.Text = 'C - Cartão') OR (DBCOB.Text = 'D - Cartão')) AND (XCod_Cartao = -1) then
    begin
    	Mensagem('INFORMAÇÃO AO USUÁRIO', 'Por favor informe o cartão que será usado na baixa.', '', 1, 1, False, 'i');
      	DBCOB.SetFocus;
      	Exit;
    end;

    {Edmar - 14/01/2015 - Comentado a instrução abaixo pois foi alterado a politica do ZION
    e não é mais necessário informar um histórico para criar ou baixar uma conta
    If DBHISTORICO.text=''
    Then Begin//caso não tenha sido informado a Conta
    ...
    begin//Foi cobrado juros/multas ou efetuado desconto
	 ...
	 end;//FINAL CONDIÇÃO (XVLRFINAL<>XVLRDEV)

    If XTipoPR='P' then
    begin
    	// - 19/01/2009: CONTROLE PAGAMENTO MÚLTIPLO
       if (DBCOB.Text='Múltiplo') then
       begin
           ...
           begin
               ...
               begin//controla os lançamentos que devem ser feitos em caixa
           ...
           if XVLRFINAL > XVLRPG then
           begin//se pago não cobre o valor devido o correto é lançar nova parcela
           	if Mensagem('A T E N Ç Ã O', 'O Valor a ser Pago é de '+FloatToStr(XVLRFINAL)+#13+'O Valor Pago é de '+FloatToStr(EDVALORPAGO.ValueNumeric)+#13+'Deseja converter a diferença em uma outra parcela?', '', 2, 3, False, 'c')=2 then
               begin
        ...
        begin
           try
               ...
               begin
                   ...
                   //deve-se lançar o valor devido para evitar confusões
...
{            	If EDVALORPAGO.ValueNumeric<=DMPESSOA.TPessoa.FieldByName('vlrcredito').AsCurrency
            ...
            Then Begin
               ...
               try
                   ...
                   Mensagem(' A T E N Ç Ã O ', 'A Movimentação Bancária com historico '+DMBANCO.TMovBanco.FieldByName('HISTORICO').AsString+' não foi baixada. A operação será abortada.', '', 1, 1, False, 'A');
                   Exit;
               end;
            End;
            end;
        ...
        Then Begin
            Try
            		...
            		Exit;
                {
	             //Edita tabela para entradas de valores para parcela
                DMCONTA.TParcCR.Edit;
                //Edmar - 09/02/2015 - Quando a cobrança for do tipo cartão, apenas muda a forma de pagamento,
                //seta o cartão que deve ser usado para a baixa e atualiza os valores de multa, juros e desconto
                if (DBCOB.Text = 'C - Cartão') OR (DBCOB.Text = 'D - Cartão') then
                begin
                   ...
                   DMCONTA.TParcCR.FieldByName('JUROS').Value:=EdJuro.ValueNumeric;
                	...
                	//caso contrario, usar o metodo padrão
   ...
   begin
       ...
       begin
        ...
        then begin
               ...
               //se não achar, busca pelo mesmo cliente com o vencimento mais proximo
...
{   //controla visualização de componentes referentes a escolha e visualização da cta corrente
   ...
   //controla visualização de componentes referentes a escolha e visualização da cta corrente
	If (DBCOB.Text='Boleto') or (DBCOB.Text='Banco') or (DBCOB.Text='Pix') or (DBCOB.Text='Cartão') or ((DBCOB.Text='Cheque') And (XTipoPR='P')) 
   Then Begin
...
//procedure utilizada para verificar a forma de lançamento referente a diferença de valores
procedure TFPadraoBaixaContas.VerificaFormaLancamentoDiferencaMaior;
var
 xvlrpg, xvlrdv: String;
begin
  inherited;
  	//Edmar - 03/11/2014 - Valida se foi selecionado mais de uma opção
  	...
  	begin
		Mensagem(' I N F O R M A Ç Ã O ', 'Escolha apenas uma opção.', '', 1, 1, False, 'I');
       Exit;
  	end;

   //se vai lançar troco atribui 0 a variavel
   ...
   begin
	...
	xvlrlanc := 0;
	//Edmar - 31/10/2014 - verifica qual opção escolheu e faz o tratamento desejado
...
//procedure utilizada para verificar a forma de lançamento referente a diferença de valores a menor
...
begin
	Try
       //Edmar - 03/11/2014 - verifica se foi selecionado alguma opção
       ...
       //Edmar - 31/10/2014 - Verifica qual opção foi selecionada
       if cbNewParc.Checked then//deseja criar uma nova parcela
       begin
			if Mensagem(' CONFIRMAÇÃO DO USUÁRIO ', 'O sistema irá gerar uma nova parcela. Deseja confirmar esta operação?' , '', 2, 3, False, 'C')= 2 then
       	begin
               if XTipoPR = 'P' then
               begin
                   if DBCOB.TEXT = 'Cheque' then
                   ...
                   begin//caso deva relançar servico
                       VLRPROD:=(EDValorJurMulta.ValueNumeric - EDVALORPAGO.ValueNumeric)/2;
                       VLRSERV:=(EDValorJurMulta.ValueNumeric - EDVALORPAGO.ValueNumeric)/2;
                   end
                   else begin
                   ...
                   begin
                       ...
                       //Edmar - 06/06/2014 - Filtra buscando a forma de pagamento para o lançamento
                       ...
                       else//Edmar - 06/06/2014 - Se não encontrar nada, atribui a forma de pagamento que já estava na conta
                   ...
                   if xSucessoBaixa = 1 then//Edmar - 30/05/2014 - Seta que gerou uma nova parcela para não lançar outro cheque
                       xGeraNovaParcela := 'S';
               end;
           end;
       end;
       ...
       //valida se não foi selecionado nenhuma opção
...
begin
  inherited;
   //verifica se nenhuma opção foi marcada e mostra advertencia e desabilita o botão de baixar
   if NOT (cbNewParc.Checked) AND NOT (cbBaixaValorPago.Checked) then
   begin
   	Mensagem(' I N F O R M A Ç Ã O ', 'Não será possível baixar uma conta a menos que uma opção esteja selecionada.', '', 1, 1, False, 'I');
  ...
  inherited;
   ...
   begin
        If XVlrCheque<=0 Then
           MessageDlg('Os valores dos cheques não foram informados.', mtWarning, [mbOK], 0)
        Else
   	     EDVALORPAGO.ValueNumeric:=XVlrCheque;
   end;

	If (StrToFloat(TiraPonto(EDVALORPAGO.Text)) > StrToFloat(TiraPonto(EDValorJurMulta.Text))) AND (XVlrCheque<=0) then
   begin//caso não tenha sido informado o valor devidamente
   ...
   begin//caso não tenha sido informado o valor
        Mensagem('INFORMAÇÃO AO USUÁRIO', 'Por favor informe o valor pago corretamente, ele não pode ser um valor nulo.', '', 1, 1, False, 'i');
        EDVALORPAGO.SetFocus;
        Exit;
   end
