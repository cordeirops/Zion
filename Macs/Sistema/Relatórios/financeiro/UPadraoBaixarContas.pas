...
uses UDMFinanc, Alxor32, AlxMessage, UDMConta, UMenu, UConsPlncta,
  ...
  ULanCHSaida, UDMEstoque, Alxorprn, Math, UDMSaida, UMDO;
...
begin
  inherited;
    ...
    //
       ...
       If (DBCOB.Text<>'Banco') And  (DBCOB.Text<>'Carteira') And (DBCOB.Text<>'Cheque') And (DBCOB.Text<>'Cartão') And (DBCOB.Text<>'Boleto')  And (DBCOB.Text<>'C - Cartão')  And (DBCOB.Text<>'D - Cartão')
...
//                Dmfinanc.TParcelaCR.FieldByName('COD_COTA').Value:=Dmfinanc.TAlxMoeda.FieldByName('COD_COTA').Value;
            ...
            //CONTROLE PARA FORMA DE PAGAMENTO CHEQUE
            	...
            	Then Begin
                    FMenu.XDTREF:=DMCONTA.WParR.FIELDBYNAME('dtlanc').AsString;
              		 If LancChEnt(XCOD_CONTA, 0,  'Bx. C.R.-'+DMCONTA.WParR.FIELDBYNAME('NUMPARC').AsString +' Venc.:'+DMCONTA.WParR.FIELDBYNAME('DTVENC').AsString+'-'+DMCONTA.WParR.FIELDBYNAME('NOME').AsString, StrToDate(EdVenc.Text), '', XVLRPG, 0, '', '', 0, '', '', 'CTAR', IntToStr(XCOD_CONTABAIXA))=True
          			 Then  //Se lançou
              			Flag:=1
                    Else
              			Flag:=0;
