...
begin
  inherited;
    ...
    //Paulo 12/09/2011: Traz a soma de cheque somente entradas
    DMPESSOA.TALX.Close;
    ...
    //Paulo 12/09/2011: Traz a soma de cheque somente saídas
    ...
    DMPESSOA.TALX.ParamByName('TIPOCAI').AsString:='S';
    DMPESSOA.TALX1.Open;
    ...
    If (FMenu.LUSUARIO.Caption='SYSTEM LORD') or (DMMacs.TALX.FieldByName('ADM').AsInteger=1)
    Then Begin
   ...
   If FileExists('C:\MZR\Arquivos\LayoutGrid\LanCaixa.lgm')
   Then
  ...
  inherited;
   BtnGravar.SetFocus;
    ...
    Then Begin //bloqueia caso não tenha sido informado o tipo Caixa
        ...
        exit;
    End;
    If (DMCAIXA.TLanCaixa.FieldByName('TIPO').AsString='Crédito') and (XCOD_PESSOACRED<=0)
    Then Begin
    ...
    If XState='INSERT'
    Then Begin
        DMCAIXA.TAlx.SQL.Add('  insert into lancaixa (DTLANC, DTREFER, COD_PLNCTAFIL, ESTRU, HISTORICO, VALOR, TIPOGERADOR, COD_GERADOR, ');
        DMCAIXA.TAlx.SQL.Add('                         TIPO, TIPCAI, COD_ABCAIXA, COD_USUARIO, DOC) ');
        DMCAIXA.TAlx.SQL.Add('          values (:DTLANC, :DTREFER, :COD_PLNCTAFIL, :ESTRU, :HISTORICO, :VALOR, :TIPOGERADOR, :COD_GERADOR, ');
        DMCAIXA.TAlx.SQL.Add('                         :TIPO, :TIPCAI, :COD_ABCAIXA, :COD_USUARIO, :DOC) ');
    ...
    Except
       ...
       MessageDlg('O Lançamento não foi efetuado!', mtWarning, [mbOK], 0);
       dbs(False);
       Exit;
    End;
    ...
    Then Begin
       ...
       If DMCAIXA.TLanCaixa.FieldByName('TIPCAI').AsString='E'
       Then Begin
    ...
    Else Begin
    	...
    	Then Begin
 			If LanCaixa (XCOD_CONTABANCO, DateToStr(Date()), DMMACS.TLoja.FieldByName('PLNCTATRANSFCXCX').AsInteger, 'Transf Cx -> Cx '+ FMenu.LCAIXA.Caption,  DBVALOR.ValueNumeric , 'TCXCX', XCod_LancamentoCX, DBTIPO.Text, 'E', DBDoc.Text, Date(), '1')=True
       	Then Begin
           	DMBANCO.IBT.CommitRetaining;
               MDO.Transac.CommitRetaining;
       	End
    ...
    //Verifica para abrir gaveta
	 If UpperCase(DBTIPO.Text)= 'CARTEIRA' Then
       ecfOpenGaveta;
...
begin
  inherited;
   ...
   End
        ...
        DMCAIXA.TAlx.Close;
        DMCAIXA.TAlx.SQL.Clear;
        DMCAIXA.TAlx.SQL.Add('select * from movbanco');
        DMCAIXA.TAlx.SQL.Add('left join chequerec on movbanco.cod_movbanco = chequerec.cod_movbanco');
        DMCAIXA.TAlx.SQL.Add('where (chequerec.cod_chequerec =:CODCHEQUE)');
        DMCAIXA.TAlx.ParamByName('CODCHEQUE').AsInteger:= XCodLancCaixa;
        DMCAIXA.TAlx.Open;
        
        If DMCAIXA.TAlx.FieldByName('FECH').AsString = 'S'
        Then Begin
           Mensagem('  I N F O R M A Ç Ã O  ', 'Este cheque ja foi movimentado e não poderá ser transferido de caixa!', '', 1,1, false, 'I');
        End
        else begin
           DMCAIXA.Talx.Close;
           DMCAIXA.TAlx.SQL.Clear;
           DMCAIXA.Talx.SQL.Add('update lancaixa set lancaixa.cod_abcaixa =:CODABCAIXA ');
           DMCAIXA.Talx.SQL.Add('where lancaixa.cod_lanc= '+(DMCAIXA.WLancCaixa.FieldByName('COD_LANC').AsString));
           DMCAIXA.TAlx.ParamByName('CODABCAIXA').AsInteger:=XCOD_CONTABANCO;
           DMCAIXA.TAlx.ExecSQL;
           DMCAIXA.IBT.CommitRetaining;

           Mensagem('  I N F O R M A Ç Ã O  ', 'Cheque transferido com sucesso!', '', 1,1, false, 'I');

           //FiltraTabela(, 'VWLANCAIXA', 'COD_ABCAIXA', FMenu.LABCAIXA.Caption, ' ORDER BY COD_LANC DESC');
...
begin
  inherited;
		...
		FMenu.LCAIXA.Caption:=DMCAIXA.TCaixaPAcess.FieldByName('DESCRICAO').AsString;
