...
uses
  ...
  FR_DBSet, FR_Class, IBEvents, gbCobranca, DrLabel, cxControls,
  cxContainer, cxEdit, cxGroupBox, cxRadioGroup, cxTextEdit,
  TFlatButtonUnit, XP_Button, ComCtrls;
  Var
   //Variavei Utilizadas para controle de lançamento automático
   XPedido: String;
   XCODPEDIDO: INTEGER;
  	XLancAut: Boolean;
   XCod_Pessoa: Integer;
...
Begin
   ...
   DMFinanc.TAlx.SQL.Add('SELECT distinct(parcelacr.cod_parcelacr), parcelacr.dtvenc, parcelacr.numboleto, ctareceber.dtlanc, parcelacr.numparc, parcelacr.valExt, parcelacr.valor, ');
   ...
   Do Begin
       ...
       If FMenu.XTIPOBOLTO='R' Then
         gbTitulo1.Sacado.Endereco.Complemento:='1'
       Else
         gbTitulo1.Sacado.Endereco.Complemento:='3';
       ...
       Then Begin//Não for uma copia do boleto
           gbTitulo1.NumeroDocumento:=XNumFisc+'-'+XnNum+':'+DMFinanc.TAlx.FieldByName('NUMPARC').AsString;
       ...
       Then Begin
           if DMBANCO.TCtaCor.FieldByName('CODCARTEIRABLT').AsString = 'SR' then
  			    gbTitulo1.NossoNumero:= ConcatZero(XnNum,10);
           if DMBANCO.TCtaCor.FieldByName('CODCARTEIRABLT').AsString = 'CR' then
               gbTitulo1.NossoNumero:= '9'+ConcatZero(XnNum,9) ;
       End;
       ...
       gbTitulo1.Sacado.Nome:=TiraCaracter(TiraAcentos(DMFinanc.TAlx.FieldByName('SACADO').AsString), '-');
...
{		If DMMACS.TLoja.FieldByName('MULTA').AsString<>'' Then
       	gbTitulo1.ValorOutrasDespesas:=(DMMACS.TLoja.FieldByName('MULTA').AsCurrency);}
...
Begin
   ...
   DMFinanc.TAlx.SQL.Clear;
   DMFinanc.TAlx.SQL.Add('SELECT distinct(parcelacr.cod_parcelacr), parcelacr.dtvenc, parcelacr.numboleto, ctareceber.dtlanc, parcelacr.numparc, parcelacr.valExt, parcelacr.valor, ');
   DMFinanc.TAlx.SQL.Add('pessoa.bairro, pessoa.cep, Pessoa.Telrel, pessoa.cpfcnpj, pessoa.endnumero, pessoa.endereco, pessoa.nome AS SACADO,ctacor.cod_ctacor,agencia.cod_agencia, ');
   DMFinanc.TAlx.SQL.Add('banco.cod_banc,banco.logobanco,CtaReceber.NumDoc, CtaReceber.TIPOGERADOR, CtaReceber.COD_GERADOR, CtaReceber.DTLanc, cidade.nome AS CIDADE, estado.uf_estado ');
   DMFinanc.TAlx.SQL.Add('FROM parcelacr Left join ctareceber ON parcelacr.cod_ctareceber = ctareceber.cod_ctareceber ');
   ...
   Then Begin
        ...
        DMFinanc.TAlx.ParamByName('CODCTARECEBER').AsInteger:=XViewRef.FieldByName('COD_PARCELACR').AsInteger;
   End;
   DMFinanc.TAlx.SQL.Add('ORDER BY cod_parcelacr');
   DMFINANC.TAlx.Open;
   XContCarne:= 0;
   QRCarne.Preview;
...
Begin
   ...
   Then Begin
	   ...
	   DMFinanc.TRel.Open;
      PadraoImp(DMMACS.TLoja.FieldByName('IMPTHERMAL').AsString);
      FSRel.LoadFromFile('C:\MZR\MACS\Rel\FrfReciboExtrato.frf');
      FSRel.ShowReport;
      RetPadraoImp;        
   End;
End;
...
Begin
    ...
    Try
      ...
      Then Begin
        ...
        If Tipo=0
        Then Begin
            FiltraTabela(DMCONTA.WParP, 'VWPARCP', 'COD_LOJA', '', ' (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) ORDER BY VWPARCP.NUMPARC');
			 //TOTALIZA CONTAS
            DMFINANC.TAlx.Close;
            DMFINANC.TAlx.SQL.Clear;
            DMFINANC.TAlx.SQL.Add('SELECT SUM(VWPARCP.VALOR) AS TOTAL FROM VWPARCP WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL))');
			 DMFINANC.TAlx.Open;
            If DMFINANC.TAlx.FieldByName('TOTAL').AsString='' Then
            	EdTotIndividual.ValueNumeric:=0
            Else
            	EdTotIndividual.ValueNumeric:=DMFINANC.TAlx.FieldByName('TOTAL').Value;
        End;
        If Tipo=1
        Then Begin
        ...
        Then Begin
            ...
            DMFINANC.TAlx.Close;
            DMFINANC.TAlx.SQL.Clear;
            DMFINANC.TAlx.SQL.Add('SELECT SUM(VWPARCP.VALOR) AS TOTAL FROM VWPARCP WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND (FECH='+#39+'S'+#39+')');
			 DMFINANC.TAlx.Open;
            If DMFINANC.TAlx.FieldByName('TOTAL').AsString='' Then
            	EdTotBaixadas.ValueNumeric:=0
            Else
            	EdTotBaixadas.ValueNumeric:=DMFINANC.TAlx.FieldByName('TOTAL').Value;
        ...
        Then Begin
            ...
            DMFINANC.TAlx.Close;
            DMFINANC.TAlx.SQL.Clear;
            DMFINANC.TAlx.SQL.Add('SELECT SUM(VWPARCP.VALOR) AS TOTAL FROM VWPARCP WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND (PREVISAO='+#39+'1'+#39+')  AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL))');
			 DMFINANC.TAlx.Open;
            If DMFINANC.TAlx.FieldByName('TOTAL').AsString='' Then
            	EdTotIndividual.ValueNumeric:=0
            Else
      ...
      Else Begin
        ...
        Then Begin
            ...
            DMFINANC.TAlx.Close;
            DMFINANC.TAlx.SQL.Clear;
            DMFINANC.TAlx.SQL.Add('SELECT SUM(VWPARCR.VALOR) AS TOTAL, SUM(vwparcr.valorpg) AS TOTALPG  FROM VWPARCR WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL))  AND ((VWPARCR.scpc<>'+#39+'1'+#39+') OR (VWPARCR.scpc is null))');
			 DMFINANC.TAlx.Open;
         	 EdTotIndividual.ValueNumeric:=DMFINANC.TAlx.FieldByName('TOTAL').AsCurrency-DMFINANC.TAlx.FieldByName('TOTALPG').AsCurrency;
            FiltraTabela(DMCONTA.WParR, 'VWPARCR', 'COD_LOJA', '', ' (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND ((FECH<>'+#39+'S'+#39+') OR (FECH IS NULL)) AND ((VWPARCR.scpc<>'+#39+'1'+#39+') OR (VWPARCR.scpc is null)) ORDER BY VWPARCR.NUMPARC');
        End;
        If Tipo=1
        Then Begin
            ...
            DMFINANC.TAlx.Close;
            DMFINANC.TAlx.SQL.Clear;
            DMFINANC.TAlx.SQL.Add('SELECT SUM(VWPARCR.VALOR) AS TOTAL, SUM(vwparcr.valorpg) AS TOTALPG  FROM VWPARCR WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND  (COD_CTARECEBER = '+#39+DMCONTA.WCtaR.FieldByName('COD_CTARECEBER').AsString+#39+')   AND ((FECH<>'+#39+'S'+#39+')  OR (FECH IS NULL)) ');
			 DMFINANC.TAlx.Open;
        ...
        Then Begin
            ...
            DMFINANC.TAlx.Close;
            DMFINANC.TAlx.SQL.Clear;
            DMFINANC.TAlx.SQL.Add('SELECT SUM(VWPARCR.VALOR) AS TOTAL FROM VWPARCR WHERE (COD_LOJA = '+#39+FMenu.LCODLOJA.Caption+#39+') AND (FECH='+#39+'S'+#39+') ');
    		 DMFINANC.TAlx.Open;
          	 EdTotBaixadas.ValueNumeric:=DMFINANC.TAlx.FieldByName('TOTAL').AsCurrency;
...
{    //Verifica se existe uma moeda
    ...
    Else Begin
    	if CbDiaMes.Checked=true then
       begin
           GeraParcela(DBNumDoc.Text+'-'+'1/'+IntToStr(QtdParcela), VerificaParcelaFixa(EdDiaMes.Text,IntToStr(Mes+1),IntToStr(Ano)) , ValorProd , ValorServ, 0, DBCOBRANCA.Text, RGJuros.ItemIndex, XDATAREF, 1);
           Mes:=Mes+1;
           If mes=12
           Then Begin
               Mes:=0;
...
//  inherited;
    ...
    Begin
        ...
        Then Begin
            ...
            Exit;
        End;
        If (XTabRef.FieldByName('COBRANCA').AsString<>'Banco') And  (XTabRef.FieldByName('COBRANCA').AsString<>'Carteira') And (XTabRef.FieldByName('COBRANCA').AsString<>'Cheque') And (XTabRef.FieldByName('COBRANCA').AsString<>'Cartão') And (XTabRef.FieldByName('COBRANCA').AsString<>'Boleto')  And (XTabRef.FieldByName('COBRANCA').AsString<>'C - Cartão')  And (XTabRef.FieldByName('COBRANCA').AsString<>'D - Cartão')
        Then Begin
    ...
    Then Begin
        ...
        Then Begin
           ...
           Then Begin
	   			DMFinanc.TAlx.Close;
	   			DMFinanc.TAlx.SQL.Clear;
	   			DMFinanc.TAlx.SQL.Add('SELECT * FROM parcelacp LEFT join ctapagar ');
      			DMFinanc.TAlx.SQL.Add('ON parcelacp.cod_ctapagar=ctapagar.cod_ctapagar ');
		   		DMFinanc.TAlx.SQL.Add('Where ctapagar.cod_ctapagar=:CODCTAPAGAR ');
		   		DMFinanc.TAlx.SQL.Add(' And (parcelacp.dtvenc = :DTVENC)');
		   		DMFinanc.TAlx.ParamByName('CODCTAPAGAR').AsInteger:=xCODCTA;
		   		DMFinanc.TALX.ParamByName('DTVENC').AsDate:=Date();
                       ...
                       Then Begin
                           ...
                           Then Begin
                               ...
                               begin
                                   ...
                                   If LanCaixa(-1, DateToStr(Date()), DMFinanc.TALX.FieldByName('COD_PLNCTA').AsInteger, 'Bx. Cta Pag.-'+DMFINANC.TAlx.FIELDBYNAME('NUMPARC').AsString + '-'+DateToStr(Date()) + '-'+DMCONTA.WParP.FIELDBYNAME('NOME').AsString, DMFinanc.TALX.FieldByName('VALOR').AsCurrency, 'CTAP', DMFinanc.TALX.FieldByName('COD_PARCELACP').AsInteger, 'CARTEIRA', 'S', DMFINANC.TAlx.FIELDBYNAME('NUMPARC').AsString, DMFinanc.TALX.FieldByName('DTREF').AsDateTime, '1')=False
                                   Then Begin
...
begin
  inherited;
  	 //Catões de Crédito e Débito devem se feitas pela opção de fatura de cartões
  ...
  inherited;
   ...
   Then begin//SE A PARCELA NÃO SE ENCONTRA FECHADA
       ...
       Then Begin//se todos os registros procurados foram encontrados entao prossegue
           LVALOR.Caption:=FormatFloat('######,##0.00', XView.FieldByName('Valor').AsCurrency);
			XTabRef.EDIT;
   ...
   Then Begin
       ...
       Then Begin // Caso a conta tenha sido feita em cheque, deve se retirar do caixa e do controle de cheque
           ...
           Else Begin
               ...
               If FiltraTabela(DMBANCO.TMovBanco, 'MOVBANCO', 'COD_MOVBANCO', '', '(movbanco.COD_GERADOR='+XViewRef.FieldByName('COD_PARCELACR').AsString+') and ((movbanco.TIPOGERADOR=' + #39 + 'CTARCH' + #39 +') or (movbanco.TIPOGERADOR=' + #39 + 'CTAR' + #39 + '))')=True
               Then Begin
                   ...
                   Then Begin
                       ...
                       Then Begin
                           ...
                           If FiltraTabela(DMCAIXA.TLanCaixa, 'LANCAIXA', 'COD_LANCAIXA', '', '(COD_GERADOR='+ #39 + IntToStr(XCOD_BANCOCHEQUE) + #39 + ') and ((TIPOGERADOR = '+ #39 + 'MCHR' + #39 +') OR (TIPOGERADOR = ' + #39 + 'CTAR' + #39 + ') OR (TIPOGERADOR = ' + #39 + 'DCHR' + #39 + ') OR (TIPOGERADOR = ' + #39 + 'CTARCH' + #39 + '))')=True
                           Then Begin//encontrado referencia em lançamento de caixa
                               //Filtra abertura de caixa para verificar se caixa ainda está aberto
                               If FiltraTabela(DMCAIXA.TAbCaixa, 'ABCAIXA', 'COD_ABCAIXA', DMCAIXA.TLanCaixa.FieldByName('COD_ABCAIXA').AsString, '')=True
                               Then Begin//encontrado referencia em Abcaixa
                       ...
                       Else Begin
                           ...
                           // VERIFICANDO SE NO CAIXA FOI MOVIMENTADO CHEQUE E SE FOI, EH EFETUADO UM LANCAIXA
                           If FiltraTabela(DMCAIXA.TLanCaixa, 'LANCAIXA', 'COD_LANCAIXA', '', '(COD_GERADOR='+ #39 + IntToStr(XCOD_BANCOCHEQUE) + #39 + ') and ((TIPOGERADOR = '+ #39 + 'MCHR' + #39 +') OR (TIPOGERADOR = ' + #39 + 'CTAR' + #39 + ') OR (TIPOGERADOR = ' + #39 + 'DCHR' + #39 + ') OR (TIPOGERADOR = ' + #39 + 'CTARCH' + #39 + ') OR (TIPOGERADOR = ' + #39 + 'LCHR' + #39 + '))')=True
                           Then Begin//encontrado referencia em lançamento de caixa
                               //Filtra abertura de caixa para verificar se caixa ainda está aberto
                               If FiltraTabela(DMCAIXA.TAbCaixa, 'ABCAIXA', 'COD_ABCAIXA', DMCAIXA.TLanCaixa.FieldByName('COD_ABCAIXA').AsString, '')=True
                               Then Begin//encontrado referencia em Abcaixa
...
begin
  inherited;
    If key=#13
    Then Begin
      edvencimento2.Text:=EdVencBaixa2.Text;
	   FiltraParcelaEdit('DTVENC');
    End;
    If Key=#27 Then
	   FiltraParcelaEdit('ODTVENC');
...
begin
    ...
    DMBANCO.TMovBanco.SQL.Add('AND ((movbanco.tipogerador = ' + #39 + 'CTAPJ' + #39 + ') OR (movbanco.tipogerador = ' + #39 + 'CTAPCH' + #39 +') OR (movbanco.tipogerador = ' + #39 + 'CTARCH' + #39 +') OR (movbanco.tipogerador = ' + #39 + 'CTPCARTAO' + #39 + '))');
...
begin
  inherited;
   ...
   //ordenar
   XViewRef.SQL.Add(' ORDER BY NOME, DTVENC, NUMPARC, NOME');
