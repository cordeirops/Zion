...
uses UMenu, UDMPessoa, UFORNECEDOR, UCliente, Alxor32, UDMConta, UDMMacs,
  ...
  TFlatEditUnit, AlxMessage, DB, UDMFinanc, UDMGEOGRAFIA;

{$R *.DFM}

//verifica qual o tipo de relatório pedido e executa sql
Procedure TFRelCtaPR.PrepareImp;
Var
	XJuncao: String;
	TIPO: STRING;
   TIPODT: STRING;
...
begin
   ...
   LDescTitulo.Caption := 'Rel. Ctas. Rateio';
  ...
  inherited;
   ...
   then begin
   	DescRecAvulso.Visible:=true;
	    Panel1.Visible:=true;
       Panel1.BringToFront;
       LDescTitulo.Caption := 'Recibo Avulso';
       edEmitente.Text:='';
       edEmitenteEnd.Text:='';
       edDocumento.Text:='';
       edValor.Text:='';
   ...
   Then Begin
       LDescTitulo.Caption:='Contas a Pagar por fornecedores';
       If (FMenu.TIPOREL='RELCTAPMENS') Then
       	LDescTitulo.Caption:='Contas a Pagar Mensal';
		If FMenu.TIPOREL='RELCTAPDOC_REC' Then
       begin
       	LDescTitulo.Caption:='Contas a Pagar Relação de Pagamentos';
           CBFiltroCliente.Enabled := false;
   ...
   Then Begin
       LDescTitulo.Caption:='Contas a Receber por clientes';
       CBFiltroCliente.Enabled := false;

       If (FMenu.TIPOREL='RELEXTRACTCLI') Then
           LDescTitulo.Caption:='Ctas á Receber c/ Dados Pessoas Físicas';
       If (FMenu.TIPOREL='RELEXTRACTPES_JUR') Then
           LDescTitulo.Caption:='Ctas á Receber c/ Dados Pessoas Jurídicas';
       If FMenu.TIPOREL='RELCTARDOC' Then
       	LDescTitulo.Caption:='Contas a Receber por Documento';
       If FMenu.TIPOREL='RELCTARDOCAGRUPADO' Then
       	LDescTitulo.Caption:='Contas a Receber por Documento Agrupado';
		If FMenu.TIPOREL='RELCTARDOCCORR' Then
       	LDescTitulo.Caption:='Contas a Receber por Documento Corrigido';
		If FMenu.TIPOREL='RELCTARDOC_REC' Then
       	LDescTitulo.Caption:='Relação de Recebimentos';
   ...
   //RELATÓRIO DE CTA RECEBER POR SACADO PARA OS DE DESPACHANTE
	If (FMenu.TIPOREL='RELCTARSACADO')
   Then Begin
       LDescTitulo.Caption:='Contas a Receber por sacado';
       CBFiltroCliente.Enabled := false;
...
begin
   LDescTitulo.Caption := 'Ctas. Pagar por Fornecedor';
...
begin
   LDescTitulo.Caption := 'Ctas. Receber de Clientes';
   RGESCOLHA.Visible := false;
   RGORDEM.Visible := false;
...
begin
   LDescTitulo.Caption := 'Relatório de Plano de Contas';
   RGESCOLHA.Visible := false;
   RGORDEM.Visible := false;
   GBSCPC.Visible := false;
...
begin
   LDescTitulo.Caption := 'Ctas. Vencidas';
   RGESCOLHA.Visible := false;
   RGORDEM.Visible := false;
...
begin
  inherited;
    ...
    then begin
       ...
       LDescTitulo.Caption := 'Recibo de Contas a Pagar';
       edEmitente.SetFocus;
...
//       EdMunicipio.Text:= DMGEOGRAFIA.TCidade.FieldByNAme('NOME').AsString;
    ...
    if (RDRECAVULSO.ItemIndex=1)
    then begin
        LDescTitulo.Caption := 'Recibo de Contas a Receber';
