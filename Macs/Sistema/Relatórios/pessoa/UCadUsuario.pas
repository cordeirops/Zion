...
begin
  inherited;
   ...
   DMMACS.TRel.SQL.Add('select registro.cod_registro,registro.data,registro.hora,registro.dt_modulo,registro.modulo,registro.comando,registro.field1,registro.texto,registro.cod_usuario,registro.usuario');
	...
	DMMACS.TRel.SQL.Add(' join pessoa on pessoa.cod_pessoa = funcionario.cod_pessoa');

   //Djonathan	19/11/2009: SE IGUAL A TRUE ENTAO MOSTRA TODOS OS USUARIOS
   if(FrmBusca1.EdDescricao.Text = '')then
	 	DMMACS.TRel.SQL.Add(' where usuario.codusuario = registro.cod_usuario')
   else
   begin
   	 DMMACS.TRel.SQL.Add(' where usuario.codusuario = :codusuario');
   ...
   begin
   	DMMACS.TRel.SQL.Add(' AND registro.data between :data and :data2 order by registro.cod_usuario;');
