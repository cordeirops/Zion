...
begin
    ...
    if (XCONTROLE = false)then begin
    	If RGTIPO.ItemIndex=0
    	Then Begin
       	 //GRAVA DADOS DE PESSOA FISICA
...
begin
  inherited;
	Try
   	...
   	Then Begin//pressionado enter, consultar
  			If XTab=True Then
  				FiltraTabela(XTabela, XSQLTABELA, XCampo, '', '(upper(CPFCNPJ) like upper('+#39+EdCpf.Text+'%'+#39+')) ORDER BY CPFCNPJ')
    		Else
 				FiltraTabela(XView, XSQLVIEW, XCampo, '',   '(upper(CPFCNPJ) like upper('+#39+EdCpf.Text+'%'+#39+')) ORDER BY CPFCNPJ');
       End;
       If Key=VK_ESCAPE
       Then Begin //pressionado esc, liberar
           LiberaDados;
...
begin
     Try
       	...
       	If Mensagem('A T E N Ç Ã O', 'CEP Encontrado!' + #13 +XListEnd.Values['TIPO_LOGRADOURO'] + ' ' + XListEnd.Values['LOGRADOURO'] + ' - ' + XListEnd.Values['BAIRRO'] + #13 + XListEnd.Values['CIDADE'] + ' - ' + XListEnd.Values['UF'] + '.' + #13 +'Atualizar informações?', '', 2, 3, False, 'c')= 2
   		Then Begin
           	DMPESSOA.TPessoa.Edit;
           	DMPESSOA.TPessoa.FieldByName('ENDERECO').AsString := UpperCase(XListEnd.Values['TIPO_LOGRADOURO']+' '+XListEnd.Values['LOGRADOURO']);
           	DMPESSOA.TPessoa.FieldByName('BAIRRO').AsString := UpperCase(XListEnd.Values['BAIRRO']);
           	DMPESSOA.TPessoa.Post;
