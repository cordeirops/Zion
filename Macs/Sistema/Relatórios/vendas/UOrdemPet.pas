...
uses
  ...
  DBCtrls, UFrmBusca, FR_DSet, FR_DBSet, FR_Class, FashionPanel, Menus;
...
type
  TFOrdemPet = class(TFPadrao)
    ...
    DBGrid2: TDBGrid;
    BtnProcProd: TBitBtn;
    EdCodInternoProd: TColorEditFloat;
    ...
    procedure EdCodInternoProdKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
begin
  inherited;
   ...
   //DMServ.TOrd.FieldByName('COD_FUNCIONARIO').AsInteger:=XCOD_FUNCIONARIO;
   DMServ.TOrd.FieldByName('COD_LOJA').AsString:=FMenu.LCODLOJA.Caption;
...
Begin
   ...
   DMESTOQUE.TSlave.sql.Add('subproduto.contrint,itprodord.operacao');
...
Begin
   ...
   EdCodInternoProd.Text := DMEstoque.TSubProd.FieldByName('CONTRINT').AsString;
...
begin
  inherited;
	FMenu.TIPOREL:='RELOSANALITICOSERVAGRUPADO';
	AbrirForm(TFRelOrdem, FRelOrdem, 0);
end;

procedure TFOrdemPet.EdCodInternoProdKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
       //Paulo 27/10/2011: Seleciona produto pelo código
       If Key=VK_Return
       Then Begin
           If FiltraTabela(DMEstoque.WSimilar, 'VWSIMILAR', 'Upper(CONTRINT)', UpperCase(EdCodInternoProd.Text), '')=True
           Then Begin
               EscreveLabelsPROD;
           End
           Else Begin
...
begin
  inherited;
       //Paulo 27/10/2011: emite relatório de produtos agrupados
       FMenu.TIPOREL:='RELOSANALITICOPRODAGRUPADO';
	    AbrirForm(TFRelOrdem, FRelOrdem, 0);
end;
