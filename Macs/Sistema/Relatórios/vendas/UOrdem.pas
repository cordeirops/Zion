...
type
  TFOrdem = class(TFCadPadrao)
    ...
    BitBtn15: TBitBtn;
    PGRAFICA: TPanel;
    EdTamanho: TColorEditFloat;
    Label56: TLabel;
    Label58: TLabel;
    EdObs: TColorEditFloat;
    ...
    procedure PTrocarExit(Sender: TObject);
...
Begin
   ...
   // passa os valores para os edits
   EDCodBarra.Text := DMEstoque.TSubProd.FieldByName('CODBARRA').AsString;
   If PCodInterno.Visible = true Then
       EDCodInterno.Text := DMEstoque.TSubProd.FieldByName('COD_INTERNO').AsString;
   If PCodFab.Visible = true Then
       EDCodFab.Text := DMEstoque.TSubProd.FieldByName('CODPRODFABR').AsString;
   If PCodComposto.Visible = true Then
       EDCodComposto.Text := DMEstoque.TSubProd.FieldByName('CODCOMPOSTO').AsString;
...
// - 19/03/2009:
   ...
   Then Begin
       ...
       Then Begin
           PGRAFICA.Visible := true;
           PGRAFICA.BringToFront;
       End;
   End
   Else Begin
...
begin
   ...
   Then Begin
       ...
       If DMMACS.TLoja.FieldByName('ATIVIDADE').AsString='INFORMATICA'
       Then Begin
           If (DBMouse.Text='') or (DBTeclado.Text='') or (DBKitmm.Text='') or (DBCabos.Text='') or (DBimpressora.Text='') or  (DBCdRecupera.Text='') or (DBMonitor.Text='') or (DbFones.Text='') or (DbEstadoEquip.Text='') or (DBLOCALAT.Text='')
           Then Begin
               Mensagem('OPÇÃO BLOQUEADA', 'Todos os dados pertinentes a periféricos que acompanham o equipamento devem ser preenchidos!', '', 1, 1, False, 'a');
               PAbertura.Visible:=true;
               PAbertura.BringToFront;
               PAbreInfo.BringToFront;
               DBMouse.SetFocus;
       ...
       XTabela.FieldByName('COD_CLIENTE').AsInteger:=XCOD_CLIENTE;
       //INFORMA CODIGO DO EQUIPAMENTO
       XTabela.FieldByName('COD_EQUIPAMENTO').AsInteger:=XCOD_EQUIPAMENTO;
       //INFORMA CODIGO DO VENDEDOR
       XTabela.FieldByName('COD_VENDEDOR').AsInteger:=XCOD_VENDEDOR;
       //INFORMA CODIGO DO RESPONSÁVEL
       XTabela.FieldByName('COD_FUNCIONARIO').AsInteger:=XCOD_FUNCIONARIO;
       ...
       //GRAVA TOTAIS
       XTabela.FieldByName('TOTORD').AsCurrency:=EdValorOrd.ValueNumeric;
       XTabela.FieldByName('TOTPROD').AsCurrency:=EdtotalProd.ValueNumeric;
       XTabela.FieldByName('TOTSERV').AsCurrency:=EdTotalserv.ValueNumeric;
       XTabela.FieldByName('TOTDESP').AsCurrency:=EdTotDesp.ValueNumeric;
       XTabela.FieldByName('DESCONTO').AsCurrency:=EdDescTOrdem.ValueNumeric;
       ...
       Case RGSituacao.ItemIndex of
           0: XTabela.FieldByName('STATUS').AsString:='ABERTO';
           1: XTabela.FieldByName('STATUS').AsString:='EXECUTANDO';
           2: XTabela.FieldByName('STATUS').AsString:='PARADO';
           3: XTabela.FieldByName('STATUS').AsString:='TERMINADO';
       End;

       // edita o ccheckbox 'DOCUMENTOS'
       If CBDocumentos.Checked = true
       Then Begin
           DMServ.TOrd.FieldByName('documentos').AsString := 'X'
       End
       Else Begin
           DMServ.TOrd.FieldByName('documentos').AsString := ''
       End;

       // edita o ccheckbox 'DESKTOP'
       If CBDesktop.Checked = true
       Then Begin
          DMServ.TOrd.FieldByName('DESKTOP').AsString := 'X'
       End
       Else Begin
           DMServ.TOrd.FieldByName('DESKTOP').AsString := ''
       End;

       // edita checkbox 'EMAILS'
       If CBEmails.Checked = true
       Then Begin
           DMServ.TOrd.FieldByName('EMAILS').AsString := 'X'
       End
       Else Begin
           DMServ.TOrd.FieldByName('EMAILS').AsString := ''
       End;

       // edita checkbox 'OUTROS'
       if CBOutros.Checked = true
       Then Begin
           DMServ.TOrd.FieldByName('OUTROS').AsString := 'X'
       End
       Else Begin
           DMServ.TOrd.FieldByName('OUTROS').AsString := ''
       End;  }
       ...
       then begin
           ...
           DMServ.TAlx1.SQL.Add('CDRECUPERA = :CDRECUPERA, MONITOR = :MONITOR, FONES = :FONES, ESTADOEQUIP = :ESTADOEQUIP');
           DMServ.TAlx1.SQL.Add('where COD_ORDEM = :OLD_COD_ORDEM');
       ...
       Case RGSituacao.ItemIndex of
           ...
           1: DMServ.TAlx1.ParamByName('STATUS').AsString:='EXECUTANDO';
           2: DMServ.TAlx1.ParamByName('STATUS').AsString:='PARADO';
...
begin
  ...
  PCodInterno.Visible:=False;
  	...
  	IF DMMacs.Tloja.FieldByName('FOCUITEMPED').AsString = 'BP'
   THEN BEGIN
	   BtnProcProd.SetFocus;
   END;
  	IF DMMacs.Tloja.FieldByName('FOCUITEMPED').AsString = 'CB'
   THEN BEGIN
      EDCodBarra.SetFocus;
  	   IF DMMacs.Tloja.FieldByName('CAMPOPESQITEMPED').AsString = 'CI'
      THEN Begin
...
procedure TFOrdem.EDCodCompostoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
	If Key=VK_Return
...
procedure TFOrdem.EDCodInternoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
	If Key=VK_Return
  ...
  inherited;
   ...
   Try
      ...
      //DMSERV.TPOrd.FieldByName('COD_ITPRODORD').AsInteger:=XCOD_ITPROD;
       ...
       DMSERV.TPOrd.ParamByName('NUMREQ').AsString := EDNumReg.Text;
...
begin
  inherited;
	...
	Then Begin
       ...
       EscreveLabelsSERV;
   End;
   EdCodcompostoServ.Refresh;
   EdCodInternoServ.Refresh;
   EDQtdServ.Refresh;
   EdDescServ.Refresh;
...
begin
  inherited;
	If Key=VK_Return
   Then Begin
		FiltraTabela(DMServ.WSubServ, 'VWSUBSERVICO', 'COD_INTERNO', EdCodInternoServ.Text, '');
		EscreveLabelsSERV;
   End;
end;

procedure TFOrdem.EdCodcompostoServKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
	If Key=VK_Return
...
begin
  inherited;
   ...
   Try
       ...
       DMServ.TSOrd.FieldByName('OBS').AsString:=EdObs.Text;
       DMServ.TSOrd.FieldByName('TAMANHO').AsCurrency:=EdTamanho.ValueNumeric;
       DMServ.TSOrd.FieldByName('OBS').AsString:=EdObs.Text;
...
begin
  inherited;
   ...
   Then Begin//caso a tabela esteja vasia não pode apagar
       ...
       EdTamanho.ValueNumeric:=DMESTOQUE.TSlaveServ.FieldByName('TAMANHO').AsCurrency;
...
procedure TFOrdem.PTrocarExit(Sender: TObject);
begin
  inherited;
	If PTrocar.Visible=True Then
       BtnDevolver.SetFocus;
end;

procedure TFOrdem.BtnDevolverClick(Sender: TObject);
begin
  inherited;
	If EDQtdDev.ValueNumeric<=0
   Then Begin
		Mensagem('OPÇÃO BLOQUEADA', 'A quantidade devolvida tem que ser superior a "0" (zero)', '', 1, 1, False, 'i');
       BtnCancelDevol.SetFocus;
       Exit;
   End;

	If (DMESTOQUE.TSlave.FieldByName('QTDRET').AsCurrency<=EDQtdDev.ValueNumeric)
   Then Begin
   ...
   Try
       ...
       DMSERV.TPOrd.FieldByName('QTD').AsCurrency:=DMSERV.TPOrd.FieldByName('QTDRET').AsCurrency-EDQtdDev.ValueNumeric;
		CalcPercent(DMSERV.TPOrd.FieldByName('DESCONTO').AsCurrency, 0, (DMSERV.TPOrd.FieldByName('QTD').AsCurrency*DMSERV.TPOrd.FieldByName('VLRUNIT').AsCurrency), 'D');
	    DMSERV.TPOrd.FieldByName('TOTAL').AsCurrency:=AlxCalculo;
		//CALCULA A REAL LUCRATIVIDADE
		CalcLuc(DMSERV.TPOrd.FieldByName('TOTAL').AsCurrency, DMSERV.TPOrd.FieldByName('DESCONTO').AsCurrency, DMSERV.TPOrd.FieldByName('QTD').AsCurrency);
   	DMSERV.TPOrd.FieldByName('LUCMOE').Value:=AlxLUCMOE;
   	DMSERV.TPOrd.FieldByName('LUCPER').Value:=AlxLUCPER;


       DMEstoque.TEstoque.edit;
		DMEstoque.TEstoque.FieldByName('ESTFISICO').AsCurrency:=DMEstoque.TEstoque.FieldByName('ESTFISICO').AsCurrency+DMSERV.TPOrd.FieldByName('QTDRET').AsCurrency;
...
procedure TFOrdem.BitBtn1Click(Sender: TObject);
begin
  inherited;
...
begin
  inherited;
   ...
   if XView.FieldByName('STATUS').AsString='PARADO'
   Then Begin
  			DBGridCadastroPadrao.Canvas.Font.Color := clRed;
			DBGridCadastroPadrao.DefaultDrawColumnCell(Rect, DataCol, Column, State);
   End;
...
procedure TFOrdem.ControleInterno1Click(Sender: TObject);
begin
  inherited;
   PCodInterno.Visible:=True;
   PCodInterno.BringToFront;
...
procedure TFOrdem.CdFab1Click(Sender: TObject);
begin
  inherited;
   PCodFab.Visible:=True;
   PCodFab.BringToFront;
...
procedure TFOrdem.C1Click(Sender: TObject);
begin
  inherited;
   PCodComposto.Visible:=True;
   PCodComposto.BringToFront;
...
procedure TFOrdem.BtnEscolheProcClick(Sender: TObject);
begin
  inherited;
  	MEscolheProc.Popup(BtnEscolheProc.Width+BtnEscolheProc.Left+60, 280);
end;
...
begin
  inherited;
  	...
  	If CBFILTRO.Text='PARADAS' Then
       XFILTRO:=' AND (STATUS='+#39+'PARADO'+#39+')';
  	If CBFILTRO.Text='EXECUTANDO' Then
       XFILTRO:=' AND (STATUS='+#39+'EXECUTANDO'+#39+')';
