...
type
  TFAtualizaBanco = class(TForm)
    ...
    function InsereNovaView(XVIEW:String; XCAMPOS:String; XSQL:String):Boolean;
    procedure BtAtualizaTabelaClick(Sender: TObject);
    //Paulo 27/03/2012: atualiza o campo da tabela
    function AtualizaTabela(XTABELA:String; XCAMPO:String; XTIPO:String):Boolean;
	 //Alex 09/04/2014: atualiza o campo da tabela
	 function AtualizaTabelaCampo(XTABELA:String; XCAMPO:String; XTIPO:String):Boolean;
    ...
    function InsereTrigger(XTABELA:String; XGENERATION:String ; XPK:String):Boolean;
    //Paulo 02/04/2012: Gera Indices
    function InsereIndice(XINDICE:String; XTABELA:String; XCAMPO:String; XUnique: Boolean):Boolean;
	...
	Try
     ...
     begin
		//Alex - 14/11/2014: Cria Tabela PERIODOFATURA com a pk
      	If not InsereTabelaNova('PERIODOFATURA', 'COD_PERIODOFATURA INTEGER NOT NULL') Then
       ...
       if InsereTrigger('PERIODOFATURA', 'GEN_PERIODOFATURA_ID', 'COD_PERIODOFATURA')=False then
       ...
       if InsereIndice('ID_PERIODOFATURA_COD_ORDEM','PERIODOFATURA','COD_ORDEM', false)=False then
       begin
...
Begin
	Try
		...
		Then Begin
      		If not InsereTabelaNova('ANTECIPACR', 'COD_ANTECIPACR INTEGER NOT NULL') Then
       	...
       	if InsereTrigger('ANTECIPACR', 'gen_antecipacr_id', 'COD_ANTECIPACR')=False then
       	...
       	if not AtualizaTabela('ANTECIPACR', 'COD_CONTAANTECIPACAO', 'INTEGER') then
	...
	Try
       //Adiciona CUSTOESTOQUEEMPRESA
		MDO.QConsulta.Close;
     	...
     	if MDO.QConsulta.IsEmpty
		Then Begin
      		If not InsereTabelaNova('CUSTOESTOQUEEMPRESA', 'COD_CUSTOESTOQUEEMPRESA INTEGER NOT NULL') Then
       		MPainel.Lines.Add('Erro ao gerar nova tabela: CUSTOESTOQUEEMPRESA');
       	if InsereGeneration('CUSTOESTOQUEEMPRESA','COD_CUSTOESTOQUEEMPRESA')=False then
           	MPainel.Lines.Add('Erro ao criar gerador para o campo COD_CUSTOESTOQUEEMPRESA da tabela CUSTOESTOQUEEMPRESA');
       	if InsereTrigger('CUSTOESTOQUEEMPRESA', 'GEN_CUSTOESTOQUEEMPRESA_ID', 'COD_CUSTOESTOQUEEMPRESA')=False then
           	MPainel.Lines.Add('Erro ao criar triger para o gerador para o campo COD_CUSTOESTOQUEEMPRESA da tabela CUSTOESTOQUEEMPRESA');
       	if not AtualizaTabela('CUSTOESTOQUEEMPRESA', 'COD_ESTOQUE', 'INTEGER') then
       		MPainel.Lines.Add('Erro ao atualizar o campo: COD_ESTOQUE da tabela: CUSTOESTOQUEEMPRESA');
       	if not AtualizaTabela('CUSTOESTOQUEEMPRESA', 'COD_ITEMPEDC', 'INTEGER') then
       		MPainel.Lines.Add('Erro ao atualizar o campo: COD_ITEMPEDC da tabela: CUSTOESTOQUEEMPRESA');
       	if not AtualizaTabela('CUSTOESTOQUEEMPRESA', 'VALORUNITARIOCUSTO', 'NUMERIC(14,2)') then
       		MPainel.Lines.Add('Erro ao atualizar o campo: VALORUNITARIOCUSTO da tabela: CUSTOESTOQUEEMPRESA');
       	if not AtualizaTabela('CUSTOESTOQUEEMPRESA', 'QUANTIDADEDISPONIVELVENDA', 'NUMERIC(10,3)') then
       ...
       //Adiciona CUSTOITEMPEDIDO
		MDO.QConsulta.Close;
     	...
     	if MDO.QConsulta.IsEmpty
		Then Begin
      		If not InsereTabelaNova('CUSTOITEMPEDIDO', 'COD_CUSTOITEMPEDIDO INTEGER NOT NULL') Then
       	...
       	if InsereTrigger('CUSTOITEMPEDIDO', 'GEN_CUSTOITEMPEDIDO_ID', 'COD_CUSTOITEMPEDIDO') = False then
           	MPainel.Lines.Add('Erro ao criar triger para o gerador para o campo COD_CUSTOITEMPEDIDO da tabela CUSTOITEMPEDIDO');
               
       	if not AtualizaTabela('CUSTOITEMPEDIDO', 'COD_CUSTOESTOQUEEMPRESA', 'INTEGER') then
       	...
       	if not AtualizaTabela('CUSTOITEMPEDIDO', 'TIPOGERADOR', 'VARCHAR(20)') then
       		MPainel.Lines.Add('Erro ao atualizar o campo: TIPOGERADOR da tabela: CUSTOITEMPEDIDO');
       	if not AtualizaTabela('CUSTOITEMPEDIDO', 'QUANTIDADE', 'NUMERIC(10,3)') then
...
Begin
	Try
		MDO.QConsulta.Close;
     	...
     	if MDO.QConsulta.IsEmpty
		Then Begin
      		If not InsereTabelaNova('ANTECIPARCR_ITENS', 'COD_ANTECIPARCR_ITENS INTEGER NOT NULL') Then
       		MPainel.Lines.Add('Erro ao gerar nova tabela: ANTECIPARCR_ITENS');
       	if InsereGeneration('ANTECIPARCR_ITENS','COD_ANTECIPARCR_ITENS')=False then
           	MPainel.Lines.Add('Erro ao criar gerador para o campo COD_ANTECIPARCR_ITENS da tabela ANTECIPARCR_ITENS');
       	if InsereTrigger('ANTECIPARCR_ITENS', 'gen_anteciparcr_itens_id', 'cod_anteciparcr_itens')=False then
           	MPainel.Lines.Add('Erro ao criar triger para o gerador para o campo cod_anteciparcr_itens da tabela ANTECIPARCR_ITENS');
       	if not AtualizaTabela('ANTECIPARCR_ITENS', 'COD_ANTECIPARCR', 'INTEGER') then
       		MPainel.Lines.Add('Erro ao atualizar o campo: COD_ANTECIPARCR da tabela: ANTECIPARCR_ITENS');
       	if not AtualizaTabela('ANTECIPARCR_ITENS', 'COD_PARCELACR', 'INTEGER') then
...
begin
      //////////////////////////////////////////////////////////////////////////
       ...
       if not AtualizaTabela('SUBPRODUTO', 'CODIGOBENEFICIOFISCAL', 'VARCHAR(10)') then
       ...
       if not AtualizaTabelaCampo('IMPNFETMP', 'DESCPRODVINCULADO', 'VARCHAR(150)') then
       ...
       if not AtualizaTabelaCampo('REGISTRO', 'FIELD1', 'VARCHAR(200)') then
       ...
       if not AtualizaTabela('CUSTOITEMPEDIDO', 'VALORCUSTOANTERIOR', 'NUMERIC(14, 2)') then
       ...
       if not ApagaCampo('CUSTOESTOQUEEMPRESA', 'COD_ITEMPEDC') then
	    ...
	    if not AtualizaTabela('CUSTOESTOQUEEMPRESA', 'TIPOGERADOR', 'VARCHAR(15)') then
       ...
       if not AtualizaTabela('ITPRODORD', 'VALORCUSTOUNITARIONOTAFISCAL', 'NUMERIC(14, 2)') then
           MPainel.Lines.Add('Erro ao adicionar o campo: VALORCUSTOUNITARIONOTAFISCAL na tabela: ITPRODORD');
       if not AtualizaTabela('ITENSPEDVEN', 'VALORCUSTOUNITARIONOTAFISCAL', 'NUMERIC(14, 2)') then
       ...
       if not InsereGeneration('COMPROD', 'COD_COMPROD') then
           MPainel.Lines.Add('Erro ao criar Generator para a tabela COMPROD')
			...
			If not InsereTrigger('COMPROD', 'GEN_COMPROD_ID', 'COD_COMPROD') Then
      ...
      //Matheus - 28/03/2016 - Cria campo CAMINHODOCS na tabela LOJA
       ...
       if not AtualizaTabela('USUARIO','FILTRAPEDDATA01', 'VARCHAR(20) CHARACTER SET ISO8859_1') then
           MPainel.Lines.Add('Erro ao atualizar o campo: FILTRAPEDDATA01 da tabela: LOJA');

       if not AtualizaTabela('USUARIO','FILTRAPEDDATA02', 'VARCHAR(20) CHARACTER SET ISO8859_1') then
      ...
      if not AtualizaTabelaCampo('LOJA','LOGINSUPORTE', 'VARCHAR(150) CHARACTER SET ISO8859_1') then
      ...
      if not InsereTabelaNova('ADICAOIMPORTACAO','COD_ADICAO INTEGER NOT NULL') then//cria a pk da tabela
      ...
      begin
        ...
        if not AtualizaTabela('ADICAOIMPORTACAO','CODIGOFABRICANTE', 'VARCHAR(60)') then
           MPainel.Lines.Add('Erro ao atualizar o campo: CODIGOFABRICANTE da tabela: ADICAOIMPORTACAO');

        if not AtualizaTabela('ADICAOIMPORTACAO','DESCONTOADICAO', 'NUMERIC(15, 2)') then
        ...
        if not InsereTrigger('ADICAOIMPORTACAO', 'GEN_ADICAOIMPORTACAO_ID', 'COD_ADICAO') then
      ...
      if not InsereTabelaNova('ITENSEXP','COD_ITENSEXP INTEGER NOT NULL') then//cria a pk da tabela
         MPainel.Lines.Add('Erro ao gerar nova tabela: ITENSEXP')
      else
      begin
      	 //Angelo - 06/07/2015 - Cria todos os outros campos da tabela
        if not AtualizaTabela('ITENSEXP','COD_ITENSPEDVEN', 'INTEGER NOT NULL') then
      ...
      if not InsereTabelaNova('VOLUMES','COD_VOLUME INTEGER NOT NULL') then//cria a pk da tabela
      ...
      if not InsereTabelaNova('ITENSVOLUME','COD_ITENSVOLUME INTEGER NOT NULL') then//cria a pk da tabela
      ...
      if not AtualizaTabela('PEDVENDA', 'NUMFISCAL_DEVOLVIDA', ' DESCRICAO_10 ') then
      ...
      if not AtualizaTabela('ITENSPEDVEN', 'ALIQ_DEVOLVIDO', ' NUMERIC(14, 2) ') then
      ...
      if not AtualizaTabela('PEDCOMPRA', 'NUMFISCAL_DEVOLVIDA', ' DESCRICAO_10 ') then
      ...
      if not InsereTabelaNova('CONTROLA_EXPEDICAO', 'COD_CONTROLE INTEGER NOT NULL') then//cria a pk da tabela
      ...
      if not AtualizaTabela('DOCFISSAIDA', 'VALORTOTALTRIBUTAESTADUAL', ' NUMERIC(14, 2) ') then
      ...
      if not AtualizaTabela('DOCFISORD', 'VALORTOTALTRIBUTAESTADUAL', ' NUMERIC(14, 2) ') then
      ...
      if not AtualizaTabela('DOCFIS', 'VALORTOTALTRIBUTAESTADUAL', ' NUMERIC(14, 2) ') then
      ...
      if not AtualizaTabela('ITENSPEDVEN', 'VALORTOTALTRIBUTAESTADUAL', ' NUMERIC(14, 2) ') then
      ...
      if not AtualizaTabela('ITENSPEDC', 'VALORTOTALTRIBUTAESTADUAL', ' NUMERIC(14, 2) ') then
      ...
      if not AtualizaTabela('ITPRODORD', 'VALORTOTALTRIBUTAESTADUAL', ' NUMERIC(14, 2) ') then
      ...
      if not InsereTabelaNova('FICHATECNICA', 'COD_SETOR INTEGER NOT NULL') then//cria a pk da tabela
      ...
      if not InsereTabelaNova('SETORPRODUCAO','COD_SETORPRODUCAO  INTEGER NOT NULL') then//cria a pk da tabela
      ...
      if not InsereTabelaNova('ITEMFICHATECNICA','COD_ITEMFICHATECNICA INTEGER NOT NULL') then//cria a pk da tabela
      ...
      begin
        ...
        if not AtualizaTabela('ITEMFICHATECNICA','COD_SETORPRODUCAO', 'INTEGER') then
           MPainel.Lines.Add('Erro ao atualizar o campo: COD_SETORPRODUCAO da tabela: ITEMFICHATECNICA');

        if not AtualizaTabela('ITEMFICHATECNICA','COD_FICHATECNICA', 'INTEGER') then
      ...
      if AtualizaTabela('IMPNFETMP', 'DESCPRODVINCULADO', 'VARCHAR(80)') = false then
      ...
      if AtualizaTabela('ORCAMENTO', 'CARACTERISTICA_PRODUTO01', ' OBS ') = false then
      ...
      if AtualizaTabela('ORCAMENTO', 'CARACTERISTICA_PRODUTO02', ' OBS ') = false then
      ...
      if AtualizaTabela('PEDCOMPRA', 'DESCRICAO_PRODMANUTENCAO', ' VARCHAR(100) ') = false then
      ...
      if AtualizaTabela('ITPRODORD', 'QTDE_HORA_SAIDA', ' NUMERIC(14,2) ') = false then
      ...
      if AtualizaTabela('ITPRODORD', 'QTDE_HORA_RETORNO', ' NUMERIC(14,2) ') = false then
      ...
      if AtualizaTabela('ITPRODORD', 'QTDE_HORA_CONSUMIDA', ' NUMERIC(14,2) ') = false then
      ...
      if AtualizaTabelaCampo('ORCAMENTO', 'PRAZO_DESPACHO', 'VARCHAR(50)') = false then
      ...
      //Alex - 27/06/2014 - Informa campo ICMSCREDALIQ na tabela ITENSPEDIDO DE VENDA
      if AtualizaTabela('ITENSPEDVEN', 'ICMSCREDVALOR', 'NUMERIC(14,2)') = false then
      ...
      if AtualizaTabela('ORCAMENTO', 'EMAIL_SOLICITANTE', 'VARCHAR(50)') = false then
      ...
      if AtualizaTabela('ITPRODORD', 'DESGASTE_SAIDA', 'NUMERIC(10, 2)') = false then
      ...
      if AtualizaTabela('ITPRODORD', 'DESGASTE_RETORNO', 'NUMERIC(10, 2)') = false then
      ...
      if AtualizaTabela('ITPRODORD', 'DESGASTE_CONSUMIDO', 'NUMERIC(10, 2)') = false then
      ...
      if AtualizaTabelaCampo('IMPNFETMP', 'QUANTIDADE', 'NUMERIC(10,2)') = false then
      ...
      if AtualizaTabela('DESPADIC','NFSE_NUMERODOCUMENTO', ' INTEGER ')=False then
      ...
      if AtualizaTabela('EMPRESA', 'VALORTOTALTRIBUTANFSE', 'NUMERIC(14,2)') = false then
      ...
      if InsereTabelaNova('IMPNFEFINANC','COD_IMPNFEFINANC INTEGER NOT NULL')=False then
      ...
      //Edmar - 04/04/2014 - Informa o campo NR_FATURA  na tabela IMPNFEFINANC
      if AtualizaTabela('IMPNFEFINANC', 'VLR_PARCELA', 'NUMERIC(14, 2)') = false then
      ...
      //Informa campo na tabela DOCFISORD NFSE_NUMERODOCUMENTO
      if AtualizaTabela('DOCFISORD','NFSE_NUMERODOCUMENTO', ' INTEGER ')=False then
      ...
      if AtualizaTabela('DOCFISORD','NFSE_CODIGOAUTENTICIDADE', ' varchar(50) ')=False then
      ...
      if AtualizaTabela('EMPRESA','NFSE_CODTRIBUTACAOMUNICIPAL', ' varchar(15) ')=False then
      ...
      if AtualizaTabela('EMPRESA','NFSE_DESCRICAOCNAE', ' varchar(100) ')=False then
      ...
      if AtualizaTabela('DOCFISORD','NFSE_XML', ' BLOB SUB_TYPE 1 SEGMENT SIZE 1 ')=False then
         	MPainel.Lines.Add('Erro ao criar campo NFSE_XML da tabela: DOCFISORD');

      //Informa campo na tabela ORDEM NFSE_PROTOCOLO
      if AtualizaTabela('DOCFISORD','NFSE_PROTOCOLO', ' VARCHAR(100) ')=False then
      ...
      if AtualizaTabela('SUBPRODUTO','COMPLEMENTOFISCAL', ' VARCHAR(500) ')=False then
      ...
      ////////////////////////////////////////////////////////////
      		//INCLUIR TABELA FATURACONTAS
      		if InsereTabelaNova('INFCOMPLEMENTAR','COD_INFCOMPLEMENTAR INTEGER NOT NULL')=False then
      		...
      		if InsereGeneration('INFCOMPLEMENTAR','COD_INFCOMPLEMENTAR')=False then
         		MPainel.Lines.Add('Erro ao criar gerador para o campo COD_INFCOMPLEMENTAR da tabela INFCOMPLEMENTAR');
      		if InsereTrigger('INFCOMPLEMENTAR', 'GEN_INFCOMPLEMENTAR_ID', 'COD_INFCOMPLEMENTAR')=False then
      		...
      		if AtualizaTabela('INFCOMPLEMENTAR','TIPOGERADOR', 'VARCHAR(10)')=False then
      ...
      //Informa campo na tabela empresa paa armazenar o valor total aproximado dos tibutos
      if AtualizaTabela('EMPRESA','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('ITENSPEDVEN','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('ITENSPEDVEN','ALIQUOTATOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('ITENSPEDC','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('ITENSPEDC','ALIQUOTATOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('ITPRODORD','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('ITPRODORD','ALIQUOTATOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('NCM','ALIQTOTALTRIBUTANAC', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('NCM','ALIQTOTALTRIBUTAIMP', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('DOCFISSAIDA','ALIQUOTATOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
         	MPainel.Lines.Add('Erro ao criar campo ALIQUOTATOTALTRIBUTA da tabela: DOCFISSAIDA');
      if AtualizaTabela('DOCFISSAIDA','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('DOCFISORD','ALIQUOTATOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
         	MPainel.Lines.Add('Erro ao criar campo ALIQUOTATOTALTRIBUTA da tabela: DOCFISORD');
      if AtualizaTabela('DOCFISORD','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      if AtualizaTabela('DOCFIS','ALIQUOTATOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
         	MPainel.Lines.Add('Erro ao criar campo ALIQUOTATOTALTRIBUTA da tabela: DOCFIS');
      if AtualizaTabela('DOCFIS','VALORTOTALTRIBUTA', ' NUMERIC(14,2) ')=False then
      ...
      ////////////////////////////// ////
      		//INCLUIR TABELA FATURACONTAS
      		if InsereTabelaNova('FATURACONTA','COD_FATURACONTA INTEGER NOT NULL')=False then
      		...
      		if InsereTrigger('FATURACONTA', 'GEN_FATURACONTA_ID', 'COD_FATURACONTA')=False then
           ...
           if AtualizaTabela('FATURACONTA','VLRTOTFATURA', ' NUMERIC(14,2) ')=False then
       	...
       	if InsereTabelaNova('ITENSFATURACONTA','COD_ITENSFATURACONTA INTEGER NOT NULL')=False then
      		...
      		if InsereTrigger('ITENSFATURACONTA', 'GEN_ITENSFATURACONTA_ID', 'COD_ITENSFATURACONTA')=False then
        	...
        	//CAMPOS TABELA ITENSFATURACONTAS
      		if AtualizaTabela('ITENSFATURACONTA','COD_FATURACONTA', 'INTEGER')=False then
      ...
      if InsereTrigger('itenspedc', 'GEN_itenspedc_ID', 'cod_itenspedc')=False then
      ...
      if AtualizaTabela('FORMPAG','MOBILE', 'CHAR(1)')=False then
	   ...
	   //INICIO TABELA DE ITENS DE GRAFICA
      		if InsereTabelaNova('IT_GRAFICA','COD_ITGRAFICA INTEGER NOT NULL')=False then
      		...
      		if InsereTrigger('IT_GRAFICA', 'GEN_IT_GRAFICA_ID', 'COD_ITGRAFICA')=False then
           ...
           if AtualizaTabela('IT_GRAFICA','DESCRICAO', ' BLOB SUB_TYPE 1 SEGMENT SIZE 1 ')=False then
      ...
      //FIM DA TABELA itens grafica
	   ...
	   //INICIO TABELA DE CCE CARTA DE CORREÇÃO ELETRONICA
           ...
           if AtualizaTabela('CCE','TEXTO', ' BLOB SUB_TYPE 1 SEGMENT SIZE 1 ')=False then
               MPainel.Lines.Add('Erro ao criar campo TEXTO da tabela: CCE');
           if AtualizaTabela('CCE','XML', ' BLOB SUB_TYPE 1 SEGMENT SIZE 1 ')=False then
               MPainel.Lines.Add('Erro ao criar campo XML da tabela: CCE');
           if AtualizaTabela('CCE','XMLRESPOSTA', ' BLOB SUB_TYPE 1 SEGMENT SIZE 1 ')=False then
      ...
      if AtualizaTabela('DOCFIS','NFEXMLCANCEL', ' BLOB SUB_TYPE 1 SEGMENT SIZE 1 ')=False then
      ...
      if InsereTabelaNova('SERVICO_NFSE','COD_SERVICO INTEGER NOT NULL')=False then
...
//Alex: 14/01/2014 Procedure para tratar em especifico o caso de aumentar o tamanho do campo de codigo de barras
Function TFAtualizaBanco.ApagaCampo(XTabelaApagar: String; XCampoApagar: String): Boolean;
begin
	Try
   	Try
           Result := True;
           ...
           begin
               MDO.QConsulta.Close;
               MDO.QConsulta.SQL.Clear;
               MDO.QConsulta.SQL.Add('ALTER TABLE '+XTabelaApagar+' DROP '+ XCampoApagar);
...
begin
      ...
      Then Begin
           ...
           XSQL_VIEW := ' SELECT  ORCAMENTO.COD_ORCAMENTO, ORCAMENTO.COD_CLIENTE, ORCAMENTO.NOMECLI, ORCAMENTO.COD_FORMPAG, ';
           XSQL_VIEW := XSQL_VIEW + ' ORCAMENTO.COD_USUARIO, ORCAMENTO.DTABERT, ORCAMENTO.NUMORC, ORCAMENTO.DTVALIDADE, ORCAMENTO.VLRTOTAL, ';
           XSQL_VIEW := XSQL_VIEW + ' FORMPAG.DESCRICAO, ORCAMENTO.COD_LOJA, PESSOA.NOME, ORCAMENTO.DESCRICAO, ORCAMENTO.EXPORTA ';
           XSQL_VIEW := XSQL_VIEW + ' FROM ORCAMENTO ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN FORMPAG ON ORCAMENTO.COD_FORMPAG = FORMPAG.COD_FORMPAG ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN FUNCIONARIO ON FUNCIONARIO.COD_FUNC =  ORCAMENTO.COD_VENDEDOR ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PESSOA ON FUNCIONARIO.COD_PESSOA = PESSOA.COD_PESSOA ';
      ...
      If cbParcR.Checked=True
      Then Begin
          XCAMPOS:= '   COD_INTERNO, COD_PARCELACR, COD_CTARECEBER, COD_COTA, COD_PARANT, COBRANCA, DTVENC, NUMPARC, VALOR, VALORPG, TIPOBOLETO, NUMCTA, DTDEBITO, FECH, CLASSIFICACAO, ';
 		   XCAMPOS := XCAMPOS + ' DESCRICAO, COD_PLNCTA, COD_FORMPAG, COD_LOJA, FORMPAG, COD_CLIENTE, COD_PESSOA, NOME, DTLANC, NUMDOC, HISTORICO, MARK, NUMBOLETO, NUMFISC, REMESSA, COD_CTACOR, ';
 		   XCAMPOS := XCAMPOS + ' SCPC, NUMCARTAO, DIASATRASO, VENDEDOR, ANTECIPADO ';
          XSQL_VIEW:=' select cliente.cod_interno, parcelacr.cod_parcelacr, parcelacr.cod_ctareceber, parcelacr.cod_cota, parcelacr.cod_parant, ';
          XSQL_VIEW:=XSQL_VIEW+'        parcelacr.cobranca,parcelacr.dtvenc, parcelacr.numparc, parcelacr.valor, parcelacr.valorpg, parcelacr.tipoblt, ctacor.numctacor, ';
          XSQL_VIEW:=XSQL_VIEW+'        parcelacr.dtdebito, parcelacr.fech, plncta.classificacao, plncta.descricao, plncta.cod_plncta, formpag.cod_formpag, ';
          XSQL_VIEW:=XSQL_VIEW+'        formpag.cod_loja, formpag.descricao, cliente.cod_cliente, pessoa.cod_pessoa, pessoa.nome, ctareceber.dtlanc, ctareceber.numdoc, ';
          XSQL_VIEW:=XSQL_VIEW+'        parcelacr.historico, parcelacr.mark, parcelacr.numboleto, parcelacr.numfisc, parcelacr.remessa, parcelacr.cod_ctacor, parcelacr.scpc, ';
          XSQL_VIEW:=XSQL_VIEW+'        ctareceber.numcartao, parcelacr.diasatraso, vwfuncionario.nome, parcelacr.antecipado from parcelacr ';
          XSQL_VIEW:=XSQL_VIEW+'        left join ctareceber ON parcelacr.cod_ctareceber = ctareceber.cod_ctareceber ';
          XSQL_VIEW:=XSQL_VIEW+'        left join ctacor on parcelacr.cod_ctacor = ctacor.cod_ctacor ';
          XSQL_VIEW:=XSQL_VIEW+'        left join plncta on ctareceber.cod_plncta = plncta.cod_plncta ';
          XSQL_VIEW:=XSQL_VIEW+'                         left join formpag ON ctareceber.cod_formpag = formpag.cod_formpag ';
          XSQL_VIEW:=XSQL_VIEW+'                         left join cliente ON ctareceber.cod_cliente = cliente.cod_cliente ';
          XSQL_VIEW:=XSQL_VIEW+'                         left join pessoa ON pessoa.cod_pessoa = cliente.cod_pessoa ';
          XSQL_VIEW:=XSQL_VIEW+'                         left join vwfuncionario on ctareceber.cod_vendedor = vwfuncionario.cod_func ';
      ...
      If CbVWCliente.Checked=True
      Then Begin
          XCAMPOS:= '  COD_CLIENTE, COD_INTERNO, COD_PESSOA, NOME, ENDERECO, BAIRRO, CPFCNPJ, COD_CIDADE, CIDADE, ATIVO, EXPORT, TELREL, SEPARAR, SCPC, COD_TABPRECO, ATUALIZAR, MARK, UF_ESTADO, ENDNUMERO, RAZAOSOCIAL ';
          XSQL_VIEW:=' SELECT cliente.cod_cliente, cliente.cod_interno, pessoa.cod_pessoa, pessoa.nome, pessoa.endereco, pessoa.bairro, pessoa.cpfcnpj, ';
          XSQL_VIEW:=XSQL_VIEW+' cidade.cod_cidade, cidade.nome, cliente.ativo, cliente.export, PESSOA.telrel, cliente.separar, cliente.scpc, ';
          XSQL_VIEW:=XSQL_VIEW+' cliente.cod_tabpreco, cliente.atualizar, cliente.mark,estado.uf_estado , pessoa.endnumero, pessoaj.razao_social ';
          XSQL_VIEW:=XSQL_VIEW+' FROM CLIENTE JOIN PESSOA ON cliente.cod_pessoa = pessoa.cod_pessoa ';
          XSQL_VIEW:=XSQL_VIEW+' left join pessoaJ on Pessoa.cod_pessoa = pessoaj.cod_pessoa ';
          XSQL_VIEW:=XSQL_VIEW+' left join  cidade ON pessoa.cod_cidade = cidade.cod_cidade ';
          XSQL_VIEW:=XSQL_VIEW+' left join estado on cidade.cod_estado = estado.cod_estado ';
      ...
      If CBVWLANCENT.Checked=True
      Then Begin
          XCAMPOS:= '      COD_LANCENT,    COD_ESTOQUE,    COD_LOJA,    COD_USUARIO,    DT_LANCAMENTO,    MOTIVO,    NUMNOTA,    QUANTIDADE,    CTRLINT,    PRODUTO,    ANTERIOR,    CODFABRICANTE,    QTD4CASAS ';
          XSQL_VIEW:=' SELECT lancent.cod_lanent, lancent.cod_estoque, lancent.cod_loja, ';
          XSQL_VIEW:=XSQL_VIEW+'       lancent.cod_usuario, lancent.dt_lancamento, lancent.motivo, ';
          XSQL_VIEW:=XSQL_VIEW+'       lancent.numnota, lancent.quantidade, subproduto.contrint, subproduto.descricao, lancent.qtdant, subproduto.codprodfabr, lancent.qtd4casas ';
          XSQL_VIEW:=XSQL_VIEW+'       FROM lancent ';
          XSQL_VIEW:=XSQL_VIEW+'       Left join estoque ON lancent.cod_estoque = estoque.cod_estoque ';
          XSQL_VIEW:=XSQL_VIEW+'       left join subproduto on subproduto.cod_subproduto = estoque.cod_subprod ';
      ...
      //Alex - 14/08/2015 - VWLANCENT - Verifica para atualizar VWCLIENTE
      If CBVWLANCSAI.Checked=True
      Then Begin
          XCAMPOS:= '     COD_LANCSAI,    COD_ESTOQUE,    COD_LOJA,    COD_USUARIO,    DT_LANCAMENTO,    MOTIVO,    NUMNOTA,    QUANTIDADE,    CTRLINT,    PRODUTO,    ANTERIOR,    CODFABRICANTE,    QTD4CASAS ';
          XSQL_VIEW:=' SELECT lancsai.cod_lancsai, lancsai.cod_estoque, lancsai.cod_loja, ';
          XSQL_VIEW:=XSQL_VIEW+'         lancsai.cod_usuario, lancsai.dt_lancamento, lancsai.motivo, ';
          XSQL_VIEW:=XSQL_VIEW+'         lancsai.numnota, lancsai.quantidade, SUBPRODUTO.contrint, SUBPRODUTO.DESCRICAO,lancsai.qtdant, subproduto.codprodfabr, lancsai.qtd4casas ';
          XSQL_VIEW:=XSQL_VIEW+'         FROM lancsai join estoque ON lancsai.cod_estoque = estoque.cod_estoque ';
          XSQL_VIEW:=XSQL_VIEW+'              left join subproduto on subproduto.cod_subproduto = estoque.cod_subprod ';
      ...
      Then Begin
          XCAMPOS:= ' COD_SERVICO, COD_GRUPOSERV, COD_SUBGRUPOSERV, COD_INTERNO, SERVICO, APLICACAO, ';
		   XCAMPOS:=XCAMPOS+'CI_GRUPO, GRUPO, CI_SUBGRUPO, SUBGRUPO, MARK';
          XSQL_VIEW:=' SELECT DISTINCT(servico.cod_servico), servico.cod_gruposerv, servico.cod_subgruposerv, ';
          XSQL_VIEW:=XSQL_VIEW+' servico.cod_interno, servico.descricao, servico.aplicacao, ';
          XSQL_VIEW:=XSQL_VIEW+' grupoprod.cod_interno, grupoprod.descricao, subgrupoprod.cod_interno, ';
          XSQL_VIEW:=XSQL_VIEW+' subgrupoprod.descricao, servico.mark ';
          XSQL_VIEW:=XSQL_VIEW+' FROM servico LEFT join subservico ON servico.cod_servico = subservico.cod_servico ';
          XSQL_VIEW:=XSQL_VIEW+' Left Join grupoprod on servico.cod_gruposerv = grupoprod.cod_grupoprod ';
          XSQL_VIEW:=XSQL_VIEW+' Left Join subgrupoprod on servico.cod_subgruposerv = Subgrupoprod.cod_Subgrupoprod ';
      ...
      Then Begin
          XCAMPOS:=' COD_FORNEC, COD_INTERNO, COD_PESSOA, NOME, ENDERECO, BAIRRO, CPFCNPJ, COD_CIDADE, CIDADE, EXPORT, ';
          XCAMPOS:=XCAMPOS+'     TELREL, INSC_EST, UF, RAZAOSOCIAL ';
          XSQL_VIEW:=' SELECT fornecedor.cod_fornec, fornecedor.cod_interno, pessoa.cod_pessoa, ';
          XSQL_VIEW:=XSQL_VIEW+' pessoa.nome, pessoa.endereco, pessoa.bairro, pessoa.cpfcnpj, ';
          XSQL_VIEW:=XSQL_VIEW+' cidade.cod_cidade, cidade.nome, fornecedor.export, pessoa.telrel,pessoaJ.insc_est,estado.uf_estado, pessoaj.razao_social ';
          XSQL_VIEW:=XSQL_VIEW+' FROM fornecedor JOIN PESSOA ON fornecedor.cod_pessoa = pessoa.cod_pessoa ';
          XSQL_VIEW:=XSQL_VIEW+' left join  cidade ON pessoa.cod_cidade = cidade.cod_cidade ';
          XSQL_VIEW:=XSQL_VIEW+' left join pessoaJ on Pessoa.cod_pessoa = pessoaj.cod_pessoa ';
          XSQL_VIEW:=XSQL_VIEW+' left join estado on cidade.cod_estado = estado.cod_estado ';
      ...
      If CBVWSimilar.Checked=True
      Then Begin
       	Try
      			XCAMPOS:='  COD_SUBPRODUTO,    CODCOMPOSTO,    DESCRICAO,    CODBARRA,    MARCA,    COD_INTERNO,    COD_PRODUTO, ';
          		XCAMPOS:= XCAMPOS + ' COD_ESTOQUE,    COD_LOJA,    ULTCOMPRA,    ULTVENDA,    ESTFISICO,    VENDVARP,    VANDATAP,    VENDATAV,    VENDVARV, ';
          		XCAMPOS:= XCAMPOS + '  CODFABRICANTE,    CONTRINT,    VLRBONIFIC,    ESPECIFICACAO,    MARK,    VLRUNITCOMPRA,    LOCALESTANTE,    ESTLOJA,    ATIVO, IPI, ST, ESTOQUE4CASAS, VLRCUSTONFE ';

               XSQL_VIEW:='';
          		XSQL_VIEW:= XSQL_VIEW+ ' SELECT  subproduto.cod_subproduto, subproduto.codcomposto, subproduto.descricao,        subproduto.codbarra, subproduto.marca, subproduto.cod_interno,        subproduto.cod_produto, ';
          		XSQL_VIEW:= XSQL_VIEW+ '  estoque.cod_estoque,estoque.cod_loja, estoque.ultcompra, estoque.ultvenda,        estoque.estfisico, estoque.vendvarp, estoque.vendatap, estoque.vendatav, ';
          		XSQL_VIEW:= XSQL_VIEW+ '  estoque.vendvarv, subproduto.codprodfabr, SUBPRODUTO.contrint, estoque.vlrbonific, subproduto.especificacao, subproduto.mark, estoque.VLRUNITCOMP, subproduto.localestante,ESTOQUE.estloja, subproduto.ativo, subproduto.ipiprod, estoque.pis, ';
          		XSQL_VIEW:= XSQL_VIEW+ '  estoque.estoque4casas, estoque.valcustonfe   ';

          		XSQL_VIEW:= XSQL_VIEW+ '  from subproduto join estoque ON subproduto.cod_subproduto = estoque.cod_subprod ';
      ...
      End;
      //VWSubServicos - Verifica para atualizar VWSubServicos
      If CBVWSubServicos.Checked=True
      Then Begin
          XCAMPOS:='    COD_SUBSERVICO,     CODCOMPOSTO,    DESCRICAO,    COD_INTERNO,    COD_SERVICO,    EMPSERV,    QTDGARANTIA,  ';
          XCAMPOS:= XCAMPOS + '    TEMPSERV,     VLRFINAL,    TIPOGARANT,    VLRTOTSUBSERV,    HORA_INI,    HORA_FINAL,    INTERVALO_INI, ';
          XCAMPOS:= XCAMPOS + '    INTERVALO_FINAL,      VLR_NORMAL,    VLR_EXTRA, ATIVO ';

          XSQL_VIEW:=' SELECT  subservico.cod_subservico, subservico.codcomposto,  SUBSERVICO.descricao, ';
          XSQL_VIEW:= XSQL_VIEW+ ' subservico.cod_interno, subservico.cod_servico, subservico.empserv, subservico.quantgarantia, ';
          XSQL_VIEW:= XSQL_VIEW+ 'subservico.tempserv, subservico.vlrfinal, subservico.tipogarantia,subservico.vlrsubserv, ';
          XSQL_VIEW:= XSQL_VIEW+ 'subservico.hora_ini, subservico.hora_final, subservico.intervalo_ini, subservico.intervalo_final, ';
          XSQL_VIEW:= XSQL_VIEW+ 'subservico.vlr_normal, subservico.vlr_extra, subservico.ativo ';
    	   XSQL_VIEW:= XSQL_VIEW+ 'from subservico left join servico ON subservico.cod_servico=servico.cod_servico ';
      ...
      If cbVWCtaCor.Checked=True
      Then Begin
          XCAMPOS:='  COD_CTACOR, NUMCTACOR, DTABERT, COD_AGENCIA, NUMAGENCIA,  DESCBANCO, SALDO, INATIVO ';

          XSQL_VIEW:=' SELECT ctacor.cod_ctacor, ctacor.numctacor, ';
          XSQL_VIEW:= XSQL_VIEW+ ' ctacor.dtabert, agencia.cod_agencia, ';
          XSQL_VIEW:= XSQL_VIEW+ ' agencia.numagencia, banco.descbanco,ctacor.saldo, ctacor.inativo ';
          XSQL_VIEW:= XSQL_VIEW+ ' from ctacor join agencia on ctacor.cod_agencia = agencia.cod_agencia ';
    	   XSQL_VIEW:= XSQL_VIEW+ ' Join banco on Banco.cod_banc = agencia.cod_banco ';
      ...
      If CbAtualizaVwOrdem.Checked=True
      Then Begin
          XCAMPOS:='     COD_ORDEM,    COD_CLIENTE,    COD_EQUIPAMENTO,    COD_FUNCIONARIO,    COD_VENDEDOR,    DTABERT,    DTPREV,    DTFECH,    NUMERO, ';
          XCAMPOS:=XCAMPOS+'    NUMORC,    STATUS,    TOTAL,    CI_CLIENTE,    CLIENTE,    EQUIPAMENTO,    COD_LOJA,    GARANT,    MARK,    FISCO, ';
          XCAMPOS:=XCAMPOS+'    NUMFISCAL,    FORMAPAG,    TIPOPAG,    PLACA,    BOX,    EXP,    EXPORTADO,    STATUSNFSE,    NUMFISCALNFSE,    NUMFISCAL_DEVOLVIDA,    numfiscalINT, DTAGENDADO ';

          XSQL_VIEW:=' SELECT ordem.cod_ordem, ordem.cod_cliente, ordem.cod_equipamento, ordem.cod_funcionario, ordem.cod_vendedor, ordem.dtabert, '+
                       ' ordem.dtprev, ordem.dtfech,  ordem.numero, ORDEM.numorc, ordem.status,  ordem.totord, cliente.cod_interno, pessoa.nome, '+
                       ' equipamento.descricao, ORDEM.cod_loja, ordem.garantia, ORDEM.mark, ORDEM.fisco, ordem.numfiscal, formpag.descricao, ordem.tipopag, '+
                       ' EQUIPAMENTO.placa, ORDEM.box, ORDEM.exp, ORDEM.exportado, ordem.statusnfse, NUMFISCALNFSE, ORDEM.NUMFISCAL_DEVOLVIDA, docfisord.numdocfisint, Ordem.DTAGENDADO '+
                       ' FROM ordem '+
                       ' Left join docfisord on ordem.cod_ordem=docfisord.cod_ordem '+
          ...
          begin
             MPainel.Lines.Add('Erro ao apagar a view: STATUSNFSE');
          end
          else begin
      ...
      begin
           ...
           XCAMPOS := ' NUMPED, COD_PEDVENDA, COD_CLIENTE, NOMECLI, COD_FORMPAG, COD_USUARIO, DTPEDVEN, ';
			XCAMPOS := XCAMPOS + ' NUMORC, NUMFISCAL, SITUACAO, VALOR, FORMPAG, COD_LOJA, COD_VENDEDOR, ';
			XCAMPOS := XCAMPOS + ' VENDEDOR, TIPO, GARANT, MARK, COBRANCA, FATURADO, FISCO, CONTROLEFRETE, NUMFISCAL_DEV, NUMFISCAL_PC ';

           XSQL_VIEW := ' SELECT PEDVENDA.NUMPED, PEDVENDA.COD_PEDVENDA, PEDVENDA.COD_CLIENTE, PEDVENDA.NOMECLI,  PEDVENDA.COD_FORMPAG, PEDVENDA.COD_USUARIO, PEDVENDA.DTPEDVEN,PEDVENDA.NUMORC, ';
			XSQL_VIEW := XSQL_VIEW + ' PEDVENDA.NUMFISCAL, PEDVENDA.SITUACAO, PEDVENDA.VALOR,  FORMPAG.DESCRICAO, FORMPAG.COD_LOJA, PEDVENDA.COD_VENDEDOR, PESSOA.NOME, PEDVENDA.TIPO,  PEDVENDA.GARANTIA, ';
			XSQL_VIEW := XSQL_VIEW + ' PEDVENDA.MARK, PEDVENDA.COBRANCA, PEDVENDA.FATURADO, PEDVENDA.FISCO, PEDVENDA.CONTROLEFRETE, PEDVENDA.NUMFISCAL_DEVOLVIDA, PEDVENDA.NUMFISCAL_PC FROM PEDVENDA JOIN FORMPAG ON PEDVENDA.COD_FORMPAG = FORMPAG.COD_FORMPAG ';
			XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN FUNCIONARIO ON FUNCIONARIO.COD_FUNC = PEDVENDA.COD_VENDEDOR ';
			XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PESSOA ON FUNCIONARIO.COD_PESSOA = PESSOA.COD_PESSOA ';
      ...
      begin
           ...
           XCAMPOS := ' COD_PEDCOMP, COD_FORNEC, COD_FORMPAG, COD_USUARIO, DTPEDCOMP, NUMPED, SITUACAO,            ';
        	XCAMPOS := XCAMPOS + ' VALOR, NOME, RAZAOSOCIAL, FORMPAG, COD_LOJA, NUMDEV, NUMFISCAL,DESTINATARIO, ';
        	XCAMPOS := XCAMPOS + ' COBRANCA, MARK, FATURADO, EXPORT, FISCO, NUMFISCAL_DEV ';

           XSQL_VIEW := ' SELECT PEDCOMPRA.COD_PEDCOMP, PEDCOMPRA.COD_FORNEC, PEDCOMPRA.COD_FORMPAG, PEDCOMPRA.COD_USUARIO, ';
           XSQL_VIEW := XSQL_VIEW + ' PEDCOMPRA.DTPEDCOMP, PEDCOMPRA.NUMPED, PEDCOMPRA.SITUACAO, PEDCOMPRA.VALOR, PESSOA.NOME, ';
           XSQL_VIEW := XSQL_VIEW + ' vwfornec.razaosocial, FORMPAG.DESCRICAO, FORMPAG.COD_LOJA, PEDCOMPRA.NUMDEV, PEDCOMPRA.NUMFISCAL, PEDCOMPRA.DESTINATARIO, ';
           XSQL_VIEW := XSQL_VIEW + ' PEDCOMPRA.COBRANCA, PEDCOMPRA.MARK, PEDCOMPRA.FATURADO, PEDCOMPRA.EXPORT, PEDCOMPRA.FISCO, PEDCOMPRA.NUMFISCAL_DEVOLVIDA ';
           XSQL_VIEW := XSQL_VIEW + ' FROM PEDCOMPRA LEFT JOIN FORNECEDOR ON PEDCOMPRA.COD_FORNEC = FORNECEDOR.COD_FORNEC ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PESSOA ON FORNECEDOR.COD_PESSOA = PESSOA.COD_PESSOA ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN vwfornec ON pedcompra.cod_fornec = vwfornec.cod_fornec ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN FORMPAG ON PEDCOMPRA.COD_FORMPAG = FORMPAG.COD_FORMPAG ';
      ...
      begin
          ...
          XCAMPOS := ' COD_INTERNO, COD_PARCELACP, COD_CTAPAGAR, COD_COTA, COD_PARANT, COBRANCA, DTVENC, ';
        	XCAMPOS := XCAMPOS + ' NUMPARC, VALOR, VALORPG, DTDEBITO, FECH, CLASSIFICACAO,DESCRICAO, ';
        	XCAMPOS := XCAMPOS + ' COD_PLNCTA, COD_FORMPAG, COD_LOJA, FORMPAG, COD_FORNEC, COD_PESSOA, ';
          XCAMPOS := XCAMPOS + ' NOME, DTLANC, NUMDOC, HISTORICO, PREVISAO, DTREFER, MARK, NUMFISC, NUMBOLETO, CHEQUES, RAZAOSOCIAL ';

           XSQL_VIEW := ' SELECT FORNECEDOR.COD_INTERNO, PARCELACP.COD_PARCELACP, PARCELACP.COD_CTAPAGAR, PARCELACP.COD_COTA, PARCELACP.COD_PARANT, ';
           XSQL_VIEW := XSQL_VIEW + ' PARCELACP.COBRANCA, PARCELACP.DTVENC, PARCELACP.NUMPARC, PARCELACP.VALOR, PARCELACP.VALORPG, ';
           XSQL_VIEW := XSQL_VIEW + ' PARCELACP.DTDEBITO, PARCELACP.FECH, PLNCTA.CLASSIFICACAO, PLNCTA.DESCRICAO, PLNCTA.COD_PLNCTA, FORMPAG.COD_FORMPAG, ';
           XSQL_VIEW := XSQL_VIEW + ' FORMPAG.COD_LOJA, FORMPAG.DESCRICAO, FORNECEDOR.COD_FORNEC, PESSOA.COD_PESSOA, PESSOA.NOME, CTAPAGAR.DTLANC, ';
           XSQL_VIEW := XSQL_VIEW + ' CTAPAGAR.NUMDOC, PARCELACP.HISTORICO, PARCELACP.PREVISAO, PARCELACP.DTREF, PARCELACP.MARK, PARCELACP.NUMFISC, ';
           XSQL_VIEW := XSQL_VIEW + ' PARCELACP.NUMBOLETO, PARCELACP.CHEQUES, PESSOAJ.RAZAO_SOCIAL ';
           XSQL_VIEW := XSQL_VIEW + ' FROM PARCELACP ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN CTAPAGAR ON PARCELACP.COD_CTAPAGAR = CTAPAGAR.COD_CTAPAGAR ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PLNCTA ON CTAPAGAR.COD_PLNCTA = PLNCTA.COD_PLNCTA ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN FORMPAG ON CTAPAGAR.COD_FORMPAG = FORMPAG.COD_FORMPAG ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN FORNECEDOR ON CTAPAGAR.COD_FORNEC = FORNECEDOR.COD_FORNEC ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PESSOA ON  FORNECEDOR.COD_PESSOA = PESSOA.COD_PESSOA ';
           XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PESSOAJ ON PESSOA.COD_PESSOA = PESSOAJ.COD_PESSOA ';
...
Begin
	Try
       ...
       Then Begin
           ...
           MDO.QAlx1.ParamByName('Codigo').AsInteger:=MDO.QConsulta.FieldByName('cod_itservord').AsInteger;
           MDO.QAlx1.Open;
           ...
           Then Begin
               ...
               MDO.Query.ParamByName('COD_ORDEM').AsInteger:=MDO.QAlx1.FieldByName('COD_ORDEM').AsInteger;
               MDO.Query.ParamByName('COD_SERVICO').AsInteger:= MDO.QAlx1.FieldByName('COD_SERVICO').AsInteger;
               MDO.Query.ParamByName('QTD').AsCurrency:=MDO.QAlx1.FieldByName('QTD').AsCurrency;
               MDO.Query.ParamByName('DESCONTO').AsCurrency:=MDO.QAlx1.FieldByName('DESCONTO').AsCurrency;
               MDO.Query.ParamByName('VLRUNIT').AsCurrency:=MDO.QAlx1.FieldByName('VLRUNIT').AsCurrency;
               MDO.Query.ParamByName('VLRUNITCUSTO').AsCurrency:=MDO.QAlx1.FieldByName('VLRUNITCUSTO').AsCurrency;
               MDO.Query.ParamByName('TOTAL').AsCurrency:=MDO.QAlx1.FieldByName('TOTAL').AsCurrency;
               MDO.Query.ParamByName('COD_FUNCIONARIO').AsInteger:=MDO.QAlx1.FieldByName('COD_FUNCIONARIO').AsInteger;
               MDO.Query.ParamByName('DATA').AsDateTime:=MDO.QAlx1.FieldByName('DATA').AsDateTime;
               MDO.Query.ParamByName('TOTENCARGO').AsCurrency:=MDO.QAlx1.FieldByName('TOTENCARGO').AsCurrency;
               MDO.Query.ParamByName('LUCMOE').AsCurrency:=MDO.QAlx1.FieldByName('LUCMOE').AsCurrency;
               MDO.Query.ParamByName('LUCPER').AsCurrency:=MDO.QAlx1.FieldByName('LUCPER').AsCurrency;
               If MDO.Query.ParamByName('COD_ITSERVORD').AsInteger=-1
               Then Begin
               ...
               MDO.Query.ParamByName('Codigo').AsInteger:=MDO.QConsulta.FieldByName('cod_itservord').AsInteger;
               MDO.Query.ExecSQL;
               MDO.Transac.CommitRetaining;
           End;
           MDO.QConsulta.Next;
...
begin
      ...
      if InsereIndice('ID_COD_SUBSERVICO','subservico','cod_subservico', false)=False then
      begin
           MPainel.Lines.Add('Erro ao gerar Indice: ID_COD_SUBSERVICO - subservico - cod_subservico');
      end;

      If CBIndiceITServOrd.Checked=True
      Then Begin
          //indice para tabela itservord CAMPO cod_ordem
          if InsereIndice('ID_cod_ordem','itservord','cod_ordem', false)=False then
          begin
          ...
          if InsereIndice('ID_cod_itservord','itservord','cod_itservord', true)=False then
          begin
      ...
      if InsereIndice('ID_CLASSIFICACAO','DESPADIC','CLASSIFICACAO', false)=False then
      begin
      		MPainel.Lines.Add('Erro ao gerar Indice: ID_CLASSIFICACAO - DESPADIC - CLASSIFICACAO');
      end;

      //indice para tabela SUBPRODUTO
      if InsereIndice('ID_COD_UNIDVENDA','SUBPRODUTO','COD_UNIDVENDA', false)=False then
      begin
      		MPainel.Lines.Add('Erro ao gerar Indice: ID_COD_UNIDVENDA - SUBPRODUTO - COD_UNIDVENDA');
      end;

      if InsereIndice('ID_COD_PERIODOFISCAL','DOCFISORD','COD_PERIODOFISCAL', false)=False then
      begin
      ...
      if InsereIndice('ID_COD_GERADOR','INFCOMPLEMENTAR','COD_GERADOR', false)=False then
      begin
      		MPainel.Lines.Add('Erro ao gerar Indice: COD_GERADOR - INFCOMPLEMENTAR - COD_GERADOR');
      end;
      if InsereIndice('IDX_COD_DOCFISCAL','INFCOMPLEMENTAR','COD_DOCFISCAL', false)=False then
      begin
      ...
      if InsereIndice('IDX_DOCFISCORD_COD_ORDEM','DOCFISORD','COD_ORDEM', true)=False then
      begin
      ...
      if InsereIndice('ID_IT_GRAFICA_COD_ITGRAFICA','IT_GRAFICA','COD_ITGRAFICA', true)=False then
      begin
...
//Paulo 27/03/2012: cria uma nova tabela no banco
function TFAtualizaBanco.InsereTabelaNova(XTABELA:String; XPK:String):Boolean;
begin
     Result:=True;
         ...
         try
             ...
             try
				//Vamos tentar marcar o campo criado como '1' garantindo que não mude o trabalho de que já usa o sistema
             	MDO.Query.Close;
             	MDO.Query.SQL.Clear;
             	MDO.Query.SQL.Add(' UPDATE lojacesso SET lojacesso.editavendedoros='+#39+'1'+#39);
             	MDO.Query.ExecSQL;
             	MDO.Transac.CommitRetaining;
             except
            	MDO.Transac.RollbackRetaining;
...
//Paulo 27/03/2012: atualiza o campo da tabela
function TFAtualizaBanco.AtualizaTabela(XTABELA:String; XCAMPO:String; XTIPO:String):Boolean;
begin
     ...
     begin
         try
             ...
             MDO.Query.Close;
             MDO.Query.SQL.Clear;
             MDO.Query.SQL.Add('ALTER TABLE '+XTABELA+ ' ADD '+XCAMPO+' '+XTIPO);
             MDO.Query.ExecSQL;
             MDO.Transac.CommitRetaining;
         except
            MDO.Transac.RollbackRetaining;
...
//Alex 09/04/2014: atualiza o campo da tabela
function TFAtualizaBanco.AtualizaTabelaCampo(XTABELA:String; XCAMPO:String; XTIPO:String):Boolean;
begin
     //Paulo 27/03/2012: gera script
     Result:=True;
     //Paulo 09/04/2012: Verifica se já existe o campo
     ...
     then   begin
         try
             ...
             MDO.Query.Close;
             MDO.Query.SQL.Clear;
             MDO.Query.SQL.Add('ALTER TABLE '+XTABELA+ ' ALTER '+XCAMPO+' TYPE '+XTIPO);
             MDO.Query.ExecSQL;
             MDO.Transac.CommitRetaining;
         except
            MDO.Transac.RollbackRetaining;
...
//Paulo 28/03/2012: gera a view
function TFAtualizaBanco.InsereNovaView(XVIEW:String; XCAMPOS:String; XSQL:String):Boolean;
begin
    Result:=True;
...
//Paulo 02/04/2012: cria nova generation
function TFAtualizaBanco.InsereGeneration(XTABELA:String; XPK:String):Boolean;
var
	XCOD:Integer;
begin
	Result:=True;
   ...
   MDO.QConsulta.ParamByName('NOMEGENERATION').AsString:='GEN_'+XTABELA+'_ID';
   MDO.QConsulta.Open;
   if MDO.QConsulta.IsEmpty then
   begin
       ...
       try
           ...
           MDO.QConsulta.Close;
           MDO.QConsulta.SQL.Clear;
           MDO.QConsulta.SQL.Add('SET GENERATOR GEN_'+XTABELA+'_ID TO '+IntToStr(XCOD));
           MDO.QConsulta.ExecSQL;
           MDO.Transac.CommitRetaining;
       except
          MDO.Transac.RollbackRetaining;
...
//Paulo 02/04/2012: Gera Trigger
function TFAtualizaBanco.InsereTrigger(XTABELA:String; XGENERATION:String; XPK:String):Boolean;
begin
    Result:=True;
...
//Paulo 02/04/2012: Gera Indices
function TFAtualizaBanco.InsereIndice(XINDICE:String; XTABELA:String; XCAMPO:String; XUnique: Boolean):Boolean;
begin
    Result:=True;
    //Paulo 09/04/2012: Verifica se já existe a TRIGGER
   MDO.QConsulta.Close;
   ...
   begin
        try
           ...
           If XUnique=True Then
           	MDO.Query.SQL.Add('CREATE UNIQUE INDEX '+XINDICE+' ON '+XTABELA+' ('+XCAMPO+')')
           Else
           	MDO.Query.SQL.Add('CREATE INDEX '+XINDICE+' ON '+XTABELA+' ('+XCAMPO+')');
           MDO.Query.ExecSQL;
           MDO.Transac.CommitRetaining;
        except
           MDO.Transac.RollbackRetaining;
...
begin
   ...
   begin
       ...
       DMESTOQUE.TCST.FieldByName('DESCRICAO').AsString := 'Tributada pelo Simples Nacional sem permissão de crédito';
    	...
    	try
           ...
           begin
               ...
               begin
                   ...
                   Get('downloads/Zion/Arquivos/update_nfe_4.11.exe', 'update_nfe_4.11.exe', True, False);
                   Quit;
               end;
           end;
           
           //Edmar - 21/05/2018 - desregistra a dll
           WinExec(PChar('cmd /k regasm /u NFe_Util_2G.dll /tlb:NFe_Util_2G.tlb'), SW_HIDE);
           ...
           MoveFile(PChar(XDiretorioSistema+'\NFe_Util'), PChar(XDiretorioSistema+'\NFe_Util_4.09'));
...
begin
   ...
   try
   	if XACAO = 'START' then
   		WinExec('command.com /c net start "Firebird Guardian - DefaultInstance"',SW_HIDE)
       else
           WinExec('command.com /c net stop "Firebird Guardian - DefaultInstance"',SW_HIDE);
   Except
   	Result:=False;
   end;  
end;
...
begin
    ...
    try
       ...
       MDO.Query.SQL.Add(' WHERE RDB$FLAGS=1 and RDB$RELATION_NAME= '+QuotedStr(xTabela));
       MDO.Query.Open;
...
begin
   ...
   begin
       ...
       XCAMPOS := ' COD_CHEQUEREC, NUMAG, NUMCTA, DTLANC, DTVENC, DTMOV, USUARIO, VALOR,NUMCHEQUE, HISTORICO, BANCO, CTACORLAN, CLASSCONTA, CONTA, FECH, COD_MOVBANCO, EMITENTE, CPFEMIT, DESTINO ';
       XSQL_VIEW := 'SELECT chequerec.cod_chequerec, chequerec.numagencia, chequerec.numconta, ';
       XSQL_VIEW := XSQL_VIEW + ' movbanco.dtlanc, movbanco.dtvenc, movbanco.dtmov, movbanco.cod_usuario, movbanco.valor, ';
       XSQL_VIEW := XSQL_VIEW + ' movbanco.numcheque, movbanco.historico, banco.descbanco, ctacor.numctacor, ';
       XSQL_VIEW := XSQL_VIEW + ' plncta.classificacao, plncta.descricao, movbanco.fech,movbanco.cod_movbanco,chequerec.emitente, chequerec.cpfemit, chequerec.destino ';
       XSQL_VIEW := XSQL_VIEW + ' FROM chequerec Left join movbanco ON chequerec.cod_movbanco = movbanco.cod_movbanco ';
       XSQL_VIEW := XSQL_VIEW + ' Left join banco    on banco.cod_banc = chequerec.cod_banco ';
       XSQL_VIEW := XSQL_VIEW + ' left join ctacor   On ctacor.cod_ctacor = movbanco.cod_ctacor ';
       XSQL_VIEW := XSQL_VIEW + ' left join plncta   On plncta.cod_plncta = movbanco.cod_plncta ';
   ...
   begin
       ...
       //recria as view
       //VWLANCENT
       XCAMPOS := ' COD_LANCENT, COD_ESTOQUE, COD_LOJA, COD_USUARIO, DT_LANCAMENTO, MOTIVO, ';
       XCAMPOS := XCAMPOS + 'NUMNOTA, QUANTIDADE, CTRLINT, PRODUTO, ANTERIOR, CODFABRICANTE';
       XSQL_VIEW := ' SELECT LANCENT.COD_LANENT, LANCENT.COD_ESTOQUE, LANCENT.COD_LOJA, ';
       XSQL_VIEW := XSQL_VIEW + '  LANCENT.COD_USUARIO, LANCENT.DT_LANCAMENTO, LANCENT.MOTIVO, ';
       XSQL_VIEW := XSQL_VIEW + '  LANCENT.NUMNOTA, LANCENT.QUANTIDADE, SUBPRODUTO.CONTRINT, SUBPRODUTO.DESCRICAO, LANCENT.QTDANT, SUBPRODUTO.CODPRODFABR ';
       XSQL_VIEW := XSQL_VIEW + '  FROM LANCENT ';
       XSQL_VIEW := XSQL_VIEW + '  LEFT JOIN ESTOQUE ON LANCENT.COD_ESTOQUE = ESTOQUE.COD_ESTOQUE ';
       XSQL_VIEW := XSQL_VIEW + '  LEFT JOIN SUBPRODUTO ON SUBPRODUTO.COD_SUBPRODUTO = ESTOQUE.COD_SUBPROD ';
       ...
       //VWLANCSAI
       XCAMPOS := ' COD_LANCSAI, COD_ESTOQUE, COD_LOJA, COD_USUARIO, DT_LANCAMENTO, MOTIVO, ';
       XCAMPOS := XCAMPOS + 'NUMNOTA, QUANTIDADE, CTRLINT, PRODUTO, ANTERIOR, CODFABRICANTE';
       XSQL_VIEW := ' SELECT LANCSAI.COD_LANCSAI, LANCSAI.COD_ESTOQUE, LANCSAI.COD_LOJA, ';
       XSQL_VIEW := XSQL_VIEW + '  LANCSAI.COD_USUARIO, LANCSAI.DT_LANCAMENTO, LANCSAI.MOTIVO, ';
       XSQL_VIEW := XSQL_VIEW + '  LANCSAI.NUMNOTA, LANCSAI.QUANTIDADE, SUBPRODUTO.CONTRINT, SUBPRODUTO.DESCRICAO,LANCSAI.QTDANT, SUBPRODUTO.CODPRODFABR ';
       XSQL_VIEW := XSQL_VIEW + '  FROM LANCSAI JOIN ESTOQUE ON LANCSAI.COD_ESTOQUE = ESTOQUE.COD_ESTOQUE ';
       XSQL_VIEW := XSQL_VIEW + '  LEFT JOIN SUBPRODUTO ON SUBPRODUTO.COD_SUBPRODUTO = ESTOQUE.COD_SUBPROD ';
       ...
       //VWSIMILAR
       XCAMPOS := ' COD_SUBPRODUTO, CODCOMPOSTO, DESCRICAO, CODBARRA, MARCA, COD_INTERNO, COD_PRODUTO, COD_ESTOQUE, COD_LOJA, ';
       XCAMPOS := XCAMPOS + 'ULTCOMPRA, ULTVENDA, ESTFISICO, VENDVARP, VANDATAP, VENDATAV, VENDVARV, CODFABRICANTE, CONTRINT, ';
       XCAMPOS := XCAMPOS + 'VLRBONIFIC, ESPECIFICACAO, MARK, VLRUNITCOMPRA, LOCALESTANTE, ESTLOJA, ATIVO ';

       XSQL_VIEW := ' SELECT  SUBPRODUTO.COD_SUBPRODUTO, SUBPRODUTO.CODCOMPOSTO, SUBPRODUTO.DESCRICAO, ';
       XSQL_VIEW := XSQL_VIEW + '  SUBPRODUTO.CODBARRA, SUBPRODUTO.MARCA, SUBPRODUTO.COD_INTERNO, SUBPRODUTO.COD_PRODUTO, ESTOQUE.COD_ESTOQUE, ';
       XSQL_VIEW := XSQL_VIEW + '  ESTOQUE.COD_LOJA, ESTOQUE.ULTCOMPRA, ESTOQUE.ULTVENDA, ESTOQUE.ESTFISICO, ESTOQUE.VENDVARP, ';
       XSQL_VIEW := XSQL_VIEW + '  ESTOQUE.VENDATAP, ESTOQUE.VENDATAV, ESTOQUE.VENDVARV, SUBPRODUTO.CODPRODFABR, SUBPRODUTO.CONTRINT, ESTOQUE.VLRBONIFIC, ';
       XSQL_VIEW := XSQL_VIEW + '  SUBPRODUTO.ESPECIFICACAO, SUBPRODUTO.MARK, ESTOQUE.VLRUNITCOMP, SUBPRODUTO.LOCALESTANTE, ';
       XSQL_VIEW := XSQL_VIEW + '  ESTOQUE.ESTLOJA, SUBPRODUTO.ATIVO ';
       XSQL_VIEW := XSQL_VIEW + '  FROM SUBPRODUTO ';
       XSQL_VIEW := XSQL_VIEW + '  JOIN ESTOQUE ON SUBPRODUTO.COD_SUBPRODUTO = ESTOQUE.COD_SUBPROD ';
   ...
   begin
      ...
      //recria a view
      XCAMPOS := ' COD_LANC, COD_ABCAIXA, COD_PLNCTAFIL, DTLANC, HISTORICO, TIPCAI, ';
      XCAMPOS := XCAMPOS + ' TIPO, VALOR, CLASSIFICACAO, COD_LOJA, DESCRICAO, ';
      XCAMPOS := XCAMPOS + ' COD_USUARIO, LOGIN, DOC, ESTRU, TIPOGERADOR, COD_GERADOR ';
      
      XSQL_VIEW := ' SELECT LANCAIXA.COD_LANC, LANCAIXA.COD_ABCAIXA, LANCAIXA.COD_PLNCTAFIL, ';
      XSQL_VIEW := XSQL_VIEW + ' LANCAIXA.DTLANC, LANCAIXA.HISTORICO, LANCAIXA.TIPCAI, LANCAIXA.TIPO, ';
      XSQL_VIEW := XSQL_VIEW + ' LANCAIXA.VALOR, PLNCTA.CLASSIFICACAO, PLNCTA.COD_FILIAL, PLNCTA.DESCRICAO, ';
      XSQL_VIEW := XSQL_VIEW + ' LANCAIXA.COD_USUARIO, USUARIO.LOGIN, LANCAIXA.DOC, LANCAIXA.ESTRU, ';
      XSQL_VIEW := XSQL_VIEW + ' LANCAIXA.TIPOGERADOR, LANCAIXA.COD_GERADOR ';
      XSQL_VIEW := XSQL_VIEW + ' FROM LANCAIXA ';
      XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PLNCTA ON LANCAIXA.COD_PLNCTAFIL = PLNCTA.COD_PLNCTA ';
      XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN USUARIO ON USUARIO.CODUSUARIO = LANCAIXA.COD_USUARIO ';
   ...
   begin
      ...
      XCAMPOS := ' COD_MOVBANCO, COBRANCA, COD_CTACOR, COD_PLNCTA, COD_USUARIO, DTLANC, ';
      XCAMPOS := XCAMPOS + ' DTMOV, DTVENC, FECH, HISTORICO, HORALANC, TIPOOP, ';
      XCAMPOS := XCAMPOS + ' VALOR, CLASSIFICACAO, CONTA, NUMCHEQUE, VER, COD_ABBANCO, MARK, DESTINATARIOCH ';

      XSQL_VIEW := ' SELECT MOVBANCO.COD_MOVBANCO, MOVBANCO.COBRANCA, MOVBANCO.COD_CTACOR, MOVBANCO.COD_PLNCTA, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.COD_USUARIO, MOVBANCO.DTLANC, MOVBANCO.DTMOV, MOVBANCO.DTVENC, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.FECH, MOVBANCO.HISTORICO, MOVBANCO.HORALANC, MOVBANCO.TIPOOP, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.VALOR, PLNCTA.CLASSIFICACAO, PLNCTA.DESCRICAO, MOVBANCO.NUMCHEQUE, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.VER, MOVBANCO.COD_ABBANCO, MOVBANCO.MARK, MOVBANCO.DESTINATARIOCH ';
      XSQL_VIEW := XSQL_VIEW + ' FROM MOVBANCO ';
      XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PLNCTA ON MOVBANCO.COD_PLNCTA=PLNCTA.COD_PLNCTA ';
      ...
      XCAMPOS := XCAMPOS + ' USUARIO, VALOR, NUMCHEQUE, HISTORICO, BANCO, CTACORLAN, CLASSCONTA, CONTA, ';
      XCAMPOS := XCAMPOS + ' FECH, COD_MOVBANCO, EMITENTE, CPFEMIT, DESTINO ';

      XSQL_VIEW := ' SELECT CHEQUEREC.COD_CHEQUEREC, CHEQUEREC.NUMAGENCIA, CHEQUEREC.NUMCONTA,  MOVBANCO.DTLANC, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.DTVENC, MOVBANCO.DTMOV, MOVBANCO.COD_USUARIO, MOVBANCO.VALOR,  MOVBANCO.NUMCHEQUE, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.HISTORICO, BANCO.DESCBANCO, CTACOR.NUMCTACOR,  PLNCTA.CLASSIFICACAO, PLNCTA.DESCRICAO, ';
      XSQL_VIEW := XSQL_VIEW + ' MOVBANCO.FECH,MOVBANCO.COD_MOVBANCO,CHEQUEREC.EMITENTE, CHEQUEREC.CPFEMIT, CHEQUEREC.DESTINO ';
      XSQL_VIEW := XSQL_VIEW + ' FROM CHEQUEREC ';
      XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN MOVBANCO ON CHEQUEREC.COD_MOVBANCO = MOVBANCO.COD_MOVBANCO ';
      XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN BANCO ON BANCO.COD_BANC = CHEQUEREC.COD_BANCO ';
	   XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN CTACOR ON CTACOR.COD_CTACOR = MOVBANCO.COD_CTACOR ';
      XSQL_VIEW := XSQL_VIEW + ' LEFT JOIN PLNCTA ON PLNCTA.COD_PLNCTA = MOVBANCO.COD_PLNCTA ';
...
var
   xQuantidadeEstoque, xQuantidadeLancada, xValorCustoProduto, xValorUltimoValorCusto: Real;
begin
   try
       ...
       begin
           ...
           xQuantidadeEstoque := MDO.QConsulta.FieldByName('ESTFISICO').AsCurrency + MDO.QAlx1.FieldByName('QTDE_VENDIDA').AsCurrency;
           {******************************}
           {
           ...
           begin
               ...
               begin
                   ...
                   MDO.QAlx2.ParamByName('COD_PEDCOMPRA').AsInteger := MDO.QAlx1.FieldByName('COD_PEDCOMPRA').AsInteger;
                   MDO.QAlx2.Open;
                   
                   xValorCustoProduto
                       := (
                           MDO.QAlx1.FieldByName('VALORTOTAL').AsCurrency
                               + (MDO.QAlx1.FieldByName('VALORTOTAL').AsCurrency / MDO.QAlx2.FieldByName('VALORTOTALITENS').AsCurrency)
                               * MDO.QAlx2.FieldByName('DIFF').AsCurrency
                           ) / MDO.QAlx1.FieldByName('QTDE').AsCurrency;

                   if xValorCustoProduto = 0 then
                       xValorCustoProduto := MDO.QAlx1.FieldByName('VALORTOTAL').AsCurrency / MDO.QAlx1.FieldByName('QTDE').AsCurrency;
                   ...
                   InserirItemCustoProduto(MDO.QAlx1.FieldByName('COD_ESTOQUE').AsInteger, MDO.QAlx1.FieldByName('COD_ITENSPEDC').AsInteger,
                               xValorCustoProduto, xQuantidadeLancada, MDO.QAlx1.FieldByName('DATA').AsDateTime, 'PEDCOMPRA');
               end;
               MDO.QAlx1.Next;
           end;
           ...
           begin
               ...
               DMESTOQUE.Alx.ParamByName('VALOR_CUSTO').AsCurrency := xValorUltimoValorCusto;
               DMESTOQUE.Alx.ParamByName('COD_ESTOQUE').AsInteger := MDO.QConsulta.FieldByName('COD_ESTOQUE').AsInteger;
       ...
       begin
           AplicaSaidaCustoEstoque(DMESTOQUE.Alx.FieldByName('COD_ESTOQUE').AsInteger,
               DMESTOQUE.Alx.FieldByName('COD_ITEM').AsInteger, 'ORDEM', DMESTOQUE.Alx.FieldByName('QTDE').AsInteger);
       ...
       On E : Exception do
       begin
           ShowMessage('O processo de atualização dos custos não foi finalizado. Error: ' + E.Message);
           MDO.Transac.RollbackRetaining;
           DMESTOQUE.TransacEstoque.RollbackRetaining;
       end;
   end;
...
begin
   try
       ...
       begin
           ...
           MDO.QAlx1.ParamByName('PERCSERV').AsCurrency := MDO.QConsulta.FieldByName('PERCSERVICO').AsCurrency;
           MDO.QAlx1.ParamByName('COD_PARCELA').AsInteger := MDO.QConsulta.FieldByName('COD_PARCELACR').AsInteger;
           MDO.QAlx1.ExecSQL;
       ...
       On E : Exception do
       begin
           ShowMessage('O processo de normalização não foi finalizado. Error: ' + E.Message);
           MDO.Transac.RollbackRetaining;
       end;
   end;
end;
