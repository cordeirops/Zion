...
Begin
	Try
		...
		Then Begin
       	...
       	if InsereTrigger('ACESSOBANCO', 'gen_ACESSOBANCO_id', 'COD_ACESSO')=False then
       	...
       	if not AtualizaTabela('ACESSOBANCO', 'NOME_COMPUTADOR', 'VARCHAR(100)') then
...
begin
 ...
 // Paulo 31/03/2012: FUNÇÃO PARA CRIAR NOVA TABELA                      //
   ...
   if InsereTrigger('CTARECEBER', 'GEN_CTARECEBER_ID', 'COD_CTARECEBER') = False then
   ...
   if InsereTrigger('PARCELACR', 'GEN_PARCELACR_ID', 'COD_PARCELACR') = False then
   ...
   if InsereTrigger('PARCELACP', 'GEN_PARCELACP_ID', 'COD_PARCELACP') = False then
   ...
   if not AtualizaTabela('PESSOAJ', 'ORGAO_MUNICIPAL_ESTADUAL', 'VARCHAR(1)') then
...
//Edmar - 13/07/2017 - Atualiza campo TEL0800 para 50 caracteres
 ...
 //Alex - 23/06/2016 - Cria campo CAMINHODOCS para cada pessoa
  ...
  if not InsereTabelaNova('ADICAOIMPORTACAO', 'COD_ADICAO INTEGER NOT NULL') then
    //cria a pk da tabela
  ...
  if not InsereTabelaNova('ITENSVOLUME', 'COD_ITENSVOLUME INTEGER NOT NULL') then
    //cria a pk da tabela
...
begin
  ...
  begin
    XCAMPOS := ' COD_ORDEM, COD_CLIENTE, COD_EQUIPAMENTO, COD_FUNCIONARIO, COD_VENDEDOR, ';
    XCAMPOS := XCAMPOS + ' DTABERT, DTPREV, DTFECH, NUMERO, NUMORC, STATUS, MONITOR, TOTAL, TPROD, TSERV, CI_CLIENTE, ';
    XCAMPOS := XCAMPOS + ' CLIENTE, EQUIPAMENTO, COD_LOJA, GARANT, MARK, FISCO, NUMFISCAL, FORMAPAG, ';
    XCAMPOS := XCAMPOS + ' TIPOPAG, PLACA, BOX, EXP, EXPORTADO, STATUSNFSE, NUMFISCALNFSE, NUMFISCAL_DEVOLVIDA, ';
...
begin
  try
    ...
    MessageDlg('Falha ao criar chave primária para a tabela estoque.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
  try
    ...
    MessageDlg('Falha ao criar Trigger ESTOQUE_AI0 para a tabela estoque.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
  try
    ...
    MessageDlg('Falha ao criar Trigger LANCENT_AIUD0 para a tabela LANCENT.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
  try
    ...
    MessageDlg('Falha ao criar Trigger LANCSAI para a tabela LANCSAI.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
  try
    ...
    MessageDlg('Falha ao criar chave primária para a tabela subproduto.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
  try
    ...
    MessageDlg('Falha ao criar chave primária para a tabela Ordem.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
   try
    ...
    MessageDlg('Falha ao criar estrutura de Movimento de Estoque.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
...
begin
  try
    ...
    MessageDlg('Falha ao criar chave primária para a tabela ITProdOrd.', mtWarning, [mbOK], 0);
    Result := False;
  end;
end;
