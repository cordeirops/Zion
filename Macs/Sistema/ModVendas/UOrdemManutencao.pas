...
type
  TFOrdemManutencao = class(TFCadPadrao)
    ...
    procedure FrmItensEdDescricaoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure BTNOPENClick(Sender: TObject);
    procedure EdProdutoDescricaoKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EdProdutoDescricaoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
var
  ...
  //VARIAVEIS
  xCodFornecedor, xCodUsuario, xCodFuncionario, xCodFormaPag, xCodEstoque, xNumPedido, xCodPedido, xCodConta, xCodContaBanco, xCodEstoqueProduto: Integer;
...
uses UDMEntrada, UDmServ, AlxMessage, UFORNECEDOR, UDMPessoa, DB, UPadrao,
  ...
  UConsPlncta, UDMConta, UDmBanco, URelEstoque, UFuncionario, UDmGeografia, UDMSAIDA,
  URelProdutos;
  ...
  inherited;
   ...
   Then Begin
       ...
       Then Begin
           ...
           DMESTOQUE.WSimilar.SQL.Add(' where subproduto.usoconsumo<>''5''  order by vwsimilar.contrint ');
   ...
   Then Begin
       ...
       Else Begin
           ...
           Then Begin
               ...
               Then Begin
                   XCodTmpEstoque:=DBGridListaProdutos.Columns[0].Field.Text;
               End;
           End
           Else Begin
               XCodTmpEstoque:=DBGridListaProdutos.Columns[0].Field.Text;
...
begin
  inherited;
   ...
   try
   	//Edmar - 09/02/2015 - Filtra buscando o produto que foi usado na manutenção
   	...
   	Mensagem('MZR Sistemas', 'Não foi possível atualizar o estoque do produto. Por favor, contate o suporte.', '', 1, 1, False, 'a');
       Exit;   	
   end;
...
Begin
   ...
   DMESTOQUE.TSlave.SQL.Add(' itenspedc.obs, itenspedc.qtdeprod, itenspedc.valunit, itenspedc.valortotal, itenspedc.data, vwsimilar.contrint  ');
...
Begin
   ...
   Try
    	...
    	Then begin
           ...
           MDO.QConsulta.SQL.Add('	vwpedc.nome, vwpedc.numped, vwpedc.valor, formpag.descricao,  formpag.cod_interno as INTERNO_FORMAPAG, fornecedor.cod_interno as INTERNO_FORNEC, pedcompra.cod_prodmanutencao');
           ...
           MDO.QConsulta.SQL.Add(' 	left join pedcompra on vwpedc.cod_pedcomp = pedcompra.cod_pedcomp');           
...
Begin
   Result:=True;
	Try
       ...
       //Edmar - 16/06/2014 - Filtra buscando o produto que está em manutenção
       if FiltraTabela(DMESTOQUE.Alx, 'VWSIMILAR', 'COD_ESTOQUE', MDO.QConsulta.FieldByName('COD_PRODMANUTENCAO').AsString, '') then
		begin
       	xCodEstoqueProduto := MDO.QConsulta.FieldByName('COD_PRODMANUTENCAO').AsInteger; 
       	EdProdutoDescricao.Text := DMESTOQUE.Alx.FieldByName('DESCRICAO').AsString;
       end;
   Except
       Result:=False;
   End;
...
begin
  inherited;
   ...
   Except
   	On E : Exception do
       begin
       	MessageDlg('Não foi possível remover o item da ordem e/ou atualizar seu estoque.'+#13+E.Message, mtWarning, [mbOK], 0);
   		DMESTOQUE.TransacEstoque.RollbackRetaining;
...
begin
   ...
   Then Begin
       ...
       PConsulta.BringToFront;
       FiltraOrdem;
	    xCodEstoqueProduto := -1;//Edmar - 16/06/2014 - seta -1 para não salvar o produto anterior na proxima OS
       inherited;
   End;
...
Begin
   ...
   End;
   {
   if xCodEstoqueProduto = -1 then
   begin
   	Mensagem('OPÇÃO BLOQUEADA', 'Por favor, selecione o produto para manutenção.', '', 1, 1, False, 'a');
   ...
   Try
       //Edmar - 09/02/2015 - Antes de atualizar o pedido com as novas informações
       //buscar o mesmo para verificar se o produto em manutenção não se alterou.
       FiltraTabela(DMSAIDA.TAlx, 'PEDCOMPRA', 'COD_PEDCOMP', IntToStr(xCodPedido), '');
		//se o produto do pedido de manutenção for diferente do produto selecionado
       //significa que foi alterado o produto em manutenção.
       //neste caso, retira o produto anterior do estoque (com seu devido lançamento) e incrementa o estoque do novo produto
       if DMSAIDA.TAlx.FieldByName('COD_PRODMANUTENCAO').AsInteger <> xCodEstoqueProduto then
       begin
       	//Edmar - 09/02/2015 - Filtra buscando o estoque do produto para incrementar a quantidade do estoque
   	...
   	MDO.Query.SQL.Add('update pedcompra set pedcompra.cod_fornec=:fornec, pedcompra.cod_formpag=:formapag, pedcompra.valor=:valor, ');
		MDO.Query.SQL.Add(' pedcompra.COD_USUARIO=:usuario, pedcompra.cod_prodmanutencao = :codprodmanu, pedcompra.descricao_prodmanutencao = :descricao ');
       ...
       MDO.Query.ParamByName('codprodmanu').AsCurrency:=xCodEstoqueProduto;
       MDO.Query.ParamByName('descricao').AsString := EdProdutoDescricao.Text;
       MDO.Query.ExecSQL;
   ...
   Except
   	On E : Exception do
       begin
       	MessageDlg('Não foi possível gravar a ordem de manuteção.'+#13+E.Message, mtWarning, [mbOK], 0);
           MDO.Transac.RollbackRetaining;
           DMESTOQUE.TransacEstoque.RollbackRetaining;
           DMSAIDA.IBT.RollbackRetaining;
           DMENTRADA.IBT.RollbackRetaining;
...
begin
  inherited;
   ...
   Then Begin
       ...
       Then Begin
       	Try
           	//Edmar - 09/02/2015 - Filtra buscando o pedido
           	FiltraTabela(DMSAIDA.TAlx, 'PEDCOMPRA', 'COD_PEDCOMP', IntToStr(xCodPedido), '');
   		...
   		Except
           	On E : Exception do
               begin
               	MessageDlg('Não foi possível excluir a ordem de manuteção e/ou atualizar o estoque do produto.'+#13+E.Message, mtWarning, [mbOK], 0);
               	DMESTOQUE.TransacEstoque.RollbackRetaining;
					DMSAIDA.IBT.RollbackRetaining;
                   MDO.Transac.RollbackRetaining;
               end;
...
begin
  inherited;
   ...
   If Mensagem('CONFIRMAÇÃO DO USUÁRIO', 'Deseja Realmente fechar a ordem '+DMENTRADA.TAlx.FieldByName('NUMPED').AsString, '', 2, 3, False, 'C')=2 then
   begin
   	If DMENTRADA.TAlx.FieldByName('SITUACAO').AsString<>'FECHADO' then
       begin
...
begin
   ...
   If FechaOrdem(IntToStr(xCodPedido), EdDtFech.Text)=True then
   begin
   	Try
	    	MDO.Transac.CommitRetaining;
       Except
       ...
       //Edmar - 12/02/2015 - Rotina para retornar o estoque do produto novamente
       if FiltraTabela(DMESTOQUE.TSlave, 'ITENSPEDC', 'COD_PEDCOMPRA', IntToStr(xCodPedido), '') then
       begin
           ...
           //percorre os itens retornando seu estoque
...
begin
  inherited;
   ...
   FMenu.XTipoRel :='FRFPRODMANUTENCAO';
...
begin
  ...
  //MAURO - FILTRA EMPRESA PARA IMPRIMIR CABEÇALHO
   ...
   if not FiltraTabela(DMGEOGRAFIA.TAlx, 'VWCIDADE', 'COD_CIDADE', DMMACS.TEmpresa.FieldByName('COD_CIDADE').AsString, '') then
   	Exit;
   ...
   DMESTOQUE.TRel.SQL.Add(' vwpedc.nome, vwpedc.numped, vwpedc.valor, vwpedc.numdev, vwsimilar.codfabricante, vwsimilar.contrint,vwfuncionario.nome as Responsavel, pedcompra.cod_prodmanutencao, vwpedc.situacao ');
   ...
   if DMESTOQUE.TRel.FieldByName('COD_PRODMANUTENCAO').AsString <> '' then
   	FiltraTabela(DMESTOQUE.Alx, 'VWSIMILAR', 'COD_ESTOQUE', DMESTOQUE.TRel.FieldByName('COD_PRODMANUTENCAO').AsString, '')
   else//Edmar - 16/06/2014 - Se a ordem não possui produto de manutenção
   	//Busca por -1 para evitar erro de sql vazia no comprovante
		FiltraTabela(DMESTOQUE.Alx, 'VWSIMILAR', 'COD_ESTOQUE', '-1', '');   
...
procedure TFOrdemManutencao.FrmItensEdDescricaoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TFOrdemManutencao.BTNOPENClick(Sender: TObject);
begin
  inherited;
 	If AbrirForm(TFProduto, FProduto, 1)='Selected' then
   Begin
   	xCodEstoqueProduto := DMESTOQUE.WSimilar.FieldByName('COD_ESTOQUE').AsInteger;
   	EdProdutoDescricao.Text := DMESTOQUE.WSimilar.FieldByName('DESCRICAO').AsString; 
   end;
end;

procedure TFOrdemManutencao.EdProdutoDescricaoKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  	If PListaProdutos.Visible=False
   Then Begin
   	If EdProdutoDescricao.Text<>''
       Then Begin
           PListaProdutos.Top:=137;
           PListaProdutos.Left:=16;
           PListaProdutos.Visible:=True;
           PListaProdutos.BringToFront;
           DMESTOQUE.WSimilar.Close;
   ...
   Then Begin
       ...
       End
       Else Begin
           If DMESTOQUE.WSimilar.Locate('descricao', UpperCase(EdProdutoDescricao.Text),[loPartialKey])=false
           Then Begin
               If DMESTOQUE.WSimilar.Locate('contrint', UpperCase(EdProdutoDescricao.Text),[loPartialKey])=false
               Then Begin
                   XCodTmpEstoque:=DBGridListaProdutos.Columns[0].Field.Text;
               End;
           End
...
procedure TFOrdemManutencao.EdProdutoDescricaoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
	If (Key=Vk_f2) and (key<>Vk_F3)
   Then Begin
       If AbrirForm(TFProduto, FProduto, 1)='Selected'
       Then Begin
           xCodEstoqueProduto := DMESTOQUE.WSimilar.FieldByName('COD_PRODMANUTENCAO').AsInteger;
           EdProdutoDescricao.Text := DMESTOQUE.WSimilar.FieldByName('DESCRICAO').AsString;
           PListaProdutos.Visible:=False;
       End;
   End;
...
procedure TFOrdemManutencao.FormClose(Sender: TObject;
