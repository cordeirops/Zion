...
type
  TFOrdemRetifica = class(TFPadrao)
    ...
    procedure EdVlrTotProdExit(Sender: TObject);
    procedure EdVlrTotServExit(Sender: TObject);
  private
...
Begin
	Try
       ...
       DMServ.WOrdemDesp.SQL.Add('   equipamento.modelo, vwordem.cliente, vwordem.formapag, vwordem.dtabert, ordem.cod_vendedor, ');
...
Begin
	Try
       ...
       Then Begin
           ...
           Then Begin
               MessageDlg('Ordem de Serviços bloqueada. Verifique se ela está em edição por outro usuário.'+#13+#10+'Use as teclas ctrl+D para desbloquear.', mtWarning, [mbOK], 0);
               Result:=False;
               Exit;
           End;
  ...
  inherited;
   ...
   Then Begin
   	...
   	FrmFormaPagamento.Repaint;
               If (DMFINANC.TFormPag.FieldByName('DESCRICAO').AsString = 'CORTESIA') or (DMFINANC.TFormPag.FieldByName('DESCRICAO').AsString = 'GARANTIA')
               Then Begin
                   CBGeraFinanceiro.Checked:=true;
                   CBGeraFinanceiro.Enabled:=false;
                   CBGeraFinanceiro.Visible:=true;
               End
               else Begin
                   If ControlAccess('GERAFINANC', '')=True
                   Then Begin
                       CBGeraFinanceiro.Visible:=true;
                       CBGeraFinanceiro.Enabled:=true;
...
begin
  inherited;
   ...
   Then Begin
       ...
       Then Begin
         If FiltraTabela(DMESTOQUE.WSimilar, 'vwsimilar', 'COD_ESTOQUE', DMESTOQUE.TSlave.FieldByName('COD_ESTOQUE').AsString, '')
         Then Begin
...
begin
  ...
  //EFETUA CONTROLE DE ACESSO
   ...
   DMPESSOA.TALX1.SQL.Add(' select * from vwfuncionario where vwfuncionario.cod_func = :codigo ');
   DMPESSOA.TALX1.ParamByName('codigo').AsInteger :=  DMSERV.WOrdemDesp.FieldByNAme('cod_vendedor').AsInteger ;
   DMPESSOA.TALX1.Open;
...
begin
  inherited;
   ...
   Then Begin
       If FiltraTabela(DMServ.WSubServ, 'vwsubservico', 'cod_subservico', DMESTOQUE.TSlaveServ.FieldByName('COD_SERVICO').AsString, '')
       Then Begin
       	//Buscamos o Item de produto
...
procedure TFOrdemRetifica.FormClose(Sender: TObject;
...
procedure TFOrdemRetifica.BtnSelecionarClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemRetifica.Cartadecorreoeletrnica1Click(Sender: TObject);
begin
  inherited;
    IniciaCCe(DMServ.WOrdemDesp.FieldByName('COD_ORDEM').AsInteger, 'ORDEM');
end;

procedure TFOrdemRetifica.EfetuardevoluodeNFe1Click(Sender: TObject);
begin
  inherited;
	//Edmar - 20/03/2015 - busca o cliente do pedido            DMServ.WOrdemDesp.FieldByName('COD_ORDEM').AsInteger;
...
procedure TFOrdemRetifica.EdVlrTotProdExit(Sender: TObject);
begin
  inherited;
   EdVlrUnitProd.ValueNumeric := EdVlrTotProd.ValueNumeric / EdQuantProd.ValueNumeric;
end;

procedure TFOrdemRetifica.EdVlrTotServExit(Sender: TObject);
begin
  inherited;
   EdVlrUnitServ.ValueNumeric :=  EdVlrTotServ.ValueNumeric /  EdQuantServ.ValueNumeric;
end;
