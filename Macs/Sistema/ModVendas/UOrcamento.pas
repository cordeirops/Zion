...
Begin
   	...
   	Exit;
  {
   ...
   begin
       ...
       //Edmar - 08/09/2014 - se o cliente for do tipo industria
       if xIndustria then
       begin//busca o valor correspondente sendo a prazo ou a vista
           If FrmFormPag1.EdDescricao.Text = 'À VISTA' then
               ...
               EDValUnit.Text := FormatCurr('0.00', DMEstoque.TEstoque.FieldByName('VENDINDP').AsCurrency);
       end
			else
       begin
           if xAtacadista then
...
Begin
   ...
   DMESTOQUE.TSlave.sql.Add(' subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS PRIMCOD, subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL2').AsString+' AS SEGCOD, subproduto.localestante ');//Jônatas 22/08/2013 - Incluido localestante
  ...
  inherited;
	Try
   	...
   	Then Begin//pressionado enter, consultar
           If (DMMACS.TEmpresa.FieldByName('MOBILE').AsString='1')
           Then Begin
               If (CbMostrarOrcamentosexportados.Checked=True)
               Then Begin
                   XView.Close;
                   XView.SQL.Clear;
                   XView.SQL.Add(' Select * from vworcamento ');
                   XView.SQL.Add(' Left join orcamento on vworcamento.cod_orcamento = orcamento.cod_orcamento ');
                   XView.SQL.Add(' where ((upper(orcamento.numorc) like '+#39+'%'+UpperCase(EdNumOrc.Text)+#39+')) and ((orcamento.exporta=0) or (orcamento.exporta is null)) ORDER BY orcamento.numorc');
                   XView.SQL.Text;
                   XView.Open;
               End
               Else Begin
                   ...
                   XView.SQL.Add(' Select * from vworcamento ');
                   XView.SQL.Add(' Left join orcamento on vworcamento.cod_orcamento = orcamento.cod_orcamento ');
                   XView.SQL.Add(' where ((upper(orcamento.numorc) like '+#39+'%'+UpperCase(EdNumOrc.Text)+#39+')) and ((orcamento.exporta=1)) ORDER BY orcamento.numorc');
                   XView.SQL.Text;
                   XView.Open;
               End;
           End
...
//  inherited;
	Try
		If Key=VK_RETURN
   	Then Begin//pressionado enter, consultar
           If (DMMACS.TEmpresa.FieldByName('MOBILE').AsString='1') and (CbMostrarOrcamentosexportados.Checked)
           Then Begin
           ...
           Else Begin
               ...
               XView.SQL.Add(' Select * from vworcamento ');
               XView.SQL.Add(' Left join orcamento on vworcamento.cod_orcamento = orcamento.cod_orcamento ');
               XView.SQL.Add(' where ((upper(orcamento.NOMECLI) like '+#39+'%'+UpperCase(EdNome.Text)+'%'+#39+')) and (orcamento.exporta=0) ORDER BY orcamento.NOMECLI, orcamento.NUMORC ');
               XView.SQL.Text;
               XView.Open;
           End;
       End;
...
begin
   ...
   //Caso o sistema esteja modando em modo Mobile (sincronismo de dados entre servidor e maquinas fora do mesmo banco de dados)
   If (DMMACS.TEmpresa.FieldByName('MOBILE').AsString='1') and (DMSAIDA.TOrc.FieldByName('EXPORTA').AsString='0')
   Then Begin
       Mensagem('B L O Q U E A D O !', 'Este orçamento já foi sincronizado com o servidor e não pode mais ser consultado/alterado!', '', 1, 1, False, 'i');
       Exit;
   End;
...
//  inherited;
   ...
   //Jônatas 08/08/2013 - Verifica se o orçamento está sendo gerado no servidor ou no mobile
   try
       ...
       If(DMMACS.TSecundario.FieldByName('MOBILE').AsString <> '1') then //SE FOR <> 1 É SERVIDOR
         XTabela.FieldByName('EXPORTA').AsInteger:=0;
       If(DMMACS.TSecundario.FieldByName('MOBILE').AsString = '1') then //SE FOR = 1 É MOBILE
         XTabela.FieldByName('EXPORTA').AsInteger:=1;
   Except
...
begin
  inherited;
   ...
   Then Begin
       ...
       EXIT;
   End;
   //VERIFICA SE POSSUI ESTOQUE SUFICIENTE PARA PODER VENDER POSTERIORMENTE
   ...
   XCodPedido:=DMSAIDA.WOrc.fieldbyname('COD_ORCAMENTO').AsString;
    ...
    // - 06/02/2009:  se for setado em loja com conversao unitaria // incluir sql's
     ...
     If DMMACS.TLoja.FieldByName('MODELOORC').AsInteger=1 Then
         FSRel.LoadFromFile('C:\MZR\MACS\Rel\FrfOrcamento.frf');//Jônatas 22/08/2013 - Relatório alterado para Mercescan com o local da estante
...
procedure TFOrcamento.BtnSincronismoClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrcamento.CbMostrarOrcamentosexportadosClick(Sender: TObject);
begin
  inherited;
   If CbMostrarOrcamentosexportados.Checked=true
   Then Begin
       ...
       XView.SQL.Add(' Select * from vworcamento ');
       XView.SQL.Add(' Left join orcamento on vworcamento.cod_orcamento = orcamento.cod_orcamento ');
       XView.SQL.Add(' ORDER BY orcamento.NOMECLI ');
       XView.SQL.Text;
       XView.Open;
   End
   Else Begin
       ...
       XView.SQL.Add(' Select * from vworcamento ');
       XView.SQL.Add(' Left join orcamento on vworcamento.cod_orcamento = orcamento.cod_orcamento ');
       XView.SQL.Add(' where ((orcamento.exporta=0)  or (orcamento.exporta is null)) ORDER BY orcamento.NOMECLI ');
       XView.SQL.Text;
       XView.Open;
   End;
end;
