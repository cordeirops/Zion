...
//        Function CalculoIcmsSubstituicaoInsustriasSimples: Boolean;
		 //Função utilizada para verificar o tipo do cliente para calculo de subtituição
...
//        Function InicializaVarUF: Boolean;
        ...
        //Conjunto de funções para inserir os Itens e atualizar Estoque
        //Função responsável por realizar todas as validações antes da inserção do item
        ...
        //Edmar - 12/03/2015 - Filtra cliente que veio selecionado na devolução
...
Var
    ...
    XAtacadista, xIndustria: Boolean; //Armazena temporariamente se o cliente é tipo atacadista ou nao
...
Begin
    Inherited;
       ...
       Try
           //Recebemos a chave primaria do cliente para tornálo fornecedores
           ...
           DMMACS.TMP.Open;
           //Selecionamos os veículos de entrada
           ...
           begin
               Try
                   ...
                   DMMACS.TMP.FieldByName('VLR8').AsCurrency   := 0; //Base Substituição
       ...
       //Paramos o processo aqui pq o que esta abaixo do exit é processo antigo
       exit;
...
Begin
    ...
    Else
	 ...
	 //Edmar - 11/03/2015 - Busca se o cliente é contribuinte de icms
    DMMACS.TALX.Close;
...
Begin
    Try
        ...
        Else Begin
           ...
           else begin//Paulo - 20/07/2010 - Para tipo de operação outros
    ...
    If FiltraTabela(DMMacs.TLoja, 'LOJA', 'COD_LOJA', FMenu.LCODLOJA.Caption, '') = True
     ...
     //VERIFICA SE O PEDIDO JÁ POSSUE DOC FI
           ...
           //Assume parcelas em campo blob de loja para após ser passado em relatório
            ...
            //Edmar - 27/01/2015 - Busca o valor total da conta e o pago, apenas se o modelo de impressão for 1 ou 10
...
Begin
   ...
   //Zeramos as variáveis
    	...
    	//RECALCULA ICMS SUBSTITUIÇÃO TRIBUTÁRIA CONFORME CONFIGURAÇÕES
    	 ...
    	 Then Begin
           ...
           Begin
               ...
               DMSAIDA.TAlx.Open;
               //Edmar - 09/12/2014 - se o CST for de substituição tributária e o tipo da operação
               //não for "Simples Remessa", calcula a substituição do item.
    ...
    Else Begin
        ...
        EdValorIpi.ValueNumeric := DMESTOQUE.TSlave.FieldByName('VALORIPI').AsCurrency;
        //TOTAL ICMS SUBSTITUICAO
        EdValoIcmsSubstituicao.ValueNumeric := DMESTOQUE.TSlave.FieldByName('VALORICMSST').AsCurrency;{ TODO : Jônatas 03/10/2013 - Verificar questão de importação de xml, ou PC normal }
        XVLRTOTALICMSSUBSTITUICAO := EdValoIcmsSubstituicao.ValueNumeric;//Jônatas 03/10/2013 - essa linha foi comentada e a de cima foi inserida
...
Begin
 ...
 //*******************************************************************************************************
  ...
  //Verifica Cliente a inserir na tabela
    ...
    //Abilita controle de aliquotas para pedido de compra
       ...
       //XTabela.FieldByName('NUMPED').AsInteger:=XNumPedido;//Jônatas 31/07/2013 - Deslocado para adaptar a situação da importação de XML
       //Jônatas 30/07/2013 - Inserir condição para verificar se é importação de XML
       FiltraTabela(DMPESSOA.WFornecedor, 'VWFORNEC', '', '', ' ORDER BY COD_FORNEC'); //Jônatas 02/08/2013 - Liberar dados para a consulta
       if XImportNfe = 1 then
       begin
       	XTabela.FieldByName('NUMPED').AsInteger := XNumfiscNFe; //Jônatas 31/07/2013 - Ao inserir um registro que vem de xml, atribuir ao número do pedido o número fiscal
           XTabela.FieldByName('CHAVENFEXML').AsString := FMenu.XChaveNFe; //Jônatas 29/07/2013 - Ao inserir um novo registro, obrigatoriamente salvar a chave
           XTabela.FieldByName('VLRFRETE').AsCurrency := XFreteNfe; //Jônatas 29/07/2013 - Idem a chave, se quiser manter o valor posteriormente
           XTabela.FieldByName('VLRSEGURO').AsCurrency := XSeguroNfe; //Jônatas 30/07/2013 - Idem
           XTabela.FieldByName('NUMFISCAL').AsString := IntToStr(XNumfiscNFe); //Jônatas 30/0702013 - Idem
           XTabela.FieldByName('COD_FORNEC').AsInteger := XcodFornecedor; //Jônatas 31/07/2013 - Alterado pois estava pegando um valor fixo(o primeiro da tabela)
       end
       else //novo regstro de forma normal
       begin
       	XTabela.FieldByName('COD_FORNEC').AsInteger := DMPESSOA.WFornecedor.FieldByName('COD_FORNEC').AsInteger; //Jônatas 31/07/2013 - Caso não seja importação de XML pega um fornecedor padrão
           XCodPessoa := DMPESSOA.WFornecedor.FieldByName('COD_FORNEC').AsInteger; //Jônatas 02/08/2013 - Teste para atualizar o valor a ser gravado no botão gravar
           XTabela.FieldByName('NUMPED').AsInteger := XNumPedido; //Jônatas 31/07/2013 - Caso seja apenas um novo registro, atribui o valor padrão conforme regras da loja
       end;        
 ...
 //************************************************
   ...
   Registra(XSQLTABELA, 'ABRE ' + XSQLTABELA, DateToStr(Date()), 'Nº: ' + IntToStr(XNumPedido) + '; Cód. pedido: ' + XCodPedido, '');
End;
 ...
 //se realmente estiver trabalhando com vendas e não com compras
    ...
    Then Begin
        ...
        Then Begin
            ...
            try
                //Edmar - 06/08/2014 - Salva o cliente assim que ele é selecionado	
                XTabela.Edit;
                XTabela.FieldByName('COD_CLIENTE').AsInteger := XCodPessoa;
                XTabela.Post;
            ...
            End;
	 		 If DMMACS.TEmpresa.FieldByName('INDUSTRIA').AsString = '1' Then
					FiltraSlave;
            	//VerificaTipoCliente;//Se a empresa usuaria for do tipo cinsustria, deveremos efetuar calculo de substituição e para isso precisamso saber o tipo do cliente final
        ...
        If AbrirForm(TFFornecedor, FFornecedor, 1) = 'Selected'
            ...
            If FiltraTabela(DMPESSOA.TFornecedor, 'FORNECEDOR', 'COD_FORNEC', DMPESSOA.WFornecedor.FieldByName('COD_FORNEC').AsString, '') = True
               ...
               //VERFICA QTO A CREDITO DE CLIENTES
                ...
                begin
                    //Edmar - 17/08/2015 - verifica se o fornecedor é estrangeiro
...
Begin
 ...
 //EFETUA CONTROLE DE ACESSO A ALTERAÇÃO DO VLR UNITARIO
    ...
    //Verifica se deve ser mostrado os filtros fechados ou não
...
Begin
	Try
        ...
        Then Begin
            ...
            Then Begin
            	 Result:=False;
                Mensagem('OPÇÃO BLOQUEADA', 'O produto não pode ser inserido a lista com o valor unitario igual ou inferior a 0.', '', 1, 1, False, 'a');
                EDValUnit.SetFocus;
                Exit;
            End;
            ...
            Then Begin
            	 Result:=False;
                Mensagem('OPÇÃO BLOQUEADA', 'O produto não pode ser inserido a lista com o valor total igual ou inferior a 0.', '', 1, 1, False, 'a');
                EDTotal.SetFocus;
                Exit;
            End;
        End;
        ...
        If DMMACS.TLoja.FieldByName('ATIVIDADE').AsString <> 'SEMENTES'
        Then Begin
            //Jônatas 04/10/2013 - Condição para importação de XML, não mostrar mensagem ao importar XML
            If XImportNfe = 0 //0 = PC feito "na mão", 1 = Importação XML
            Then Begin
                DMESTOQUE.TSlave.Open;
        ...
        Then Begin
            ...
            Then Begin
               ...
               Else Begin
                   Result:=False;
                  	Mensagem('   A T E N Ç Ã O   ', 'Reinicie o Sistema e tente novamente!', '', 1, 1, false, 'A');
               End;
    ...
    Try
       ...
       //**************************************************
        ...
        Else Begin
            Try
                ...
                //se for uma importação de XML, realiza o rateio do valor total do custo dos itens
                if XImportNfe = 1 then
                begin
                   ...
                   MDO.QAlx1.Open;
                   //busca a diferença entre o valor total dos itens e total do pedido
                   ...
                   begin
                       //rateia a diferença para os itens recuperando o custo da mercadoria
                       ...
                       if xCustoNfe_ValorFinal = 0 then//quando não existir diferença salva o proprio valor total
       ...
       //*********************************************
        Try
            ...
            //Edmar - 20/02/2014 - linha comentada para corrigir o erro do pedido de compra que não salvava
        	XTransaction.CommitRetaining;
           MDO.Transac.CommitRetaining;
        Except
    ...
    Then Begin //caso a tabela esteja vasia não pode apagar
     ...
     //Atualiza Estoque
       ...
       //prepara estoque fisico
        ...
        //Alex: 07/03/2019 - Desativar o lançamento de custo do item que foi criado no momento da inserção do item
        If XSQLTABELA = 'PEDCOMPRA' Then
...
Begin
    ...
    If Key=vk_f8 Then
    begin
       Mensagem('   A T E N Ç Ã O   ', 'Essa opção foi desabilitada. Para excluir-lo consulte o pedido, exclua todos os itens e clique em cancelar', '', 1, 1, false, 'I');
    	exit;
    end;
    Inherited;
...
Begin
   // variaveis que armazenam o valor do pedido de venda e compra
    ...
    If ((XView.FieldByName('SITUACAO').AsString <> 'FECHADO') And (XView.FieldByName('SITUACAO').AsString <> 'EXPEDIÇÃO') And (XView.FieldByName('SITUACAO').AsString <> 'CANCELADO') And (XView.FieldByName('SITUACAO').AsString <> 'DEVOLVIDO')) Or (XView.FieldByName('FORMPAG').AsString = 'CADERNETA')
        Then Begin
   ...
   //informa fornecedores
            ...
            //Filtra a pessoa para buscar se ela é estrangeira
      ...
      //Informa outros valores
        ...
        Except
            ...
            XNumPedido := 0;
        End;
      //Jônatas 29/07/2013 - Adiciona aqui o local para leitura da chaveda NFe a partir do banco de dados,
      //caso haja valores, caso contrário, fica como ''(null)
	     If XSQLTABELA <> 'PEDVENDA'
        Then Begin
            Try
...
{	 	 If DMMACS.TEmpresa.FieldByName('INDUSTRIA').AsString = '1' Then
        	VerificaTipoCliente(XCodPessoa);//Se a empresa usuaria for do tipo cinsustria, deveremos efetuar calculo de substituição e para isso precisamso saber o tipo do cliente final}
        FiltraSlave; 
    ...
    If (XView.FieldByName('SITUACAO').AsString = 'ABERTO') or (XView.FieldByName('SITUACAO').AsString = 'FATURAMENT')
        ...
        //FILTRA LOJA PARA VERIFICAR CONFIGURAÇÕES
           ...
           // configuração em loja que jah deixa desativado a opção para gerar financeiro
            ...
            //Edmar - 19/03/2015 - Se for qualquer operação diferente de pedido de venda ou pedido de compra
            //deixa o não gerar financeiro marcado automaticamente
        ...
        End
         	...
         	XImportNfe := 0;//Edmar - 11/04/2014 - Não o pedido, finaliza a importação

    End

    Else if XView.FieldByName('SITUACAO').AsString = 'EXPEDICAO' then
        Mensagem('OPÇÃO BLOQUEADA', 'Este pedido encontra-se em Expedição e não pode ser fechado!', '', 1, 1, False, 'i')
...
Begin
   ...
   //Bloqueia opção a vista para determinadas formas de pagamento
    If ((CBPagamento.Text <> 'Carteira') And (CBPagamento.Text <> 'Banco') And (CBPagamento.Text <> 'Chq. Terc.') And (CBPagamento.Text <> 'Crédito') and ((CBPagamento.Text = 'Cartão') and (DMMACS.TLoja.FieldByName('MOVCAIXACARTAO').AsString<>'1'))) And (XView.FieldByName('FORMPAG').AsString = 'À VISTA')
        Then Begin
        Mensagem('OPÇÃO BLOQUEADA', 'A forma de pagamento ' + CBPagamento.Text + ' não permite fechar o pedido como À Vista!', '', 1, 1, False, 'i');
        CBPagamento.SetFocus;
        Exit;
    End;
   ...
   //If (fmenu.XSimplesRemessa=True) OR (fmenu.XRemessaGarantia=True) or (FMenu.XOutrosEnt=true) or (FMenu.XComplementacao=true)Then
    ...
    Then Begin
    //VERIFICA HÁ NECESSIDADE DE IMPRIMIR E IMPRIME CUPOM FISCAL
        If XSQLTABELA = 'PEDVENDA'
        Then Begin
            FiltraTabela(DMCAIXA.TCaixa, 'CAIXA', 'COD_CAIXA', FMenu.LCODCAIXA.Caption, '');
            If DMCAIXA.TCaixa.FieldByName('TIPOCUPOM').AsString = 'ECF'
        ...
        //Vamos verificar se a compra deve emitir uma nfe de emissão propria de entrada
            ...
            try
                ...
                begin
                   //Edmar - 02/04/2014 - Código válido para contas a pagar
                   ...
                   //
                   //
                   //Edmar - 03/04/2014 - Plano de contas padrão dependendo se é a vista ou a prazo
        ...
        End;
        XImportNfe := 0;//Edmar - 11/04/2014 - Seta que finalizou o processo de importação
        MDO.Transac.CommitRetaining;
    End;
...
Begin
   ...
   //PASSA VALORES ATUAIS
	 Label8.Caption:='A L Í Q U O T A S - '+DMESTOQUE.TSubProd.FieldByName('DESCRICAO').AsString;
...
Begin
   ...
   //19/06/2009: calcula PMC se foi informado um valor para a MVA
    ...
    //Usaremos os comandos abaixo para registrar a atualização de preços em históricos
...
Begin
   ...
   //controla visualização de componentes referentes a escolha e visualização da cta corrente
    If (CBPagamento.Text = 'Banco') Or ((CBPagamento.Text = 'Cheque') And (XSQLTABELA = 'PEDCOMPRA')) Or ((CBPagamento.Text = 'Cartão') and (XSQLTABELA <> 'PEDCOMPRA') and (DMMACS.TLoja.FieldByName('MOVCAIXACARTAO').AsString<>'1'))
        Then Begin
...
//           FFiscal.MAdicionais.Lines.Text:=DMENTRADA.TFiscPC.FieldByName('OBS').AsString;
        ...
        Else Begin
           ...
           //Jônatas 26/07/2013 - Iserir a chave da NFe no banco
           //DMENTRADA.TFiscPC.FieldByName('NFECHAVE').AsString:=FMenu.XChaveNFe;//Desnecessário aqui
...
//Edmar - 28/01/2015 - Procedimento auxiliar para mostrar o comprovante de número 10.
//Nesse comprovante tem algumas validações a mais que devem ser feita, para evitar retrabalho
...
begin
   ...
   begin
   	...
   	//Se não tiver despesas no pedido, abre comprovante personalizado
...
//Edmar - 17/08/2015 - Verifica se o cliente é estrangeiro e mostra ou não
