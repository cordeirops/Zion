...
type
  TFRelEstoque = class(TFPadrao)
    ...
    GBProdutoInt: TGroupBox;
    Label1: TLabel;
    Label4: TLabel;
    RbTodas: TRadioButton;
    RbExcluir: TRadioButton;
    CbEstoquep: TCheckBox;
  ...
  private
   ...
   Procedure ImpCarregamento;
    ...
    //Filtro por Subproduto para manutenção
    Procedure FiltroSubProdutoManutencao;
...
Begin
   ...
   begin
      ...
      DMESTOQUE.TProduto.Next;
      		...
      		ShowMessage('Cód.: '+DMESTOQUE.TProduto.FieldByName('COD_INTERNO').AsString+' - Descrição: '+DMESTOQUE.TProduto.FieldByName('DESCRICAO').AsString);
      end;
   end;
   DMESTOQUE.TransacEstoque.CommitRetaining;
...
Begin
   LimpaSQL;
   DMESTOQUE.TRel.SQL.Add('  Select ordem.numero As numped, PRODUTO.descricao AS PRODUTO, ordem.comissao as vlrcomis, SUBPRODUTO.CONTRINT, ordem.dtfech as dtpedven , subproduto.codcomposto, itprodord.qtd as qtdeprod, subproduto.descricao as NOMEPRODUTO, ');
...
Begin
   LimpaSQL;
   DMESTOQUE.TRel.SQL.Add(' Select pedvenda.numped, pedvenda.TIPO, PRODUTO.descricao AS PRODUTO, pedvenda.vlrcomis, SUBPRODUTO.CONTRINT, pedvenda.dtpedven , subproduto.codcomposto, itenspedven.qtdeprod, subproduto.descricao as NOMEPRODUTO, ');
   DMESTOQUE.TRel.sql.Add(' subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS PRIMCOD, subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL2').AsString+' AS SEGCOD, ');
...
Begin
   ...
   Try
   	...
   	DMESTOQUE.TRel.SQL.Add(' left join subgrupoprod on subgrupoprod.cod_subgrupoprod = produto.cod_subgrupoprod ');
   	DMESTOQUE.TRel.SQL.Add(' WHERE (ESTOQUE.COD_LOJA='+FMenu.LCODLOJA.Caption+')  AND (PEDVENDA.SITUACAO<>'+#39+'CANCELADO'+#39+')');
    if CbEstoquep.Checked=True Then
    DMESTOQUE.TRel.SQL.Add(' AND (subproduto.est_permanente=1) ')
   else
    DMESTOQUE.TRel.SQL.Add(' AND (subproduto.est_permanente is null or subproduto.est_permanente = 0)');
   	FiltroGrupo;
   	FiltroSubGrupo;
   	FiltroMarca;
		FiltroCampAux;
...
Begin
   ...
   DMESTOQUE.TRel.SQL.Add(' left join grupoprod on grupoprod.cod_grupoprod = produto.cod_grupoprod ');
   DMESTOQUE.TRel.SQL.Add(' left join subgrupoprod on subgrupoprod.cod_subgrupoprod = produto.cod_subgrupoprod ');
   DMESTOQUE.TRel.SQL.Add(' WHERE (ESTOQUE.COD_LOJA='+FMenu.LCODLOJA.Caption+') AND (SUBPRODUTO.ATIVO <> ''N'') ');
   if CbEstoquep.Checked=True Then
    DMESTOQUE.TRel.SQL.Add(' AND (subproduto.est_permanente=1) ')
   else
    DMESTOQUE.TRel.SQL.Add(' AND (subproduto.est_permanente is null or subproduto.est_permanente = 0)');
   ...
   Then Begin
	...
	End;
   LimpaSQL;
   DMESTOQUE.TRel.SQL.Add(' select subproduto.cod_interno AS CI_SUBPRODUTO, SUBPRODUTO.CodComposto, SUBPRODUTO.CONTRINT, SUBPRODUTO.DESCRICAO AS NOMEPRODUTO, subproduto.codprodfabr AS REFERENCIA, subproduto.marca AS MARCA, ');
   DMESTOQUE.TRel.sql.Add(' subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS PRIMCOD, subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL2').AsString+' AS SEGCOD, subproduto.localestante AS LOCAL, ');
   ...
   DMESTOQUE.TRel.SQL.Add(' left join grupoprod on grupoprod.cod_grupoprod = produto.cod_grupoprod ');
   DMESTOQUE.TRel.SQL.Add(' left join subgrupoprod on subgrupoprod.cod_subgrupoprod = produto.cod_subgrupoprod ');
   DMESTOQUE.TRel.SQL.Add(' WHERE (ESTOQUE.COD_LOJA='+FMenu.LCODLOJA.Caption+') and (estoque.estfisico<>estoque.estcalc) and (subproduto.ativo<>''N'')');
   FiltroGrupo;
   FiltroSubGrupo;
   FiltroMarca;
	FiltroCampAux;
...
Begin
   ...
   DMESTOQUE.TRel.sql.Add(' subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS PRIMCOD, subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL2').AsString+' AS SEGCOD, ');
   DMESTOQUE.TRel.SQL.Add('  subproduto.descricao as NOMEPRODUTO, subproduto.marca, itenspedc.descpro, itenspedc.embprod, itenspedc.frete, itenspedc.ALIQICMS AS ICMS, ');
...
Begin
	LimpaSQL;
   DMESTOQUE.TRel.SQL.Add(' select subproduto.cod_interno AS CI_SUBPRODUTO, SUBPRODUTO.CodComposto, SUBPRODUTO.CONTRINT, SUBPRODUTO.DESCRICAO AS SUBPRODUTO, subproduto.codprodfabr AS REFERENCIA, subproduto.marca AS MARCA, ');
   DMESTOQUE.TRel.sql.Add(' subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS PRIMCOD, subproduto.est_permanente,subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL2').AsString+' AS SEGCOD, subproduto.localestante AS LOCAL, unidade.sigla_unid,');
   DMESTOQUE.TRel.SQL.Add(' estoque.estfisico, estoque.ULTCOMPRA AS DATA, ESTOQUE.estreserv, estoque.estped, estoque.estsaldo, estoque.vendvarv, estoque.vendvarp, estoque.vlrunitcomp as valunit, estoque.valrep, estoque.lucratividade, estoque.cod_loja, ');
   DMESTOQUE.TRel.SQL.Add(' produto.cod_interno AS CI_PRODUTO, produto.descricao AS PRODUTO, grupoprod.cod_interno AS CI_GRUPO, grupoprod.descricao AS GRUPO, subgrupoprod.cod_interno AS CI_SUBGRUPO, subgrupoprod.descricao AS SUBGRUPO ');
   DMESTOQUE.TRel.SQL.Add(' from subproduto left join estoque ON subproduto.cod_subproduto = estoque.cod_subprod ');
   ...
   Then Begin
      ...
      If (FMenu.TIPOREL='POSEST2') Then
	        FSRel.LoadFromFile('C:\MZR\MACS\Rel\FrfPosEstoque_Class2.frf');
...
Begin
	If (FrmSubProduto1.EdDescricao.Text<>'') And (FrmSubProduto2.EdDescricao.Text<>'')
   Then Begin//haverá seleção por subproduto
...
Begin
   ...
   DMESTOQUE.Alx3.SQL.Add(' itenspedc.qtdeprod, subproduto.descricao as NOMEPRODUTO, itenspedc.VALUNIT, ');
   ...
   DMESTOQUE.Alx2.SQL.Add(' itenspedven.qtdeprod, subproduto.descricao as NOMEPRODUTO, Itenspedven.OPERACAO,');
...
Begin
   ...
   End;
	If (FMenu.TIPOREL='POSEST') or (FMenu.TIPOREL='LISTACONTAGEM') or (FMenu.TIPOREL='POSEST2')
   Then Begin
		If (FMenu.TIPOREL='POSEST2') Then
			DMMACS.TLoja.FieldByName('TMP2').AsString:='Estoque/Relatórios/Estoque/Lista de Produtos Classificados'
		else
...
begin
  inherited;
	GBProdutoInt.Visible := False;
     
	If FMenu.TIPOREL='POSEST' Then
   begin
       Caption:='Relatório de Posição do Estoque';
		GBProdutoInt.Visible := True;
       RbTodas.Checked := True;
       RbExcluir.Checked := False;
   end;
	If FMenu.TIPOREL='LANCENT' Then
       Caption:='Relatório de Lançamentos de Entrada';
	If FMenu.TIPOREL='LANCSAI' Then
       Caption:='Relatório de Lançamentos de Saída';
	If FMenu.TIPOREL='LISTVEND'
   Then Begin
       Caption:='Relatório de Lista de Preços para vendas';
   End;
	If FMenu.TIPOREL='ENTXSAI' Then
       Caption:='Relatório de Confronto Entradas e Saidas';
	If FMenu.TIPOREL='CURVAABC' Then
       Caption:='Relatório de Curva ABC de Produtos';
   If FMenu.TIPOREL='LISTEST' Then
      Caption:='Listagem de Estoque';
   //MAURO 03/06/2013 - Relatorio de produtos manutenção
   if FMenu.TIPOREL = 'FRFPRODMANUTENCAO' then
   	Caption:='Relatório de Produtos com Manutenção';

   //****************************
   if FMenu.TIPOREL='EXPXML' then
   	Caption:='Relatório de Exportação XML';
   ...
   //INTITULA RALATORIO DE EXPORTACAO PARA XML
	If FMenu.TIPOREL='EXPXML'
   Then Begin
   	Caption:='Exportar Itens de venda para XML';
       BtnImprimir.Caption:='&Exportar';
   End;
   //INTITULA RALATORIO DE PRODUTOS COMPRADOS
	If FMenu.TIPOREL='FRFPRODCOMPRA_CLASS_PROD'
   Then Begin
   	Caption:='Relatório de produtos comprados';
		CBEstPos.Visible:=False;
		CBEstZerado.Visible:=False;
   End;
   //INTITULA RALATORIO DE PRODUTOS DEVOLVIDOS
	If FMenu.TIPOREL='FRFPRODCOMPRA_DEV_CLASS_PROD'
   Then Begin
   	Caption:='Relatório de produtos devolvidos';
		CBEstPos.Visible:=False;
		CBEstZerado.Visible:=False;
   End;
   //INTITULA RALATORIO DE ESTOQUE CALCULADO
	If FMenu.TIPOREL='ESTCALC'
   Then Begin
   	Caption:='Relatório de Estoque Calculado';
		CBEstPos.Visible:=False;
		CBEstZerado.Visible:=False;
   End;
   //INTITULA RALATORIO DE ESTOQUE COM ESTOQUE MINIMO E MAXIMO
	If FMenu.TIPOREL='ESTMAXMIN'
   Then Begin
   	Caption:='Relatório de Estoque com Quant. Máx/Mín.';
   ...
   If FMenu.TIPOREL='FRFPRODVENDA_CLASS_PROD'
   Then Begin
   	Caption:='Relatório de produtos vendidos';
		CBEstPos.Visible:=False;
		CBEstZerado.Visible:=False;
   End;
	If  FMenu.TIPOREL='FRRESUMOVC'
   Then Begin
		Caption:='Relatório Resumo de Compras e Vendas';
   ...
   End;
	...
	If  (FMenu.TIPOREL='LSTESTPEDCLIDESCCUPOM')
   Then Begin
		Caption:='Lista Com Descrição de Cupom';
       BtnImprimir.Caption:='&Salvar';
   End;
   ...
   Then begin
		Caption:='Relatório de Entradas para Correção';
       PData.Visible:=True;
   end;

	If  (FMenu.TIPOREL='LANCSAILOTE') or (FMenu.TIPOREL='LANCSAI')
   Then begin
		Caption:='Relatório de Saídas para Correção';
       PData.Visible:=True;
   end;
...
begin
  inherited;
	If fmenu.TIPOREL='FRFPRODVENDA_TotalProd' Then
       Caption:='Relatório de Carregamento'  
   Else
       Caption:='Relatório de Estoque';
   If FMenu.TIPOREL='RELTRIBUTA' Then
      Caption:='Lista de Produtos por Tributação';
   If FMenu.TIPOREL='LISTEST' Then
      Caption:='Listagem de Estoque';
...
procedure TFRelEstoque.FiltroSubProdutoManutencao;
begin
	If (FrmSubProduto1.EdDescricao.Text<>'') And (FrmSubProduto2.EdDescricao.Text<>'')
   Then Begin//haverá seleção por subproduto                                                                                                      
   	DMESTOQUE.TRel.SQL.Add(' AND (upper(pedcompra.descricao_prodmanutencao) between upper(:SUBPROD1) AND upper(:SUBPROD2)) ');
       DMESTOQUE.TRel.ParamByName('SUBPROD1').AsString:=FrmSubProduto1.EdDescricao.Text;
       DMESTOQUE.TRel.ParamByName('SUBPROD2').AsString:=FrmSubProduto2.EdDescricao.Text;
   End;
end;
