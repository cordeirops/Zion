...
type
  TFCadCFOP = class(TFCadPadraoDesc)
    ...
    cbNaoGeraSt: TCheckBox;
    cbNaoGeraIcmsOp: TCheckBox;
...
begin
  inherited;
    ...
    XLiberaDados:='';
    LiberaDados;
    DBGridCadastroPadrao.DataSource := DMESTOQUE.DSCFOP;
end;
...
begin
  inherited;
    ...
    cbNCI.Checked := False;
    cbNaoGeraSt.Checked := False;
    cbNaoGeraIcmsOp.Checked := False;
end;
...
begin
    ...
    Then Begin
       Mensagem('INFORMAÇÃO AO USUÁRIO', 'A CFOP que informado já se encontra cadastrado. Terá que informar novo valor!', '', 1, 1, False, 'i');
       DBCFOP.SetFocus;
       Exit;
    End;
    ...
    //Edmar - 24/08/2015 - Verifica se o CFOP vai gerar ST
    if cbNaoGeraSt.Checked then
    begin
       XTabela.Edit;
       XTabela.FieldByName('NAOGERAST').AsString := '1';
       XTabela.Post;
    end
    else begin
       XTabela.Edit;
       XTabela.FieldByName('NAOGERAST').AsString := '0';
       XTabela.Post;
    end;
    
    //Edmar - 24/08/2015 - Verifica se o CFOP vai gerar icms de operação propria    
    if cbNaoGeraIcmsOp.Checked then
    begin
       XTabela.Edit;
       XTabela.FieldByName('NAOGERAICMSOP').AsString := '1';
       XTabela.Post;
    end
    else begin
       XTabela.Edit;
       XTabela.FieldByName('NAOGERAICMSOP').AsString := '0';
       XTabela.Post;
    end;    
...
{   SelectRegistro('PLNCTA', 'COD_PLNCTA', DMESTOQUE.TCFOP.FieldByName('COD_PLNCTAFIL').AsString);
   ...
   // - 03/04/2009: marca se empresa nao calcula imposto
    ...
    if XTabela.FieldByName('NAOGERAST').AsString = '1' then
       cbNaoGeraSt.Checked := True
    else
       cbNaoGeraSt.Checked := False;

    if XTabela.FieldByName('NAOGERAICMSOP').AsString = '1' then
       cbNaoGeraIcmsOp.Checked := True
    else
       cbNaoGeraIcmsOp.Checked := False;
end;
