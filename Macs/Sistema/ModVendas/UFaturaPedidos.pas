...
uses
  ...
  FR_Class, ColorMaskEdit, frxClass, frxDBSet, UCredDev, Menus, EditFloat;
...
type
  TFFaturaPedidos = class(TFPadraoRodaPe)
    ...
    frxFatura: TfrxReport;
    frxDBFaturados: TfrxDBDataset;
    frxDBPedFat: TfrxDBDataset;
    Btncreditos: TBitBtn;
    Label16: TLabel;
    LVlrCredDev: TLabel;
    PRelDev: TPopupMenu;
    RelatriodeDevoluesFaturadas1: TMenuItem;
    frxDBDev: TfrxDBDataset;
    frxDBDev1: TfrxDBDataset;
    Label17: TLabel;
    edDesconto: TFloatEdit;
    ...
    procedure RelatriodeDevoluesFaturadas1Click(Sender: TObject);
	...
	Try
       ...
       End;
     	...
     	then begin
         	...
         	Then Begin
           	Result := False;
           	//Edmar - 07/08/2014 - Se a conta foi gravada corretamente, fecha os pedidos
           	if (FMenu.LancAutConta) or (FMenu.LancChSai) or (FMenu.LancCaixa) then
               begin
               	MDO.Query.Close;
               	MDO.Query.SQL.Clear;
...
Begin
   ...
   End;
    ...
    Then Begin
		...
		//Edmar - 22/07/2014 - Comentado pois o código abaixo não fazia a tratativa de selecionar todos os cp
       //e também foi deixado apenas uma rotina pois ela servia para os dois casos (tanto marcar quanto desmarcar)
{    	If tipo=true
       Then Begin
        DMESTOQUE.Alx1.First;
        While not DMESTOQUE.Alx1.Eof do
...
Begin
   ...
   Then Begin
   	...
   	Then Begin
       	...
       	LValor.Caption:='Total R$: '+FormatFloat('0.00', DMESTOQUE.Alx2.FieldByName('Valor').AsCurrency-edDesconto.ValueNumeric);
   	End;
		//Passa o valor p/ label
   	DMESTOQUE.Alx2.Close;
   	DMESTOQUE.Alx2.SQL.Clear;
   ...
   Else Begin
       ...
       Then Begin
           ...
           Then Begin
               XTOTAL:= DMESTOQUE.Alx2.FieldByName('Valor').AsCurrency;
               LValor.Caption:='Total R$: '+FormatFloat('0.00', DMESTOQUE.Alx2.FieldByName('Valor').AsCurrency-edDesconto.ValueNumeric);
               FMenu.XValorFatura:=StrToFloat(FormatFloat('0.00', DMESTOQUE.Alx2.FieldByName('Valor').AsCurrency-edDesconto.ValueNumeric));
           End;
...
begin
  inherited;
   ...
   If (DMESTOQUE.Alx2.FieldByName('COD_FORNEC').AsString = '') then
   begin
    LVlrCredDev.Caption := '0,00';

    MDO.QAlx2.Close;
    MDO.QAlx2.SQL.Clear;
    MDO.QAlx2.SQL.Add(' UPDATE PEDVENDA SET PEDVENDA.MARK = '''' WHERE PEDVENDA.MARK = ''X''');
    MDO.QAlx2.ExecSQL;
...
begin
  inherited;
   ...
   XTOTAL := (XTOTAL - StrToCurr(LVlrCredDev.Caption)-edDesconto.ValueNumeric);
   ...
   Then Begin
       ...
       EdNumParc.SetFocus;
       XTOTAL := DMESTOQUE.Alx2.FieldByName('Valor').AsCurrency-edDesconto.ValueNumeric;
       Exit;
   End;
   ...
   Then Begin
       Mensagem('   A T E N Ç Ã O   ','CAIXA FECHADO. ESSA OPERAÇÃO SÓ PODERÁ SER FEITA COM O CAIXA ABERTO.','',1,1,false,'a');
       Exit;
   End;
   ...
   try
       ...
       Then Begin
           ...
           Begin
               ...
               Then Begin
                   ...
                   MDO.QAlx1.Close;
                   MDO.QAlx1.SQL.Clear;
                   MDO.QAlx1.SQL.Add(' UPDATE PEDVENDA SET PEDVENDA.COD_FATURA ='+IntToStr(XCodFatura)+' WHERE PEDVENDA.MARK = ''X''');
                   MDO.QAlx1.ExecSQL;
           ...
           Then Begin
               ...
               Except
                   DMSAIDA.IBT.RollbackRetaining;
                   MessageDlg('[1390] - Falha na fatura!'+#13+#10+'Os dados dos pedidos de venda não serão retornados.', mtWarning, [mbOK], 0);
               End;
       ...
       End
       Else Begin
           If XTIPOAUX='FATORDEM' then
               Mensagem('ATENÇÃO', 'Os faturamentos de ordem foram cancelados!', '', 1, 1, False, 'a')
           else
               Mensagem('ATENÇÃO', 'Os faturamentos de pedidos foram cancelados!', '', 1, 1, False, 'a');

           MDO.Transac.RollbackRetaining;
       End;
   except
	    MDO.Transac.RollbackRetaining;
		MessageDlg('[1587] - Falha na fatura!'+#13+#10+'Os pedidos não foram devidamente faturados. Os pedidos serão marcados como Abertos, porém verifique o financeiro.', mtWarning, [mbOK], 0);
   end;
   LVlrCredito.Caption:='';
   LVlrCredDev.Caption:='';
...
Begin
   ...
   Try
       ...
       {MDO.QAlx2.Close;
       MDO.QAlx2.SQL.Clear;
       MDO.QAlx2.SQL.Add(' SELECT PEDVENDA.COD_PEDVENDA FROM PEDVENDA WHERE PEDVENDA.COD_FATURA ='+CodFatura);
       MDO.QAlx2.Open;
       MDO.QAlx2.First;
       ...
       begin
        ...
        MDO.QConsulta.SQL.Add(' UPDATE PEDVENDA SET PEDVENDA.COD_FATURA='''' WHERE PEDVENDA.COD_PEDVENDA='+MDO.QAlx2.FieldByName('COD_PEDVENDA').AsString);
...
procedure TFFaturaPedidos.BtnRelatorioClick(Sender: TObject);
begin
  inherited;
  DMMACS.TALX.Close;
  DMMACS.TALX.SQL.Clear;
  DMMACS.TALX.SQL.Add(' SELECT FATURA.NUMDOC, FATURA.DATA, FATURA.VALOR, FATURA.NOMECLI FROM FATURA');
  DMMACS.TALX.SQL.Add(' WHERE (FATURA.CANCELADO =''N'') AND (FATURA.COD_FATURA = :COD)');
  DMMACS.TALX.ParamByName('COD').AsInteger := DMENTRADA.TFatura.FieldByName('COD_FATURA').AsInteger;
  DMMACS.TALX.Open;
  ...
  frxDBPedFat.DataSource := DMMACS.DSALX;
  frxDBFaturados.DataSource := DMENTRADA.DAlx;
  frxFatura.LoadFromFile('C:\MZR\MACS\Rel\FrfRelFaturados.fr3');
  frxFatura.ShowReport();
end;

procedure TFFaturaPedidos.BtncreditosClick(Sender: TObject);
begin
  inherited;
   if DMESTOQUE.Alx2.FieldByName('COD_FORNEC').AsString = '' then
   begin
     MessageDlg('Antes de selecionar os créditos selecione uma conta para fatura.', mtWarning, [mbOK], 0);
     Exit;
   end;

   FMenu.XCodCliente := DMESTOQUE.Alx2.FieldByName('COD_FORNEC').AsString;
   AbrirForm(TFCredDev, FCredDev, 0);

   MDO.Query.Close;
   MDO.Query.SQL.Clear;
   MDO.Query.SQL.Add(' SELECT SUM(PEDVENDA.VALOR) AS VLRCRED FROM PEDVENDA WHERE PEDVENDA.MARK = ''X''');
   MDO.Query.Open;

   LVlrCredDev.Caption := FormatFloat('0.00', MDO.Query.FieldByName('VLRCRED').AsCurrency);
   LVlrCredDev.Refresh;
   LVlrCredDev.Repaint;
end;

procedure TFFaturaPedidos.RelatriodeDevoluesFaturadas1Click(
  Sender: TObject);
begin
  inherited;
  MDO.QConsulta.Close;
  MDO.QConsulta.SQL.Clear;
  MDO.QConsulta.SQL.Add(' SELECT FATURA.DATA, FATURA.NUMDOC, FATURA.NOMECLI, FATURA.VALOR');
  MDO.QConsulta.SQL.Add(' FROM FATURA');
  MDO.QConsulta.SQL.Add(' WHERE FATURA.COD_FATURA='+QuotedStr(DMENTRADA.TFatura.FieldByName('COD_FATURA').AsString));
  MDO.QConsulta.Open;

  MDO.Query.Close;
  MDO.Query.SQL.Clear;
  MDO.Query.SQL.Add(' SELECT PEDVENDA.NOMECLI AS NOME, PEDVENDA.DTPEDVEN, PEDVENDA.COD_PEDVENDA, PEDVENDA.NUMPED, PEDVENDA.VALOR AS VLRDEV');
  MDO.Query.SQL.Add(' FROM PEDVENDA');
  MDO.Query.SQL.Add(' WHERE PEDVENDA.COD_FATURA ='+QuotedStr(DMENTRADA.TFatura.FieldByName('COD_FATURA').AsString));
  MDO.Query.Open;

  frxDBDev.DataSet := MDO.Query;
  frxDBDev1.DataSet := MDO.QConsulta;
  frxFatura.LoadFromFile('C:\MZR\MACS\Rel\FrfRelDevol.fr3');
  frxFatura.ShowReport();

end;

procedure TFFaturaPedidos.edDescontoExit(Sender: TObject);
begin
  inherited;
	FiltraPedido('ONUMPARC');
end;
