...
type
  TFCliente = class(TFCadPadraoPes)
    ...
    procedure EdBuscaBairroKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  private
    ...
    Procedure LocalizaUltCli;// localiza o ultimo cliente cadastrado e insere seu codigo interno na Hint
    procedure contadorCli;// - 27/03/2009: conta quantidade de clientes exibidos na grid

  public
    { Public declarations }
    Procedure FiltraServot;//procedure utilizada para filtrar produtos e servot referente ao cliente
	 Procedure GeraHist; //Procedure responsavel por montar um historico do cliente
   ...
   // Imprimi dados da tabela ordem de serviço no memo
   ...
   //- 19/02/2009: procedimento que buscará todos os dados da empresa para gerar relatório no RAVE
   ...
   //- 19/02/2009: imprime uma linha para melhor visualização dos dados no RAVE
   procedure linha(x: Integer; tipo :Boolean);
   //- 19/02/2009: busca dados do cliente para gerar relatório no RAVE
   ...
   //- 20/02/2009: compara se ja chegou no final da página e seta a impressão para uma nova
   procedure compara;
   //- 20/02/2009: imprime cabecalho em cada nova pagina
   procedure cabecalho;
   //- 20/02/2009: busca informações de venda
   procedure GeraInformacoes;
   //Procedure Para selecionar todas as tabelas de preço disponiveis
   Procedure SelecionaTabelaPreco;
   //Angelo - 18/06/2015 - Define a ordem de tabulação dinamicamente
...
var
  ...
  //- 19/02/2009: variáveis que armazenam quantidade, soma, media, data de parcelas, pedidos
  x1, x2, x3, x4, x5, x6, descforma1, descforma2 :String; // - 20/02/2009: e outras informações do cliente para relatório do RAVE
  auxlinha, xrel: Integer; // - 20/02/2009: variável para controlar linha de impressão e quantidade de itens comprados pelo cliente
  somaitens, somaserv, itens, serv: Real; // - 20/02/2009: variável que acumula o valor total dos itens comprados pelo cliente
  XCOD_TRANSP: Integer; // - 20/02/2009: código da transportadora do cliente
  XcodNome,XcodCid:Integer;
  XcodEst,Xbairro:String;

  XCODCLIINI, XCODCLIFIM:Integer;//Paulo 28/07/2011: variaveis para impressão do rel de vendas dos clientes
...
uses AlxMessage, Alxor32, UDMPessoa, UDMMacs, UMenu, URelPadraoPes,
  ...
  UDMGEOGRAFIA, DateUtils, UFORNECEDOR, UCadCidade, UCadEstado, UMDO;
...
Procedure TFCliente.LocalizaUltCli;// localiza o ultimo cliente cadastrado e insere seu codigo interno na Hint
Begin
   DMMACS.TALX.Close;
   DMMACS.TALX.SQL.Clear;
   DMMACS.TALX.SQL.Add(' select * from cliente order by cliente.cod_cliente desc');
   DMMACS.TALX.Open;
   DBINTERNO.Hint:='Último cliente cadastrado: '+DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
   DBCODINTERNOPJ.Hint:='Último cliente cadastrado: '+DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
End;
///////////////////////////////////////
//SQL PARA RELATORIOS DE CTAS A RECEBER
///////////////////////////////////////
...
Begin
	   ...
	   DMMACS.TRel.SQL.Add(' WHERE (FORMPAG.COD_LOJA='+FMenu.LCODLOJA.Caption+') And (ctareceber.cod_cliente=:COD_CLIENTE)');
	   DMMACS.TRel.SQL.Add(' And ((parcelacr.fech<>'+#39+'S'+#39+') OR (parcelacr.fech IS NULL))');
      DMMACS.TRel.ParamByName('COD_CLIENTE').AsString:=XView.FIELDBYNAME('COD_CLIENTE').AsString;
      DMMACS.TRel.SQL.Add(' order by pessoa.nome, ctareceber.numdoc, ctareceber.dtlanc, parcelacr.dtvenc ');
      DMMACS.TRel.Open;

  	   DMMACS.TLoja.Edit;
      DMMACS.TLoja.FieldByName('TMP2').AsString:='Cadastro/Clientes/Relatório/Extrato de contas';
      DMMACS.TLoja.Post;
...
Begin
   ...
   // Busca dados de Ordem de Serviço para o memo do historico
   Historico_OS;

End;

Procedure TFCLIENTE.FiltraServot;//procedure utilizada para filtrar produtos e servot referente ao cliente
Begin
   DMPESSOA.TALX.Close;
...
begin
  ...
  If (DMMACS.TLoja.FieldByName('ATIVIDADE').AsString='OTICA')
  Then Begin
       ...
       TCPessoa.Tabs.Add('F&icha Ótica');
  End
  Else Begin
       ...
       TCPessoa.Tabs.Add('C&omplemento');
       TCPessoa.Tabs.Add('&Financeiro');
       TCPessoa.Tabs.Add('&Histórico');
  End;

	//Paulo 17/06/2011: Para grid de petshop
   If DMMACS.TLoja.FieldByName('atividade').AsString = 'PETSHOP'
   Then Begin
  ...
  // - 20/04/2009: verificar se arquivo de layout foi salvo anteriormente
   If FileExists('C:\MZR\Arquivos\LayoutGrid\cliente.lgm')
   Then
...
begin
  inherited;
    ...
    End;
    If (TCPessoa.ActiveTab=2) And (RGTIPO.ItemIndex=1)
    Then Begin
...
begin
  inherited;
    //Se o usuário não digitar um valor é adicionado o valor zero aos campos de receitas
    ...
    if OUTREC.Text = '' Then
       OUTREC.Text:='0';
    //Se o usuário não digitar um valor é adicionado o valor zero aos campos de despesas
    if DALUG.Text = '' Then
       DALUG.Text:='0';
    if DVEIC.Text = '' Then
       DVEIC.Text:='0';
    ...
    // Comparando o estado civil da pessoa para calculo do limite de crédito.
        Case RGCivil.ItemIndex of
        -1:  Begin
				 Mensagem('INFORMAÇÃO AO USUÁRIO', 'Selecione o estado cívil do cliente para que o cálculo seja efetivado!' , '', 1, 1, False, 'i');
            end;
        0:  Begin
...
begin
   ...
   If ControlAccess('EXCLCLI', 'M')=False Then
    ...
    If CheckRelation(XPkTabela, XView.FieldByName(XPkTabela).AsString, ['CTARECEBER', 'PEDVENDA', 'ORDEM'], 3)=False Then
        inherited
    Else
        Mensagem('INFORMAÇÃO AO USUÁRIO', 'Este Cliente possui referências pendentes e não pode ser apagado!' , '', 1, 1, False, 'i');
...
begin  
   Try
       ...
       If VerifExitVal('CLIENTE', XPkTabela, XTabela.fieldbyname(XPkTabela).AsString, XPkTabela,  XTabela.fieldbyname(XPkTabela).AsString)=True
       Then Begin
            Mensagem('INFORMAÇÃO AO USUÁRIO', 'Este Código Interno já existe!', '', 1, 1, False, 'i');
       End
       Else Begin
           XTabela.Edit;
           ...
           //Edmar - 10/03/2015 - Verifica se a opção de cliente industria e de simples nacional
           //não está marcada
           if not CBSimplesNacional.Checked AND not cbIndustria.Checked then
           	//se nenhuma das duas opções estiver marcada
           ...
           //Paulo 04/07/2011: Para controlar frequência de venda
           ...
           //Edmar - 10/03/2015 - Valida se o cliente é contribuinte de ICMS ou não
           if CbContribuinteIcms.Text = 'Sim' then
           	XTabela.FieldByName('CONTRIBICMS').AsString := '1'//Contribuinte
           else
           	if CbContribuinteIcms.Text = 'Isento' then
					XTabela.FieldByName('CONTRIBICMS').AsString := '2'//isento
               else
					XTabela.FieldByName('CONTRIBICMS').AsString := '9';//não contribuinte
                   
           inherited;

       End;
   Except
        Mensagem('Mzr Sistemas - INFORMAÇÃO', 'Falha na tentativa de gravação de novo cliente. Verifique se as informaçõe foram gravadas. Se o problema persistir reinicie o sistema.', '', 1, 1, false, 'i');
...
begin
  inherited;
   If CBMes.Text<>''
   Then Begin//MÊS VÁLIDO executa sql e abre relatório
...
begin
  ...
  Try     
       ...
       //Localiza Vendedor
       If FiltraTabela(DMMACS.TUsuario, 'USUARIO', 'CODUSUARIO', DMPESSOA.TCliente.FieldByName('COD_USUARIO').AsString,'' ) = True
       Then Begin
           //Localiza Vendedor
           If FiltraTabela(DMPESSOA.VWFuncionario, 'VWFUNCIONARIO', 'COD_FUNC', DMMACS.TUsuario.FieldByName('COD_FUNCIONARIO').AsString,'' ) = True
           Then Begin
       ...
       End;
       If filtraTabela(DMPESSOA.WFornecedor, 'VWFORNEC', 'COD_FORNEC', DMPESSOA.TPessoa.FieldByName('COD_TRANSPORTADORA').AsString, '') = True
       then begin
       ...
       if XTabela.FieldByName('CONTRIBICMS').AsString = '1' then//Contribuinte
           ...
           if XTabela.FieldByName('CONTRIBICMS').AsString = '2' then//isento
               CbContribuinteIcms.Text := 'Isento'
           else
               CbContribuinteIcms.Text := 'Não';//não contribuinte
           
        // - 03/04/2009: se for do tipo pessoa juridica
        If RGTIPO.ItemIndex <> 0
        Then Begin
            If DMPESSOA.TPessoaJ.FieldByName('INDUSTRIA').AsString = '1'
            Then
   ...
   Except
        Mensagem('Mzr Sistemas - INFORMAÇÃO', 'Falha na tentativa de consulta de cadastro. Se o problema persistir reinicie o sistema.', '', 1, 1, false, 'i');
         // - 28/04/2009: fechar possiveis transações abertas
        DMMACS.Transaction.RollbackRetaining;
        DMMACS.IBTCodigo.RollbackRetaining;
        DMPESSOA.TransacPessoa.RollbackRetaining;
...
begin
  ...
  Try
    inherited;
  Except
        Mensagem('Mzr Sistemas - INFORMAÇÃO', 'Falha na tentativa de inserção de novo cliente. Se o problema persistir reinicie o sistema.', '', 1, 1, false, 'i');
         // - 28/04/2009: fechar possiveis transações abertas
        DMMACS.Transaction.RollbackRetaining;
        DMMACS.IBTCodigo.RollbackRetaining;
        DMPESSOA.TransacPessoa.RollbackRetaining;
...
begin
  inherited;
   ...
   Then Begin
       ...
       Then Begin
	       If SelectRegistro('VWFUNCIONARIO', 'COD_FUNC', FrmVendedor.EDCodigo.Text) = True
...
begin
  inherited;
   ...
   Then Begin
       ...
       If Mensagem('A T E N Ç Ã O ! ! !', 'DESEJA REALMENTE INATIVAR O CLIENTE '+#13+XView.FieldByName(XCampo).AsString+'?', '', 2, 3, False, 'c')=2
       Then Begin
...
procedure TFCliente.EdNomeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
Var
...
begin
 inherited;
	Try
       ...
       End;
       If Key=VK_RETURN Then //CONAGEM DE CLIENTE E DE CREDITO DE ANTECIPAÇÃO
           contadorCli;
  ...
  inherited;
   ...
   Then Begin
       ...
       Then Begin
           ...
           If FiltraTabela(DMFINANC.TFormPag, 'FORMPAG', 'COD_FORMPAG', FrmFormPag.EDCodigo.Text, comp)
           Then Begin
              XCodFormPag:=DMFINANC.TFormPag.FieldByName('COD_FORMPAG').AsInteger;
              FrmFormPag.EDCodigo.Text:=DMFINANC.TFormPag.FieldByName('COD_FORMPAG').AsString;
              FrmFormPag.EdDescricao.Text:=DMFINANC.TFormPag.FieldByName('DESCRICAO').AsString;
...
begin
  Try
        ...
        MOrdem.Lines.Clear;
         inherited;
   Except
        Mensagem('Mzr Sistemas - INFORMAÇÃO', 'Falha na tentativa de cancelamento de cadastro. Se o problema persistir reinicie o sistema.', '', 1, 1, false, 'i');
         // - 28/04/2009: fechar possiveis transações abertas
        DMMACS.Transaction.RollbackRetaining;
        DMMACS.IBTCodigo.RollbackRetaining;
        DMPESSOA.TransacPessoa.RollbackRetaining;
...
begin
  inherited;
   ...
   Then Begin
       DMPESSOA.TCliente.Edit;
       If DMPESSOA.TCliente.FieldByName('SCPC').AsString='1'
       Then Begin
       	DMPESSOA.TCliente.FieldByName('SCPC').AsString:='0';
       	DMPESSOA.TCliente.FieldByName('OBSFINANCEIRA').AsString:='';
       	If Mensagem('Confirmação do usuário', 'Deseja excluir todas as contas deste cliente no scpc?', '', 2, 3, False, 'c')=2
       	Then Begin
               DMMACS.TALX.Close;
               DMMACS.TALX.SQL.Clear;
       ...
       Else Begin
           XInformCliente:='';
			InputQuery('Solicitação', 'Informe a observação financeira para o cliente:', XInformCliente);
       	DMPESSOA.TCliente.FieldByName('SCPC').AsString:='1';
       	DMPESSOA.TCliente.FieldByName('OBSFINANCEIRA').AsString:=XInformCliente;
       	If Mensagem('Confirmação do usuário', 'Deseja incluir todas as contas deste cliente no scpc?', '', 2, 3, False, 'c')=2
       	Then Begin
               DMMACS.TALX.Close;
               DMMACS.TALX.SQL.Clear;
               DMMACS.TALX.SQL.Add(' select * from vwparcr where vwparcr.cod_cliente=:CLIENTE ');
...
procedure TFCliente.DBGridCadastroPadraoDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  inherited;
...
begin
  ...
  //- 19/02/2009: vamos lá, o botão chama o componente RVRel(RVSystem), do RAVE
  ...
  //- 19/02/2009: abaixo está comentado o código q gerava arquivo texto com algumas informações do cliente
  //- 19/02/2009: não será mais utilizado pois será usado um relatório do rave com mais informações do cliente
  ...
  { If IOResult = 0 then
   ...
   // DEPOIS DE IMPRIMIR PEDIDO DE VENDA, IMPRIMI AGORA ORDEM DE SERVIÇO
...
begin
 ...
 {  If Not DMServ.TOrd.IsEmpty
   Then Begin
       ...
       Writeln(xtexto,'');
       XLinha := 'Ordem de Serviço..:';
       Writeln(xtexto,xlinha);

       Writeln(xtexto,'');
       XLinha := '   Nº ORD. SERVIÇO      DT. ORD. SERVIÇO      FORM. PAG.                         VALOR(R$)';
       Writeln(xtexto,xlinha);
...
// Imprimi arquivo com dados da tabela ordem de serviço
...
begin
   ...
   If DMServ.Alx.FieldByName('cont').AsInteger > 0
   Then Begin
       MOrdem.Lines.Add('Ordem de Serviço..:');
       MOrdem.Lines.Add('   Quantidade: ' + IntToStr(DMServ.Alx.FieldByname('CONT').AsInteger)+
       ' - Valor Total..:'+FormatFloat('0.00', DMServ.Alx.FieldByName('TOTAL').AsCurrency)+
       ' - Média por Ordem..: '+FormatFloat('0.00', DMServ.Alx.FieldByName('TOTAL').AsCurrency/DMServ.Alx.FieldByName('CONT').AsInteger));
   End;

   // maior e menor valor do ordem de serviço jah registrado p cliente
   DMServ.Alx.Close;
   DMServ.Alx.SQL.Clear;
   ...
   Then Begin
       ...
       '  - Número..: '+DMServ.Alx.FieldByName('numero').AsString);
   End;
   If not DMServ.Alx.IsEmpty
   Then Begin
       DMServ.Alx.Last;
   ...
   // dados das ordens de serviço do cliente
   DMServ.WOrdem.Close;
  ...
  { // apenas seleciona historico do cliente sobre maior e menor compra, documentos e contas em atraso...
   ...
   Then Begin
       Try
           ...
           // grava dados
           XLinha := '   NÚM. PEDIDO        DT. PEDIDO VENDA        FORM. PAG.                         VALOR(R$)';
       ...
       Except
           Mensagem('   A T E N Ç Ã O   ', 'Falha na impressão do histórico do cliente.','',1,1,false,'E');
           Exit;
       End;
   End;
   ...
   Then Begin
       ...
       XLinha := XLinha + '   Quantidade..: ' + IntToStr(DMMACS.TALX.FieldByName('CONT').AsInteger) + '  - Valor Total..: '+FormatFloat('0.00', DMMACS.TALX.FieldByName('TOTAL').AsCurrency)+ '  - Média por Pedido..: '+FormatFloat('0.00', DMMACS.TALX.FieldByName('TOTAL').AsCurrency/DMMACS.TALX.FieldByName('CONT').AsInteger);
   ...
   Then Begin
       ...
       Writeln(XTexto, XLinha);
       XLinha := '   DT. LANC.          DT. VENC.           NÚM. PARC.            FORM. PAG.                           VALOR(R$)';
       Writeln(XTexto, XLinha);
...
begin
    ...
    begin
        // armazena o total de dias em atraso de todas as contas. Utiliza uma função que retorna a diferenca entre as datas
    ...
    Then Begin
        ...
        x3:=FloatToStr(XMediaDias); // - 19/02/2009: media de dias em atraso de cada parcela para gerar relatório do RAVE
    End
    Else Begin
       LMedia.Caption := '0';
    End;
...
procedure TFCliente.EdNomeKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFCliente.EdCodigoKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If  FMenu.XCONSULTARAPIDA
   Then Begin
...
procedure TFCliente.EdCpfKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If  FMenu.XCONSULTARAPIDA
   Then Begin
     // - 16/02/2009: pesquisa cpf/cnpj do cliente na tabela aberta
   DMPESSOA.WCliente.Locate('CPFCNPJ',EdCpf.Text, [loPartialKey,loCaseInsensitive]);
  End;
   contadorCli;
end;

// - 19/02/2009: procedimento que busca dados da empresa e cliente para gerar relatório no RAVE
...
// - 19/02/2009: impressão do rave (componente, linha, coluna, texto, fonte, tamanho, cor)
procedure TFCliente.Imprime(RVRel: TRVSystem; Linha: Double; Coluna:Double; Texto:String; Fonte:String; TamFonte:Integer; CorFonte:TColor; Tipo:Boolean);
begin
...
// - 19/02/2009: x = linha da pagina / tipo = tracejada ou continua / bld = negrito ou não
...
// - 19/02/2009: On print do componente do Rave, antes de imprimir ele vai setar os locais que serão impressas as informações do cliente
procedure TFCliente.RVRelPrint(Sender: TObject);
var
   pp, up :String; // - 20/02/2009: variáveis que armazenam data do primeiro e ultimo pedido e primeira e ultima ordem
   difp, aux :Real; // - 20/02/2009: variáveis que guardam a diferença de dias entre duas datas
   atraso, adiantado :Integer; // - 20/02/2009: variável que guarda a diferença entre data de vencimento e data do relatório
begin
  inherited;

  // - 19/02/2009: CABEÇALHO

        cabecalho;  // - 02/03/2009: imprime cabeçalho de página

        // - 19/02/2009: dados do cliente
        Imprime(RVRel,31,5, 'Cliente: ' +DMPESSOA.TCliente.fieldByName('COD_INTERNO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('NOME').AsString ,'Verdana',8,clBlack, true);
        Imprime(RVRel,31,150, 'CPF/CNPJ: ' +DMPESSOA.TPessoa.fieldByName('CPFCNPJ').AsString ,'Verdana',8,clBlack, false);
        Imprime(RVRel,35,5, 'Endereço: ' +DMPESSOA.TPessoa.fieldByName('ENDERECO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('BAIRRO').AsString,'Verdana',8,clBlack, false);
        Imprime(RVRel,35,150, 'CEP: ' +DMPESSOA.TPessoa.fieldByName('CEP').AsString,'Verdana',8,clBlack, false);
        Imprime(RVRel,39,5, 'Cidade: ' +DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString,'Verdana',8,clBlack, false);
        Imprime(RVRel,39,150, 'Fone: ' +DMPESSOA.TPessoa.fieldByName('TELREL').AsString ,'Verdana',8,clBlack, true);

        auxlinha:=43;

  linha(auxlinha, true); // - 19/02/2009: linha separatória


        // - 19/02/2009: INFORMAÇÕES COMERCIAIS
        Imprime(RVRel,auxlinha,5, 'Informações comerciais:','Verdana',10,clBlack, true);
        ...
        informacoescli('P','Ultima'); // - 19/02/2009: busca todas as informações da PRIMEIRO e ULTIMO pedido de compra do cliente
        if x4 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Primeiro Pedido de Compra: '+x4+ '   No Valor de: R$ ' +x5+ '   em: ' +x6+ '    pagamento em: '+descforma1,'Verdana',8,clBlack, false);
             up:=x6; // - 20/02/2009: data do último pedido
             pp:=x3; // - 20/02/2009: data do primeiro pedido
             difp:=DaysBetween(StrToDate(pp), StrToDate(up)); // - 20/02/2009: calcula a diferença de dias entre as datas
             auxlinha:=auxlinha+4;
             compara;
        end;
        if x1 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Último Pedido de Compra: '+x1+ '   No Valor de: R$ ' +x2+ '   em: ' +x3+ '    pagamento em: '+descforma2,'Verdana',8,clBlack, false);
        ...
        informacoescli('P','Maior'); // - 19/02/2009: busca todas as informações da MAIOR e MENOR compra do cliente
        if x1 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Maior Pedido de Compra: '+x1+ '   No Valor de: R$ ' +x2+ '   em: ' +x3+ '    pagamento em: '+descforma1,'Verdana',8,clBlack, false);
             auxlinha:=auxlinha+4;
             compara;
        end;
        if x4 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Menor Pedido de Compra: '+x4+ '   No Valor de: R$ ' +x5+ '   em: ' +x6+ '    pagamento em: '+descforma2,'Verdana',8,clBlack, false);
             auxlinha:=auxlinha+6;
             compara;
        end;


        informacoescli('I','Pedido'); // - 19/02/2009: busca todas as informações dos itens comprados pelo cliente
        if itens > 0
        then begin
             somaitens:=somaitens/itens;
             FormatCurr('0.00', somaitens);
             Imprime(RVRel,auxlinha,10, 'Média de Valor de Produto: R$ ' +CurrToStr(somaitens) ,'Verdana',8,clBlack, true);
             auxlinha:=auxlinha+4;
             compara;
        end;


        informacoescli('P','Pedido'); // - 19/02/2009: busca todas as informações dos PEDIDOS do cliente
        if x1 <> ''
        then begin
             if difp <> 0 // - 20/02/2009: se a diferença de dias não for zero (caso de um unico pedido)
             then begin
                  if difp < 30 // - 20/02/2009: se a diferença de datas for menor que 30 dias
                  then begin
                       Imprime(RVRel,auxlinha,10, 'Média de Valor de Compra Mensal: R$ ' +x2 ,'Verdana',8,clBlack, true);
                  end
                  else begin
                       difp:=(StrToCurr(x2)/difp)*30; // - 20/02/2009: calcula a média de compra por mês
                       Imprime(RVRel,auxlinha,10, 'Média de Valor de Compra Mensal: R$ ' +FloatToStr(StrToFloat(FormatFloat('0.00',difp))) ,'Verdana',8,clBlack, true);
                  end;
                  auxlinha:=auxlinha+6;
                  compara;
             end;

             Imprime(RVRel,auxlinha,10, 'Quantidade de Pedidos: '+x1+ '   Valor Total dos Pedidos: R$ ' +x2+ '   Média do Valor de Compra: R$ ' +x3,'Verdana',8,clBlack, true);
             auxlinha:=auxlinha+6;
             compara;
        end;
         ...
         ///////////////////////////////////////
        // - 20/02/2009: ORDENS DE SERVIÇO//
        ///////////////////////////////////////

        informacoescli('O','Ultima'); // - 19/02/2009: busca todas as informações da PRIMEIRO e ULTIMO ordem de serviço do cliente
        if x4 <> ''
        then begin

             linha(auxlinha, false); // - 19/02/2009: só imprime linha separatória se tiver ordens de serviço

             Imprime(RVRel,auxlinha,10, 'Primeira Ordem de Seviço: '+x4+ '   No Valor de: R$ ' +x5+ '   em: ' +x6+ '    pagamento em: '+descforma1,'Verdana',8,clBlack, false);
             up:=x6;
             pp:=x3;
        ...
        if x1 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Última Ordem de Seviço: '+x1+ '   No Valor de: R$ ' +x2+ '   em: ' +x3+ '    pagamento em: '+descforma2,'Verdana',8,clBlack, false);
             auxlinha:=auxlinha+4;
             compara;
        end;


        informacoescli('O','Maior'); // - 19/02/2009: busca todas as informações da MAIOR e MENOR compra do cliente
        if x1 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Maior Ordem de Seviço: '+x1+ '   No Valor de: R$ ' +x2+ '   em: ' +x3+ '    pagamento em: '+descforma1,'Verdana',8,clBlack, false);
             auxlinha:=auxlinha+4;
             compara;
        end;
        if x4 <> ''
        then begin
             Imprime(RVRel,auxlinha,10, 'Menor Ordem de Seviço: '+x4+ '   No Valor de: R$ ' +x5+ '   em: ' +x6+ '    pagamento em: '+descforma2,'Verdana',8,clBlack, false);
             auxlinha:=auxlinha+6;
             compara;
        end;


        informacoescli('I','Ordem'); // - 19/02/2009: busca todas as informações dos itens comprados pelo cliente em ordens
        if itens > 0
        then begin
             somaitens:=somaitens/itens;
             Imprime(RVRel,auxlinha,10, 'Média de Valor de Produtos em Ordem: R$ ' +CurrToStr(somaitens) ,'Verdana',8,clBlack, true);
             auxlinha:=auxlinha+4;
             compara;
        end;
        if serv > 0
        then begin
             ...
             Imprime(RVRel,auxlinha,10, 'Média de Valor de Serviços em Ordem: R$ ' +CurrToStr(somaserv) ,'Verdana',8,clBlack, true);
             auxlinha:=auxlinha+4;
             compara;
        end;


        informacoescli('O','Ordem'); // - 19/02/2009: busca todas as informações dos PEDIDOS do cliente
        if x1 <> ''
        then begin
             if difp < 30 // - 20/02/2009: se a diferença de datas for menor que 30 dias
             then begin
                  Imprime(RVRel,auxlinha,10, 'Média de Valor de Ordens de Serviço por Mês: R$ ' +x2 ,'Verdana',8,clBlack, true);
             end
             else begin
                  difp:=(StrToCurr(x2)/difp)*30; // - 20/02/2009: calcula a média do valor das ordens por mês
                  Imprime(RVRel,auxlinha,10, 'Média de Valor de Ordens de Serviço por Mês: R$ ' +FloatToStr(StrToFloat(FormatFloat('0.00',difp))) ,'Verdana',8,clBlack, true);
             end;
             auxlinha:=auxlinha+6;
             compara;

             Imprime(RVRel,auxlinha,10, 'Quantidade de Ordens de Seviço: '+x1+ '   Valor Total das Ordens: R$ ' +x2+ '   Média do Valor de Compra: R$ ' +x3,'Verdana',8,clBlack, true);
             auxlinha:=auxlinha+6;
             compara;
        end;
    ...
    linha(auxlinha, True); // - 19/02/2009: linha separatória
        ...
        informacoescli('C','ParcVencidas'); // - 19/02/2009: busca todas as informações das PARCELAS do cliente
        if x1 <> ''
        then begin
             Imprime(RVRel,auxlinha,5, 'Quantidade de parcelas atrasadas: '+x1+ '   No Valor de: R$ ' +x2+ '   Média de dias de atraso: ' +x3,'Verdana',10,clBlack, true);
             auxlinha:=auxlinha+7;
             compara;
        end;

        informacoescli('C','Vencidas'); // - 19/02/2009: busca todas as informações das PARCELAS do cliente
        if not DMMACS.TALX.IsEmpty
        then begin
             Imprime(RVRel,auxlinha,5, 'Parcelas Vencidas','Verdana',10,clBlack, true);
             auxlinha:=auxlinha+5;
             compara;

             while not DMMACS.TALX.Eof do
             begin
                  atraso:=DaysBetween(DMMACS.TALX.fieldByName('DTVENC').AsDateTime, Date());
                  Imprime(RVRel,auxlinha,10, 'Número da parcela: '+DMMACS.TALX.fieldByName('NUMPARC').AsString+ '   Com vencimento em: ' +DMMACS.TALX.fieldByName('DTVENC').AsString+ '   No valor de: R$ ' +DMMACS.TALX.fieldByName('VALOR').AsString+ '   Atraso de: ' +IntToStr(atraso)+ ' dias.' ,'Verdana',8,clBlack, false);
                  auxlinha:=auxlinha+4; // - 20/02/2009: controla linha de impressão
                  compara; // - 20/02/2009: compara se está no fim da pagina
                  DMMACS.TALX.Next; // - 20/02/2009: próximo registro
             end;
        end;
    ...
    linha(auxlinha, True); // - 19/02/2009: linha separatória
        ...
        informacoescli('C','ParcVencer'); // - 19/02/2009: busca todas as informações das PARCELAS do cliente
        if x1 <> ''
        then begin
             Imprime(RVRel,auxlinha,5, 'Quantidade de parcelas a vencer: '+x1+ '   No Valor de: R$ ' +x2,'Verdana',10,clBlack, true);
             auxlinha:=auxlinha+7;
             compara;
        end;


        informacoescli('C','Vencer'); // - 19/02/2009: busca todas as informações das PARCELAS do cliente
        if not DMMACS.TALX.IsEmpty
        then begin
             Imprime(RVRel,auxlinha,5, 'Parcelas a Vencer','Verdana',10,clBlack, true);
             auxlinha:=auxlinha+5;
             compara;
             ...
             begin
                  ...
                  Imprime(RVRel,auxlinha,10, 'Número da parcela: '+DMMACS.TALX.fieldByName('NUMPARC').AsString+ '   Com vencimento em: ' +DMMACS.TALX.fieldByName('DTVENC').AsString+ '   No valor de: R$ ' +DMMACS.TALX.fieldByName('VALOR').AsString,'Verdana',8,clBlack, false);
                  auxlinha:=auxlinha+4; // - 20/02/2009: controla linha de impressão
                  compara; // - 20/02/2009: compara se está no fim da pagina
                  DMMACS.TALX.Next; // - 20/02/2009: próximo registro
             end;
    ...
    linha(auxlinha, True); // - 19/02/2009: linha separatória
        ...
        atraso:=0; // - 19/02/2009: variáveis de controle de inadimplência
        adiantado:=0; // - 19/02/2009: variáveis de controle de inadimplência
        aux:=0; // - 19/02/2009: variáveis de controle de inadimplência


        informacoescli('C','Inadimplencia'); // - 19/02/2009: busca todas as informações das PARCELAS do cliente para comparação de inadimplencia
        if not DMMACS.TALX.IsEmpty // - 19/02/2009: se existem parcelas pagas
        then begin
             while not DMMACS.TALX.Eof do // - 19/02/2009: enquanto não for o fim
             begin
                  FiltraTabela(DMMACS.TLoja, 'LOJA', '', '', '');
                  if DMMACS.TLoja.FieldByName('DIASCARE').AsInteger > 0  // - 02/03/20029: compara em loja os dias de carencia para controle de inadimplência
                  then begin
                  ...
                  if (DMMACS.TALX.fieldByName('DTVENC').AsDateTime < DMMACS.TALX.fieldByName('DTDEBITO').AsDateTime) and (DaysBetween(DMMACS.TALX.fieldByName('DTVENC').AsDateTime, DMMACS.TALX.fieldByName('DTDEBITO').AsDateTime) > StrToInt(x1))
                  then begin
                       atraso:=atraso+1;
                  end
                  else begin
             ...
             atraso:=(atraso*100);
             aux:=(atraso/aux); // - 20/02/2009: recebe o calculo da porcentagem de inadimplência nas parcelas ja pagas
             FormatCurr('0.00', aux);
             Imprime(RVRel,auxlinha,10, 'Em ' +CurrToStr(aux) + '% das parcelas pagas, houve atraso maior que ' +x1+ ' dias.' ,'Verdana',8,clBlack, true);
...
// - 20/02/2009: imprime cabeçalho de pagina a cada nova pagina
...
begin
        ...
        Imprime(RVRel,19,75,'Histórico do Cliente','Verdana',14,clBlack, True);
...
// - 19/02/2009: busca informações comerciais do cliente
procedure TFCliente.informacoescli(tipo: String; inf: String);
begin
     // - 19/02/2009: limpa variáveis de controle
     x1:='';
     x2:='';
     x3:='';
     x4:='';
    ...
    if tipo = 'P'  // - 20/02/2009: SE FOR INFORMAÇÕES DOS PEDIDOS
    then begin
         if inf = 'Pedido' // - 19/02/2009: se for informações sobre os pedidos
         then begin
              // - 19/02/2009: filtra quantidade de pedidos do cliente, soma do valor dos mesmos e média de valor que o cliente compra
              DMMACS.TALX.Close;
              DMMACS.TALX.SQL.Clear;
              DMMACS.TALX.SQL.Add(' SELECT Count(Pedvenda.cod_Pedvenda) AS CONT, SUM(PEDVENDA.VALOR) AS TOTAL FROM PEDVENDA ');
              DMMACS.TALX.SQL.Add(' Left Join cliente on pedvenda.cod_cliente = cliente.cod_cliente ');
              ...
              then begin
                   ...
                   then begin
                       ...
                       DMMACS.TALX.ParamByName('DATA1').AsDateTime:=StrToDate(EdDataIni.Text);
                       DMMACS.TALX.ParamByName('DATA2').AsDateTime:=StrToDate(EdDataFim.Text);
                   end;
                   DMMACS.TALX.ParamByName('CODCLIENTE').AsString:=DMESTOQUE.Alx1.fieldByName('COD_CLIENTE').AsString;
              end
              else begin
                   DMMACS.TALX.ParamByName('CODCLIENTE').AsString:=XTabela.FieldByName('COD_CLIENTE').AsString;
              end;
              ...
              Then Begin
                   If DMMACS.TALX.FieldByName('CONT').AsInteger>0
                   Then Begin
                        x1:=IntToStr(DMMACS.TALX.FieldByName('CONT').AsInteger); // - 19/02/2009: quantidade de pedidos
                        x2:=DMMACS.TALX.FieldByName('TOTAL').AsString; // - 19/02/2009: soma do valor dos pedidos
                        x3:=FloatToStr(StrToFloat( FormatFloat('0.00',DMMACS.TALX.FieldByName('TOTAL').AsCurrency/DMMACS.TALX.FieldByName('CONT').AsInteger))); // - 19/02/2009: média do valor dos pedidos
                   End;
              End;
         end else

         if inf = 'Maior' // - 19/02/2009: se for informações da maior e menor pedido de venda
         then begin
              ...
              Then Begin
                   DMMACS.TALX.First;
                   x1:=DMMACS.TALX.FieldByName('numped').AsString; // - 19/02/2009: numero do pedido da maior compra
                   x2:=DMMACS.TALX.FieldByName('valor').AsString; // - 19/02/2009: valor da maior compra
                   x3:=DMMACS.TALX.FieldByName('dtpedven').AsString; // - 19/02/2009: data da maior compra
                   descforma1:=DMMACS.TALX.FieldByName('descricao').AsString;
                   If DMMACS.TALX.RecordCount>1
                   Then Begin
                        DMMACS.TALX.Last;
                        x4:=DMMACS.TALX.FieldByName('numped').AsString; // - 19/02/2009: numero do pedido da menor compra
                        x5:=DMMACS.TALX.FieldByName('valor').AsString; // - 19/02/2009: valor da menor compra
                        x6:=DMMACS.TALX.FieldByName('dtpedven').AsString; // - 19/02/2009: data da menor compra
                        descforma2:=DMMACS.TALX.FieldByName('descricao').AsString;
                   End;
              End;
         end else

         if inf = 'Ultima' // - 19/02/2009: se for informações da primeiro e ultimo pedido de venda
         then begin
              ...
              Then Begin
                   DMMACS.TALX.First;
                   x2:=DMMACS.TALX.FieldByName('valor').AsString; // - 19/02/2009: valor da ultima compra
                   x3:=DMMACS.TALX.FieldByName('dtpedven').AsString; // - 19/02/2009: data da ultima compra
                   x1:=DMMACS.TALX.FieldByName('numped').AsString; // - 19/02/2009: numero do pedido da ultima compra
                   descforma2:=DMMACS.TALX.FieldByName('descricao').AsString;
                   If DMMACS.TALX.RecordCount>1
                   Then Begin
                        DMMACS.TALX.Last;
                        x5:=DMMACS.TALX.FieldByName('valor').AsString; // - 19/02/2009: valor da primeira compra
                        x6:=DMMACS.TALX.FieldByName('dtpedven').AsString;  // - 19/02/2009: data da primeira compra
                        x4:=DMMACS.TALX.FieldByName('numped').AsString; // - 19/02/2009: numero do pedido da primeira compra
                        descforma1:=DMMACS.TALX.FieldByName('descricao').AsString;
                   End;
              End;
         end;
    ...
    if tipo = 'O' // - 20/02/2009: SE FOR INFORMAÇÕES DAS ORDENS
    then begin
         if inf = 'Ordem' // - 20/02/2009: se for informações sobre as ordens do cliente
         then begin
              ...
              If DMServ.Alx.FieldByName('cont').AsInteger > 0
              Then Begin
                   x1:=DMServ.Alx.FieldByname('CONT').AsString; // - 20/02/2009: quantidade de ordens
                   x2:=DMServ.Alx.FieldByName('TOTAL').AsString; // - 20/02/2009: soma do valor das ordens
                   x3:=FloatToStr(StrToFloat(FormatFloat('0.00',DMServ.Alx.FieldByName('TOTAL').AsCurrency/DMServ.Alx.FieldByName('CONT').AsInteger))); // - 20/02/2009: média do valor das ordens
              End;
         end else

         if inf = 'Maior' // - 20/02/2009: se for informações sobre as ordens do cliente
         then begin
              // maior e menor valor do ordem de serviço jah registrado p cliente
              DMServ.Alx.Close;
              DMServ.Alx.SQL.Clear;
         ...
         if inf = 'Ultima' // - 20/02/2009: se for informações sobre as ordens do cliente
         then begin
              //meira e ultima ordem de servico do cliente
              DMServ.Alx.Close;
              DMServ.Alx.SQL.Clear;
    ...
    if tipo = 'C' // - 20/02/2009: SE FOR INFORMAÇÕES DAS CONTAS A RECEBER
    then begin
         if inf = 'ParcVencidas' // - 19/02/2009: se for informações das parcelas
         then begin
              ...
              if xrel = 1
              then begin
                   DMMACS.TALX.ParamByName('COD_CLIENTE').AsString:=DMESTOQUE.Alx1.fieldByName('COD_CLIENTE').AsString;
              end
              else begin
                   DMMACS.TALX.ParamByName('COD_CLIENTE').AsInteger:=XTabela.FieldByName('COD_CLIENTE').AsInteger;
              end;
              ...
              Then Begin
                   If DMMACS.TALX.FieldByName('CONT').AsInteger>0
                   Then Begin
                        x1:=DMMACS.TALX.FieldByName('CONT').AsString; // - 19/02/2009: quantidade de parcelas em atraso
                        x2:=DMMACS.TALX.FieldByName('TOTAL').AsString; // - 19/02/2009: soma do valor total de parcelas em atraso
                   End;
              End;
         ...
         if inf = 'Vencidas' // - 19/02/2009: se for informações gerais de todas parcelas vencidas
         then begin
         ...
         if inf = 'ParcVencer' // - 19/02/2009: se for informações das parcelas
         then begin
              // - 19/02/2009: documentos em atraso
              DMMACS.TALX.Close;
              DMMACS.TALX.SQL.Clear;
         ...
         if inf = 'Vencer' // - 19/02/2009: se for informações gerais de todas parcelas vencidas
         then begin
              //filtra parcelas em atraso do cliente
              DMMACS.TALX.Close;
              DMMACS.TALX.SQL.Clear;
         ...
         if inf = 'Inadimplencia' // - 19/02/2009: se for informações gerais de todas parcelas pagas
         then begin
              // - 19/02/2009: filtra parcelas pagas do cliente
              DMMACS.TALX.Close;
              DMMACS.TALX.SQL.Clear;
              DMMACS.TALX.SQL.Add(' select * From vwparcr ');
              DMMACS.TALX.SQL.Add(' where (vwparcr.fech='+#39+'S'+#39+') and (vwparcr.cod_cliente=:COD_CLIENTE) ');
              DMMACS.TALX.ParamByName('COD_CLIENTE').AsInteger:=XTabela.FieldByName('COD_CLIENTE').AsInteger;
              DMMACS.TALX.Open;
              DMMACS.TALX.First;
         end;
    ...
    if tipo = 'I' // - 20/02/2009: SE FOR RELACIONADO A ITENS DE PEDIDOS E SERVIÇOS DE ORDENS
    then begin
         if inf = 'Pedido' // - 20/02/2009: filtra todos os pedidos do cliente
         then begin
              DMFINANC.TAlx.Close;
              DMFINANC.TALX.SQL.Clear;
              DMFINANC.TALX.SQL.Add(' select *  From vwpedv ');
              ...
              then begin
                   ...
                   then begin
                       ...
                       DMFINANC.TALX.ParamByName('DATA1').AsDateTime:=StrToDate(EdDataIni.Text);
                       DMFINANC.TALX.ParamByName('DATA2').AsDateTime:=StrToDate(EdDataFim.Text);
                   end;
                   DMFINANC.TALX.ParamByName('COD_CLIENTE').AsString:=DMESTOQUE.Alx1.fieldByName('COD_CLIENTE').AsString;
              end
              else begin
                   DMFINANC.TALX.ParamByName('COD_CLIENTE').AsInteger:=XTabela.FieldByName('COD_CLIENTE').AsInteger;
              end;
              ...
              if not DMFINANC.TAlx.IsEmpty // - 20/02/2009: se encontrou algum pedido do cliente
              then begin

                   itens:=0; // - 20/02/2009: zera controle de quantidade de produtos comprados
                   somaitens:=0; // - 20/02/2009: zera controle da soma dos produtos comprados pelo cliente

                   while not DMFINANC.TAlx.Eof do // - 20/02/2009: enquanto não for o fim dos pedidos
                   begin
                        DMESTOQUE.Alx.Close;
                        DMESTOQUE.Alx.SQL.Clear;
                        ...
                        DMESTOQUE.Alx.ParamByName('COD').AsString:=DMFINANC.TAlx.fieldByName('COD_PEDVENDA').AsString;
                        DMESTOQUE.Alx.Open;

                        itens:=itens+DMESTOQUE.Alx.FieldByName('QTD').AsInteger; // - 20/02/2009: acumula a soma da quantidade de itens comprados
                        somaitens:=somaitens+DMESTOQUE.Alx.FieldByName('VALOR').AsCurrency; // - 20/02/2009: acumula a soma do valor dos itens comprados

                        DMFINANC.TAlx.Next; // - 20/02/2009: próximo registro
                   end;
              end;
         end else

         if inf = 'Ordem' // - 20/02/2009: filtra todas as ordens do cliente
         then begin
              DMServ.WOrdem.Close;
              DMServ.WOrdem.SQL.Clear;
              DMServ.WOrdem.SQL.Add('select * from vwordem');
              ...
              if not DMServ.WOrdem.IsEmpty // - 20/02/2009: se encontrou alguma ordem do cliente
              then begin

                   itens:=0; // - 20/02/2009: zera controle de quantidade de produtos comprados
                   somaitens:=0; // - 20/02/2009: zera controle da soma dos produtos comprados pelo cliente

                   while not DMServ.WOrdem.Eof do // - 20/02/2009: enquanto não for o fim dos pedidos
                   begin
                        DMESTOQUE.Alx.Close;
                        DMESTOQUE.Alx.SQL.Clear;
                        ...
                        DMESTOQUE.Alx.ParamByName('COD').AsString:=DMServ.WOrdem.fieldByName('COD_ORDEM').AsString;
                        DMESTOQUE.Alx.Open;

                        itens:=itens+DMESTOQUE.Alx.FieldByName('QTD').AsInteger; // - 20/02/2009: acumula a soma da quantidade de itens comprados
                        somaitens:=somaitens+DMESTOQUE.Alx.FieldByName('VALOR').AsCurrency; // - 20/02/2009: acumula a soma do valor dos itens comprados

                        DMServ.WOrdem.Next; // - 20/02/2009: próximo registro
                   end;

                   DMServ.WOrdem.First; // - 20/02/2009: volta ao primeiro registro para calcular os serviços

                   serv:=0; // - 20/02/2009: zera controle de quantidade de serviços prestados
                   somaserv:=0; // - 20/02/2009: zera controle da soma dos serviços prestados

                   while not DMServ.WOrdem.Eof do // - 20/02/2009: enquanto não for o fim dos pedidos
                   begin
                        if DMMACS.TLoja.FieldByName('ATIVIDADE').AsString <> 'EQUIPE' // - 20/02/2009: se não for a mercescan
                        Then begin
                             DMESTOQUE.Alx.Close;
                             DMESTOQUE.Alx.SQL.Clear;
                             ...
                             serv:=itens+DMESTOQUE.Alx.FieldByName('QTD').AsInteger; // - 20/02/2009: acumula a soma da quantidade de itens comprados
                             somaserv:=somaserv+DMESTOQUE.Alx.FieldByName('VALOR').AsCurrency; // - 20/02/2009: acumula a soma do valor dos itens comprados

                             DMServ.WOrdem.Next; // - 20/02/2009: próximo registro
                        end
                        else begin  // - 20/02/2009: se for a mercescan busca o valor de serviços na tabela de despesas
                             DMESTOQUE.Alx.Close;
                             DMESTOQUE.Alx.SQL.Clear;
...
begin
   ...
   FiltraTabela(DMPESSOA.TCliente,'CLIENTE','COD_CLIENTE',DMPESSOA.WCliente.FieldByName('COD_CLIENTE').AsString,''); // - 19/02/2009: filtra dados do cliente

   FiltraTabela(DMPESSOA.TPessoa,'PESSOA','COD_PESSOA',DMPESSOA.TCliente.FieldByName('cod_pessoa').AsString,''); // - 19/02/2009: filtra dados da pessoa

   FiltraTabela(DMGEOGRAFIA.TCidade,'CIDADE','COD_CIDADE',DMPESSOA.TPessoa.FieldByName('cod_cidade').AsString,''); // - 19/02/2009: filtra cidade do cliente

   FiltraTabela(DMGEOGRAFIA.TEstado,'ESTADO','COD_ESTADO',DMGEOGRAFIA.TCidade.fieldByName('cod_estado').AsString,''); // - 19/02/2009: filtra estado do cliente
...
begin
    ...
    if (EdNome.Text <> '') and (EdNome.Text <> 'Nome')  then
        DMPESSOA.TAux1.SQL.Add(' and (upper(pessoa.nome)) like (upper('+#39+EdNome.Text+'%'+#39+'))');

    if (EdCodigo.Text <> '') and (EdCodigo.Text <> 'Código') then
        DMPESSOA.TAux1.SQL.Add(' and (upper(cliente.cod_interno)) like (upper('#39+EdCodigo.Text+'%'+#39+'))');

    if (EdCpf.Text <> '') and (EdCpf.Text <> 'CPF/CNPJ') then
        DMPESSOA.TAux1.SQL.Add(' and (upper(pessoa.cpfcnpj)) like (upper('#39+EdCpf.Text+'%'+#39+'))');
    ...
    //ALEX 11/11/2011: mostra crédito do cliente
    lbCreditoCli.Caption:='Crédito por Antecipações: '+FormatFloat('0.00', DMPESSOA.TAux1.FieldByName('VLRCREDITO').AsCurrency);
end;
...
begin
  inherited;
   ...
   // - 20/05/2009: relatório de vendas
   ...
   LTituloRel.Caption:='Relatório de Vendas';
...
begin
  ...
  then begin
       ...
       //  - 20/05/2009: enquanto não for o fim dos clientes
       while not DMESTOQUE.Alx1.Eof do
       begin
           DMESTOQUE.Alx2.Close;
           DMESTOQUE.Alx2.SQL.Clear;
           DMESTOQUE.Alx2.SQL.Add('select * From vwparcr ');
           DMESTOQUE.Alx2.SQL.Add('where (vwparcr.dtvenc < :DATA) AND (vwparcr.fech<>'+#39+'S'+#39+') and (vwparcr.cod_cliente = :CODCLIENTE)');
           DMESTOQUE.Alx2.ParamByName('DATA').AsDateTime:=Date();
           DMESTOQUE.Alx2.ParamByName('CODCLIENTE').AsInteger:=DMESTOQUE.Alx1.fieldByName('COD_CLIENTE').AsInteger;
           DMESTOQUE.Alx2.Open;

           if not DMESTOQUE.Alx2.IsEmpty
           then begin
               if RGPessoa.ItemIndex = 1 // - 20/05/2009: se forem só clientes q pagam em dia
               then begin
                   DMESTOQUE.Alx1.Next;
               end
               else begin
...
begin
   ...
   try
       ...
       FiltraTabela(DMPESSOA.TPessoa, 'PESSOA', 'COD_PESSOA', DMESTOQUE.Alx1.fieldByName('COD_PESSOA').AsString,'');
       xdata:=DMPESSOA.TPessoa.fieldByName('DATA_INI').AsString;

       // - 20/05/2009: busca informações dos pedidos
       ...
       // - 20/05/2009: busca informações dos itens
       ...
       // - 20/05/2009: busca informações das parcelas vencidas
       ...
       DMESTOQUE.Alx4.ParamByName('DESC1').AsString := DMESTOQUE.Alx1.fieldByName('NOME').AsString;
       DMESTOQUE.Alx4.ParamByName('INT1').AsInteger := DMESTOQUE.Alx1.fieldByName('COD_INTERNO').AsInteger;
       //Edmar - 05/05/2015 - Altera o tipo de dado que será incluido
...
begin
  inherited;
   // - 03/06/2009: relatório de clientes cadastrados por data de cadastro
   PCadastro.SendToBack;
   PCadastro.Visible:=False;
   PConsulta.SendToBack;
   PConsulta.Visible:=False;
   ...
   LTituloRel.Caption:='Relatório de Clientes Cadastrados';
...
begin
  inherited;
   // - 03/06/2009: relatório de clientes divididos por cidade
   PCadastro.SendToBack;
   PCadastro.Visible:=False;
   PConsulta.SendToBack;
   PConsulta.Visible:=False;
   ...
   RGPessoa.Visible:=false;
   LTituloRel.Caption:='Relatório de Clientes por Cidade';
...
begin
  inherited;
   ...
   DMPESSOA.TCliente.Post;   }
	...
	//Edmar - 08/06/2015 - Foi alterado a forma de marcação e desmarcação do cliente   
   try
   	MDO.Query.Close;
       MDO.Query.SQL.Clear;
       ...
       MDO.Query.ParamByName('CLIENTE').AsInteger := DMPESSOA.WCliente.FieldByName('COD_CLIENTE').AsInteger;
       MDO.Query.ExecSQL;

       MDO.Transac.CommitRetaining;       
   except
   	ShowMessage('Não foi possível desmarcar o Cliente '+DMPESSOA.WCliente.FieldByName('NOME').AsString);
...
begin
  inherited;
	...
	//Edmar - 08/06/2015 - Foi alterado a forma de marcação do cliente
	try
		MDO.Query.Close;
       MDO.Query.SQL.Clear;
   	...
   	ShowMessage('Não foi possível marcar os Clientes.');
       MDO.Transac.RollbackRetaining;   
   end;
...
procedure TFCliente.EdBuscaBairroKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
  var
  	XSCPC, XFiltro: String;
begin
...
begin
  inherited;
	try
       //Paulo 03/03/2011: Verifica se existe algum filtro selecionado, senão manda a mensagem e
       //aborda a tarefa
       if(XcodNome = 0) and(XcodCid =0) and (XcodEst = '') and (Xbairro = '')and (cbClienteSelecionado.Checked=False)then
       begin
         MessageDlg('Você deve selecionar pelo menos um filtro pra imprimir as etiquetas', mtWarning, [mbOK], 0);
         exit;
       end;
      ...
      { //Paulo 03/03/2011: Determino o tamanho da folha com base das configurações
       ...
       //Paulo 09/06/2011: Determino o tamanho da fonte com base das configurações
...
begin
  inherited;
      ...
      Then Begin
          FrmBuscaNome.EdDescricao.Text:=DMPESSOA.WCliente.FieldByName('NOME').AsString;
          FrmBuscaNome.EDCodigo.text:=DMPESSOA.WCliente.FieldByName('COD_CLIENTE').AsString;
...
begin
  inherited;
    ...
    If AbrirForm(TFCadCidade, FCadCidade, 1)='Selected'
    Then Begin
        FrmBuscaCid.EdDescricao.Text:=DMGEOGRAFIA.WCidade.FieldByName('NOME').AsString;
        FrmBuscaCid.EDCodigo.text:=DMGEOGRAFIA.WCidade.FieldByName('COD_CIDADE').AsString;
        FrmBuscaCid.Repaint;
...
begin
  inherited;
      ...
      Then Begin
          FrmBuscaUF.EdDescricao.Text:=DMGEOGRAFIA.WEstado.FieldByName('DESCRICAO').AsString;
          FrmBuscaUF.EDCodigo.text:=DMGEOGRAFIA.WEstado.FieldByName('COD_ESTADO').AsString;
          FrmBuscaUF.Repaint;
...
begin
  inherited;
       //Paulo 03/03/2011: Junta os dados do endereço e joga no caption
       LbQEndereco.Caption:=DMPESSOA.TRel.FieldByName('endereco').AsString+', '+DMPESSOA.TRel.FieldByName('endnumero').AsString;
       LbQBairro.Caption:=DMPESSOA.TRel.FieldByName('bairro').AsString;
       LbQCidade.Caption:=DMPESSOA.TRel.FieldByName('cidade').AsString+' - '+DMPESSOA.TRel.FieldByName('uf_estado').AsString ;
...
begin
  inherited;
       ...
       if (FrmBuscaCliInicial.EDCodigo.text = '') and (FrmBuscaCliFinal.EDCodigo.text<>'') then
       begin
       ...
       DMMACS.TLoja.Edit;
       if (EdDtInicial.Text <> '  /  /    ') and (EdDtFinal.Text <> '  /  /    ') then
           DMMACS.TLoja.FieldByName('TMP2').AsString:='De '+EdDtInicial.Text+' a '+EdDtFinal.Text
       else
       ...
       begin
        ...
        begin
         ...
         DMPESSOA.TRel.ParamByName('CLIINI').AsInteger:=StrToInt(FrmBuscaCliInicial.EDCodigo.text);
         DMPESSOA.TRel.ParamByName('CLIFIM').AsInteger:=StrToInt(FrmBuscaCliFinal.EDCodigo.text);
        end;
        DMPESSOA.TRel.Open;
        ...
        begin
         ...
         DMPESSOA.TALX.ParamByName('CODCLI').AsInteger:=DMPESSOA.TRel.FieldByName('cod_cliente').AsInteger;
         DMPESSOA.TALX.ParamByName('SIT').AsString:='FECHADO';
         if (EdDtInicial.Text <> '  /  /    ') and (EdDtFinal.Text <> '  /  /    ')then
         begin
          DMPESSOA.TALX.SQL.Add('and pedvenda.dtfech between :dtIni and :dtFim');
          DMPESSOA.TALX.ParamByName('DTINI').AsDate:=StrToDate(EdDtInicial.Text);
         ...
         while not DMPESSOA.TALX.Eof do
         begin
           XTotalVenda:=XTotalVenda+DMPESSOA.TALX.FieldByName('valor').AsCurrency;

           DMPESSOA.TALX.Next;
         end;

         //Paulo 29/07/2011: Seleciona dados de ordem de serviço
         ...
         DMPESSOA.TALX1.ParamByName('CODCLI').AsInteger:=DMPESSOA.TRel.FieldByName('cod_cliente').AsInteger;
         DMPESSOA.TALX1.ParamByName('SIT').AsString:='FECHADO';
         if (EdDtInicial.Text <> '  /  /    ') and (EdDtFinal.Text <> '  /  /    ')then
         begin
          DMPESSOA.TALX1.SQL.Add('and ordem.dtentrega between :dtIni and :dtFim');
          DMPESSOA.TALX1.ParamByName('DTINI').AsDate:=StrToDate(EdDtInicial.Text);
         ...
         begin
          XTotalOrdem:=XTotalOrdem+DMPESSOA.TALX1.FieldByName('totord').AsCurrency;
         ...
         begin
          ...
          if DMPESSOA.TALX.FieldByName('dtfech').AsDateTime > DMPESSOA.TALX1.FieldByName('dtentrega').AsDateTime then
            DMMACS.TMP.FieldByName('DATA1').AsDateTime:=DMPESSOA.TALX.FieldByName('dtfech').AsDateTime
          else
            DMMACS.TMP.FieldByName('DATA1').AsDateTime:=DMPESSOA.TALX.FieldByName('dtentrega').AsDateTime;
           ...
           begin
            ...
            begin
             ...
             DMMACS.TMP.FieldByName('DATA1').AsDateTime:=DMPESSOA.TALX1.FieldByName('dtentrega').AsDateTime;
        ...
        //Paulo 29/07/2011: Seleciona os dados para o relatório
        DMMACS.TMP.Close;
        DMMACS.TMP.SQL.Clear;
...
begin
  inherited;
      ...
      If AbrirForm(TFCliente, FCliente, 1)='Selected'
      Then Begin
          FrmBuscaCliInicial.EdDescricao.Text:=DMPESSOA.WCliente.FieldByName('NOME').AsString;
          FrmBuscaCliInicial.EDCodigo.text:=DMPESSOA.WCliente.FieldByName('COD_CLIENTE').AsString;
...
begin
  inherited;
      ...
      If AbrirForm(TFCliente, FCliente, 1)='Selected'
      Then Begin
          FrmBuscaCliFinal.EdDescricao.Text:=DMPESSOA.WCliente.FieldByName('NOME').AsString;
          FrmBuscaCliFinal.EDCodigo.text:=DMPESSOA.WCliente.FieldByName('COD_CLIENTE').AsString;
...
begin
  inherited;
       ...
       If (DMMACS.TLoja.FieldByName('atividade').AsString = 'PETSHOP')and(DBCPF.Text='') Then
           cbVendaVista.Checked:=True
       else
           cbVendaVista.Checked:=False;
end;
...
begin
  inherited;
	...
	//Edmar - 08/06/2015 - Foi alterado a forma de desmarcação do cliente
	try
   	MDO.Query.Close;
       MDO.Query.SQL.Clear;
   	...
   	ShowMessage('Não foi possível desmarcar os Clientes.');
       MDO.Transac.RollbackRetaining;   
   end;
...
begin
  inherited;
       //Paulo 01/11/2011: mostra crédito do cliente
       FiltraTabela(DMPESSOA.TPessoa,'PESSOA','COD_PESSOA',IntToStr(DMPESSOA.WCliente.FieldByName('COD_PESSOA').AsInteger),'');
       lbCreditoCli.Caption:='Crédito por Antecipações: '+FormatFloat('0.00', DMPESSOA.TPessoa.FieldByName('VLRCREDITO').AsCurrency);
...
procedure TFCliente.RelMalaDiretaRave;  // Metodo que  gera o relatorio Mala Direta Usando Rave Reports
var
	xNrLines:  Integer ;
   ...
   xContadorEof: Integer;
   xTotalClientes: Integer;

 begin
	with  RvMalaDireta.BaseReport do
   begin
	    xTamanhoEtiquetas := (((StrToInt(EdFonte.text) * 0.3 / 10) * 5) + 2.8 ); //2,8 corresponde a o espaçamento entre as etiquetas, ja contando com o espaço entre as linhas
       xQuantidadeEtiquetas := StrToInt(EdQtdeEtqPagina.Text);//Ceil(25.6/xTamanhoEtiquetas);
       ...
       begin
           ...
           PrintLeft(Copy(DMPESSOA.TRel.FieldbyName('NOME').AsString, 1, StrToInt(EdLargura.text)),(StrToFloat(EdMargem.Text)/25.0));
           NewLine;
           PrintLeft(Copy(DMPESSOA.TRel.FieldbyName('ENDERECO').AsString + ', ' + DMPESSOA.TRel.FieldbyName('ENDNUMERO').AsString, 1, StrToInt(EdLargura.text)),(StrToFloat(EdMargem.Text)/25.0));
           NewLine;
           PrintLeft(Copy(DMPESSOA.TRel.FieldbyName('BAIRRO').AsString, 1, StrToInt(EdLargura.text)),(StrToFloat(EdMargem.Text)/25.0));
           NewLine;
           PrintLeft(Copy(DMPESSOA.TRel.FieldbyName('CIDADE').AsString + ' - '+ DMPESSOA.TRel.FieldbyName('UF_ESTADO').AsString, 1, StrToInt(EdLargura.text)),(StrToFloat(EdMargem.Text)/25.0));
           NewLine;
           PrintLeft(Copy(DMPESSOA.TRel.FieldbyName('CEP').AsString, 1, StrToInt(EdLargura.text)),(StrToFloat(EdMargem.Text)/25.0));
           DMPESSOA.TRel.Next;
...
begin
  ...
  if Trim(EdRazaoSocial.Text) = ''
    then begin
        EdRazaoSocial.Text := 'Razão Social';
 ...
 inherited;
	Try
   	...
   	Then Begin//pressionado enter, consultar
           ...
           Then Begin
               If XTab=True Then
                   FiltraTabela(XTabela, XSQLTABELA, 'RAZAOSOCIAL', '', '(upper(RAZAOSOCIAL) like upper('+#39+EdRazaoSocial.Text+'%'+#39+')) AND (vwcliente.ativo=0) '+XSCPC+XFiltro+' ORDER  BY RAZAOSOCIAL')
               Else
                   FiltraTabela(XView, XSQLVIEW, 'RAZAOSOCIAL', '',   '(upper(RAZAOSOCIAL) like upper('+#39+EdRazaoSocial.Text+'%'+#39+')) AND (vwcliente.ativo=0)  '+XSCPC+XFiltro+'  ORDER BY RAZAOSOCIAL');
           End
           Else Begin
               If XTab=True Then
                   FiltraTabela(XTabela, XSQLTABELA, 'RAZAOSOCIAL', '', '(upper(RAZAOSOCIAL) like upper('+#39+EdRazaoSocial.Text+'%'+#39+')) AND ((vwcliente.ativo<>0) or (vwcliente.ativo is null)) '+XSCPC+XFiltro+' ORDER  BY RAZAOSOCIAL')
               Else
                   FiltraTabela(XView, XSQLVIEW, 'RAZAOSOCIAL', '',   '(upper(RAZAOSOCIAL) like upper('+#39+EdRazaoSocial.Text+'%'+#39+')) AND ((vwcliente.ativo<>0) or (vwcliente.ativo is null)) '+XSCPC+XFiltro+'  ORDER BY RAZAOSOCIAL');
           End;
       End;
...
procedure TFCliente.EdRazaoSocialKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  If FMenu.XCONSULTARAPIDA
   Then Begin
     // - 16/02/2009: pesquisa nome cliente na tabela aberta
     DMPESSOA.WCliente.Locate('RAZAOSOCIAL',EdRazaoSocial.Text, [loPartialKey,loCaseInsensitive]);
    //contadorCli;
   End;
end;
