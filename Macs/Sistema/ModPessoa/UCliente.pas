...
var
  ...
  // - 20/02/2009: e outras informações do cliente para relatório do RAVE
  auxlinha, xrel: Integer;
  // - 20/02/2009: variável para controlar linha de impressão e quantidade de itens comprados pelo cliente
  somaitens, somaserv, itens, serv: Real;
  // - 20/02/2009: variável que acumula o valor total dos itens comprados pelo cliente
  ...
  //Paulo 28/07/2011: variaveis para impressão do rel de vendas dos clientes
...
begin
  ...
  DMMACS.TALX.Open;
  DBINTERNO.Hint := 'Último cliente cadastrado: ' +
    DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
  DBCODINTERNOPJ.Hint := 'Último cliente cadastrado: ' +
    DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
end;
...
begin
  ...
  DMMACS.TLoja.FieldByName('TMP2').AsString :=
    'Cadastro/Clientes/Relatório/Extrato de contas';
  DMMACS.TLoja.Post;
  ...
  begin
    ...
    begin
      ...
      MResumo.Lines.Add('   Quantidade..: ' +
        ...
        '  - Média por Pedido..: ' + FormatFloat('0.00',
        DMMACS.TALX.FieldByName('TOTAL').AsCurrency /
...
begin
  inherited;
  if CBMes.Text <> '' then
  begin //MÊS VÁLIDO executa sql e abre relatório
...
begin
  ...
  begin
    ...
    if Mensagem('A T E N Ç Ã O ! ! !', 'DESEJA REALMENTE INATIVAR O CLIENTE ' + #13 + XView.FieldByName(XCampo).AsString + '?', '', 2, 3, False, 'c') = 2 then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Mensagem('Confirmação do usuário', 'Deseja excluir todas as contas deste cliente no scpc?', '', 2, 3, False, 'c') = 2 then
      begin
        DMMACS.TALX.Close;
        DMMACS.TALX.SQL.Clear;
    ...
    begin
      ...
      if Mensagem('Confirmação do usuário', 'Deseja incluir todas as contas deste cliente no scpc?', '', 2, 3, False, 'c') = 2 then
      begin
        DMMACS.TALX.Close;
        DMMACS.TALX.SQL.Clear;
        DMMACS.TALX.SQL.Add(' select * from vwparcr where vwparcr.cod_cliente=:CLIENTE ');
...
begin
  ...
  //- 19/02/2009: trabalha nos milimetros da página
  RVRel.SystemPReview.RulerType := rtBothCm;
  //- 19/02/2009: mostra régua superior e lateral
  RVRel.Execute;
...
begin
  ...
  begin
    ...
    // - 19/02/2009: media de dias em atraso de cada parcela para gerar relatório do RAVE
  end
  else
  begin
    LMedia.Caption := '0';
...
var
  pp, up: string;
  // - 20/02/2009: variáveis que armazenam data do primeiro e ultimo pedido e primeira e ultima ordem
  difp, aux: Real;
  // - 20/02/2009: variáveis que guardam a diferença de dias entre duas datas
  atraso, adiantado: Integer;
  // - 20/02/2009: variável que guarda a diferença entre data de vencimento e data do relatório
begin
  inherited;
  difp := 0;
  ...
  Imprime(RVRel, auxlinha, 5, 'Informações comerciais:', 'Verdana', 10, clBlack,
    true);
  ...
  // - 19/02/2009: busca todas as informações da PRIMEIRO e ULTIMO pedido de compra do cliente
  if x4 <> '' then
  begin
    ...
    // - 20/02/2009: calcula a diferença de dias entre as datas
    auxlinha := auxlinha + 4;
    compara;
  end;
  if x1 <> '' then
  begin
    Imprime(RVRel, auxlinha, 10, 'Último Pedido de Compra: ' + x1 +
  ...
  // - 19/02/2009: busca todas as informações da MAIOR e MENOR compra do cliente
  if x1 <> '' then
  begin
  ...
  // - 19/02/2009: busca todas as informações dos itens comprados pelo cliente
  if itens > 0 then
  begin
    somaitens := somaitens / itens;
    FormatCurr('0.00', somaitens);
    Imprime(RVRel, auxlinha, 10, 'Média de Valor de Produto: R$ ' +
  ...
  // - 19/02/2009: busca todas as informações dos PEDIDOS do cliente
  if x1 <> '' then
  begin
    if difp <> 0
      {// - 20/02/2009: se a diferença de dias não for zero (caso de um unico pedido)} then
    begin
      if difp < 30
        {// - 20/02/2009: se a diferença de datas for menor que 30 dias} then
      begin
        Imprime(RVRel, auxlinha, 10, 'Média de Valor de Compra Mensal: R$ ' + x2,
          'Verdana', 8, clBlack, true);
      end
      else
      begin
        difp := (StrToCurr(x2) / difp) * 30;
        // - 20/02/2009: calcula a média de compra por mês
        Imprime(RVRel, auxlinha, 10, 'Média de Valor de Compra Mensal: R$ ' +
    ...
    Imprime(RVRel, auxlinha, 10, 'Quantidade de Pedidos: ' + x1 +
      '   Valor Total dos Pedidos: R$ ' + x2 + '   Média do Valor de Compra: R$ '
 ...
 ///////////////////////////////////////
  ...
  // - 19/02/2009: busca todas as informações da PRIMEIRO e ULTIMO ordem de serviço do cliente
  if x4 <> '' then
  begin

    linha(auxlinha, false);
    // - 19/02/2009: só imprime linha separatória se tiver ordens de serviço

    Imprime(RVRel, auxlinha, 10, 'Primeira Ordem de Seviço: ' + x4 +
      '   No Valor de: R$ ' + x5 + '   em: ' + x6 + '    pagamento em: ' +
      descforma1, 'Verdana', 8, clBlack, false);
    up := x6;
    pp := x3;
  ...
  if x1 <> '' then
  begin
    Imprime(RVRel, auxlinha, 10, 'Última Ordem de Seviço: ' + x1 +
      '   No Valor de: R$ ' + x2 + '   em: ' + x3 + '    pagamento em: ' +
      descforma2, 'Verdana', 8, clBlack, false);
    auxlinha := auxlinha + 4;
    compara;
  ...
  if x1 <> '' then
  begin
    Imprime(RVRel, auxlinha, 10, 'Maior Ordem de Seviço: ' + x1 +
      '   No Valor de: R$ ' + x2 + '   em: ' + x3 + '    pagamento em: ' +
      descforma1, 'Verdana', 8, clBlack, false);
    auxlinha := auxlinha + 4;
    compara;
  ...
  if x4 <> '' then
  begin
    Imprime(RVRel, auxlinha, 10, 'Menor Ordem de Seviço: ' + x4 +
      '   No Valor de: R$ ' + x5 + '   em: ' + x6 + '    pagamento em: ' +
      descforma2, 'Verdana', 8, clBlack, false);
    auxlinha := auxlinha + 6;
    compara;
  ...
  // - 19/02/2009: busca todas as informações dos itens comprados pelo cliente em ordens
  if itens > 0 then
  begin
    somaitens := somaitens / itens;
    Imprime(RVRel, auxlinha, 10, 'Média de Valor de Produtos em Ordem: R$ ' +
      CurrToStr(somaitens), 'Verdana', 8, clBlack, true);
    auxlinha := auxlinha + 4;
    compara;
  end;
  ...
  begin
    ...
    Imprime(RVRel, auxlinha, 10, 'Média de Valor de Serviços em Ordem: R$ ' +
  ...
  if x1 <> '' then
  begin
    if difp < 30 {// - 20/02/2009: se a diferença de datas for menor que 30 dias} then
    begin
      Imprime(RVRel, auxlinha, 10,
        'Média de Valor de Ordens de Serviço por Mês: R$ ' + x2, 'Verdana', 8,
        clBlack, true);
    end
    else
    begin
      difp := (StrToCurr(x2) / difp) * 30;
      // - 20/02/2009: calcula a média do valor das ordens por mês
      Imprime(RVRel, auxlinha, 10,
        'Média de Valor de Ordens de Serviço por Mês: R$ ' +
    ...
    Imprime(RVRel, auxlinha, 10, 'Quantidade de Ordens de Seviço: ' + x1 +
      '   Valor Total das Ordens: R$ ' + x2 + '   Média do Valor de Compra: R$ ' +
      x3, 'Verdana', 8, clBlack, true);
    auxlinha := auxlinha + 6;
    compara;
  end;
  ...
  // - 19/02/2009: busca todas as informações das PARCELAS do cliente
  if x1 <> '' then
  begin
    Imprime(RVRel, auxlinha, 5, 'Quantidade de parcelas atrasadas: ' + x1 +
      '   No Valor de: R$ ' + x2 + '   Média de dias de atraso: ' + x3, 'Verdana',
      10, clBlack, true);
    auxlinha := auxlinha + 7;
    compara;
  end;
  ...
  begin
    ...
    begin
      atraso := DaysBetween(DMMACS.TALX.fieldByName('DTVENC').AsDateTime,
        Date());
      Imprime(RVRel, auxlinha, 10, 'Número da parcela: ' +
  ...
  // - 19/02/2009: busca todas as informações das PARCELAS do cliente para comparação de inadimplencia
  ...
  begin
    ...
    begin
      ...
      if DMMACS.TLoja.FieldByName('DIASCARE').AsInteger > 0
        {// - 02/03/20029: compara em loja os dias de carencia para controle de inadimplência} then
      begin
    ...
    atraso := (atraso * 100);
    aux := (atraso / aux);
    // - 20/02/2009: recebe o calculo da porcentagem de inadimplência nas parcelas ja pagas
    FormatCurr('0.00', aux);
...
begin
  ...
  if tipo = 'P' {// - 20/02/2009: SE FOR INFORMAÇÕES DOS PEDIDOS} then
  begin
    if inf = 'Pedido' {// - 19/02/2009: se for informações sobre os pedidos} then
    begin
      ...
      begin
        ...
        begin
          ...
          x3 := FloatToStr(StrToFloat(FormatFloat('0.00',
            DMMACS.TALX.FieldByName('TOTAL').AsCurrency /
            DMMACS.TALX.FieldByName('CONT').AsInteger)));
          // - 19/02/2009: média do valor dos pedidos
        end;
      end;
    end
    else if inf = 'Maior'
      {// - 19/02/2009: se for informações da maior e menor pedido de venda} then
    begin
      ...
      {// - 19/02/2009: se for informações da primeiro e ultimo pedido de venda} then
    begin
  ...
  else if tipo = 'O' {// - 20/02/2009: SE FOR INFORMAÇÕES DAS ORDENS} then
  begin
    if inf = 'Ordem'
      {// - 20/02/2009: se for informações sobre as ordens do cliente} then
    begin
      ...
      begin
        ...
        // - 20/02/2009: média do valor das ordens
      end;
    end
    else if inf = 'Maior'
      {// - 20/02/2009: se for informações sobre as ordens do cliente} then
  ...
  else if tipo = 'C' {// - 20/02/2009: SE FOR INFORMAÇÕES DAS CONTAS A RECEBER} then
  begin
    if inf = 'ParcVencidas'
      {// - 19/02/2009: se for informações das parcelas} then
    begin
      ...
      {// - 19/02/2009: se for informações gerais de todas parcelas vencidas} then
    begin
    ...
    begin
      ...
      {// - 19/02/2009: se for informações gerais de todas parcelas pagas} then
    begin
    ...
    {// - 20/02/2009: SE FOR RELACIONADO A ITENS DE PEDIDOS E SERVIÇOS DE ORDENS} then
  begin
    ...
    begin
      ...
      begin
        ...
        while not DMFINANC.TAlx.Eof do
          // - 20/02/2009: enquanto não for o fim dos pedidos
        begin
          DMESTOQUE.Alx.Close;
          DMESTOQUE.Alx.SQL.Clear;
    ...
    begin
      ...
      begin
        ...
        DMServ.WOrdem.First;
        // - 20/02/2009: volta ao primeiro registro para calcular os serviços

        serv := 0;
        // - 20/02/2009: zera controle de quantidade de serviços prestados
        somaserv := 0;
        // - 20/02/2009: zera controle da soma dos serviços prestados

        while not DMServ.WOrdem.Eof do
          // - 20/02/2009: enquanto não for o fim dos pedidos
        begin
          if DMMACS.TLoja.FieldByName('ATIVIDADE').AsString <> 'EQUIPE'
            {// - 20/02/2009: se não for a mercescan} then
          begin
            DMESTOQUE.Alx.Close;
            DMESTOQUE.Alx.SQL.Clear;
          ...
          begin // - 20/02/2009: se for a mercescan busca o valor de serviços na tabela de despesas
            DMESTOQUE.Alx.Close;
            DMESTOQUE.Alx.SQL.Clear;
...
begin
  ...
  lbCreditoCli.Caption := 'Crédito por Antecipações: ' + FormatFloat('0.00',
    ...
    begin
      ...
      begin
        if RGPessoa.ItemIndex = 1
          {// - 20/05/2009: se forem só clientes q pagam em dia} then
        begin
          DMESTOQUE.Alx1.Next;
        end
        else
...
//Edmar - 08/06/2015 - Foi alterado a forma de marcação e desmarcação do cliente
  try
    ...
    ShowMessage('Não foi possível desmarcar o Cliente ' +
...
begin
  ...
  try
    ...
    begin
      MessageDlg('Por favor, informe a descrição da tabela de preço', mtWarning,
        [mbOK], 0);
...
begin
  ...
  begin
    ...
    //2,8 corresponde a o espaçamento entre as etiquetas, ja contando com o espaço entre as linhas
