...
uses
  ...
  RpRender, RpRenderRTF, cxCalendar;

type
  TFOrdemGrafica = class(TFPadrao)
    PConsulta: TPanel;
    ...
    TabSheet3: TTabSheet;
    BitBtn8: TBitBtn;
    EdDtAbertura: TDateTimePicker;
    EdDtPrevisao: TDateTimePicker;
    MDetalhamentoProd: TMemo;
    Label4: TLabel;
    EdDescricaoDespesa: TEdit;
    Label6: TLabel;
    EdTotalDespesa: TColorEditFloat;
    Label7: TLabel;
    EdQuantidadeDespesa: TColorEditFloat;
    Label8: TLabel;
    EdVlrUnitDespesa: TColorEditFloat;
    BitBtn2: TBitBtn;
    BitBtn3: TBitBtn;
    ...
    ComprovanteemMeiaPagina1: TMenuItem;
    FrmObra: TFrmBusca;
    ...
    procedure EdDepartamentoKeyPress(Sender: TObject; var Key: Char);
    ...
    procedure EdDepartamentoEnter(Sender: TObject);
    procedure EdDepartamentoExit(Sender: TObject);
    ...
    procedure RvOrdem1BeforePrint(Sender: TObject);

    procedure RvOrdem1Print(Sender: TObject);
    ...
    procedure FrmAtendenteEDCodigoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure FrmresponsavelEDCodigoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EdDescricaoDespesaKeyPress(Sender: TObject; var Key: Char);
    procedure EdVlrUnitDespesaKeyPress(Sender: TObject; var Key: Char);
    procedure EdQuantidadeDespesaExit(Sender: TObject);
    procedure EdQuantidadeDespesaKeyPress(Sender: TObject; var Key: Char);
    procedure BitBtn2Click(Sender: TObject);
    procedure BitBtn3Click(Sender: TObject);
    procedure Cartadecorreoeletrnica1Click(Sender: TObject);
    procedure ComprovanteemMeiaPagina1Click(Sender: TObject);
    procedure FrmDepartamentoBTNOPENClick(Sender: TObject);
  ...
  private
    ...
    //LIMPA O CAMPO DE DESEPESAS
	 procedure LimpaCamposDesp;
   ...
   procedure RemoverProduto;
	//Remove o item adicional de grafica e imprime no memo o seu texto
	procedure RemoverItemAdicionalGrafica;
   // remover serviço selecionado
   procedure RemoverServico;
   // remover despesas selecionado
   procedure RemoverDespesas;
   // - 19/02/2009: impressão do rave (componente, linha, coluna, texto, fonte, tamanho, cor)
   ...
   // Procedure para preencher a combo de impressar disponiveis
   Procedure SelecionaImpressoras;
   ...
   //Imprime pelo rave report o comprovante de abertura numero 01
   Procedure ImpCompAbert1;
	//Imprime pelo rave report o comprovante de abertura numero 02
	Procedure ImpCompAbert2;
   //Imprime Cabeçalho do relatorio de comprovante 01
   Procedure ImpCompAbert1Cabecalho;
	//Imprime Cabeçalho do relatorio de comprovante 02
	Procedure ImpCompAbert2Cabecalho;
   ...
   //Imprime Cabeçalho do relatorio de comprovante 01
   Procedure ImpCompAbert1FiltraDados;  
   //Imprime dados de produtos comprovante 01
   Procedure ImpCompAbert1Produtos;
   //Imprime dados de servicos comprovante 01
   Procedure ImpCompAbert1Servicos;
   //Imprime dados de despesas comprovante 01
   Procedure ImpCompAbert1Despesas;
   //Imprime dados de rodape comprovante 01
   Procedure ImpCompAbert1Rodape;
   //Impressão de comprovante em meia pagina
	Procedure ImpCompMeiaPagina;
	//Jônatas 24/06/2013 - Imprime cabeçalho do comprovante de Meia página
	//Pode pegar os dados do ImpCompAbert1FiltraDados
	//Pode imprimir a imagem pelo PrintImage
	Procedure ImpCompMeiaPagina_Cabecalho;
	//Jônatas 24/06/2013 - Imprime produtos do comprovante de Meia página
	Procedure ImpCompMeiaPagina_Produtos;
	//Jônatas 24/06/2013 - Imprime serviços do comprovante de Meia página
	Procedure ImpCompMeiaPagina_Servicos;
	//Jônatas 24/06/2013 - Imprime despesas do comprovante de Meia página
	Procedure ImpCompMeiaPagina_Despesas;
	//Jônatas 24/06/2013 - Imprime o rodapé do comprovante de Meia página
	Procedure ImpCompMeiaPagina_Rodape;
  public
...
var
  FOrdemGrafica: TFOrdemGrafica;

  //VARIAVEIS RESPONSÁVEIS POR ARMAZENAR OS VALORES DAS CHAVES PRIMÁRIAS E ESTRANGEIRAS UTILIZADAS NA ORDEM
  XCOD_ORDEM, XCOD_CLIENTE, XCOD_EQUIPAMENTO, XCOD_CONTABANCO, XCOD_VENDEDOR, XCOD_ATENDENTE, XCOD_RESPONSAVEL, XCOD_FUNCIONARIO, XCOD_FORMPAG, XCOD_CONTA, XCOD_CONTASERV: INTEGER;
  ...
  XVLRUNITVISTA, XVLRTVISTA, XVLRTOTAL, XVLRTPROD, XVLRTSERV, XVLRDESP: REAL;// ARMARZENA VALOR TOTAL DO PEDIDO
...
//   XSQLTABELA: String; // Nome da tabela no BD para consultas Sql
   ...
   XCod_PedCompra, XCod_PedVendaDev, XCod_FornecedorDev, XCodSlaveDev, XCOD_FISCAL, XCOD_OBRA: Integer; //ALEX : 19/01/09 - Variaveis auxiliares para controle de devolução de ordem de serviço
   ...
   XImprimeMaisProdutos, XImprimeMaisServicos, XImprimeMaisDespesas, XNovaPagina: Boolean;//Variaveis para controlar a imressão de produtos,serviços e despesas no comprovante, caso algum destes seja true, significa que temos que puxar nova pagina e continuar impressão destes
   XQuantProdutosImprimir, XQuantServicosImp, XquantDespImprimir: Smallint;// Variaveis para controlar a quantidade de cada item a ser impresso, necessário para podermos controlar corretamente o avanço de paginas

	XTipoComp : String;//Jônatas 24/06/2013 - controle do tipo de comprovante para o comprovante de meia página
	XNumVia : Smallint;//Jônatas 24/06/2013 - para a exibição do número da via no comprovante demeia página
implementation
...
//Jônatas 24/06/2013 - Imprime o rodapé do comprovante de Meia página
Procedure TFOrdemGrafica.ImpCompMeiaPagina_Rodape;

Var
	i: Smallint;
	XLinhaImpObs: Integer;
	TamTexto: Integer;
Begin
 Try
       XLinImp:=XLinImp+2;
       //Imprime cabeçalho dos itens
       //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
       IF XNumVia <> 1 then
          XLinImp:=XLinImp+4;
       Imprime(RvOrdem1,XLinImp,5,'----------------------------------------------------------------------------------------------------------------------------','Verdana',12,clBlack, false);
       XLinImp:=XLinImp+4;
       XLinhaImpObs:=XLinImp;
       MObs.Lines.Text:=DMServ.TOrd.FieldByName('observacao').AsString;
       If MObs.Text<>''
       Then Begin
        //imprime as linhas da observação
           Imprime(RvOrdem1, XLinhaImpObs,8, 'Observação:' ,'Verdana',10,clBlack, True);
           XLinhaImpObs:=XLinhaImpObs+3;
       End;
       i:=0;
       //Jônatas 28/06/2013 - pega o numero de caracteres do texto para controle da impressão da linha
       TamTexto:=MObs.GetTextLen;
       while i<TamTexto do
       Begin
            Imprime(RvOrdem1, XLinhaImpObs, 10, copy(MObs.Lines.Text,0,65) ,'Verdana',6,clBlack, false);
            XLinhaImpObs:=XLinhaImpObs+3;
            //limitar o número de linhas para não atrapalhar a impressão
            If XLinhaImpObs>=117 then
              break;
            //Jônats 28/06/2013 - remove do texto a ser impresso o que já foi impresso
            MObs.Lines.Text:=copy(MObs.Lines.Text,66,MObs.GetTextLen);
            i:=i+66;
       End;

       //Nome do venddor e a forma de pagamento
       Imprime(RvOrdem1, XLinImp,103, 'Vendedor: '+DMPESSOA.VWFuncionario.FieldByName('NOME').AsString ,'Verdana', 8,clBlack, false);
       //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
       IF XNumVia = 1 then
          Imprime(RvOrdem1, XLinImp,163, 'Forma Pagto: '+DMFINANC.TFormPag.FieldByName('descricao').Asstring ,'Verdana', 8,clBlack, false);
       XLinImp:=XLinImp+4;
       //Condição do pagamento e o Sub total
       //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
       IF XNumVia = 1
       then begin
            Imprime(RvOrdem1, XLinImp,103, 'Cond. Pagto: '+DMServ.TOrd.FieldByName('tipopag').AsString ,'Verdana', 8,clBlack, false);
            Imprime(RvOrdem1, XLinImp,163, 'Sub Total: '+FormatFloat('0.00', XVLRTOTAL) ,'Verdana', 9,clBlack, true);
            XLinImp:=XLinImp+4;
       end;
       //Previsão de Entrega e desconto
       Imprime(RvOrdem1, XLinImp,103, 'Previsão Ent.: '+DMServ.TOrd.FieldByName('dtprev').AsString ,'Verdana', 8,clBlack, false);
       //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
       IF XNumVia = 1
       then
            Imprime(RvOrdem1, XLinImp,163, 'Desc.: '+FormatFloat('0.00', DMServ.TOrd.FieldByName('DESCONTO').AsCurrency)+'%'  ,'Verdana', 8,clBlack, false);
       XLinImp:=XLinImp+4;
       //Responsável e Total da ordem de serviço
       Imprime(RvOrdem1, XLinImp, 103, 'Responsável: '+DMMACS.TALX.FieldByName('NOME').AsString ,'Verdana', 8,clBlack, false);
       //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
       IF XNumVia = 1
       then
           Imprime(RvOrdem1, XLinImp,163, 'Total Ordem: '+FormatFloat('0.00', DMServ.TOrd.FieldByName('totord').AsCurrency) ,'Verdana', 9,clBlack, true);

  //If XLinhaImpObs>XLinImp Then
        //XLinImp:=XLinhaImpObs;

       XLinImp:=XLinImp+15;
       //Imprime cabeçalho dos itens
       // imprimir linhas para assinatura
       Imprime(RvOrdem1,XLinImp,5,'     ________________________________________                  ________________________________________     ','Verdana',10,clBlack, false);
       XLinImp:=XLinImp+4;
       // imprimir descrição para assinatura
       Imprime(RvOrdem1,XLinImp,5,'                   CLIENTE/RESPONSÁVEL                                                          CONFERENTE                   ','Verdana',10,clBlack, false);
       XLinImp:=XLinImp+3;
       //linha pra dividir o final do rodapé
       Imprime(RvOrdem1,XLinImp,5,'__________________________________________________________________________________________________','Verdana',10,clBlack, false);
       XLinImp:=XLinImp+3;
       //mensagem
       Imprime(RvOrdem1,XLinImp,8,'Autorizo a produção e/ou confecção dos serviços e produtos acima discriminados, de acordo com o prazo de entrega e pagamento estipulados nesta ','Verdana',7,clBlack, false);
       XLinImp:=XLinImp+3;
       //mensagem
       Imprime(RvOrdem1,XLinImp,8,'Ordem de Serviços e responsabilizo-me sobre os mesmos. ','Verdana',7,clBlack, false);
       XLinImp:=XLinImp+2;
       //fim do rodapé
       Imprime(RvOrdem1,XLinImp,5,'__________________________________________________________________________________________________','Verdana',10,clBlack, false);
...
End;
//Jônatas 24/06/2013 - Imprime despesas do comprovante de Meia página
Procedure TFOrdemGrafica.ImpCompMeiaPagina_Despesas;
...
Begin
 Try
    ...
    If not DMESTOQUE.Alx4.IsEmpty
       Then Begin
           //Imprime cabeçalho dos itens
         Imprime(RvOrdem1,XLinImp,5,'-------------------------------------------------------------------------------------------------------------------------------------','Verdana',10,clBlack, false);
         XLinImp:=XLinImp+3;
       End;
       XFlag:=0;
       While (not DMESTOQUE.Alx4.Eof) and (XFlag=0) do
       Begin
           //dados das despesas
           Imprime(RvOrdem1, XLinImp, 8, DMESTOQUE.Alx4.fieldByName('despesa').AsString ,'Verdana', 7,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, 93, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('QTD').AsCurrency) ,'Verdana', 7,clBlack, true);
           //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
           IF XNumVia = 1
           Then Begin
               Imprime(RvOrdem1, XLinImp, 123, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('vlrfinal').AsCurrency) ,'Verdana', 7,clBlack, true);
               Imprime(RvOrdem1, XLinImp, 153, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 7,clBlack, true);
               Imprime(RvOrdem1, XLinImp, 178, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('VLRTOTFIN').AsCurrency) ,'Verdana', 7,clBlack, true);
           End;
           DMESTOQUE.Alx4.Next;
           XquantDespImprimir:=XquantDespImprimir-1;
           //Jônatas 28/06/2013 - a pedido, limite de linhas da primeira via
           IF XNumVia = 1
           Then Begin
               If XLinImp>=93
               Then Begin
                XFlag:=1;
                   XImprimeMaisDespesas:=True;
                   XNovaPagina:=True;
               End;
           End
           Else Begin//vias 2 e 3
               If XLinImp>=96
               Then Begin
                XFlag:=1;
                   XImprimeMaisDespesas:=True;
           ...
           XLinImp:=XLinImp+3;
       End;
       If XquantDespImprimir=0 Then
        XImprimeMaisDespesas:=False;
...
End;
//Jônatas 24/06/2013 - Imprime serviços do comprovante de Meia página
Procedure TFOrdemGrafica.ImpCompMeiaPagina_Servicos;
 ...
 Try
    If not DMESTOQUE.TDesp.IsEmpty
       Then Begin
         ...
         XLinImp:=XLinImp+3;
       End;
       While (not DMESTOQUE.TDesp.Eof) and (XFlag=0) do
       Begin
           //dados das despesas
           Imprime(RvOrdem1, XLinImp, 8, DMESTOQUE.TDesp.fieldByName('despesa').AsString ,'Verdana',7,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, 93, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('QTD').AsCurrency) ,'Verdana', 7,clBlack, true);
           //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
           IF XNumVia = 1
           Then Begin
               Imprime(RvOrdem1, XLinImp, 123, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('vlrfinal').AsCurrency) ,'Verdana', 7,clBlack, true);
               Imprime(RvOrdem1, XLinImp, 153, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 7,clBlack, true);
               Imprime(RvOrdem1, XLinImp, 178, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('VLRTOTFIN').AsCurrency) ,'Verdana', 7,clBlack, true);
           End;
           DMESTOQUE.TDesp.Next;
           XQuantServicosImp:=XQuantServicosImp-1;
           //Jônatas 28/06/2013 - a pedido, limite de linhas da primeira via
           IF XNumVia = 1
           Then Begin
               If XLinImp>=93
           ...
           XLinImp:=XLinImp+4;
       End;
       If XQuantServicosImp=0 Then
        XImprimeMaisServicos:=False;
...
End;
//Jônatas 24/06/2013 - Imprime produtos do comprovante de Meia página
Procedure TFOrdemGrafica.ImpCompMeiaPagina_Produtos;
...
Begin
 Try
       //Jônatas 25/06/2013 - Dá pra fazer alterações em todas a funções
       If XQuantProdutosImprimir>0
       Then Begin
           XFlag:=0;
           While (not DMESTOQUE.TSlave.Eof) and (XFlag=0) do
           Begin
               XLinImp:=XLinImp+1;
               //dados dos produtos
               Imprime(RvOrdem1, XLinImp, 8, DMESTOQUE.TSlave.fieldByName('DESCRICAO').AsString ,'Verdana', 7,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp, 93, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('QTD').AsCurrency) ,'Verdana', 7,clBlack, true);
               //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
               IF XNumVia = 1
               Then Begin
                   Imprime(RvOrdem1, XLinImp, 123, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('VLRUNIT').AsCurrency) ,'Verdana', 7,clBlack, true);
                   Imprime(RvOrdem1, XLinImp, 153, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 7,clBlack, true);
                   Imprime(RvOrdem1, XLinImp, 178, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('TOTAL').AsCurrency) ,'Verdana', 7,clBlack, true);
               End;
               If Trim(DMESTOQUE.TSlave.fieldByName('DESCGRAFICA').AsString)<>''
               Then Begin
                   MObs.Lines.Text:=DMESTOQUE.TSlave.fieldByName('DESCGRAFICA').AsString;
                   XLinImp:=XLinImp+3;
                   i:=0;
                   //Jônatas 28/06/2013 - pega o numero de caracteres do texto para controle da impressão da linha
                   TamTexto:=MObs.GetTextLen;
                   while i<TamTexto do
                   Begin
                        Imprime(RvOrdem1, XLinImp, 15, copy(MObs.Lines.Text,0,135) ,'Verdana',6,clBlack, false);
                        XLinImp:=XLinImp+3;
                        //Jônats 28/06/2013 - remove do texto a ser impresso o que já foi impresso
                        MObs.Lines.Text:=copy(MObs.Lines.Text,136,MObs.GetTextLen);
                        i:=i+136;
                   End;
                    //Substituído
                   {
                   for i := 0 to MObs.Lines.Count -1 do
                   Begin
                       //imprime as linhas de observação
                       Imprime(RvOrdem1, XLinImp, 18, MObs.Lines[i] ,'Verdana',8,clBlack, false);
                       XLinImp:=XLinImp+3;
                   End;}
               End
               Else Begin
...
//Pode pegar os dados do ImpCompAbert1FiltraDados
//Pode imprimir a imagem pelo PrintImage
Procedure TFOrdemGrafica.ImpCompMeiaPagina_Cabecalho;
...
Begin
     Try
       ...
       try
        If FileExists('logoEmpresa.bmp')
           Then Begin
               ...
               PrintImage(RvOrdem1, XLinImp, 8, 30, 30,  Bitmap);// via 1
           End;
       finally
         //  FreeAndNil(Bitmap);
       end;
       XLinImp:=XLinImp+10;
  //dados da empresa
       //Telefone e número da ordem de serviço
       Imprime(RvOrdem1, XLinImp,53, 'Fone: '+DMMACS.TEmpresa.fieldByName('fone').AsString + ' - '+ DMMACS.TEmpresa.fieldByName('tel0800').AsString ,'Verdana',12,clBlack, true);
       Imprime(RvOrdem1,XLinImp,153,'OS núm. ['+DMServ.TOrd.FieldByName('NUMERO').AsString+']','Verdana',10,clBlack, True);
  XLinImp:=XLinImp+4;
       //Endereço e data da impressão
       Imprime(RvOrdem1, XLinImp,53, 'Endereço: '+DMMACS.TEmpresa.fieldByName('ENDERECO').AsString + ', '+ DMMACS.TEmpresa.fieldByName('NUMERO').AsString+' / '+DMMACS.TEmpresa.fieldByName('BAIRRO').AsString+ ' : ' +DMMACS.TEmpresa.fieldByName('CEP').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,153, 'Imp.: '+DateToStr(Date()) + ' - ' + TimeToStr(Time()) ,'Verdana',8,clBlack, false);
  XLinImp:=XLinImp+4;
       //Cidade, Estado e Data de Abertura
       Imprime(RvOrdem1, XLinImp,53, DMGEOGRAFIA.TCidade.fieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,153, 'Dt. Abertura: '+DMServ.TOrd.FieldByName('dtabert').AsString ,'Verdana',8,clBlack, false);
  XLinImp:=XLinImp+4;
       //Data de Fechamento
       Imprime(RvOrdem1, XLinImp,153, 'Dt. Fechamento: '+DMServ.TOrd.FieldByName('dtfech').AsString ,'Verdana',6,clBlack, false);
  XLinImp:=XLinImp+4;
       //Status
       Imprime(RvOrdem1, XLinImp,153, 'Status: '+DMServ.TOrd.FieldByName('status').AsString ,'Verdana',8,clBlack, True);
       XLinImp:=XLinImp+4;
       //Número da via e da página
       Imprime(RvOrdem1, XLinImp,153, 'Via: [0'+IntToStr(XNumVia)+'] -' ,'Verdana',8,clBlack, True);
       Imprime(RvOrdem1, XLinImp,172, 'Pagina: ['+IntToStr(XNumPagina)+']' ,'Verdana',8,clBlack, True);
  XLinImp:=XLinImp+6;
       //divisão para mostrar os dados do cliente
       Imprime(RvOrdem1, XLinImp,5,'----------------------------------------------------------------------------------------------------------------------------','Verdana',10,clBlack, false);
       XLinImp:=XLinImp+2;
       //dados do cliente
       //Nome e CPF ou CNPJ
       Imprime(RvOrdem1, XLinImp,8, 'Cliente: ' +DMPESSOA.TCliente.fieldByName('COD_INTERNO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('NOME').AsString ,'Verdana',8,clBlack, true);
       Imprime(RvOrdem1, XLinImp,153, 'CPF/CNPJ: ' +DMPESSOA.TPessoa.fieldByName('CPFCNPJ').AsString ,'Verdana',8,clBlack, false);
       XLinImp:=XLinImp+4;
       //Endereço e CEP
       Imprime(RvOrdem1, XLinImp,8, 'Endereço: ' +DMPESSOA.TPessoa.fieldByName('ENDERECO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('BAIRRO').AsString,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,153, 'CEP: ' +DMPESSOA.TPessoa.fieldByName('CEP').AsString,'Verdana',8,clBlack, false);
       XLinImp:=XLinImp+4;
       //Cidade, Estado e Telefone
       Imprime(RvOrdem1, XLinImp,8, 'Cidade: ' +DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,153, 'Fone: ' +DMPESSOA.TPessoa.fieldByName('TELREL').AsString ,'Verdana',8,clBlack, true);
       XLinImp:=XLinImp+4;
       //Contato
       Imprime(RvOrdem1, XLinImp,8, 'Contato: ' + DMServ.TOrd.FieldByName('contato').AsString,'Verdana',8,clBlack, false);
  XLinImp:=XLinImp+2;
       //Imprime cabeçalho dos itens
       ...
       //Jônatas 28/06/2013 - a pedido, mostrar valores somente na primeira via
       IF XNumVia = 1 then
          Imprime(RvOrdem1, XLinImp,8, 'Produtos/Serviços/Despesas                             Quantidade         Vlr. Unitário          Desc.              Vlr. Total' ,'Verdana', 9,clBlack, false)
       Else
          Imprime(RvOrdem1, XLinImp,8, 'Produtos/Serviços/Despesas                             Quantidade' ,'Verdana', 9,clBlack, false);
       XLinImp:=XLinImp+3;
   ...
   End;
End;
Procedure TFOrdemGrafica.ImpCompMeiaPagina;

Var Looping: Boolean;

Begin
     Try
         //Chama o filtradados do outro comprovante pois será a mesma coisa
         ImpCompAbert1FiltraDados;
         XNumPagina:=1;
         XLinImp:=8;
         //Define as variaveis de impressão como true para poder imprimir pela primeira vez
         XImprimeMaisProdutos:=True;
         XImprimeMaisServicos:=True;
         XImprimeMaisDespesas:=True;

         XNovaPagina:=False;
         //controle para a continuidade da impressão
         Looping:=True;
         //for XNumVia from 1 to 3 do
         For XNumVia := 1 to 3 do
         Begin
               While Looping = true do
               Begin
                    ImpCompMeiaPagina_Cabecalho;
                 If XNovaPagina=False Then
                     ImpCompMeiaPagina_Produtos;
                 If XNovaPagina=False Then
                     ImpCompMeiaPagina_Servicos;
                 If XNovaPagina=False Then
                     ImpCompMeiaPagina_Despesas;
                 XLinImp:=96;
                 ImpCompMeiaPagina_Rodape;
                 //Jônatas 25/06/2013 - troquei as condições para o looping ser falso pois atualizava o
                 //numero de páginas sem nem entrar nas outras funções pois ainda
                 //não tinha chegado na sua execução
                 If (XQuantProdutosImprimir=0) and (XQuantServicosImp=0) and (XquantDespImprimir=0) Then
                     Looping:=False;

                 If (XNovaPagina=True) and (Looping=True)
                 Then Begin
                     XNumPagina:=XNumPagina+1;
                     RvOrdem1.BaseReport.NewPage;
                     XLinImp:=8;
                     XNovaPagina:=false;
                 End;
               End;
               //Inicia a contagem de páginas novamente
               XNumPagina:=1;
               //vai para a nova página a não ser que seja a última
               If XNumVia <> 3
               Then Begin
                   RvOrdem1.BaseReport.NewPage;
                   XLinImp:=8;
                   XNovaPagina:=false;
                   //chama o filtra dados pra começar novamente e "resetar" as quantidades
                   ImpCompAbert1FiltraDados;
                   //seta a necessidade de imprimir produtos, serviços e despesas pra true
                   XImprimeMaisProdutos:=True;
                   XImprimeMaisServicos:=True;
                   XImprimeMaisDespesas:=True;
                   //seta o controle de cotinuidade pra true
                   Looping:=true;
               End;
         End;
     Except
...
End;
//Função utilizada para retornar o numero corrente para o pedido e incremantar na empresa
Function TFOrdemGrafica.RetornaNumPed: Integer;
Var
	XFlag: Integer;
Begin
   Result:=-1;
...
End;
//Função Auxiliar para Devolução de Nota Fiscal - Verifica Configurações necessárias para devolução
Function TFOrdemGrafica.DevNotaFiscal_VerificaConfigs: Boolean;
Begin
	Result:=True;
   Try
...
End;
//Função Auxiliar para Devolução de Nota Fiscal - Tentar registrar o cliente da nota fiscal de saída como fornecedor da NF de entrada
Function TFOrdemGrafica.DevNotaFiscal_SetaClienteFornecedor: Boolean;
Begin
	Result:=True;
   Try
...
End;
//Função Auxiliar para Devolução de Nota Fiscal - Efetua Estorno do Financeiro
Function TFOrdemGrafica.DevNotaFiscal_EstornoFinanceiro: Boolean;
Begin
	Result:=True;
   Try
...
//Filtra Tabela Slave e calcula total
Procedure TFOrdemGrafica.FiltraSlaveDev(XCod_PedSlave: Integer);
Begin
...
End;
//Função Auxiliar para Devolução de Nota Fiscal - Gera Pedido de compra e lançao itens
Function TFOrdemGrafica.DevNotaFiscal_EfetuaDevolucao: Boolean;
Var
	XProxPed: Integer;
Begin
	Result:=True;
...
//Função Auxiliar para Devolução de Nota Fiscal - Fecha o pedido e seta dados do fiscal
Function TFOrdemGrafica.DevNotaFiscal_SetaFiscal: Boolean;
Begin
	Result:=True;
   Try
...
//Função Auxiliar para Devolução de Nota Fiscal - Fecha o pedido e seta dados do fiscal
Function TFOrdemGrafica.DevNotaFiscal_EmiteFiscal: Boolean;
Begin
	Result:=True;
   Try
   	//Confirma alterações
...
//Função que efetua a devolução total de Nota Fiscal Normal e Nota Fiscal Eletronica, emitindo as Notas Fiscais de entrada Correspondente
Function TFOrdemGrafica.DevNotaFiscal(XCod_PedDev: Integer): Boolean;
Begin
	Result:=True;
   Try
...
//Função utilizada para buscar o cliente atravez do nome o cpfcnpj
Function TFOrdemGrafica.BuscaCliente(XTipo: String; XValor: String):Boolean;
Begin
	//Limpa SQl
   DMPESSOA.TALX.Close;
...
//Função utilizada para buscar o veiculo do cliente atravez da placa
Function TFOrdemGrafica.BuscaPlaca(XValor: String):Boolean;
Begin
End;

//Função Para calcular desconto da ordem
Function TFOrdemGrafica.EfetuaDesconto(OrigemDesconto: String; TipoDesconto: String): Boolean;
Begin
...
End;
//Função Utilizada para gerar a tela do fiscal a partir da tela de consulta
Function TFOrdemGrafica.AbreFiscal: Boolean;
Begin
	Result:=True;
   Try
...
//Paulo 19/04/2012: Função Utilizada para gerar a tela do fiscal para serviços a partir da tela de consulta
Function TFOrdemGrafica.AbreFiscalServicos: Boolean;
Begin
	Result:=True;
   Try
		DMServ.WOrdem.FieldByName('COD_CLIENTE').AsInteger;
...
//Gera documento fiscal para a ordem
Function TFOrdemGrafica.GeraFiscal: Boolean;
Begin
   ...
   If (XCOD_VENDEDOR=-1)
   Then Begin
       Mensagem('OPÇÃO BLOQUEADA', 'Por Favor. Informe o vendedor', '', 1, 1, False, 'a');
...
function TFOrdemGrafica.verificaPessoa(msg:Boolean):Boolean;
Begin
End;

//Djonathan 23/11/2009 : Funcao que retorna o minimo e o maximo de uma tabela
Function TFOrdemGrafica.EncontraValores(XSqlTabelaA: String; XSQLTabelaAux: String; XInicial: String; XFinal: String; XDesc: String): Integer;
...
function TFOrdemGrafica.ComparaValores(XSqlTabelaA: String;XCodigoB: String; XCount: Integer): Boolean;
begin
		DMESTOQUE.Alx.Close;
   	DMESTOQUE.Alx.SQL.Clear;
...
function TFOrdemGrafica.ComparaValoresInternos(XSqlTabelaA: String;XView: String;XCodigoV: String;XCodigoB: String;XCount: Integer): Boolean;
begin
   	DMESTOQUE.Alx.SQL.Clear;
		DMESTOQUE.Alx.Close;
...
Function TFOrdemGrafica.DevTotPed(XCod_PedDev: Integer): Boolean;
Var
	XTotPedDev: Real;
Begin
	Try
...
procedure TFOrdemGrafica.limpar(pessoa,equipamento:Boolean);
Begin
End;

// form activate
procedure TFOrdemGrafica.FormActivate(Sender: TObject);
begin
  inherited;
   ...
   Else Begin
       ...
       End;
	End;
   If FileExists('C:\MZR\Arquivos\LayoutGrid\Ordemgraf.lgm')
   Then
       DBGridConsulta.Columns.LoadFromFile('C:\MZR\Arquivos\LayoutGrid\Ordemgraf.lgm');
   ...
   begin
       DBGridConsulta.Top := 133;
       DBGridConsulta.Height := 472;
       BtnSelecionar.Visible := True;
   end
   else begin//form de cadastro
       DBGridConsulta.Top := 108;
       DBGridConsulta.Height := 494;
       BtnSelecionar.Visible := False;
   end;

end;

procedure TFOrdemGrafica.AtualizaGridConsulta(FILTRO :STRING);
begin
   ...
   DMServ.WOrdem.SQL.Add(' WHERE (COD_LOJA='+FMenu.LCODLOJA.Caption+')');

   DMServ.WOrdem.SQL.Add(' AND COD_ORDEM IS NOT NULL ');

   //FILTRO PARA EDIT'S
   if FILTRO = 'NUMERO' THEN
       DMServ.WOrdem.SQL.Add(' AND vwordem.numero = '+#39+TRIM(EDNum.Text)+#39)
   ELSE
   IF FILTRO = 'DEPARTAMENTO' THEN
       DMServ.WOrdem.SQL.Add(' AND (upper (vwordem.EQUIPAMENTO)) like (upper ('+#39+EdDepartamento.Text+'%'+#39+'))')
   ELSE
   ...
   IF FILTRO = '' THEN BEGIN
       ...
       If CBFILTRO.Text='ABERTAS' Then
           DMServ.WOrdem.SQL.Add(' AND (STATUS='+#39+'ABERTO'+#39+')')
       else
       If CBFILTRO.Text='FECHADAS' Then
           DMServ.WOrdem.SQL.Add(' AND (STATUS='+#39+'FECHADO'+#39+')')
       else
       If CBFILTRO.Text='TERMINADAS' Then
           DMServ.WOrdem.SQL.Add(' AND (STATUS='+#39+'TERMINADO'+#39+')')
       Else
       If CBFILTRO.Text='NFE2' Then
           DMServ.WOrdem.SQL.Add(' AND (FISCO='+#39+'NFE2'+#39+')')
       Else
       If CBFILTRO.Text='CANCELADO' Then
           DMServ.WOrdem.SQL.Add(' AND (STATUS='+#39+'CANCELADO'+#39+')');

   END;

   //ORDENAÇÃO COMBO
	If CBORDEM.Text='NUMERO'Then //ordenar pelo número
   	DMServ.WOrdem.SQL.Add(' ORDER BY NUMERO, STATUS DESC, BOX, DTABERT DESC')
	else
   If CBORDEM.Text='CLIENTE'Then //ordenar pelo cliente
   	DMServ.WOrdem.SQL.Add(' ORDER BY CLIENTE, STATUS DESC,BOX,DTABERT DESC')
	else
   If CBORDEM.Text='DATA'Then //ordenar pela data
   	DMServ.WOrdem.SQL.Add(' ORDER BY DTABERT DESC, STATUS DESC, BOX')
   else
   If CBORDEM.Text='DEPARTAMENTO'Then //ordenar pela data
   	DMServ.WOrdem.SQL.Add(' ORDER BY EQUIPAMENTO, DTABERT DESC, STATUS DESC, BOX')
   else
   If CBORDEM.Text = 'BOX' Then  //ordenar pelo box
       DMServ.WOrdem.SQL.Add(' ORDER BY BOX, PLACA, STATUS DESC, STATUS, DTABERT DESC');
   If CBORDEM.Text = 'FISCAL' Then  //ordenar pelo box
       DMServ.WOrdem.SQL.Add(' ORDER BY numfiscal, PLACA, STATUS DESC, STATUS, DTABERT DESC');
...
procedure TFOrdemGrafica.CBFILTROPropertiesChange(Sender: TObject);
begin
  inherited;
end;
...
procedure TFOrdemGrafica.DBGridConsultaDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  inherited;
...
procedure TFOrdemGrafica.EDNumKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TFOrdemGrafica.EDBoxKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
end;
//PRESS ENTER CAMPO PLACA LOCALIZAR
procedure TFOrdemGrafica.EdDepartamentoKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   ...
   then
       ...
       if  Length(EdDepartamento.Text) < 4 then
   ...
   if key = #13
   then BEGIN
       AtualizaGridConsulta('DEPARTAMENTO');
       //PesquisaEquipCliente(EdDepartamento.Text);
       EdDepartamento.SelectAll;
   END;

  if key = #27
   then begin
      CBORDEM.Text := 'DEPARTAMENTO';
      AtualizaGridConsulta('');
      //CBORDEM.Text := 'BOX';
      EdDepartamento.SelectAll;
   end;
...
procedure TFOrdemGrafica.EDCliente1KeyPress(Sender: TObject; var Key: Char);
var
   pos:integer;
begin
  inherited;
...
procedure TFOrdemGrafica.EDNumEnter(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDNumExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EdDepartamentoEnter(Sender: TObject);
begin
  inherited;
   if EdDepartamento.Text = 'DEPARTAMENTO' then
        EdDepartamento.Text := ''
   else
    EdDepartamento.SelectAll;

   EdDepartamento.Font.Color := clBlack;
end;

procedure TFOrdemGrafica.EdDepartamentoExit(Sender: TObject);
begin
  inherited;
   if Trim(EdDepartamento.Text) = '' then
        EdDepartamento.Text := 'DEPARTAMENTO';

   EdDepartamento.Font.Color := clGray;
end;

procedure TFOrdemGrafica.EDCliente1Enter(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDCliente1Exit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.BtnNovoClick(Sender: TObject);
begin
   DMMACS.Transaction.CommitRetaining;
  ...
  //inherited;
   ...
   //Prepara combo de impressoras
   SelecionaImpressoras;
   ...
   Try
       ...
       EdDtAbertura.DateTime:=Date();
       DMServ.TOrd.FieldByName('dtentrega').AsDateTime:=Date();
       EdDtPrevisao.DateTime:=Date();
       ...
       If DMPESSOA.TALX.IsEmpty Then
           FiltraTabela(DMPESSOA.TALX, 'vwfuncionario', '', '', '');

       XCOD_VENDEDOR:=DMPESSOA.TALX.FieldByName('COD_FUNC').AsInteger;
       XCOD_FUNCIONARIO:=DMPESSOA.TALX.FieldByName('COD_FUNC').AsInteger;
       XCOD_ATENDENTE:=DMPESSOA.TALX.FieldByName('COD_FUNC').AsInteger;
       XCOD_RESPONSAVEL:=DMPESSOA.TALX.FieldByName('COD_FUNC').AsInteger;

       FrmVendedor.EDCodigo.Text:=DMPESSOA.TALX.FieldByName('COD_INTERNO').AsString;
       FrmVendedor.EdDescricao.Text:=DMPESSOA.TALX.FieldByName('NOME').AsString;
       FrmVendedor.Repaint;

       Frmresponsavel.EDCodigo.Text:=DMPESSOA.TALX.FieldByName('COD_INTERNO').AsString;
       Frmresponsavel.EdDescricao.Text:=DMPESSOA.TALX.FieldByName('NOME').AsString;

       FrmAtendente.EDCodigo.Text:=DMPESSOA.TALX.FieldByName('COD_INTERNO').AsString;
       FrmAtendente.EdDescricao.Text:=DMPESSOA.TALX.FieldByName('NOME').AsString;
...
Procedure TFOrdemGrafica.LimpaCampos;
begin
...
procedure TFOrdemGrafica.LimpaCamposProd;
begin
   ...
   LProduto.Caption:='';
   LCst.Caption:='';
   MDetalhamentoProd.Lines.Clear;
end;

procedure TFOrdemGrafica.LimpaCamposServ;
begin
...
procedure TFOrdemGrafica.LimpaCamposDesp;
begin
   EdTotalDespesa.Text := '0,00';
   EdVlrUnitDespesa.Text := '0,00';
   EdTotDesp.Text := '0,00';
   EdQuantidadeDespesa.Text := '0,00';
   EdDescricaoDespesa.Clear;
end;


//press key codigo do cliente
procedure TFOrdemGrafica.FrmClienteEDCodigoKeyDown(Sender: TObject;
...
// press botao cliente
procedure TFOrdemGrafica.FrmClienteBTNOPENClick(Sender: TObject);
begin
 // inherited;
...
procedure TFOrdemGrafica.PesquisaCodigoEquipamento(codigo:String; cliente:boolean);
var
   Tmp:Boolean;
begin
   // letras maiusculas
   ...
   Then Begin
       ...
       DMESTOQUE.Alx.SQL.Add('where upper(equipamento.placa) like upper ('+#39+codigo+#39+')  AND (EQUIPAMEnTO.ATIVO='+#39+'1'+#39+')');
       DMESTOQUE.Alx.Open;
       DMESTOQUE.Alx.first;
       if not DMESTOQUE.alx.IsEmpty
       then begin
           XCOD_EQUIPAMENTO:=DMESTOQUE.Alx.FieldByName('COD_EQUIPAMENTO').AsInteger;
           FrmVendedor.EdDescricao.Text:=DMESTOQUE.Alx.FieldByName('DESCRICAO').AsString;
           if cliente
           then begin
       ...
       Else Begin
          XCOD_EQUIPAMENTO := -1;
          FrmVendedor.EdDescricao.Text := '';
          FrmVendedor.EDCodigo.SelectAll;
       End;
   FrmCliente.Repaint;
   FrmEquipe.Repaint;
   FrmVendedor.Repaint;
...
procedure TFOrdemGrafica.FBEquipPlacaEDCodigoExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.FBEquipPlacaEDCodigoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
end;

// click botao equipamento
procedure TFOrdemGrafica.FBEquipPlacaBTNOPENClick(Sender: TObject);
begin
  inherited;
   ...
   Then Begin
       XCOD_ATENDENTE:=DMPESSOA.VWFuncionario.FieldByName('cod_func').AsInteger;
       FrmAtendente.EDCodigo.Text := DMPESSOA.VWFuncionario.FieldByName('COD_INTERNO').AsString;
       FrmAtendente.EDCodigo.Hint := DMPESSOA.VWFuncionario.FieldByName('NOME').AsString;
       FrmAtendente.EdDescricao.Text := DMPESSOA.VWFuncionario.FieldByName('NOME').AsString;
       FrmAtendente.EDCodigo.SetFocus;
   End
   Else Begin
       XCOD_ATENDENTE:=-1;
       FrmAtendente.EDCodigo.Text:='';
       FrmAtendente.EDCodigo.Hint := '';
       FrmAtendente.EdDescricao.Text:='';
   End;
   FrmAtendente.Repaint;
end;

//pesquisa pelo codigo da forma de pagamento
Procedure TFOrdemGrafica.PesquisaCodigoPagamento(codigo:String);
begin
   // letras maiusculas
   codigo:= Trim(UpperCase(codigo));
...
procedure TFOrdemGrafica.FrmFormPag1EDCodigoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  If Key=VK_RETURN
...
procedure TFOrdemGrafica.FrmFormPag1BTNOPENClick(Sender: TObject);
begin
  inherited;
...
// exit codigo equipamento
procedure TFOrdemGrafica.FrmFormPag1EDCodigoExit(Sender: TObject);
begin
  inherited;
   if FrmFormPag1.EDCodigo.Text<>''
     Then Begin
...
//seleciona forma de pagamento e vendedor de acordo com o cliente
Procedure TFOrdemGrafica.SelecionaFormaPagamento;
Begin
...
procedure TFOrdemGrafica.FBEquipPlacaEDCodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
end;
//click limpar forma de pagamento
procedure TFOrdemGrafica.FrmFormPag1BTNMINUSClick(Sender: TObject);
begin
  inherited;
...
//click botao produto
procedure TFOrdemGrafica.BtnProcProdClick(Sender: TObject);
var
  aux:String;
begin
  inherited;
...
//Filtra as tabelas e escreve em edits e labels as informações dos produtos selecionado
Procedure TFOrdemGrafica.EscreveLabelsPROD;
Begin
...
//opções de pesquisa para o produto
procedure TFOrdemGrafica.TipoPesquisaProduto(tipo:String);
begin
...
procedure TFOrdemGrafica.CdigodeBarra1Click(Sender: TObject);
begin
  inherited;
       TipoPesquisaProduto('BARRA');
end;

procedure TFOrdemGrafica.ControleInterno1Click(Sender: TObject);
begin
  inherited;
   TipoPesquisaProduto('');
end;

procedure TFOrdemGrafica.CdigoFabricante1Click(Sender: TObject);
begin
  inherited;
   TipoPesquisaProduto('FABRICANTE');
end;

procedure TFOrdemGrafica.Descrio1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDCodFabKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
   str : String;
begin
   ...
   Then Begin
       ...
       Else Begin
           ...
           Then begin
           	...
           	FiltraTabela(DMESTOQUE.WSimilar, 'VWSIMILAR', 'COD_LOJA', '', ' (COD_LOJA='+FMenu.LCODLOJA.Caption+') AND (upper(CODFABRICANTE) like upper('+#39+EDCodFab.Text+'%'+#39+')) ORDER BY DESCRICAO');
           End
           Else Begin
           	FiltraTabela(DMESTOQUE.WSimilar, 'VWSIMILAR', 'COD_LOJA', '', ' (COD_LOJA='+FMenu.LCODLOJA.Caption+') AND (upper(DESCRICAO) like upper('+#39+EDCodFab.Text+'%'+#39+')) ORDER BY DESCRICAO');
           End;
       End;
   End;
  end;
...
procedure TFOrdemGrafica.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFOrdemGrafica.EDQuantidadeExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDDescKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TFOrdemGrafica.EDQuantidadeKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   if Key = #13 then
       EDDesc.SetFocus;
...
procedure TFOrdemGrafica.EDDescExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDValUnitExit(Sender: TObject);
begin
  inherited;
   If (EDQuantidade.ValueNumeric<>0) And (EDValUnit.ValueNumeric<>0)
   Then Begin
...
procedure TFOrdemGrafica.EDValUnitKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   if key = #13 then
       EDTotal.SetFocus;
...
procedure TFOrdemGrafica.EDTotalKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   If Key=#13
   Then Begin
...
procedure TFOrdemGrafica.EDTotalExit(Sender: TObject);
begin
  inherited;
   CalcPercent(0, (EDQuantidade.ValueNumeric*EDValUnit.ValueNumeric)- EDTotal.ValueNumeric, (EDQuantidade.ValueNumeric*EDValUnit.ValueNumeric), 'D');
	EDDesc.ValueNumeric:=AlxPercento;
...
procedure TFOrdemGrafica.BtnInsertProdClick(Sender: TObject);
Var
...
begin
  ...
  { // verifica se jah foi selecionado o equipamento
   ...
   Then Begin
       Mensagem('   A T E N Ç Ã O   ','Selecione um Equipamento antes de prosseguir.','',1,1,false,'a');
   ...
   End;
     Try
       ...
       MDO.QConsulta.SQL.Add('select estoque.cod_estoque, subproduto.cod_cst, estoque.cod_subprod, subproduto.cod_subproduto, subproduto.quantgarantia, subproduto.tipogarantia');
       MDO.QConsulta.SQL.Add('from estoque left join subproduto on estoque.cod_subprod = subproduto.cod_subproduto');
       MDO.QConsulta.SQL.Add('where estoque.cod_estoque = :codigo');
       MDO.QConsulta.ParamByName('codigo').AsInteger := DMEstoque.WSimilar.FieldByName('COD_ESTOQUE').AsInteger;
       MDO.QConsulta.Open;
       ...
       If ( MDO.QConsulta.FieldByname('quantgarantia').AsString <> '') and ( MDO.QConsulta.FieldByname('tipogarantia').AsString <> '') and (SubstituiCaracter(MDO.QConsulta.FieldByname('tipogarantia').AsString,' ','') <> 'Hs')
       Then Begin
           // função para retornar a data de termino da garantia
           XDtGarantia:= DateToStr(Calcula_Garantia(MDO.QConsulta.FieldByname('quantgarantia').AsString,MDO.QConsulta.FieldByname('tipogarantia').AsString));
       End;
       // verifica se no produto foi lancado algum tipo de garantia. O tipo de garantia deve ser em horas
       If (MDO.QConsulta.FieldByname('quantgarantia').AsString <> '') and (MDO.QConsulta.FieldByname('tipogarantia').AsString <> '') and ((SubstituiCaracter(MDO.QConsulta.FieldByname('tipogarantia').AsString,' ','') = 'Hs') or (SubstituiCaracter(MDO.QConsulta.FieldByname('tipogarantia').AsString,' ','') = 'Km'))
       Then Begin
           // função para retornar a data de termino da garantia
            XDtGarantia:= '01/01/1900';
       End;
       If (SubstituiCaracter(MDO.QConsulta.FieldByname('tipogarantia').AsString,' ','') = 'Km')
       Then Begin
           ...
           XQtdGarantia := DMESTOQUE.Alx.FieldByname('KMATUAL').AsInteger + MDO.QConsulta.FieldByname('quantgarantia').AsInteger;
       End;
   Except
       XQtdGarantia:=0;
       XDtGarantia:='';
   ...
   Try
       ...
       DMSERV.TPOrd.ParamByName('COD_CST').AsInteger := MDO.QConsulta.FieldByName('COD_CST').AsInteger;
       ...
       Try
           ...
           DMServ.IBT.CommitRetaining;
           Try
           	//Insere descritivo para o item
               If MDetalhamentoProd.Text<>''
               Then Begin
                   ...
                   MDO.Query.Close;
                   MDO.Query.SQL.Clear;
                   MDO.Query.SQL.Add(' INSERT INTO IT_GRAFICA (COD_ITPRODORDEM, DESCRICAO ) VALUES (:COD_ITPRODORDEM, :DESCRICAO ) ');
                   MDO.Query.ParamByName('COD_ITPRODORDEM').AsString:=MDO.QConsulta.FieldByName('codigo').AsString;
                   MDO.Query.ParamByName('DESCRICAO').AsString:=MDetalhamentoProd.Text;
                   MDO.Query.ExecSQL;
...
//FILTRA OS REGISTROS DE PRODUTOS E SERVIÇOS PERTENCENTES A ORDEM
Procedure TFOrdemGrafica.FiltraSlave;
var
	XDESCMOEDA: real;
Begin
   XVLRTVISTA:=0;
   ...
   //CALCULA TOTAL
	DMESTOQUE.TSlave.Close;
	DMESTOQUE.TSlave.SQL.Clear;
	DMESTOQUE.TSlave.SQL.Add('Select count(itprodord.cod_itprodord) as QuantProdutos, Sum(ITPRODORD.total) as TOTAL From ITPRODORD');
	DMESTOQUE.TSlave.SQL.Add('Where ITPRODORD.COD_ORDEM = :CODORDEM ');
	DMESTOQUE.TSlave.ParamByName('CODORDEM').AsInteger:=XCOD_ORDEM;
	DMESTOQUE.TSlave.Open;

   XQuantProdutosImprimir:=DMESTOQUE.TSlave.FieldByName('QuantProdutos').AsInteger;
	If XCONTROLECONSULTA = False Then
	...
	DMESTOQUE.TSlave.SQL.Add(' itprodord.baseicms, itprodord.vlricms, itprodord.aliqicms, cst.cod_sit_trib, itprodord.descnf as DESCONTONF, it_grafica.descricao as DESCGRAFICA, ');
   DMESTOQUE.TSlave.sql.Add(' subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL1').AsString+' AS PRIMCOD, subproduto.'+DMMACS.TLoja.FieldByName('CODPRODREL2').AsString+' AS SEGCOD ');
	DMESTOQUE.TSlave.SQL.Add(' From itprodord left Join estoque on itprodord.cod_estoque = estoque.cod_estoque ');
	DMESTOQUE.TSlave.SQL.Add(' Left Join subproduto on estoque.cod_subprod = subproduto.cod_subproduto ');
	DMESTOQUE.TSlave.SQL.Add(' left join it_grafica on itprodord.cod_itprodord = it_grafica.cod_itprodordem ');
   ...
   DMESTOQUE.Alx2.Close;
   DMESTOQUE.Alx2.SQL.Clear;
   DMESTOQUE.Alx2.SQL.Add('select  count(despadic.cod_despadic) as QuantServicos, Sum(despadic.vlrtotfin) AS VLRTOTFIN From despadic Where (despadic.gerador='+#39+'ORDEM'+#39+') AND (despadic.cod_gerador=:COD_GERADOR)');
   DMESTOQUE.Alx2.ParamByName('COD_GERADOR').AsInteger:=XCOD_ORDEM;
   DMESTOQUE.Alx2.Open;
   XQuantServicosImp:=DMESTOQUE.Alx2.FieldByName('QuantServicos').AsInteger;
...
{   DMESTOQUE.TDesp.Close;
   ...
   DMESTOQUE.TDesp.SQL.Add('despadic.cod_funcionario, despadic.cfop, DESPADIC.COMISSAO, DESPADIC.DESCNF ');
   ...
   //TRABALHANDO COM DESPESAS ADICIONAIS
   DMESTOQUE.Alx2.Close;
   DMESTOQUE.Alx2.SQL.Clear;
   DMESTOQUE.Alx2.SQL.Add('select count(despadic.cod_despadic) as QuantDespesas, Sum(despadic.vlrtotfin) AS VLRTOTFIN From despadic Where (despadic.gerador='+#39+'ORDEMD'+#39+') AND (despadic.cod_gerador=:COD_GERADOR)');
   DMESTOQUE.Alx2.ParamByName('COD_GERADOR').AsInteger:=XCOD_ORDEM;
   DMESTOQUE.Alx2.Open;
   XquantDespImprimir:=DMESTOQUE.Alx2.FieldByName('QuantDespesas').AsInteger;

  	EdTotDesp.ValueNumeric:=DMESTOQUE.Alx2.FieldByName('VLRTOTFIN').AsCurrency;
   EdTotDesp.Repaint;
   XVLRDESP:=DMESTOQUE.Alx2.FieldByName('VLRTOTFIN').AsCurrency;

   DMESTOQUE.Alx4.Close;
   DMESTOQUE.Alx4.SQL.Clear;
   DMESTOQUE.Alx4.SQL.Add('select * from despadic');
   DMESTOQUE.Alx4.SQL.Add('where (despadic.cod_gerador= :cod) and (despadic.gerador='+#39+'ORDEMD'+#39+')');
   DMESTOQUE.Alx4.SQL.Add('order by despadic.cod_despadic desc');
   DMESTOQUE.Alx4.ParamByName('cod').AsInteger:=XCOD_ORDEM;
   DMESTOQUE.Alx4.Open;

	XCONTROLECONSULTA := False;
   //ATRIBUI TOTAL VENDIDO SEM DESCONTO À VARIAVEL
   XVLRTOTAL:=XVLRTPROD+XVLRTSERV+XVLRDESP;
End;

// função para calcular data de garantia
function TFOrdemGrafica.Calcula_Garantia(Qtde, Tipo: string): TDateTime;
begin
...
procedure TFOrdemGrafica.BtnDeleteProdClick(Sender: TObject);
begin
  inherited;
   ...
   Then Begin//caso a tabela esteja vasia não pode apagar
       ...
       then begin
         ...
         //Verifica os dados adicionais (texto complementar dos itens e apaga
         RemoverItemAdicionalGrafica;
         //remove o produto e atualiza estoque
         RemoverProduto;
...
procedure TFOrdemGrafica.RemoverTudo1Click(Sender: TObject);
begin
  inherited;
...
//Remove o item adicional de grafica e imprime no memo o seu texto
procedure TFOrdemGrafica.RemoverItemAdicionalGrafica;
begin
	Try
       MDO.QConsulta.Close;
       MDO.QConsulta.SQL.Clear;
       MDO.QConsulta.SQL.Add(' Select * from it_grafica Where it_grafica.cod_itprodordem=:CodigoItem ');
       MDO.QConsulta.ParamByName('CodigoItem').AsInteger:=DMESTOQUE.TSlave.FieldByName('cod_itprodord').AsInteger;
       MDO.QConsulta.Open;
       If not MDO.QConsulta.IsEmpty
       Then Begin
       	MDetalhamentoProd.Text:=MDO.QConsulta.FieldByName('DESCRICAO').AsString;
           MDO.Query.Close;
           MDO.Query.SQL.Clear;
           MDO.Query.SQL.Add('  delete from it_grafica Where it_grafica.cod_itprodordem=:CodigoItem  ');
       	MDO.Query.ParamByName('CodigoItem').AsInteger:=DMESTOQUE.TSlave.FieldByName('cod_itprodord').AsInteger;
           MDO.Query.ExecSQL;
           Try
           	MDO.Transac.CommitRetaining;
           Except
...
//remover produto selecionado
procedure TFOrdemGrafica.RemoverProduto;
var
...
procedure TFOrdemGrafica.EDesricaoServicoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
   if key = #13 then
...
procedure TFOrdemGrafica.EValorServKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if key = #13 then begin
       EQtdServ.Text := '1,00';
...
procedure TFOrdemGrafica.EValorServExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EQtdServKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   if Key = #13 then
       EDescServ.SetFocus;
...
procedure TFOrdemGrafica.EDescServExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDescServKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   if key = #13 then begin
       ETotalServItem.ValueNumeric:=(EValorServ.ValueNumeric*EQtdServ.ValueNumeric);
 ...
 //on exit qnt serviço
procedure TFOrdemGrafica.EQtdServExit(Sender: TObject);
begin
  inherited;
   ETotalServItem.ValueNumeric:=(EValorServ.ValueNumeric*EQtdServ.ValueNumeric);
end;
...
//pesquisa codigo equipe
Procedure TFOrdemGrafica.PesquisaCodigoEquipe(codigo:String);
begin
   // letras maiusculas
   codigo:= Trim(UpperCase(codigo));
...
procedure TFOrdemGrafica.FrmEquipeEDCodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TFOrdemGrafica.FrmEquipeEDCodigoExit(Sender: TObject);
begin
  inherited;
   if FrmEquipe.EDCodigo.Text<>''
     Then Begin
 ...
 // click botao equipe 
procedure TFOrdemGrafica.FrmEquipeBTNOPENClick(Sender: TObject);
begin
  inherited;
 ...
 //btn inserir serviço
procedure TFOrdemGrafica.BtnInserirClick(Sender: TObject);
var
...
begin
  inherited;
   ...
   If (EQtdServ.ValueNumeric = 0) and (EValorServ.Text <> '0,00')
   Then Begin
  ...
  //Paulo 30/06/2011: Não deixa inserir um serviço com o valor zerado
   ...
   Try
       ...
       XCOD_ITDESP := DMESTOQUE.TDesp.FieldByName('COD_DESPADIC').AsInteger;
   ...
   End;
      ...
      { DMESTOQUE.TDesp.Close;
       DMESTOQUE.TDesp.SQL.Clear;
       DMESTOQUE.TDesp.SQL.Add('insert into DESPADIC');
       DMESTOQUE.TDesp.SQL.Add('(COD_GERADOR, GERADOR, DESPESA, QTD, VLRREAL, VLRFINAL,');
       DMESTOQUE.TDesp.SQL.Add('LUCMOE, LUCPER, VLRTOTFIN, DESCONTO, MARCA, UNIDADE, COD_EQUIPE, COD_FUNCIONARIO)');
       ...
       FiltraTabela(DMESTOQUE.TDesp, 'DESPADIC', '', '', '');
       ...
       DMESTOQUE.TDesp.SQL.Add('LUCMOE=:LUCMOE, LUCPER=:LUCPER, VLRTOTFIN=:VLRTOTFIN, DESCONTO=:DESCONTO, MARCA=:MARCA, UNIDADE=:UNIDADE, COD_EQUIPE=:COD_EQUIPE, ');
       DMESTOQUE.TDesp.SQL.Add('COD_FUNCIONARIO=:COD_FUNCIONARIO, CST= :CST, COMISSAO= :COMISSAO');
       ...
       DMESTOQUE.TDesp.ParamByName('DESPESA').AsString := EDesricaoServico.Text;
       DMESTOQUE.TDesp.ParamByName('QTD').AsCurrency := EQtdServ.ValueNumeric;
       //DMESTOQUE.TDesp.ParamByName('VLRREAL').AsCurrency := ;
       DMESTOQUE.TDesp.ParamByName('VLRFINAL').AsCurrency := EValorServ.ValueNumeric;
       //DMESTOQUE.TDesp.ParamByName('LUCMOE').AsCurrency := ;
       //DMESTOQUE.TDesp.ParamByName('LUCPER').AsCurrency := ;
       DMESTOQUE.TDesp.ParamByName('VLRTOTFIN').AsCurrency := ETotalServItem.ValueNumeric;
       DMESTOQUE.TDesp.ParamByName('DESCONTO').AsCurrency := EDescServ.ValueNumeric;
       //DMESTOQUE.TDesp.ParamByName('MARCA').AsString := ;
       //DMESTOQUE.TDesp.ParamByName('UNIDADE').AsString := ;
       DMESTOQUE.TDesp.ParamByName('COD_EQUIPE').AsString := FrmEquipe.EDCodigo.Text;
       DMESTOQUE.TDesp.ParamByName('COD_FUNCIONARIO').AsInteger := XCod_usuario;
       DMESTOQUE.TDesp.ParamByName('CST').AsString := '041';
       DMESTOQUE.TDesp.ParamByName('COMISSAO').AsFloat := StrToFloat(lbComissaoServ.Caption);
       //Paulo 31/10/2011: Para identificar se o serviço é de terceiros ou não
       If (cbTerceiros.Checked=true) Then
           DMESTOQUE.TDesp.ParamByName('UNIDADE').AsString := '1'
       else
           DMESTOQUE.TDesp.ParamByName('UNIDADE').AsString := '0';
       DMESTOQUE.TDesp.ParamByName('VLRREAL').AsCurrency := edCustoTerceiros.ValueNumeric;
...

{
   ...
   //XCOD_ITDESP := DMESTOQUE.TDesp.FieldByName('COD_DESPADIC').AsInteger;
   //DMESTOQUE.TDesp.FieldByName('COD_DESPADIC').AsInteger:=XCOD_ITDESP;
   DMESTOQUE.TDesp.FieldByName('DESPESA').AsString:=EDesricaoServico.Text;
   DMESTOQUE.TDesp.FieldByName('QTD').AssTRING:=EQtdServ.Text;
   DMESTOQUE.TDesp.FieldByName('VLRFINAL').AsCurrency:=EValorServ.ValueNumeric;
   DMESTOQUE.TDesp.FieldByName('VLRTOTFIN').AsCurrency:=ETotalServItem.ValueNumeric;
   DMESTOQUE.TDesp.FieldByName('DESCONTO').Asstring:=EDescServ.text;
   ...
   DMESTOQUE.TDesp.FieldByName('COD_FUNCIONARIO').AsInteger := XCod_usuario;   }
...
//click botao remover
procedure TFOrdemGrafica.BBExcluirClick(Sender: TObject);
begin
  inherited;
      DMESTOQUE.Alx1.Close;
      DMESTOQUE.Alx1.SQL.Clear;
...
// remover serviço selecionado
procedure TFOrdemGrafica.RemoverServico;
var
...
// remover despesas selecionado
procedure TFOrdemGrafica.RemoverDespesas;
var
   XAPAGA: Integer; //controla para que sejama apagados todos os serviços da ordem exportada
begin
  inherited;
   ...
   then begin
      ...
      DMESTOQUE.Alx.SQL.Add('and despadic.cod_despadic= :cod2');
      DMESTOQUE.Alx.ParamByName('cod1').AsInteger:=XCOD_ORDEM;
      DMESTOQUE.Alx.ParamByName('cod2').AsInteger:=DMESTOQUE.Alx4.fieldbyname('COD_DESPADIC').AsInteger;
      DMESTOQUE.Alx.ExecSQL;
      DMESTOQUE.TransacEstoque.CommitRetaining;
      FiltraSlave;
  end
...
procedure TFOrdemGrafica.RemoverTudo2Click(Sender: TObject);
begin
  inherited;
...
//click botao gravar
procedure TFOrdemGrafica.BtnGravarClick(Sender: TObject);
Var
...
begin
  inherited;
   ...
   If (XCOD_ATENDENTE=-1) or (FrmAtendente.EDCodigo.Text = '')
   Then Begin
       Mensagem('OPÇÃO BLOQUEADA', 'Por Favor. Informe o atendente', '', 1, 1, False, 'a');
       FrmAtendente.EDCodigo.SetFocus;
       Exit;
   End;

   If (XCOD_RESPONSAVEL=-1) or (Frmresponsavel.EDCodigo.Text = '')
   Then Begin
       Mensagem('OPÇÃO BLOQUEADA', 'Por Favor. Informe o resposável', '', 1, 1, False, 'a');
   ...
   //INFORMA CODIGO DO USUARIO
   DMServ.TOrd.FieldByName('COD_USUARIO').AsInteger:=StrToInt(FMenu.LCODUSUARIO.Caption);
   //INFORMA CODIGO DO ATENDENTE
   DMServ.TOrd.FieldByName('COD_EQUIPAMENTO').AsInteger:=XCOD_ATENDENTE;
   //INFORMA CODIGO DO VENDEDOR
   DMServ.TOrd.FieldByName('COD_VENDEDOR').AsInteger:=XCOD_VENDEDOR;
   //INFORMA CODIGO DO RESPONSÁVEL
   DMServ.TOrd.FieldByName('COD_FUNCIONARIO').AsInteger:=XCOD_RESPONSAVEL;
   ...
   DMServ.TOrd.FieldByName('COD_FORMPAG').AsInteger:=XCOD_FORMPAG;
   DMServ.TOrd.FieldByName('observacao').AsString:=MObs.Lines.Text;
   //DMServ.TOrd.FieldByName('cod_equipamento').AsInteger:=XCOD_OBRA;
   //GRAVA TOTAIS
   DMServ.TOrd.FieldByName('TOTORD').AsCurrency:=EdValorOrd.ValueNumeric;
   DMServ.TOrd.FieldByName('TOTPROD').AsCurrency:=EdtotalProd.ValueNumeric;
   DMServ.TOrd.FieldByName('TOTSERV').AsCurrency:=EdTotalserv.ValueNumeric;
   ...
   DMServ.TOrd.FieldByName('DESCPROD').AsCurrency:=EdDescTProd.ValueNumeric;
   DMServ.TOrd.FieldByName('DESCSERV').AsCurrency:=EdDescTServ.ValueNumeric;
   DMServ.TOrd.FieldByName('IMPRESSORA').AsString:=CbImpressora.Text;
   DMServ.TOrd.FieldByName('dtabert').AsDateTime:=EdDtAbertura.Date;
   DMServ.TOrd.FieldByName('dtprev').AsDateTime:=EdDtPrevisao.Date;
...
//click em cancelar
procedure TFOrdemGrafica.BtnCancelarClick(Sender: TObject);
begin
    FMenu.XTIPOBOLTO:='';
    FiltraTabela(DMServ.TOrd, 'ORDEM', 'COD_ORDEM', IntToStr(XCOD_ORDEM), '');
    FiltraSlave;
    // se nao possui servico e nem produtos eh exibido a mensagem - SEGUNDO SOLICITAÇÃO É OBRIGATORIO PODER GRAVAR SEM SERVICO
    If (DMESTOQUE.Alx4.IsEmpty) AND (DMESTOQUE.TSlave.IsEmpty) AND (DMESTOQUE.alx3.IsEmpty)
    Then Begin
       If Mensagem('   A T E N Ç Ã O   ','A Ordem de Serviço atual não possui produtos e serviços inclusos. Deseja apagá-la ?','',2,3,false,'A')=2
       Then Begin
           //Apaga ORDEM
...
//click fiscal
procedure TFOrdemGrafica.BtnDadosAdicClick(Sender: TObject);
begin
  inherited;
  GeraFiscal;
end;
//click botao apagar
procedure TFOrdemGrafica.BtnApagarClick(Sender: TObject);
begin
  inherited;
   //EFETUA CONTROLE DE ACESSO
...
procedure TFOrdemGrafica.BtnConsultarClick(Sender: TObject);
begin
   ...
   End;
       ...
       Then Begin
          ...
          End;
          //ATENDENTE
          If FiltraTabela(DMMACS.TALX, 'vwfuncionario', 'cod_func', DMServ.TOrd.FieldByName('COD_EQUIPAMENTO').AsString, '')=True
          Then Begin
               XCOD_ATENDENTE:=DMMACS.TALX.FieldByName('cod_func').AsInteger;
               FrmAtendente.EDCodigo.Text:=DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
               FrmAtendente.EdDescricao.Text:=DMMACS.TALX.FieldByName('nome').AsString;
          End
          Else Begin
               XCOD_ATENDENTE:=-1;
               FrmAtendente.EDCodigo.Text:='';
          ...
          End;
          //VENDEDOR
          If FiltraTabela(DMMACS.TALX, 'vwfuncionario', 'cod_func', DMServ.TOrd.FieldByName('COD_VENDEDOR').AsString, '')=True
          Then Begin
               XCOD_VENDEDOR:=DMMACS.TALX.FieldByName('cod_func').AsInteger;
               FrmVendedor.EDCodigo.Text:=DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
               FrmVendedor.EdDescricao.Text:=DMMACS.TALX.FieldByName('nome').AsString;
          End
          Else Begin
               xCOD_VENDEDOR:=-1;
               FrmVendedor.EDCodigo.Text:='';
          ...
          End;
          //RESPONSÁVEL
          If FiltraTabela(DMMACS.TALX, 'vwfuncionario', 'cod_func', DMServ.TOrd.FieldByName('COD_FUNCIONARIO').AsString, '')=True
          Then Begin
               XCOD_RESPONSAVEL:=DMMACS.TALX.FieldByName('cod_func').AsInteger;
               Frmresponsavel.EDCodigo.Text:=DMMACS.TALX.FieldByName('COD_INTERNO').AsString;
               Frmresponsavel.EdDescricao.Text:=DMMACS.TALX.FieldByName('nome').AsString;
          End
          Else Begin
               XCOD_RESPONSAVEL:=-1;
          ...
          EdValorOrd.ValueNumeric:=EdTotalOrd.ValueNumeric;
		   SelecionaImpressoras;
          CbImpressora.Text:=DMServ.TOrd.FieldByName('IMPRESSORA').AsString;
          EdDtAbertura.Date:=DMServ.TOrd.FieldByName('dtabert').AsDateTime;
          EdDtPrevisao.Date:=DMServ.TOrd.FieldByName('dtprev').AsDateTime;
          MObs.Lines.Text:=DMServ.TOrd.FieldByName('observacao').AsString;
...
procedure TFOrdemGrafica.EdDepartamentoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
end;
//modo pesquisa
procedure TFOrdemGrafica.SelecionarModoPesquisa(mostrar:Boolean; tipo:String);
begin
end;

procedure TFOrdemGrafica.PesquisaGrid(str:String);
begin
   if XTipoPes = 'CLIENTE'
   THEN BEGIN
       DMPESSOA.TALX.close;
...
procedure TFOrdemGrafica.EDCliente1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
   key2:Char;
   POS:INTEGER;
...
procedure TFOrdemGrafica.PPesquisaExit(Sender: TObject);
begin
  inherited;
   SelecionarModoPesquisa(false,'');
end;
function TFOrdemGrafica.ProxPesq:String;
begin
...
function TFOrdemGrafica.AntPesq:String;
begin
   Result := '';
   if XTipoPes = 'CLIENTE'
   THEN BEGIN
...
function TFOrdemGrafica.RetNomeGrid:String;
begin
   Result := '';
   if XTipoPes = 'CLIENTE'
   THEN BEGIN
...
procedure TFOrdemGrafica.EDCliente1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
end;
...
procedure TFOrdemGrafica.CBFILTROClick(Sender: TObject);
begin
  inherited;
   AtualizaGridConsulta('');
end;
//click em relatórios
procedure TFOrdemGrafica.BtnRelatorioClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.CompdeAbertura1Click(Sender: TObject);
Var
...
begin
  ...
  //EFETUA CONTROLE DE ACESSO
   ...
   //- 19/02/2009: dentro deste componente no evento OnPrint que será controlado o que será impresso no relatório
   // buscadados; //- 19/02/2009: procedimento que buscará todos os dados da empresa para serem impressas no relatório
   RvOrdem1.SystemPreview.FormState := wsMaximized;
   RvOrdem1.DefaultDest := rdPreview; //- 19/02/2009: visualizar o relatório direto
   RvOrdem1.SystemPrinter.Units := unMM; //- 19/02/2009: trabalha nos milimetros da página
   RvOrdem1.SystemPReview.RulerType := rtBothCm; //- 19/02/2009: mostra régua superior e lateral
   RvOrdem1.Execute;


end;
//click popup garantia
procedure TFOrdemGrafica.Garantia1Click(Sender: TObject);
begin
  inherited;
  // dados da loja
...
//click fiscal
procedure TFOrdemGrafica.BtnFiltrarClick(Sender: TObject);
var
...
begin
 ...
 //click botao fechar
procedure TFOrdemGrafica.BtnFechaPedClick(Sender: TObject);
begin
  inherited;
  //EFETUA CONTROLE DE ACESSO
...
// travar paines principais
procedure TFOrdemGrafica.TravarPaines(oi:boolean);
begin
...
procedure TFOrdemGrafica.FrmFormPagBTNOPENClick(Sender: TObject);
begin
  inherited;
   If AbrirForm(TFFormPag, FFormPag, 1)='Selected'
   Then Begin
...
procedure TFOrdemGrafica.FrmFormPagBTNMINUSClick(Sender: TObject);
begin
  inherited;
   XCOD_FORMPAG:=-1;
   FrmFormPag.EDCodigo.Text:='';
...
procedure TFOrdemGrafica.BtnMoedaCancelarClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.FrmFormPagEDCodigoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
   If Key=VK_RETURN
...
procedure TFOrdemGrafica.CBPagamentoExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.DBGCTAKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TFOrdemGrafica.FrmContaEDCodigoKeyPress(Sender: TObject;
  var Key: Char);
Var
	Oper:Char;
begin
...
procedure TFOrdemGrafica.FrmContaBTNOPENClick(Sender: TObject);
Var
	Oper: Char;
begin
  inherited;
...
procedure TFOrdemGrafica.FrmContaBTNMINUSClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.FrmContaServEDCodigoKeyPress(Sender: TObject;
  var Key: Char);
Var
	Oper:Char;
begin
...
procedure TFOrdemGrafica.FrmContaServBTNOPENClick(Sender: TObject);
   Var
	Oper: Char;
begin
  inherited;
...
procedure TFOrdemGrafica.FrmContaServBTNMINUSClick(Sender: TObject);
begin
  inherited;
   //Codigo da tabela local =0
   XCOD_CONTASERV:=-1;
...
procedure TFOrdemGrafica.BtnMoedaOKClick(Sender: TObject);
var
   XCodPedidoGerador :Integer; // - 16/03/2009: guarda codigo do pedido
   XFlag : Boolean; // - 13/03/2009: verifica se deve imprimir NF depois do cupom conforme configurado em caixa
   XNumAuxECF:String; // - 16/03/2009: armazena numero auxiliar cupom
...
Function TFOrdemGrafica.FechaOrd(CODPEDIDO: String; DTFECH: String) : Boolean;
Var
...
procedure TFOrdemGrafica.FrmFormPagExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.b(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  State: TOwnerDrawState);
begin
  inherited;
...
procedure TFOrdemGrafica.BtnCancelamentosClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.CancTotClick(Sender: TObject);
   Var
...
procedure TFOrdemGrafica.CancFechaClick(Sender: TObject);
Var
	XCodPedidoTroca: String;
begin
 // inherited;
...
//Função utilizada para cancelar/estornar todos os lançamentos emitidos por uma determinada OS
Function TFOrdemGrafica.CancelLanOrd(CodPedido: String):Boolean;
Var
...
procedure TFOrdemGrafica.ReeimprimirNF1Click(Sender: TObject);
begin
  inherited;
    //AbrirForm(TFSENHA, FSENHA, 0);
    //repassa resultado da tela de autenticação de senha para as variáveis de controle
...
procedure TFOrdemGrafica.ExportarPedido1Click(Sender: TObject);
begin
  inherited;
   inherited;
...
// exporta as ordens de servico selecionadas para outro banco, gerando novas ordens com os mesmos produtos, servicos, funcionario, equipe, etc
procedure TFOrdemGrafica.Exporta_OrdensServico;
var
       ...
       Begin
           ...
           Begin
               ...
               Try
                   ...
                   DMEXPORTA.TProduto.ParamByName('COD_CST').AsInteger := VerificaCST(DMESTOQUE.TProduto.FieldByName('COD_CST').AsInteger);
...
// Conecta banco de ddos para exportar Ordens
Function TFOrdemGrafica.ConectaBancoDados(XEndereco: string): boolean;
begin
   Try
...
// verifica se jah existe o funcionario no banco de fora, e se na estiver jah cadastra na hora
Function TFOrdemGrafica.VerificaFuncionario(Atividade: String; Cod_funcionario: string): integer;
var
...
// verifica se o cliente esta cadastrado no banco, se nao tiver jah cadastra na hora
Function TFOrdemGrafica.VerificaCliente(Cod_Cliente: integer): integer;
VAR
...
// verifica se existe a cidade do cliente, se nao jah cadastra
Function TFOrdemGrafica.VerificaCidade: integer;
begin
...
// verifica se existe o estado, se nao tiver jah cadastra, caso cntrario apenas recolhe os dados importantes
Function TFOrdemGrafica.VerificaEstado: integer;
BEGIN
...
// VERIFICA SE NO BANCO DE FORA ESTAH CADASTRADO O EQUIPAMENTO DA ORDEM, SE NAO TIVER JAH CADASTRA
function TFOrdemGrafica.VerificaEquipamento(Cod_equipamento: string): integer;
VAR
...
// busca forma de pagamento, se necessario cadastra a nova forma
Function TFOrdemGrafica.Forma_Pagamento: integer;
VAR
   XCOD_AUX :INTEGER;
begin
   Result:=-1;
...
// procura dados na tabela unidade tanto no banco local como o de fora e insere novos dads se necessario
Function TFOrdemGrafica.DadosUnidade: integer;
VAR
   XCOD_AUX :INTEGER;
begin
...
// VERIFICA CST PARA EXPORTAÇÃO
Function TFOrdemGrafica.VerificaCST(XCod_Cst: Integer): integer;
...
begin
     ...
     Then Begin
         ...
         Then Begin
         	  MessageDlg('Não encontrado cst: '+DMESTOQUE.TCST.FieldByName('descricao').AsString, mtWarning, [mbOK], 0);
             DMEXPORTA.TCst.Close;
             DMEXPORTA.TCst.SQL.Clear;
          ...
          End
          Else Begin
         	  MessageDlg('Encontrado cst: '+DMESTOQUE.TCST.FieldByName('descricao').AsString, mtWarning, [mbOK], 0);
          ...
          MessageDlg('Alterando cst de codigo: '+IntToStr(Result), mtWarning, [mbOK], 0);
          DMEXPORTA.TCst.ExecSQL;
          MessageDlg('Alterado com sucesso o codigo: '+IntToStr(Result), mtWarning, [mbOK], 0);
     End
     Else Begin
     		Result:=-1;
     End;
...
// VERIFICA SE NO BANCO DE FORA TEM A EQUIPE CADASTRADA JUNTO COM SEU RESPONSAVEL E FUNCIONARIO
function TFOrdemGrafica.VerificaEquipe(Cod_equipe: string; Cod_responsavel: string): integer;
VAR
   XCOD_AUX :INTEGER;
begin
	Try
...
// CADASTRO DE FUNCIONARIO
Function TFOrdemGrafica.CadastroFuncionario: integer;
var
...
// VERTIFICA A FORMA DE PAGAMENTO NO CADASTRO DO CLIENTE
Function TFOrdemGrafica.FormaPgto_Cliente: integer;
VAR
   XCOD_AUX :INTEGER;
begin
...
procedure TFOrdemGrafica.DBGridConsultaDblClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EdDescTProdExit(Sender: TObject);
begin
  inherited;
   {CalculaComissaoProd;
...
// Procedure para preencher a combo de impressar disponiveis
Procedure TFOrdemGrafica.SelecionaImpressoras;
begin
	Try
        DMMACS.Transaction.CommitRetaining;
        DMMACS.TALX.Close;
        DMMACS.TALX.SQL.Clear;
        DMMACS.TALX.SQL.Add(' select distinct(ordem.impressora) as impressoras from ordem ');
        DMMACS.TALX.Open;
        DMMACS.TALX.First;
        CbImpressora.Properties.Items.Clear;
        CbImpressora.Text:=DMMACS.TALX.FieldByName('impressoras').asstring;        
        While not DMMACS.TALX.Eof do
        Begin
           CbImpressora.Properties.Items.Add(DMMACS.TALX.FieldByName('impressoras').asstring);
           DMMACS.TALX.Next;
        End;
   Except
...
End;
// Quando eh informado o valor de desconto do total dos produtos em "%" eh efetuado calculo para cada produto da ordem
Procedure TFOrdemGrafica.CalcPercentProd;
begin
...
procedure TFOrdemGrafica.EdtotalProdExit(Sender: TObject);
begin
  inherited;
...
//FUNÇÃO QUE EFETUA CALCULO DE COMISSÃO BASEADO NO VALOR TOTAL DO PEDIDO E NÃO NA SOMA DO TOTAL DE PRODUTOS
Function TFOrdemGrafica.CalcComisTotVend: Boolean;
Begin
...
procedure TFOrdemGrafica.EdDescTServExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EdTotalservExit(Sender: TObject);
begin
  inherited;
...
//click filtro
procedure TFOrdemGrafica.cxGroupBox1Click(Sender: TObject);
begin
  inherited;
  AtualizaGridConsulta('ORIGEM');
end;
//CLICK ORDEM
procedure TFOrdemGrafica.cxGroupBox2Click(Sender: TObject);
begin
  inherited;
   AtualizaGridConsulta('ORIGEM');
end;
//ON EXIT CODIGO DO CLIENTE
procedure TFOrdemGrafica.FrmClienteEDCodigoExit(Sender: TObject);
begin
  inherited;
...
//verifa se nao ordem passada no parametro existem (produtos - prod / serviços - serv) conforme solicitado e retorna true caso encontrar
function TFOrdemGrafica.VerificaProdutosServicos(NumOrdem:Integer; prod:Boolean; serv:Boolean):Boolean;
begin
...
Function TFOrdemGrafica.AtualizarFuncionarioBancoExterno(codFunInt:integer;codFunExt:integer):Boolean;
var
...
procedure TFOrdemGrafica.CBFILTROExit(Sender: TObject);
begin
  inherited;
  AtualizaGridConsulta('');
end;
//CLOSE DO FORM
procedure TFOrdemGrafica.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  if XESTADO = 'INSERT'
  THEN BEGIN
...
//KEY PRESS NA GRID
procedure TFOrdemGrafica.DBGridConsultaKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if key = #27
...
procedure TFOrdemGrafica.PesquisaEquipCliente(codigo:String);
var
...
begin
   ...
   Then Begin
       ...
       DMESTOQUE.Alx.SQL.Clear;
       DMESTOQUE.Alx.SQL.Add('select * from equipamento');
       DMESTOQUE.Alx.SQL.Add('where (upper(equipamento.COD_EQUIPAMENTO) like upper ('+#39+codigo+#39+')) AND (EQUIPAMENTO.ATIVO='+#39+'1'+#39+')');
       DMESTOQUE.Alx.Open;
       if not DMESTOQUE.alx.IsEmpty
       then begin
...
procedure TFOrdemGrafica.Devoluo1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.Cancelar1Click(Sender: TObject);
Var
	XCodPedidoTroca: String;
   XCODLANEST: Integer;
   XQTDLOTE: Real;
...
procedure TFOrdemGrafica.CommesmoCliente1Click(Sender: TObject);
begin
    DMESTOQUE.Alx.Close;
    DMESTOQUE.Alx.SQL.Clear;
...
procedure TFOrdemGrafica.funcaocordup;
begin
  try
...
procedure TFOrdemGrafica.Verificar1Click(Sender: TObject);
begin
...
procedure TFOrdemGrafica.RelatriosdeOrdens1Click(Sender: TObject);
begin
...
function TFOrdemGrafica.VerificaPlacaAtiva(placa:String):Boolean;
begin
    Result := False;
...
procedure TFOrdemGrafica.EdFiscalEnter(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EdFiscalExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EdFiscalKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
   if key = #13
   THEN BEGIN
...
procedure TFOrdemGrafica.FrmCidadeBTNOPENClick(Sender: TObject);
begin
  inherited;
	 //LIBERA TODAS OS ESTADOS PARA CONSUTA
...
procedure TFOrdemGrafica.FrmCidadeBTNMINUSClick(Sender: TObject);
begin
  inherited;
   //Codigo da tabela local =0
   XCOD_CIDADE:=-1;
...
procedure TFOrdemGrafica.BitBtn8Click(Sender: TObject);
var
	XSUCCESS: Boolean;
   XDESCERRO: String;
...
procedure TFOrdemGrafica.Duplicata1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.QRDUPLICATABeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  inherited;
  	Try
...
procedure TFOrdemGrafica.BtnLiberaExportaClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.OrdporSubstituioTributria1Click(Sender: TObject);
begin
  inherited;
    	FMenu.TIPOREL:='ORDST';
     	AbrirForm(TFRelMecanica, FRelMecanica, 0);
...
procedure TFOrdemGrafica.BitBtn1Click(Sender: TObject);
Var
	ContParc, Cod_Tmp: Integer;
   Xdesc1, Xdesc2, Xdesc3: String;
begin
...
procedure TFOrdemGrafica.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
	XCodTmpEstoque:=DBGrid1.Columns[0].Field.Text;
end;

procedure TFOrdemGrafica.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.Valida1Click(Sender: TObject);
var
...
procedure TFOrdemGrafica.BRastrearClick(Sender: TObject);
var
...
procedure TFOrdemGrafica.EspelhoNotaFiscal1Click(Sender: TObject);
var
	XCODDOC: Integer;
begin
  inherited;
...
procedure TFOrdemGrafica.SintticodeOrdens1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.AnalticodeOrdens1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.ServiosPorEquipe1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.DBGridConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   ...
   Then Begin
     Try
       ...
       DBGridConsulta.Columns.SaveToFile('C:\MZR\Arquivos\LayoutGrid\Ordemgraf.lgm');
...
procedure TFOrdemGrafica.ServiosporEquipecomFrota1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EfetuarDevoluodeNotaFiscal1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.VisualizarDanfe1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.VisualizarnoSefaz1Click(Sender: TObject);
begin
  inherited;
	 FMzrNfe := TFMzrNFe.Create(FMzrNfe);//cria o formulario de transmissão da Nfe
...
procedure TFOrdemGrafica.ConsultarStatusNFe1Click(Sender: TObject);
begin
  inherited;
	 FMzrNfe := TFMzrNFe.Create(FMzrNfe);//cria o formulario de transmissão da Nfe
...
procedure TFOrdemGrafica.RecuperarDados1Click(Sender: TObject);
Var
	XChaveNfe: String;
begin
  inherited;
...
procedure TFOrdemGrafica.VerificarServidorSefaz1Click(Sender: TObject);
begin
  inherited;
		FMzrNfe := TFMzrNFe.Create(FMzrNfe);//cria o formulario de transmissão da Nfe
...
procedure TFOrdemGrafica.CancelarNFe1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.RelatriodeDanfes1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.ListaNfe1Click(Sender: TObject);
begin
  inherited;
	AbrirForm(TFListaNfe, FListaNfe, 0);
end;

procedure TFOrdemGrafica.InutilizarNumerao1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.EDCodFabKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
   ...
   Then Begin
       ...
       Then Begin
           ...
           Then Begin
               DMESTOQUE.WSimilar.Close;
               DMESTOQUE.WSimilar.SQL.Clear;
               DMESTOQUE.WSimilar.SQL.Add(' SELECT * FROM VWSIMILAR  where vwsimilar.contrint like upper('+#39+EDCodFab.Text+'%'+#39+')');
               DMESTOQUE.WSimilar.Open;
               XOrdem:='CI';
           End;
       End;
       ...
       Then Begin
           ...
           Then Begin
               DMESTOQUE.WSimilar.Close;
               DMESTOQUE.WSimilar.SQL.Clear;
               DMESTOQUE.WSimilar.SQL.Add(' SELECT * FROM VWSIMILAR where vwsimilar.codfabricante like upper('+#39+EDCodFab.Text+'%'+#39+')');
               DMESTOQUE.WSimilar.Open;
               XOrdem:='CF';
           End;
       End;
       ...
       Then Begin
           ...
           Then Begin
               DMESTOQUE.WSimilar.Close;
               DMESTOQUE.WSimilar.SQL.Clear;
               DMESTOQUE.WSimilar.SQL.Add(' SELECT * FROM VWSIMILAR where vwsimilar.descricao like upper('+#39+EDCodFab.Text+'%'+#39+')');
               DMESTOQUE.WSimilar.Open;
               XOrdem:='DS';
           End;
       End;
...
procedure TFOrdemGrafica.SalvarXMLDistribuio1Click(Sender: TObject);
var
   Xcfop:String;
begin
  inherited;
...
procedure TFOrdemGrafica.DBGrid4DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  inherited;
...
procedure TFOrdemGrafica.RelatriodeOrdenseProdutos1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.cbTerceirosClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.RelatriodeOrdenseServios1Click(Sender: TObject);
begin
  inherited;
...
//Paulo 29/11/2011:busca a comissão do responsavel da equipe
procedure TFOrdemGrafica.RetornaComissao(CodResp: Integer);
begin
  inherited;
       DMMACS.TALX.Close;
       DMMACS.TALX.SQL.Clear;
...
procedure TFOrdemGrafica.CompAbertura2vias1Click(Sender: TObject);
begin
  inherited;
  		//XComp2Vias:=True;
       ...
       Xcop:=1;
		RvOrdem1.SystemPreview.FormState := wsMaximized;
   	RVOrdem1.DefaultDest := rdPreview; { poderia ser rdPrinter para impressora    }
       RVOrdem1.SystemPReview.RulerType := rtBothCm; // Para mostrar a reguá n preview
       RVOrdem1.Execute;
end;
...
procedure TFOrdemGrafica.RvOrdem1BeforePrint(Sender: TObject);
begin
  inherited;
...
// - 19/02/2009: impressão do rave (componente, linha, coluna, texto, fonte, tamanho, cor)
procedure TFOrdemGrafica.Imprime(RVRel: TRVSystem; Linha: Double; Coluna:Double; Texto:String; Fonte:String; TamFonte:Integer; CorFonte:TColor; Tipo:Boolean);
begin
...
procedure TFOrdemGrafica.RvOrdem1Print(Sender: TObject);
begin
  inherited;
    If XTipoComp='MEIAPAGINA'
    then begin
        //função de meia página aqui
        ImpCompMeiaPagina;
    End
    Else Begin
    	If DMMACS.TLoja.FieldByName('MODELOORDEM').AsInteger = 1 Then
       	ImpCompAbert1;
    	If DMMACS.TLoja.FieldByName('MODELOORDEM').AsInteger = 2 Then
       	ImpCompAbert2;
		If (DMMACS.TLoja.FieldByName('MODELOORDEM').AsInteger <=0) or (DMMACS.TLoja.FieldByName('MODELOORDEM').AsInteger = Null) or (DMMACS.TLoja.FieldByName('MODELOORDEM').AsInteger > 2) Then
			MessageDlg('Configure o formato de seu comprovante para Ordens de Serviço!'+#13+'Modelos disponíveis: [1 e 2]', mtWarning, [mbOK], 0);
	 End;
end;

procedure TFOrdemGrafica.GerarXMLporPerodo1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.ICMSIndevido1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.BandFinalBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QrAbertOrdem13BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  inherited;
    xTotalProd:=0;
...
procedure TFOrdemGrafica.BandProdBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.ProdutoseServios1Click(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.QrAbertOrdem14MeiaPaginaBeforePrint(
...
procedure TFOrdemGrafica.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QRBand6BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QrAbertOrdem14CompletoBeforePrint(
  Sender: TCustomQuickRep; var PrintReport: Boolean);
begin
  inherited;
    xTotalProd:=0;
...
procedure TFOrdemGrafica.QRBand8BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QRSubDetail3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
    xTotalProd:=xTotalProd+DMESTOQUE.TSlave.fieldByName('TOTAL').AsCurrency;
...
procedure TFOrdemGrafica.QRBand12BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
  	 If ((xTotalProd+xTotalServ)-DMServ.TOrd.fieldByName('TOTORD').AsCurrency)<=0 Then
...
procedure TFOrdemGrafica.RecuperareEnviarXMLdeCancelamento1Click(
  Sender: TObject);
begin
  inherited;
       FMzrNfe := TFMzrNFe.Create(FMzrNfe);//cria o formulario de transmissão da Nfe
...
procedure TFOrdemGrafica.QRSubDetail2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QRSubDetail4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
    xTotalServ:=xTotalServ+DMESTOQUE.TDesp.fieldByName('VLRTOTFIN').AsCurrency;
...
procedure TFOrdemGrafica.QrAbertOrdem15MeiaPaginaBeforePrint(
  Sender: TCustomQuickRep; var PrintReport: Boolean);
begin
  inherited;
    xTotalProd:=0;
...
procedure TFOrdemGrafica.QRBand14BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QRSubDetail5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
    xTotalProd:=xTotalProd+DMESTOQUE.TSlave.fieldByName('TOTAL').AsCurrency;
...
procedure TFOrdemGrafica.QRSubDetail6BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
    xTotalServ:=xTotalServ+DMESTOQUE.TDesp.fieldByName('VLRTOTFIN').AsCurrency;
...
procedure TFOrdemGrafica.QRBand17BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
  	 If ((xTotalProd+xTotalServ)-DMServ.TOrd.fieldByName('TOTORD').AsCurrency)<=0 Then
...
procedure TFOrdemGrafica.QRBand19BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TFOrdemGrafica.QRSubDetail7BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
    xTotalProd:=xTotalProd+DMESTOQUE.TSlave.fieldByName('TOTAL').AsCurrency;
...
procedure TFOrdemGrafica.QRSubDetail8BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
    xTotalServ:=xTotalServ+DMESTOQUE.TDesp.fieldByName('VLRTOTFIN').AsCurrency;
...
procedure TFOrdemGrafica.QRBand22BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
  	 If ((xTotalProd+xTotalServ)-DMServ.TOrd.fieldByName('TOTORD').AsCurrency)<=0 Then
...
procedure TFOrdemGrafica.FrmVendedorBTNOPENClick(Sender: TObject);
begin
  inherited;
  	FiltraTabela(DMPESSOA.VWFuncionario, 'vwfuncionario', '', '', ' ORDER BY NOME');//LIBERA TODOS OS REGISTROS PARA CONSULTA
   If AbrirForm(TFFuncionario, FFuncionario, 1)='Selected'
   Then Begin
       ...
       FrmVendedor.EDCodigo.Hint := DMPESSOA.VWFuncionario.FieldByName('NOME').AsString;
   ...
   Else Begin
       XCOD_VENDEDOR:=-1;
       FrmVendedor.EDCodigo.Text:='';
       FrmVendedor.EDCodigo.Hint := '';
       FrmVendedor.EdDescricao.Text:='';
   End;
   FrmVendedor.Repaint;
end;

procedure TFOrdemGrafica.FrmresponsavelBTNOPENClick(Sender: TObject);
begin
  inherited;
  	FiltraTabela(DMPESSOA.VWFuncionario, 'vwfuncionario', '', '', ' ORDER BY NOME');//LIBERA TODOS OS REGISTROS PARA CONSULTA
   If AbrirForm(TFFuncionario, FFuncionario, 1)='Selected'
   Then Begin
       XCOD_RESPONSAVEL:=DMPESSOA.VWFuncionario.FieldByName('cod_func').AsInteger;
       Frmresponsavel.EDCodigo.Text := DMPESSOA.VWFuncionario.FieldByName('COD_INTERNO').AsString;
       Frmresponsavel.EDCodigo.Hint := DMPESSOA.VWFuncionario.FieldByName('NOME').AsString;
   ...
   Else Begin
       XCOD_RESPONSAVEL:=-1;
       Frmresponsavel.EDCodigo.Text:='';
       Frmresponsavel.EDCodigo.Hint := '';
       Frmresponsavel.EdDescricao.Text:='';
   End;
   Frmresponsavel.Repaint;
end;

procedure TFOrdemGrafica.FrmAtendenteEDCodigoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
   If Key = VK_return
   Then Begin
       If FiltraTabela(DMPESSOA.VWFuncionario, 'vwfuncionario', 'cod_interno', '', ' COD_INTERNO='+#39+FrmAtendente.EDCodigo.Text+#39)=True
       Then Begin//CASO CONSEGUIU SELECIONAR CLIENTE
           XCOD_ATENDENTE:=DMPESSOA.VWFuncionario.FieldByName('cod_func').AsInteger;
           FrmAtendente.EdDescricao.Text:=DMPESSOA.VWFuncionario.FieldByName('NOME').AsString;
           FrmAtendente.EDCodigo.Hint := DMPESSOA.VWFuncionario.FieldByName('NOME').AsString;
...
procedure TFOrdemGrafica.FrmVendedorEDCodigoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
   If Key = VK_return
...
procedure TFOrdemGrafica.FrmresponsavelEDCodigoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
   If Key = VK_return
...
procedure TFOrdemGrafica.EdDescricaoDespesaKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
   if key = #13 then
       EdVlrUnitDespesa.SetFocus;
end;

procedure TFOrdemGrafica.EdVlrUnitDespesaKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if key = #13 then begin
       EdQuantidadeDespesa.Text := '1,00';
       EdQuantidadeDespesa.SetFocus;
  end;
end;

procedure TFOrdemGrafica.EdQuantidadeDespesaExit(Sender: TObject);
begin
  inherited;
   EdTotalDespesa.ValueNumeric:=(EdVlrUnitDespesa.ValueNumeric*EdQuantidadeDespesa.ValueNumeric);
end;

procedure TFOrdemGrafica.EdQuantidadeDespesaKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
   if Key = #13 then
       EdTotalDespesa.SetFocus;
end;

procedure TFOrdemGrafica.BitBtn2Click(Sender: TObject);
var
   xcod_despesa :Integer;
begin
  inherited;
   ...
   // eh necesario informar a descricao de uma equipe
   If EdDescricaoDespesa.Text = ''
   Then Begin
       Mensagem('   A T E N Ç Ã O   ','É necessário informar a descrição da Despesa.','',1,1,false,'I');
       EdDescricaoDespesa.SetFocus;
       Exit;
   End;

   // se o servico nao possuir valor eh exibido uma mensagem para o
   //usuario se ele tem certeza que deseja inserir um servico sem valor
   If EdTotalDespesa.ValueNumeric = 0 Then
       Mensagem('   A T E N Ç Ã O   ','O valor da Despesa é igual a ZERO! ','',1,1,false,'I');

   If EdQuantidadeDespesa.ValueNumeric = 0
   Then Begin
       Mensagem('   A T E N Ç Ã O   ','A quantidade da Despesa é igual a ZERO! ','',1,1,false,'I');
       //ETotalServItem.SetFocus;
       //Exit;
   End;
   ...
   Try
       ...
       FiltraTabela(DMESTOQUE.TDesp, 'DESPADIC', 'cod_despadic', '1', '');
       ...
       DMESTOQUE.TDesp.ParamByName('DESP').AsInteger := xcod_despesa;
       DMESTOQUE.TDesp.ParamByName('COD_GERADOR').AsInteger := XCOD_ORDEM;
       DMESTOQUE.TDesp.ParamByName('GERADOR').AsString := 'ORDEMD';
       DMESTOQUE.TDesp.ParamByName('DESPESA').AsString := EdDescricaoDespesa.Text;
       DMESTOQUE.TDesp.ParamByName('QTD').AsCurrency := EdQuantidadeDespesa.ValueNumeric;
       DMESTOQUE.TDesp.ParamByName('VLRFINAL').AsCurrency := EdVlrUnitDespesa.ValueNumeric;
       DMESTOQUE.TDesp.ParamByName('VLRTOTFIN').AsCurrency := EdTotalDespesa.ValueNumeric;
       DMESTOQUE.TDesp.ParamByName('CST').AsString := '041';
       DMESTOQUE.TDesp.ExecSQL;
       DMESTOQUE.TransacEstoque.CommitRetaining;
   Except
...
//Imprime pelo rave report o comprovante de abertura numero 01
Procedure TFOrdemGrafica.ImpCompAbert1;
Var
 XLooping: Boolean;
Begin
 Try
       ...
       XNumPagina:=1;
       XLinImp:=8;
       //Jônatas 25/06/2013 - Alterei aqui pois o fim da página estava saindo fora
       //da impressão na via [02]
  		XLinAddvia2:=148;
       ...
       XNovaPagina:=False;
       XLooping:=True;
       While XLooping=True do
       Begin
           ImpCompAbert1Cabecalho;
           If XNovaPagina=False Then
            ImpCompAbert1Produtos;
           If XNovaPagina=False Then
            ImpCompAbert1Servicos;
           If XNovaPagina=False Then
            ImpCompAbert1Despesas;
           XLinImp:=96;
           ImpCompAbert1Rodape;
           //Jônatas 25/06/2013 - troquei as condições para o looping ser falso pois atualizava o
           //numero de páginas sem nem entrar nas outras funções pois ainda
           ...
           If (XNovaPagina=True) and (XLooping=True)
           Then Begin
         		XNumPagina:=XNumPagina+1;
            	RvOrdem1.BaseReport.NewPage;
          		XLinImp:=8;
...
End;
//Imprime pelo rave report o comprovante de abertura numero 02
Procedure TFOrdemGrafica.ImpCompAbert2;
Var
 XLooping: Boolean;
Begin
 Try
...
//Imprime Cabeçalho do relatorio de comprovante 01
Procedure TFOrdemGrafica.ImpCompAbert1FiltraDados;
Begin
	Try
       // repassando valor do cidgo da ordem para variavel
       XCOD_ORDEM := DMServ.WOrdem.FieldByname('cod_ordem').AsInteger;
       ...
       FiltraTabela(DMServ.TOrd,'ORDEM','COD_ORDEM',IntToStr(XCOD_ORDEM),'');
       //Vendedor
		FiltraTabela(DMPESSOA.VWFuncionario, 'vwfuncionario', 'cod_func', DMServ.TOrd.FieldByName('COD_VENDEDOR').AsString, '');
       //Responsavel
		FiltraTabela(DMMACS.TALX, 'vwfuncionario', 'cod_func', DMServ.TOrd.FieldByName('COD_FUNCIONARIO').AsString, '');
       //Forma de Pagamento
       FiltraTabela(DMFINANC.TFormPag, 'FORMPAG', 'COD_FORMPAG', DMServ.TOrd.FieldByName('COD_FORMPAG').AsString, '');
		FiltraSlave;
       DMESTOQUE.TSlave.First;
...
procedure TFOrdemGrafica.PrintImage(RVSystem:TRvSystem; Row,Col,ImageWidth,ImageHeight: Double; Bitmap: TBitmap; FileName:String='');
var
...
//Imprime Cabeçalho do relatorio de comprovante 01
//Imprime Cabeçalho do relatorio de comprovante 01
Procedure TFOrdemGrafica.ImpCompAbert1Cabecalho;
var
	Bitmap: TBitmap;
Begin
	Try
       ...
       try
           ...
           Then Begin
               ...
               Bitmap := TBitmap.Create;
               Bitmap.LoadFromFile('logoEmpresa.bmp');
               //PrintImage(RvOrdem1:TRvSystem; Row,Col,ImageWidth,ImageHeight: Double; Bitmap: TBitmap; FileName:String='');
               PrintImage(RvOrdem1, XLinImp, 5, 30, 30,  Bitmap);// via 1
               PrintImage(RvOrdem1, XLinImp+XLinAddvia2, 5, 30, 30,  Bitmap);// via 2
           End;
       finally
         //  FreeAndNil(Bitmap);
       end;
       ...
       // via 1
       Imprime(RvOrdem1, XLinImp,50, 'Fone: '+DMMACS.TEmpresa.fieldByName('fone').AsString + ' - '+ DMMACS.TEmpresa.fieldByName('tel0800').AsString ,'Verdana',12,clBlack, true);
       Imprime(RvOrdem1,XLinImp,{150}158,'OS núm. ['+DMServ.TOrd.FieldByName('NUMERO').AsString+']','Verdana',10,clBlack, True);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,50, 'Fone: '+DMMACS.TEmpresa.fieldByName('fone').AsString + ' - '+ DMMACS.TEmpresa.fieldByName('tel0800').AsString ,'Verdana',12,clBlack, true);
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,{150}158,'OS núm. ['+DMServ.TOrd.FieldByName('NUMERO').AsString+']','Verdana',10,clBlack, True);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,50, 'Endereço: '+DMMACS.TEmpresa.fieldByName('ENDERECO').AsString + ', '+ DMMACS.TEmpresa.fieldByName('NUMERO').AsString+' / '+DMMACS.TEmpresa.fieldByName('BAIRRO').AsString+ ' : ' +DMMACS.TEmpresa.fieldByName('CEP').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,{150}158, 'Imp.: '+DateToStr(Date()) + ' - ' + TimeToStr(Time()) ,'Verdana',8,clBlack, false);
		// via 2
       Imprime(RvOrdem1, XLinImp + XLinAddvia2,50, 'Endereço: '+DMMACS.TEmpresa.fieldByName('ENDERECO').AsString + ', '+ DMMACS.TEmpresa.fieldByName('NUMERO').AsString+' / '+DMMACS.TEmpresa.fieldByName('BAIRRO').AsString+ ' : ' +DMMACS.TEmpresa.fieldByName('CEP').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp + XLinAddvia2,{150}158, 'Imp.: '+DateToStr(Date()) + ' - ' + TimeToStr(Time()) ,'Verdana',8,clBlack, false);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,50, DMGEOGRAFIA.TCidade.fieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,{150}158, 'Dt. Abertura: '+DMServ.TOrd.FieldByName('dtabert').AsString ,'Verdana',8,clBlack, false);
		// via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,50, DMGEOGRAFIA.TCidade.fieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,{150}158, 'Dt. Abertura: '+DMServ.TOrd.FieldByName('dtabert').AsString ,'Verdana',8,clBlack, false);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,{150}158, 'Dt. Fechamento: '+DMServ.TOrd.FieldByName('dtfech').AsString ,'Verdana',8,clBlack, false);
		// via 2
       Imprime(RvOrdem1, XLinImp + XLinAddvia2,{150}158, 'Dt. Fechamento: '+DMServ.TOrd.FieldByName('dtfech').AsString ,'Verdana',8,clBlack, false);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,{150}158, 'Status: '+DMServ.TOrd.FieldByName('status').AsString ,'Verdana',8,clBlack, True);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,{150}158, 'Status: '+DMServ.TOrd.FieldByName('status').AsString ,'Verdana',8,clBlack, True);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,{150}158, 'Via: [01] - Pagina: ['+IntToStr(XNumPagina)+']' ,'Verdana',8,clBlack, True);
		// via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,{150}158, 'Via: [02] - Pagina: ['+IntToStr(XNumPagina)+']' ,'Verdana',8,clBlack, True);

       XLinImp:=XLinImp+6;
       // via 1
       Imprime(RvOrdem1, XLinImp,5,'----------------------------------------------------------------------------------------------------------------------------','Verdana',10,clBlack, false);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5,'----------------------------------------------------------------------------------------------------------------------------','Verdana',10,clBlack, false);

		XLinImp:=XLinImp+2;
       // - 19/02/2009: dados do cliente
       // via 1
       Imprime(RvOrdem1, XLinImp,5, 'Cliente: ' +DMPESSOA.TCliente.fieldByName('COD_INTERNO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('NOME').AsString ,'Verdana',8,clBlack, true);
       Imprime(RvOrdem1, XLinImp,{150}158, 'CPF/CNPJ: ' +DMPESSOA.TPessoa.fieldByName('CPFCNPJ').AsString ,'Verdana',8,clBlack, false);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5, 'Cliente: ' +DMPESSOA.TCliente.fieldByName('COD_INTERNO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('NOME').AsString ,'Verdana',8,clBlack, true);
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,{150}158, 'CPF/CNPJ: ' +DMPESSOA.TPessoa.fieldByName('CPFCNPJ').AsString ,'Verdana',8,clBlack, false);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,5, 'Endereço: ' +DMPESSOA.TPessoa.fieldByName('ENDERECO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('BAIRRO').AsString,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,{150}158, 'CEP: ' +DMPESSOA.TPessoa.fieldByName('CEP').AsString,'Verdana',8,clBlack, false);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5, 'Endereço: ' +DMPESSOA.TPessoa.fieldByName('ENDERECO').AsString+ ' - ' +DMPESSOA.TPessoa.fieldByName('BAIRRO').AsString,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,{150}158, 'CEP: ' +DMPESSOA.TPessoa.fieldByName('CEP').AsString,'Verdana',8,clBlack, false);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,5, 'Cidade: ' +DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,{150}158, 'Fone: ' +DMPESSOA.TPessoa.fieldByName('TELREL').AsString ,'Verdana',8,clBlack, true);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5, 'Cidade: ' +DMGEOGRAFIA.TCidade.FieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,{150}158, 'Fone: ' +DMPESSOA.TPessoa.fieldByName('TELREL').AsString ,'Verdana',8,clBlack, true);
		XLinImp:=XLinImp+4;
       Imprime(RvOrdem1, XLinImp,5, 'Contato: ' + DMServ.TOrd.FieldByName('contato').AsString,'Verdana',8,clBlack, false);
		XLinImp:=XLinImp+2;
       //Imprime cabeçalho dos itens
       // via 1
       Imprime(RvOrdem1,XLinImp,5,'----------------------------------------------------------------------------------------------------------------------------','Verdana',10,clBlack, false);
       ...
       XLinImp:=XLinImp+3;
       // via 1
       //Edmar - 07/01/2015 - Linha comentada pois foi reorganizado o tamanho dos campos no comprovante. Quando a descrição era grande demais,
       //ela ultrapassava seu limite e continuava sem cima da quantidade (solicitação 280)
       //Imprime(RvOrdem1, XLinImp,5, 'Produtos/Serviços/Despesas                             Quantidade         Vlr. Unitário          Desc.              Vlr. Total' ,'Verdana', 9,clBlack, true);
       Imprime(RvOrdem1, XLinImp,5, 'Produtos/Serviços/Despesas                                              Quantidade      Vlr. Unitário        Desc.        Vlr. Total' ,'Verdana', 9,clBlack, true);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5, 'Produtos/Serviços/Despesas                                              Quantidade'{         Vlr. Unitário          Desc.              Vlr. Total' },'Verdana', 9,clBlack, true);
...
End;
//Imprime Cabeçalho do relatorio de comprovante 02
Procedure TFOrdemGrafica.ImpCompAbert2Cabecalho;
var
	Bitmap: TBitmap;
Begin
	Try
       ...
       try
           ...
           Then Begin
               ...
               Bitmap.LoadFromFile('logoEmpresa.bmp');
               //PrintImage(RvOrdem1:TRvSystem; Row,Col,ImageWidth,ImageHeight: Double; Bitmap: TBitmap; FileName:String='');
               PrintImage(RvOrdem1, XLinImp, 5, 109, 21,  Bitmap);// via 1
               PrintImage(RvOrdem1, XLinImp+XLinAddvia2, 5, 109, 21,  Bitmap);// via 2
           End;
       finally
         //  FreeAndNil(Bitmap);
       end;
		//Dados da Coluna da Direita com informações da OS
		//1º via
       Imprime(RvOrdem1,XLinImp,{150}158,'OS núm. ['+DMServ.TOrd.FieldByName('NUMERO').AsString+']','Verdana',10,clBlack, True);
       Imprime(RvOrdem1, XLinImp+4,{150}158, 'Imp.: '+DateToStr(Date()) + ' - ' + TimeToStr(Time()) ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+8,{150}158, 'Dt. Abertura: '+DMServ.TOrd.FieldByName('dtabert').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+12,{150}158, 'Dt. Fechamento: '+DMServ.TOrd.FieldByName('dtfech').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+16,{150}158, 'Status: '+DMServ.TOrd.FieldByName('status').AsString ,'Verdana',8,clBlack, True);
       Imprime(RvOrdem1, XLinImp+20,{150}158, 'Via: [01] - Pagina: ['+IntToStr(XNumPagina)+']' ,'Verdana',8,clBlack, True);
		//2º via
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,{150}158,'OS núm. ['+DMServ.TOrd.FieldByName('NUMERO').AsString+']','Verdana',10,clBlack, True);
       Imprime(RvOrdem1, XLinImp+4+XLinAddvia2,{150}158, 'Imp.: '+DateToStr(Date()) + ' - ' + TimeToStr(Time()) ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+8+XLinAddvia2,{150}158, 'Dt. Abertura: '+DMServ.TOrd.FieldByName('dtabert').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+12+ XLinAddvia2,{150}158, 'Dt. Fechamento: '+DMServ.TOrd.FieldByName('dtfech').AsString ,'Verdana',8,clBlack, false);
       Imprime(RvOrdem1, XLinImp+16+XLinAddvia2,{150}158, 'Status: '+DMServ.TOrd.FieldByName('status').AsString ,'Verdana',8,clBlack, True);
       Imprime(RvOrdem1, XLinImp+20+XLinAddvia2,{150}158, 'Via: [02] - Pagina: ['+IntToStr(XNumPagina)+']' ,'Verdana',8,clBlack, True);

       XLinImp:=XLinImp+25;
		// - 19/02/2009: dados da empresa
       // via 1
       Imprime(RvOrdem1, XLinImp,5, 'Fone: '+DMMACS.TEmpresa.fieldByName('fone').AsString + ' - '+ DMMACS.TEmpresa.fieldByName('tel0800').AsString ,'Verdana',12,clBlack, true);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5, 'Fone: '+DMMACS.TEmpresa.fieldByName('fone').AsString + ' - '+ DMMACS.TEmpresa.fieldByName('tel0800').AsString ,'Verdana',12,clBlack, true);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,5, 'Endereço: '+DMMACS.TEmpresa.fieldByName('ENDERECO').AsString + ', '+ DMMACS.TEmpresa.fieldByName('NUMERO').AsString+' / '+DMMACS.TEmpresa.fieldByName('BAIRRO').AsString+ ' : ' +DMMACS.TEmpresa.fieldByName('CEP').AsString ,'Verdana',8,clBlack, false);
		// via 2
       Imprime(RvOrdem1, XLinImp + XLinAddvia2,5, 'Endereço: '+DMMACS.TEmpresa.fieldByName('ENDERECO').AsString + ', '+ DMMACS.TEmpresa.fieldByName('NUMERO').AsString+' / '+DMMACS.TEmpresa.fieldByName('BAIRRO').AsString+ ' : ' +DMMACS.TEmpresa.fieldByName('CEP').AsString ,'Verdana',8,clBlack, false);

		XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,5, DMGEOGRAFIA.TCidade.fieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString ,'Verdana',8,clBlack, false);
		// via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,5, DMGEOGRAFIA.TCidade.fieldByName('NOME').AsString+ ' - ' +DMGEOGRAFIA.TEstado.fieldByName('UF_ESTADO').AsString ,'Verdana',8,clBlack, false);
...
//Imprime dados de produtos comprovante 01
Procedure TFOrdemGrafica.ImpCompAbert1Produtos;
Var
I, XFlag: Integer;
Begin
 Try
       ...
       Then Begin
           ...
           Begin
               ...
               XLinImp:=XLinImp+1;
               // via 1
           	//Edmar - 07/01/2015 - Valores comentados para salvar qual era os valores antigos antes da alteração de tamanho dos campos
           	//Solicitação (280)
               Imprime(RvOrdem1, XLinImp, 5, DMESTOQUE.TSlave.fieldByName('DESCRICAO').AsString ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp, {90}115, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('QTD').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp, {120}140, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('VLRUNIT').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp, {150}165, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp, {175}185, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('TOTAL').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               // via 2
               Imprime(RvOrdem1, XLinImp+XLinAddvia2, 5, DMESTOQUE.TSlave.fieldByName('DESCRICAO').AsString ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp+XLinAddvia2, 115, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('QTD').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               {Imprime(RvOrdem1, XLinImp+XLinAddvia2, 120, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('VLRUNIT').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp+XLinAddvia2, 150, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               Imprime(RvOrdem1, XLinImp+XLinAddvia2, 175, FormatFloat('0.00', DMESTOQUE.TSlave.fieldByName('TOTAL').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
               }
               If Trim(DMESTOQUE.TSlave.fieldByName('DESCGRAFICA').AsString)<>''
               Then Begin
                   MObs.Lines.Text:=Trim(DMESTOQUE.TSlave.fieldByName('DESCGRAFICA').AsString);
                   XLinImp:=XLinImp+3;
                   for i := 0 to MObs.Lines.Count -1 do
                   Begin
                       //via 1
                       Imprime(RvOrdem1, XLinImp, 15, MObs.Lines[i] ,'Verdana',8,clBlack, false);
                       //via 2
                       Imprime(RvOrdem1, XLinImp+XLinAddvia2, 15, MObs.Lines[i] ,'Verdana',8,clBlack, false);
                       XLinImp:=XLinImp+3;
                   End;
               End
               Else Begin
               ...
               Then Begin
                   XFlag:=1;
                   //Jônatas 25/06/2013 - alterei aqui
                   //XImprimeMaisProdutos:=True;
                   XNovaPagina:=True;
               End;
           End;
...
End;
//Imprime dados de servicos comprovante 01
Procedure TFOrdemGrafica.ImpCompAbert1Servicos;
Var
	XFlag: Integer;
Begin
	Try
       ...
       Begin
           ...
           //Edmar - 07/01/2015 - Valores comentados para salvar qual era os valores antigos antes da alteração de tamanho dos campos
           //Solicitação (280)
           Imprime(RvOrdem1, XLinImp, 5, DMESTOQUE.TDesp.fieldByName('despesa').AsString ,'Verdana',11,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {90}115, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('QTD').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {120}140, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('vlrfinal').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {150}165, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {175}185, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('VLRTOTFIN').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           // via 2
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 5, DMESTOQUE.TDesp.fieldByName('despesa').AsString ,'Verdana',11,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 115, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('QTD').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           {Imprime(RvOrdem1, XLinImp+XLinAddvia2, 120, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('vlrfinal').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 150, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 175, FormatFloat('0.00', DMESTOQUE.TDesp.fieldByName('VLRTOTFIN').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           }
           DMESTOQUE.TDesp.Next;
           XQuantServicosImp:=XQuantServicosImp-1;
           If XLinImp>=93
...
//Imprime dados de despesas comprovante 01
Procedure TFOrdemGrafica.ImpCompAbert1Despesas;
Var
	XFlag: Integer;
Begin
	Try
       ...
       Begin
           ...
           //Edmar - 07/01/2015 - Valores comentados para salvar qual era os valores antigos antes da alteração de tamanho dos campos
           //Solicitação (280)
           Imprime(RvOrdem1, XLinImp, 5, DMESTOQUE.Alx4.fieldByName('despesa').AsString ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {90}115, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('QTD').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {120}140, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('vlrfinal').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {150}165, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp, {175}185, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('VLRTOTFIN').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           //via 2
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 5, DMESTOQUE.Alx4.fieldByName('despesa').AsString ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 115, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('QTD').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           {Imprime(RvOrdem1, XLinImp+XLinAddvia2, 120, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('vlrfinal').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 150, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('DESCONTO').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           Imprime(RvOrdem1, XLinImp+XLinAddvia2, 175, FormatFloat('0.00', DMESTOQUE.Alx4.fieldByName('VLRTOTFIN').AsCurrency) ,'Verdana', 9,clBlack, FALSE);
           }
           DMESTOQUE.Alx4.Next;
           XquantDespImprimir:=XquantDespImprimir-1;
...
//Imprime dados de rodape comprovante 01
Procedure TFOrdemGrafica.ImpCompAbert1Rodape;
Var
i: Smallint;
XLinhaImpObs: Integer;
Begin
 Try
  ...
  XLinhaImpObs:=XLinImp;
       ...
       Then Begin
        //via 1
           ...
           Imprime(RvOrdem1, XLinhaImpObs,5, 'Observação:' ,'Verdana',10,clBlack, True);
        //via 2
           Imprime(RvOrdem1, XLinhaImpObs+XLinAddvia2,5, 'Observação:' ,'Verdana',10,clBlack, True);
  ...
  for i := 0 to MObs.Lines.Count -1 do
       Begin
        //via 1
           Imprime(RvOrdem1, XLinhaImpObs,5, copy(MObs.Lines[i],0,45) ,'Verdana',8,clBlack, false);
        //via 2
           Imprime(RvOrdem1, XLinhaImpObs+XLinAddvia2,5, copy(MObs.Lines[i],0,45) ,'Verdana',8,clBlack, false);
           XLinhaImpObs:=XLinhaImpObs+3;
           //via 1
           Imprime(RvOrdem1, XLinhaImpObs,5, copy(MObs.Lines[i],46,90) ,'Verdana',8,clBlack, false);
           //via 2
           Imprime(RvOrdem1, XLinhaImpObs+XLinAddvia2,5, copy(MObs.Lines[i],46,90) ,'Verdana',8,clBlack, false);
           XLinhaImpObs:=XLinhaImpObs+3;
           //limitar o número de linhas para não atrapalhar a impressão
           If XLinhaImpObs>=117 then
              break;
       ...
       //via 1
       Imprime(RvOrdem1, XLinImp, 90, 'Vendedor: '+DMPESSOA.VWFuncionario.FieldByName('NOME').AsString ,'Verdana', 8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,150, 'Forma Pagto: '+DMFINANC.TFormPag.FieldByName('descricao').Asstring ,'Verdana', 8,clBlack, false);
       //via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2, 90, 'Vendedor: '+DMPESSOA.VWFuncionario.FieldByName('NOME').AsString ,'Verdana', 8,clBlack, false);
       //Imprime(RvOrdem1, XLinImp+XLinAddvia2,150, 'Forma Pagto: '+DMFINANC.TFormPag.FieldByName('descricao').Asstring ,'Verdana', 8,clBlack, false);

       XLinImp:=XLinImp+4;
       //via 1
       Imprime(RvOrdem1, XLinImp,90, 'Cond. Pagto: '+DMServ.TOrd.FieldByName('tipopag').AsString ,'Verdana', 8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,150, 'Sub Total: '+FormatFloat('0.00', XVLRTOTAL) ,'Verdana', 9,clBlack, true);
       //via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,90, 'Cond. Pagto: '+DMServ.TOrd.FieldByName('tipopag').AsString ,'Verdana', 8,clBlack, false);
       //Imprime(RvOrdem1, XLinImp+XLinAddvia2,150, 'Sub Total: '+FormatFloat('0.00', XVLRTOTAL) ,'Verdana', 9,clBlack, true);

       XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1, XLinImp,90, 'Previsão Ent.: '+DMServ.TOrd.FieldByName('dtprev').AsString ,'Verdana', 8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,150, 'Desc.: '+FormatFloat('0.00', DMServ.TOrd.FieldByName('DESCONTO').AsCurrency)+'%'  ,'Verdana', 8,clBlack, false);
       // via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,90, 'Previsão Ent.: '+DMServ.TOrd.FieldByName('dtprev').AsString ,'Verdana', 8,clBlack, false);
       //Imprime(RvOrdem1, XLinImp+XLinAddvia2,150, 'Desc.: '+FormatFloat('0.00', DMServ.TOrd.FieldByName('DESCONTO').AsCurrency)+'%' ,'Verdana', 8,clBlack, false);

       XLinImp:=XLinImp+4;
       //Via 1
       Imprime(RvOrdem1, XLinImp, 90, 'Responsável: '+DMMACS.TALX.FieldByName('NOME').AsString ,'Verdana', 8,clBlack, false);
       Imprime(RvOrdem1, XLinImp,150, 'Total Ordem: '+FormatFloat('0.00', DMServ.TOrd.FieldByName('totord').AsCurrency) ,'Verdana', 9,clBlack, true);
       //Via 2
       Imprime(RvOrdem1, XLinImp+XLinAddvia2,90, 'Responsável: '+DMMACS.TALX.FieldByName('NOME').AsString ,'Verdana', 8,clBlack, false);
       //Imprime(RvOrdem1, XLinImp+XLinAddvia2,150, 'Total Ordem: '+FormatFloat('0.00', DMServ.TOrd.FieldByName('totord').AsCurrency) ,'Verdana', 9,clBlack, true);
  ...
  //If XLinhaImpObs>XLinImp Then
       ...
       Imprime(RvOrdem1,XLinImp,5,'     ________________________________________                  ________________________________________     ','Verdana',10,clBlack, false);
       // via 2
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,5,'     ________________________________________                  ________________________________________     ','Verdana',10,clBlack, false);

       XLinImp:=XLinImp+4;
       // via 1
       Imprime(RvOrdem1,XLinImp,5,'                   CLIENTE/RESPONSÁVEL                                                          CONFERENTE                   ','Verdana',10,clBlack, false);
       // via 2
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,5,'                   CLIENTE/RESPONSÁVEL                                                          CONFERENTE                   ','Verdana',10,clBlack, false);

       XLinImp:=XLinImp+3;
       //via 1
       Imprime(RvOrdem1,XLinImp,5,'__________________________________________________________________________________________________','Verdana',10,clBlack, false);
       //via 2
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,5,'__________________________________________________________________________________________________','Verdana',10,clBlack, false);

       XLinImp:=XLinImp+3;
       //via 1
       Imprime(RvOrdem1,XLinImp,5,'Autorizo a produção e/ou confecção dos serviços e produtos acima discriminados, de acordo com o prazo de entrega e pagamento estipulados nesta ','Verdana',7,clBlack, false);
       //via 2
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,5,'Autorizo a produção e/ou confecção dos serviços e produtos acima discriminados, de acordo com o prazo de entrega e pagamento estipulados nesta ','Verdana',7,clBlack, false);

       XLinImp:=XLinImp+3;
       //via 1
       Imprime(RvOrdem1,XLinImp,5,'Ordem de Serviços e responsabilizo-me sobre os mesmos. ','Verdana',7,clBlack, false);
       //via 2
       Imprime(RvOrdem1,XLinImp+XLinAddvia2,5,'Ordem de Serviços e responsabilizo-me sobre os mesmos. ','Verdana',7,clBlack, false);
   ...
   End;
End;
procedure TFOrdemGrafica.BitBtn3Click(Sender: TObject);
begin
  inherited;
    DMESTOQUE.Alx1.Close;
    DMESTOQUE.Alx1.SQL.Clear;
    ...
    DMESTOQUE.Alx1.sql.Add('left join equipe on despadic.cod_equipe = equipe.cod_equipe');
    DMESTOQUE.Alx1.sql.Add('where despadic.cod_despadic= :cod');
    DMESTOQUE.Alx1.ParamByName('cod').AsString:= DMESTOQUE.Alx4.fieldbyname('COD_DESPADIC').AsString;
    DMESTOQUE.Alx1.Open;

    If Length(DMESTOQUE.Alx1.FieldByName('DESPESA').AsString) > 20
    Then Begin
         If Mensagem('   A T E N Ç Ã O   ','Deseja realmente excluir a Despesa ' + #39 + Copy(DMESTOQUE.Alx1.FieldByname('DESPESA').AsString,0,20) + #39 + '. ?','',2,3,false,'I')=3 Then
             Exit;
    End
    Else Begin
         If Mensagem('   A T E N Ç Ã O   ','Deseja realmente excluir a Despesa ' + #39 + DMESTOQUE.Alx1.FieldByname('DESPESA').AsString + #39 + ' ?','',2,3,false,'I')=3 Then
             Exit;
    End;
    //volta valores do serviço para as grids de edição
    EdDescricaoDespesa.Text:=DMESTOQUE.Alx1.fieldbyname('DESPESA').AsString;
    EdTotalDespesa.Text:=DMESTOQUE.Alx1.fieldbyname('VLRTOTFIN').AsString;
    EdQuantidadeDespesa.Text:=DMESTOQUE.Alx1.fieldbyname('QTD').AsString;
    EdVlrUnitDespesa.Text:=DMESTOQUE.Alx1.fieldbyname('VLRFINAL').AsString;
    RemoverDespesas;
end;

procedure TFOrdemGrafica.Cartadecorreoeletrnica1Click(Sender: TObject);
begin
  inherited;
    IniciaCCe(DMServ.WOrdem.FieldByName('cod_ordem').AsInteger, 'ORDEM');
end;

procedure TFOrdemGrafica.ComprovanteemMeiaPagina1Click(Sender: TObject);
begin
  //inherited;
  inherited;
  	//escolhe o tipo de comprovante para ao chamar o rave escolher as funções de meia página
  	XTipoComp:='MEIAPAGINA';
  	//chamar o CompdeAbertura1Click(Sender) pra fazer as filtragens necessárias
  	CompdeAbertura1Click(Sender);
  	//Setar o XTipoComp:=''; aqui para imprimir os outros tipos de comprovante normalmente
  	XTipoComp:='';
end;

procedure TFOrdemGrafica.FrmDepartamentoBTNOPENClick(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.FrmObraEDCodigoExit(Sender: TObject);
begin
  inherited;
...
procedure TFOrdemGrafica.BtnSelecionarClick(Sender: TObject);
begin
  inherited;
